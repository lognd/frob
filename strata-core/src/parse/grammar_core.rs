...
// The `vmodel_node`/`vmodel_edge` productions that used to live here moved
// to `grammar_vmodel.rs` (T-3260) once this file crossed the LARGE001
// threshold.
// ---------------------------------------------------------------------
// Parser
// ---------------------------------------------------------------------
...
impl Parser {
    ...
    fn parse_access_attr(&mut self, attrs: &mut Vec<String>) -> Result<(), ParseError> {
        ...
        match mode.as_str() {
            ...
            _ => return self.err("access mode must be one of read|append|alpha|write|exclusive"),
        }
    ...
    fn parse_configuration(&mut self, ast: &mut ModuleAst) -> Result<(), ParseError> {
        ...
        while !self.at_symbol('}') {
            if self.at_keyword("entity") {
                ...
                return self.err(
                    "expected 'entity NAME' or 'architecture NAME' inside configuration block",
                );
            }
            if self.at_symbol(';') {
                self.advance();
