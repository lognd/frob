...
Tickets needed a story-point size so `frob ticket start`/flow/sprint show
can enforce and report on sizing, per the owner's 2026-09-20 directive.
Added Ticket.points (Fibonacci scale, validated at write time like
milestone), a `frob ticket points`/`tokens` setter pair, and a hard
refusal at `frob ticket start` for an unsized queued ticket (single
`--unsized-ack REASON` override, chosen deliberately over a repo-wide
exemption date since the owner's own ticket body argues no backfill of
existing queued tickets is needed -- the check only trips when a ticket
is actually started). Points-weighted rollups (total/done/ETA) were
added to `sprint show`, and points-per-hour / tokens-per-point
calibration to `flow`, both mined from the same git-history transition
source `sprint_velocity` already trusts (generalized via a new
`target_state` parameter). Automatic session-transcript token mining
(the amendment's second half) was left out of scope for this pass --
only the manual `frob ticket tokens` path shipped; see Filed below.
...
```
 ...
 src/frob/_cli_parsers/_ticket/__init__.py          |    50 +-
 .../_cli_parsers/_ticket/_closeout_evidence.py     |    24 +
 src/frob/_cli_parsers/_ticket/_metadata.py         |    97 +-
 ...
 src/frob/app/ticket_runner/__init__.py             |   147 +-
 src/frob/app/ticket_runner/_close_cmd.py           |    42 +-
 src/frob/app/ticket_runner/_land_cmd.py            |   743 +-
 src/frob/app/ticket_runner/_lifecycle.py           |   171 +-
 ...
 tests/test_tickets_points.py                       |   237 +
 ...
 uv.lock                                            |     2 +-
 2964 files changed, 377078 insertions(+), 15867 deletions(-)
```

### Evidence
- `tests/test_tickets_points.py::TestValidatePoints::test_valid_value_accepted` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestValidatePoints::test_invalid_value_refused` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSetPoints::test_valid_value_sets_field` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSetPoints::test_invalid_value_refused` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSetUnsizedAck::test_ack_sets_both_fields` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSetUnsizedAck::test_reason_missing_refuses` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSetTokens::test_sets_fields` (pytest node id, verified passing when recorded)
...
- `tests/test_tickets_points.py::TestTicketPointsPerHour::test_unsized_or_unstarted_ticket_excluded` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestTicketTokensPerPoint::test_sized_and_tokened_calibrates` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestTicketTokensPerPoint::test_no_qualifying_ticket_returns_none` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestSprintViewPoints::test_points_rollup_and_eta` (pytest node id, verified passing when recorded)
