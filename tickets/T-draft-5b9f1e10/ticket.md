---
id: T-draft-5b9f1e10
title: 'migration: split module serve out of design/frob.strata into design/serve.strata
...
acceptance:
- text: Given the land, when it completes, then design/serve.strata exists with its
...
- text: 'Given the repository at this commit, when the strata checker runs, then it
  ...
  evidence: []
- text: Given every cross-module flow involving serve, when the linker runs, then
  ...
  evidence: []
- text: Given the templated-assume gate, when it runs against design/serve.strata,
...
land_commit: null
---
Migration leaf for module `serve` (members: serve). Leaf consumer: out-degree 7, fan-in 2.

One-time reviewed rewrite, by hand, module by module (owner decision D-M6:
there is no sys-split tool; the owner rejected one). Write design/serve.strata with:
  - `module serve` header (dotted path form);
