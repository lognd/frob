...
frob explore commands (map/outline/xref/docs-search). Before this fix,
frob.lang._parse_file_with_artifact_cache was always an uncached
passthrough under explore, since nothing stamped
PARSE_ARTIFACT_CACHE_ENV for it -- 1643 uncached parses measured on
frob explore xref alone (found while working T-5135's perf audit;
descoped from that ticket because explore_runner.py had a live
cross-worktree lease collision with T-4690 at the time).

Deliberately does NOT reuse frob.gates's own worker-stamp helper, which
also creates/migrates the cache db -- explore is a single, short-lived,
read-oriented process with no sibling workers to share a freshly built
cache with, so creating one here would leave an empty db behind on a
repo that has never run frob check. Only stamps the env var when
.frob/parse-artifacts.db already exists.

Also declares the new env.read/env.write capability sites in
design/frob.strata (cli node's explore_runner.py, testsuite node's new
positive-control test) and bumps the SYS111 ratchet ceilings the
unscoped land sweep's SELFAUDIT001 flagged -- confirmed a targeted
`frob check --only sys` run reports zero SELFAUDIT001 findings tied to
this diff (the remaining findings are pre-existing, unrelated debt:
html_render/narrative interface gaps, SYS119 templated assumes, and a
T-4563 glob-form pending-auto-accept note).

Evidence: test_stamps_env_when_cache_db_exists is the positive control
(the env var is unset before this fix's code path runs, so this test
would fail without it); test_does_not_stamp_when_no_cache_db_exists
proves it never writes/creates on a repo with no cache yet.
...
```
 ...
 tickets/T-5201/done-report.md                      |  2985 +++
 ...
 uv.lock                                            |     2 +-
 2965 files changed, 379133 insertions(+), 15857 deletions(-)
```

### Evidence
- `tests/unit/test_explore_runner_parse_artifact_cache.py::TestStampReadOnlyParseArtifactCacheEnv::test_stamps_env_when_cache_db_exists` (pytest node id, verified passing when recorded)
- `tests/unit/test_explore_runner_parse_artifact_cache.py::TestStampReadOnlyParseArtifactCacheEnv::test_does_not_stamp_when_no_cache_db_exists` (pytest node id, verified passing when recorded)
