...
T-5132 shipped the points=None start refusal as an unconditional default,
which broke ~40 pre-existing tests across 7 files in this repo's own
suite (every test that creates-then-starts a ticket without sizing it
first) and would surprise every consumer repo the identical way. Made the
refusal opt-in: AppConfig.ticket_points_required (default False), set via
the [tool.frob] ticket_points_required pyproject key or the one-shot
--require-points CLI flag (OR semantics, registered in _BOOL_FLAGS).
--unsized-ack remains the per-ticket override on top. This repo's own
pyproject.toml sets the key true, so its own queued tickets are still
refused exactly as T-5132 originally shipped -- only the direct-AppConfig
test fixtures (which never read pyproject.toml) and other consumer repos
see the new default-off behavior. Ran all 7 originally-broken files plus
T-5132's own test suite: one pre-existing, unrelated failure remains
(TestLedgerAutoCommitEnumeratedOverDispatchTable, missing points/set/
tokens verb accounting -- confirmed failing on dev baseline too, not
...
```
 ...
 docs/modules/tickets-data-storage.md               |   125 +
 ...
 src/frob/_cli_parsers/_ticket/_progress.py         |   174 +-
 src/frob/_cli_parsers/_verify.py                   |    16 +-
 src/frob/app/_check_chunking_baseline.py           |     9 -
 src/frob/app/_config_external.py                   |    67 +-
 ...
 src/frob/app/ticket_runner/_lifecycle.py           |   405 +-
 ...
 tickets/T-5287/done-report.md                      |  3285 +++
 ...
 uv.lock                                            |     2 +-
 3256 files changed, 427679 insertions(+), 16943 deletions(-)
```

### Evidence
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_unsized_queued_ticket_refuses` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_unsized_queued_ticket_starts_cleanly_when_not_required` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_sized_ticket_starts_cleanly` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_unsized_ack_bypasses_refusal` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_full_start_cli_refuses_on_unsized_ticket` (pytest node id, verified passing when recorded)
- `tests/test_tickets_points.py::TestStartUnsizedRefusal::test_full_start_cli_starts_cleanly_when_not_required` (pytest node id, verified passing when recorded)
