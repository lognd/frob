...
Adds frob.sql._extract: tree-sitter literal extraction from SQL-executing
call sites across Python (cursor.execute/.raw()/SQLAlchemy text()),
TS/JS (Prisma $queryRaw/$executeRaw), and Rust (sqlx query!/query_as!/
query_scalar! macros), feeding sqlfluff's parser downstream. A non-
literal composition reaching a sink is flagged WEBSEC107 (CWE-89),
excluding psycopg's safe sql.SQL/.Identifier composition per the T-5141
corpus. sql_relevance() is the reusable owner-directive predicate that
makes sqlfluff required-for-family in 5148-2's tool-registry entry
(sqlfluff's absence is unmeasured, not a failure, when no SQL surface
exists). sql now depends on lang in frob.toml's [arch.layering.allow].
No direct file reads live in this module (raw_tree's own read is already
covered under frob.lang's node), so no design/frob.strata edit was
needed for SELFAUDIT001.
...
```
 ...
 tests/unit/sql/test_extract.py                     |   173 +
 ...
 uv.lock                                            |     2 +-
 4040 files changed, 645041 insertions(+), 23495 deletions(-)
```

### Evidence
- `tests/unit/sql/test_extract.py::test_extract_sql_literals_python_cursor_execute` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_extract_sql_literals_python_django_raw` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_extract_sql_literals_python_sqlalchemy_text` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_extract_sql_literals_ts_prisma_query_raw` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_extract_sql_literals_rust_sqlx_query_macro` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_injection_findings_python_fstring_positive` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_injection_findings_python_literal_negative` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_injection_findings_psycopg_composition_excluded` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_injection_findings_ts_prisma_template_positive` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_injection_findings_ts_prisma_literal_negative` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_relevance_true_for_call_site_fixture` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_extract.py::test_sql_relevance_false_for_plain_repo` (pytest node id, verified passing when recorded)
