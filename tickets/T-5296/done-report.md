...
frob-exports flagged 3 packages (src/frob, src/frob/arch, src/frob/vet)
with public symbols that exist in their submodules but were never
re-exported through the package's own __init__.py: frob.doctor's
relevant_tool_findings/RelevantTool* family, arch._layering.check_
layering_edges, and vet._osv's query_advisories/OsvQueryError/
OsvQueryFailure. Added each to the appropriate __init__.py's import
block and __all__ list; no behavior change, pure export-surface fix.
tests/unit/test_exports.py is fully green (16/16) after the fix.
...
```
 ...
 tickets/T-draft-d2f89b58/ticket.md                 |    41 +
 ...
 uv.lock                                            |     2 +-
 3364 files changed, 431889 insertions(+), 17069 deletions(-)
```
