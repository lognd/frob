...
Adds WEBPERF101-106/108, the markup-level half of the T-5140 web-app
epic's Core Web Vitals rule family: img width/height (CLS), lazy-load on
offscreen img/iframe, srcset, font-display, head-script defer/async,
webpack/vite bundle-size budget, and viewport meta.

Reuses T-5364's _seo_substrate tree-walk substrate (frob.lang.raw_tree
single parse dispatch plus its HTML/JSX tag-name/attrs/head helpers)
rather than standing up a second tree-sitter walk, generalizing its
single-target _find_head search into a whole-document _iter_elements
walker for the body-level rules (101-103).

WEBPERF107 (source-maps-in-prod) is deliberately excluded -- owned by
T-5143-3, not duplicated per the ticket body's own scope note.

Exposes websec_findings(root, frameworks) -> tuple[Violation, ...], the
family's pkgutil-discovery hook convention, but _taint_gate only scans
frob.webapp._websec_* modules today; gate discovery for the _webperf_
prefix lands with T-draft-553232aa. An xfail(strict=True) end-to-end
test proves this gap and will flip to a hard failure (forcing the
marker's removal) the moment discovery widens.
...
```
 ...
 uv.lock                                            |   265 +-
 4550 files changed, 790656 insertions(+), 23544 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf101_positive-WEBPERF101-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf101_negative-WEBPERF101-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf102_positive-WEBPERF102-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf102_negative-WEBPERF102-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf103_positive-WEBPERF103-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf103_negative-WEBPERF103-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf104_positive-WEBPERF104-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf104_negative-WEBPERF104-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf105_positive-WEBPERF105-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf105_negative-WEBPERF105-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf106_positive-WEBPERF106-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf106_negative-WEBPERF106-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf108_positive-WEBPERF108-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_fixture[webperf108_negative-WEBPERF108-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf107_never_emitted` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_webperf_markup_findings_no_framework_short_circuits_via_hook` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_webperf_markup.py::test_websec_findings_discovery_hook_emits_violation` (pytest node id, verified passing when recorded)
