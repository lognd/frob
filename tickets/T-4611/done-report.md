...
T-3233's land spent 27.3s of forced-synchronous verification because the
quarantine-raised ERROR line named only a COUNT of undisposed findings,
never which (rule, file) pairs -- triaging required a separate `frob
verify dispose`-read round trip mid-land.

Fix: _quarantine_undisposed_summary (src/frob/app/ticket_runner/
_land_cmd.py) now enumerates each undisposed finding's deduplicated
(rule_id, file) identity inline, capped at 8 with a "+N more" suffix
beyond that, so the first log line alone is enough to triage. Added to
scope with reason: this is the one source file that emits the log line
this ticket exists to enrich.
...
```
 ...
 uv.lock                                            |     2 +-
 2817 files changed, 284113 insertions(+), 15065 deletions(-)
```

### Evidence
- `tests/unit/test_land_cmd_quarantine.py::TestQuarantineUndisposedSummary::test_enumerates_rule_and_file_identities_inline` (pytest node id, verified passing when recorded)
- `tests/unit/test_land_cmd_quarantine.py::TestQuarantineUndisposedSummary::test_deduplicates_repeated_rule_file_pairs` (pytest node id, verified passing when recorded)
- `tests/unit/test_land_cmd_quarantine.py::TestQuarantineUndisposedSummary::test_caps_named_identities_and_suffixes_the_remainder` (pytest node id, verified passing when recorded)
