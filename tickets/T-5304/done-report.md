...
LAUNCH-family checklist findings needed to be reported without ever
blocking a gate, and the owner explicitly rejected a never-fail flag
bolted onto WARN. Added Severity.ADVISORY as a fourth, real Severity
member in frob.findings, wired it through [gates.severity] override
parsing, diagnostic rendering (note-severity), and the frob check
summary/exit-code path so n_err (the sole exit-status/quarantine/
ratchet gate) never counts an advisory finding. docs/modules/gates.md
gained a dedicated Advisory (T-5304) section mirroring the existing
Unresolved (T-1664) contract, and every switch found via `Severity.`
grep in the touched files got a positive-control test proving an
all-advisory result exits 0 while still surfacing its count.
...
```
 ...
 tickets/T-5300/ticket.md                           |   270 +
 tickets/T-5301/ticket.md                           |   171 +
 tickets/T-5302/ticket.md                           |   213 +
 tickets/T-5303/ticket.md                           |   298 +
 ...
 tickets/T-5376/ticket.md                           |    45 +
 tickets/T-5377/ticket.md                           |    44 +
 tickets/T-5378/ticket.md                           |    43 +
 tickets/T-5379/ticket.md                           |    44 +
 ...
 uv.lock                                            |     2 +-
 3898 files changed, 545600 insertions(+), 23282 deletions(-)
```

### Evidence
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_advisory_is_a_distinct_severity_value` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_advisory_count_counts_only_advisory_violations` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_advisory_maps_to_note_not_warning` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_advisory_only_family_result_exits_zero_and_is_not_quarantined` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_errors_still_fail_the_family_regardless_of_advisory` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityAdvisory::test_advisory_count_shown_as_its_own_term_not_folded_into_warn` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestGatesSummaryAdvisory::test_advisory_only_summary_line_prints_the_advisory_count_and_exits_zero` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityOverridesAdvisory::test_advisory_string_accepted_as_a_per_rule_override_value` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_gates_summary.py::TestSeverityOverridesAdvisory::test_apply_severity_overrides_re_severities_to_advisory` (pytest node id, verified passing when recorded)
