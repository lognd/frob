...
A frob:waive with only reason= prose on a T-5335 sqlfluff performance
finding cannot prove the flagged query's plan was actually checked --
only that someone asserted it was fine. Reusing INV008's own "declared
but unproven" two-part obligation shape (a real bound artifact required,
not just a text claim) rather than inventing a new directive keeps this
one class of waiver honest without adding a second obligation DSL:
frob:waive's already-generic attribute grammar accepts explain="..."
with zero frob.graph.dsl changes. Rule-id scope is pkgutil-discovered
from frob.sql submodules (T-5308's own convention) so a later SQL rule
family opts in without editing this gate. Not wired into frob check's
live dispatch yet -- same "gate lands before registration" posture
T-5335's own sqlfluff plugin and T-5334's extraction substrate both
already carry; a later leaf does the wiring.
...
```
 ...
 tests/fixtures/sql/explain/proven_waiver.py        |     5 +
 tests/fixtures/sql/explain/unproven_waiver.py      |     5 +
 ...
 uv.lock                                            |     2 +-
 4276 files changed, 721986 insertions(+), 23508 deletions(-)
```

### Evidence
- `tests/unit/test_sql_explain_obligation.py::test_bare_waiver_with_no_explain_attr_fires` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_explain_attr_naming_a_missing_file_still_fires` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_explain_attr_outside_the_artifact_dir_still_fires` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_explain_attr_naming_a_real_artifact_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_non_obligated_rule_is_never_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_non_waive_edge_is_ignored` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_real_fixture_pair_unproven_fires_proven_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_sql_explain_obligation.py::test_discover_obligation_rule_prefixes_includes_default` (pytest node id, verified passing when recorded)
