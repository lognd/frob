...
CI run 35951365410's windows-latest job failed 4 tests on POSIX-path
assumptions plus a 5th (same test_token_usage.py file, missed in initial
triage) on Windows clock-tick granularity. Fixed all 5, each root-caused
against the real code (not guessed):
- CONFIGPATH001 never checked PurePosixPath().is_absolute().
- test_bulk.py's rel_path.split("/")[-1] no-ops on Windows (rel_path is
  str(Path), backslash-separated) -- use Path(rel_path).name.
- test_token_usage.py's fake-adapter fixture keyed a raw POSIX literal
  against a Path(...)-converted lookup.
- _token_usage.py's budget-expiry check used strict > against a captured
  deadline; with a 0.0s budget, several tiny-line iterations can land on
  the identical time.monotonic() tick on Windows, so > never tripped --
  changed to >=.
- test_docarch_structural.py compared Violation.file (str(Path(rel)))
  against a bare forward-slash literal.
All 5 measured directly on the real Windows mirror (winrun, not reasoned
about): before the fix, all 5 failed with the exact CI symptom; after,
all 40 tests across the 4 touched test files pass, confirmed on two
consecutive Windows runs for the timing-sensitive fix. Also 12/12 (and
40/40 full set) pass on Linux, no regressions.
...
```
 ...
 tickets/T-5476/ticket.md                           |    74 +
 tickets/T-5477/ticket.md                           |   117 +
 tickets/T-5478/ticket.md                           |   143 +
 ...
 uv.lock                                            |   265 +-
 4539 files changed, 795063 insertions(+), 23553 deletions(-)
```
...
- `tests/gates/test_docarch_structural.py::TestDocarch002RatchetSeverity::test_baselined_finding_stays_warn_new_one_errors` (pytest node id, verified passing when recorded)
