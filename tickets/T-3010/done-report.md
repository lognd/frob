...
Adds T-3004 section 6's milestone-scoped closure: a sixth, opt-in closure
rule (check_milestone_closure) over the existing V-model graph kernel so
a configuration can bind a partial architecture as long as every
uncovered obligation is a declared, reasoned gap, never silent omission.
Mirrors vmodel_check's existing PyO3/gate shape end to end (Rust unit
tests, Python PyO3-boundary tests, gate-level tests) and generalises
frob.lang._support's FacetState.KNOWN_GAP registry pattern to V-model
artifacts (MilestoneGap/MILESTONE_GAP_REGISTRY) per the owner's dispatch
instruction. Verified with cargo test (227 passed) and the targeted
pytest suite (19 new tests passed) plus a clean vmodel/milestone_closure
gate sweep against the touched files.
...
```
 ...
 docs/design/registry/check-coverage.yaml           |  1462 +-
 ...
 src/frob/gates/__init__.py                         |   872 +-
 ...
 strata-core/src/graph/vmodel/closure.rs            |   161 +-
 strata-core/src/graph/vmodel/mod.rs                |    11 +-
 strata-core/src/lib.rs                             |   195 +-
 ...
 tests/gates/test_milestone_closure.py              |   171 +
 ...
 tests/unit/strata/test_vmodel_check.py             |    72 +
 ...
 uv.lock                                            |   265 +-
 4361 files changed, 748249 insertions(+), 23542 deletions(-)
```

### Evidence
- `tests/gates/test_milestone_closure.py::TestMilestoneGapNodeIds::test_refuses_a_blank_reason` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_quiet_on_no_design_dir` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_quiet_no_vmodel_declarations` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_check.py::TestMilestoneClosureCheck::test_fires_on_an_ungapped_uncovered_obligation` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_check.py::TestMilestoneClosureCheck::test_quiet_when_gap_is_declared` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_check.py::TestMilestoneClosureCheck::test_quiet_when_covered_by_a_verifying_test` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_check.py::TestMilestoneClosureCheck::test_partial_coverage_three_of_five_with_two_gaps_passes` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_check.py::TestMilestoneClosureCheck::test_same_configuration_with_an_undeclared_missing_obligation_fails` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_fires_msclose001_on_an_ungapped_uncovered_obligation` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_quiet_when_gap_is_declared` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_partial_milestone_with_gap_passes_the_ungapped_case_still_fires` (pytest node id, verified passing when recorded)
- `tests/gates/test_milestone_closure.py::TestMilestoneClosureGate::test_default_milestone_reads_frob_toml` (pytest node id, verified passing when recorded)
