...
CI run 35951365410's windows-latest job errored collecting tests/unit/
test_ticket_verbs_wait.py: import fcntl at module scope (fcntl is
POSIX-only) raised ModuleNotFoundError before either TestDispatchWait
test's own os.name == "nt" runtime skip could execute. Guarded the
import with try/except ImportError, the exact pattern frob.process.
_lock.portable_flock_acquire already uses for the same platform gap
(T-2918/T-2934/T-3506). Measured directly on the real Windows mirror
(winrun, not reasoned about): before the fix, collection failed with the
identical ModuleNotFoundError; after the fix, all 5 tests in the file
pass (the two POSIX-only tests skip cleanly via their existing runtime
check). Also verified 5/5 pass on Linux (no regression).
...
```
 ...
 tests/unit/test_ticket_verbs_wait.py               |   204 +
 ...
 tickets/T-draft-4f61e007/ticket.md                 |    53 +
 ...
 uv.lock                                            |   265 +-
 4537 files changed, 790204 insertions(+), 23547 deletions(-)
```
...
- `tests/unit/test_ticket_verbs_wait.py::TestDispatchWait::test_budget_exhausted_names_holder` (pytest node id, verified passing when recorded)
