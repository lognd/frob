...
Why: the DSL directive story (T-4703) measured 30,915 directive lines and 1,712 runs of
3+ consecutive ones; the reverse frob:tests copy on the production symbol is the single
biggest source, so it is removed first, before any later leaf's stack counts are measured.

What: dsl.parse_directives now reorients a test-side frob:tests declaration (comment in a
test-shaped file, naming the production symbol) into the one canonical
implementation->test edge shape, so TDD001's backwards check never sees the raw
test-side orientation. A cross-file lint (frob.graph._redundant_test_declarations) flags
a still-production-side declaration as redundant, naming delete vs. move. Self-reference
(a test naming itself) stays a valid, deliberately-tolerated parse; TDD001 continues to
flag it for ordering purposes, which is a different, non-contradictory concern from parsing.

Left open: the Tier-A fix (actual delete/move rewrite) needs frob.gates._fix_engine_text,
outside this leaf's src/frob/graph/* scope -- filed as T-draft-1c09ccb2 rather than
widening scope.
...
```
 ...
 src/frob/graph/__init__.py                         |   173 +-
 ...
 uv.lock                                            |     2 +-
 2818 files changed, 284535 insertions(+), 15059 deletions(-)
```

### Evidence
- `tests/unit/graph/test_dsl.py::TestTestSideDeclarationReorientation::test_test_side_declaration_is_reoriented_to_canonical_shape` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestTestSideDeclarationReorientation::test_legacy_production_side_declaration_is_unchanged` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestTestSideDeclarationReorientation::test_self_referential_test_side_declaration_is_left_alone` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestTestSideDeclarationReorientation::test_reoriented_edge_is_never_backwards_by_tdd001s_own_predicate` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestRedundantTestDeclarationLint::test_both_sides_declared_is_flagged_as_deletable` (pytest node id, verified passing when recorded)
