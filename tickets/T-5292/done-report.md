...
Ran `frob ticket reconcile --strip-stale-fields --apply` against this
repo's own live ledger (T-5305 landed first, coordinator confirmed
done). It exposed two real bugs in T-5305's own implementation the
tmp_path-fixture tests never caught:

1. It wrote EVERY stale ticket unconditionally, including ones leased
   to another live worktree, hitting write_ticket's own T-1617
   ownership guard mid-batch (refused on the very first leased ticket
   alphabetically) instead of completing the run. Fixed: leased
   tickets are now skipped and reported separately
   (skipped_leased_ticket_ids), never written -- a new unit test
   (test_leased_ticket_is_skipped_not_written) proves it with a real
   `git worktree add` + `transition(...IN_PROGRESS)` fixture, not a
   faked lease file.
2. The CLI dispatcher never called commit_full_ledger_change at all --
   a real --apply run against this repo's ledger (101 tickets touched)
   left the worktree dirty with no commit, which would DirtyMain-block
   a concurrent `frob ticket land`. Fixed: routed through
   commit_full_ledger_change, same --no-commit opt-out shape every
   other ledger-mutating verb already has.

Verification of the actual acceptance criterion (WARNING disappears
from a fresh `frob ticket list`): confirmed directly -- before the run,
`frob ticket list 2>&1 | grep carries unknown` matched ~80 ids; after,
it matches EXACTLY the 4 tickets that were actively leased to another
worktree at run time (T-5308, T-5309, T-5311, T-5334) -- correctly
skipped, not a residual bug. Committed the 101-ticket ledger healing
itself as its own commit, separate from the code fix commits.

tests/gates_suite/test_tick.py::TestTick008UnknownLedgerFields::
test_real_repo_ledger_is_tick008_clean itself is NOT included in this
ticket's bound evidence and remains red: it asserts zero TICK008
findings, but the 4 actively-leased tickets above still carry their
stale fields (by design -- reconcile must never write a ticket another
worktree currently owns) until they land and can be reconciled. This
is the SAME structural shape T-5396's done-report already disclosed
for SYS119/test_sys_gate_zero_violations: a zero-tolerance test against
a live, constantly-churning multi-agent ledger cannot realistically
stay green without either waiting for in-flight tickets to land or
violating a real safety invariant -- not something this ticket's own
fix can or should force. Re-running --strip-stale-fields --apply once
T-5308/T-5309/T-5311/T-5334 land will close the remaining gap.

Hand-checked the two things the dry-run does not measure (T-draft-
c7aa1ed2): no new file-reading module added (existing _reconcile.py
extended only); no docs touched.

### Changed
```
 .claude/hooks/_root_write_guard_lib.py             |    24 +-
 .claude/hooks/_shellscan.py                        |    93 +
 ...
 .claude/hooks/sync-claude-config.py                |   236 +-
 ...
 .../registry/capability-via-ratchet.lock.json      |   130 +-
 docs/design/registry/check-coverage.yaml           |  1452 +-
 ...
 docs/modules/webapp-a11y.md                        |    84 +
 ...
 src/frob/gates/__init__.py                         |   835 +-
 ...
 src/frob/gates/_taint_gate.py                      |    40 +-
 ...
 src/frob/tickets/_models.py                        |   484 +-
 ...
 src/frob/webapp/_a11y_substrate.py                 |   343 +
 ...
 .../webapp/websec2xx/django_compliant/manage.py    |    10 +
 .../webapp/websec2xx/django_compliant/settings.py  |    13 +
 .../webapp/websec2xx/django_violating/manage.py    |    10 +
 ...
 .../webapp/websec2xx/flask_compliant/app.py        |    13 +
 .../websec2xx/flask_compliant/requirements.txt     |     2 +
 .../webapp/websec2xx/flask_violating/app.py        |     9 +
 ...
 tickets/T-3004/ticket.md                           |    15 +-
 tickets/T-3008/ticket.md                           |    11 +-
 tickets/T-3010/ticket.md                           |    11 +-
 tickets/T-3020/ticket.md                           |    50 -
 tickets/T-3022/ticket.md                           |    29 +-
 tickets/T-3032/done-report.md                      |  2399 ++
 tickets/T-3032/ticket.md                           |    99 +-
 tickets/T-3047/ticket.md                           |    11 +-
 tickets/T-3048/ticket.md                           |    11 +-
 tickets/T-3049/ticket.md                           |    11 +-
 tickets/T-3053/ticket.md                           |    39 +-
 tickets/T-3063/ticket.md                           |    30 +-
 tickets/T-3067/ticket.md                           |    30 +-
 tickets/T-3068/ticket.md                           |    11 +-
 ...
 tickets/T-4516/ticket.md                           |    59 +
 tickets/T-4518/done-report.md                      |  3294 +++
 tickets/T-4518/ticket.md                           |    63 +
 ...
 tickets/T-4560/ticket.md                           |    66 +
 tickets/T-4561/done-report.md                      |  3417 +++
 tickets/T-4561/ticket.md                           |    97 +
 ...
 tickets/T-5034/ticket.md                           |    52 +
 tickets/T-5035/done-report.md                      |  2816 +++
 tickets/T-5035/ticket.md                           |    67 +
 ...
 tickets/T-5274/ticket.md                           |    61 +
 tickets/T-5275/done-report.md                      |  3805 ++++
 tickets/T-5275/ticket.md                           |    53 +
 ...
 uv.lock                                            |     2 +-
 4077 files changed, 662756 insertions(+), 23504 deletions(-)
```
