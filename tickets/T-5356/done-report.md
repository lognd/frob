...
Adds the WEBSEC authz substrate's route/handler owner-check walker
(WEBSEC401 shape): scan_python_handlers (Flask/FastAPI/Django, via a
caller-parsed tree-sitter Node) and scan_rails_controller (Rails, text-
regex, no ruby grammar available) flag a handler that reaches an ORM
lookup/filter call with no current_user/request.user/g.user reference
anywhere in its body, same documented-heuristic posture as PERF008.
Never imports frob.lang/frob.gates (webapp is a leaf layer with no
allowed imports until T-5307 lands) -- takes a caller-parsed Node/text
and returns its own AuthzFinding model instead, same shape T-5313's
_a11y_substrate.py already established. Builds on T-5302's
FrameworkKind/detect_frameworks without re-detecting frameworks.
Express is out of scope: FrameworkKind has no EXPRESS member and adding
one is a _detect.py change outside this ticket's declared scope.
...
```
 ...
 tests/fixtures/webapp/websec4xx/fastapi/main.py    |    20 +
 tests/fixtures/webapp/websec4xx/flask/app.py       |    20 +
 ...
 tests/test_lang_conformance_gate.py                |    94 +-
 ...
 tests/unit/test_webapp_websec_authz_substrate.py   |   136 +
 ...
 tickets/T-5356/done-report.md                      |  4006 ++++
 ...
 uv.lock                                            |     2 +-
 3988 files changed, 630637 insertions(+), 23491 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_websec_authz_substrate.py::TestAuthIdentifierVocabulary::test_matches_ticket_vocabulary` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestFlask::test_owner_filtered_route_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestFlask::test_unfiltered_route_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestFastAPI::test_owner_filtered_handler_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestFastAPI::test_unfiltered_handler_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestDjango::test_owner_filtered_view_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestDjango::test_unfiltered_view_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestRailsController::test_owner_filtered_action_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestRailsController::test_unfiltered_action_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestNonHandlerHelpersAreIgnored::test_plain_helper_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestRailsController::test_non_controller_file_yields_nothing` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_authz_substrate.py::TestUnsupportedLanguage::test_non_python_language_is_an_error` (pytest node id, verified passing when recorded)
