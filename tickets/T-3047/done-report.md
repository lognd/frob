...
Adds T-3004 section 8's typed review/decision records to the V-model
graph kernel: a new `review` node kind (requires commit+reason,
provenance as data) and a required `reason` attr on `decision` nodes
(owner review superseding the prior schema-doc note that decision
carried no required attr). `review` is now also a legal `decides` edge
source. Verified with 4 new Rust unit tests (construction refusal, a
reasoned decision constructs, review requires both attrs, and a
supersedes round-trip through the graph via forward_closure) plus 7 new
Python-level tests at the same PyO3 boundary the other node/edge kinds
are exercised through. Two pre-existing tests in T-3010's
tests/unit/strata/test_vmodel_check.py break as a direct, measured
consequence of the decision-reason requirement; that file is held by
T-3010's live scope lease and cannot be claimed here -- the exact fix is
saved as a patch (scratchpad/t3047-fix-for-t3010-test_vmodel_check.patch)
for whoever owns that file once the lease frees. docs/strata/vmodel.md
update filed as T-6511 for the same reason.
...
```
 ...
 strata-core/src/graph/vmodel/closure.rs            |   172 +-
 strata-core/src/graph/vmodel/mod.rs                |   143 +-
 strata-core/src/lib.rs                             |   302 +-
 ...
 tests/unit/strata/test_vmodel_review_decision.py   |   159 +
 ...
 uv.lock                                            |   265 +-
 4461 files changed, 774862 insertions(+), 23552 deletions(-)
```

### Evidence
- `tests/unit/strata/test_vmodel_review_decision.py::TestDecisionNodeRequiresReason::test_fires_a_construction_error_on_a_bare_decision_node` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestDecisionNodeRequiresReason::test_quiet_when_reason_is_present` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestReviewNodeRequiresCommitAndReason::test_fires_on_a_bare_review_node` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestReviewNodeRequiresCommitAndReason::test_quiet_when_commit_and_reason_are_present` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestSupersedesRoundTrip::test_reasoned_supersedes_edge_between_two_decisions_round_trips` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestSupersedesRoundTrip::test_supersedes_edge_missing_reason_is_a_construction_error` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_vmodel_review_decision.py::TestSupersedesRoundTrip::test_review_node_can_decide_an_artifact` (pytest node id, verified passing when recorded)
