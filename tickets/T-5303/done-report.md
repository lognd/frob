...
A11Y contrast-ratio, target-size, outline:none, and hidden-text checks
(T-5146-3, T-5146-4) and SEO's markup checks (T-5147-3) all need .css and
.scss files reachable through frob.lang.parse_file before any
tree-sitter-query rule in those stories can run. Confirmed
tree-sitter-language-pack bundles "css" and "scss" grammars, then wired
both extensions into _EXTENSION_TABLE, added the _WALKERS/COMMENT_TYPES
dispatch entries, and wrote a thin _walk_css.py walker (top-level
rule_set -> CLASS symbol, top-level $-prefixed SCSS variable -> CONST
symbol; CSS has no visibility keyword so every symbol is public).
Positive-control fixtures (sample.css, sample.scss) assert a rule set and
a nested selector both parse to a non-empty raw tree.
Filed T-5386 for the remaining capability/dup/docblock FACETS
wiring (mirrors zig's T-3513 precedent) and registered it as a disclosed
KNOWN_GAP in _support.py so LANG003 does not flag it.
...
```
 ...
 src/frob/lang/_walk_css.py                         |   161 +
 ...
 uv.lock                                            |     2 +-
 3902 files changed, 545742 insertions(+), 23245 deletions(-)
```

### Evidence
- `tests/test_lang_css.py::TestCss::test_css_is_a_supported_language` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestCss::test_parse_css_sample` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestCss::test_positive_control_raw_tree_is_non_empty` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestCss::test_top_level_rule_set_is_a_class_symbol` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestCss::test_nested_rule_inside_media_is_not_a_top_level_symbol` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_scss_is_a_supported_language` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_parse_scss_sample` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_positive_control_raw_tree_is_non_empty_nested_selector` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_dollar_variable_is_a_const_symbol` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_top_level_rule_set_is_a_class_symbol` (pytest node id, verified passing when recorded)
- `tests/test_lang_css.py::TestScss::test_line_comment_is_recognized` (pytest node id, verified passing when recorded)
