...
T-5306 completes the T-5141 WEBSEC injection/output-encoding corpus by
claiming the two remaining reserved rule ids (WEBSEC107/108) that
T-5307's substrate left unclaimed: Rails' explicit unescaped-output ERB
tag `<%== %>` (distinct from WEBSEC106's `.html_safe`/`raw(...)` regex
pair) and PHP's raw-superglobal echo, a language the substrate never
reads at all. Both are TEXT-REGEX, same posture as the substrate's own
WEBSEC104-106 finders, since neither shape has a tree-sitter grammar
frob ships.

Rather than hand-editing the leased `_taint_gate.py` call site, this
leaf exposes the module-level `websec_findings(root, frameworks)` hook
T-5308's discovery mechanism (landed on dev mid-ticket) picks up
automatically via `pkgutil.iter_modules` -- proven end-to-end by a real
`taint_gate()` call over the planted `webesc108_positive` fixture in
`test_taint_gate_discovers_websec_xss_hook`, not just the module's own
standalone function.
...
```
 ...
 .../headers_log/websec120_negative/views.py        |     5 +
 .../headers_log/websec120_positive/manage.py       |     1 +
 .../headers_log/websec120_positive/views.py        |     2 +
 ...
 uv.lock                                            |     2 +-
 4180 files changed, 695710 insertions(+), 23508 deletions(-)
```

### Evidence
- `tests/unit/test_websec_xss.py::test_taint_gate_discovers_websec_xss_hook` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_xss_findings_fixture[webesc107_positive-WEBSEC107-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_xss_findings_fixture[webesc107_negative-WEBSEC107-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_xss_findings_fixture[webesc108_positive-WEBSEC108-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_xss_findings_fixture[webesc108_negative-WEBSEC108-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_xss_findings_no_framework_short_circuits` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_findings_discovery_hook_emits_violation` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_xss.py::test_websec_findings_discovery_hook_empty_frameworks_short_circuits` (pytest node id, verified passing when recorded)
