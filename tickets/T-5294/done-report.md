...
TEST002/3/4/7/9's package/system/pair-level detectors all derive a
lookup "base" node id from a TESTS edge's `src` endpoint only, but a
`frob:tests` directive can bind with the source symbol on EITHER
endpoint depending on which of the two documented conventions was used
(_unit_test_edges's own T-0336 docstring already explains this for
TEST001/002's per-symbol lookup, but TEST003/004/007/009's
package/system/pair-level aggregation, the macro-stand-in check in
_edge_has_execution_evidence, and the per-case count in _case_count all
still checked src alone). This silently collapsed every "directive
above the test" edge to the c/cpp-style "count as 1" structural
fallback instead of its real collected-case count, undercounting
exactly the shape these tests exercise. Fixed 3 call sites (TEST003/
TEST004/TEST007's own package-index build) to use the dual-endpoint
_unit_test_edges instead of the now-dead target-only _test_edges
(removed), and fixed _case_count/_edge_has_execution_evidence/
_pair_covered to try both src and target for node-id and macro-symbol
matching, mirroring the pattern _edge_has_execution_evidence's plain
node-id check already used. tests/gates_suite/test_test_gate.py is
fully green (119/119). Found-while-working: COV006 + rule-fixability
regressions unrelated to this diff, filed separately (T-5341).
...
```
 ...
 src/frob/gates/__init__.py                         |   649 +-
 ...
 tickets/T-draft-382f12d0/ticket.md                 |    37 +
 ...
 uv.lock                                            |     2 +-
 3724 files changed, 476990 insertions(+), 22357 deletions(-)
```

### Evidence
- `tests/gates_suite/test_test_gate.py::TestTestGate::test_test003_satisfied_by_parametrized_case_with_dot_in_case_id` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestTestGate::test_test003_satisfied_by_parametrized_test_node_id` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestTestGate::test_test004_passes_with_enough_e2e` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestTestGate::test_test002_parametrized_test_counts_each_case` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestTestGate::test_test003_satisfied_by_proptest_macro_block` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestConventionUnitBinding::test_test009_satisfied_by_e2e_edge` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_test_gate.py::TestPairLevelIntegration::test_test007_passes_when_boundary_tested` (pytest node id, verified passing when recorded)
