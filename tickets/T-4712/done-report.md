...
Why: leaf 2 (T-4711) documented owner decision 2 -- a wrap break may land only at
token separation, never inside a symbol path, an anchor, or a quoted value -- but
the wrapper itself (_wrap_cut_point) still cut at any space, including inside a
quoted vitest-style describe title. This leaf narrows that one wrapper.

What: _wrap_cut_point now skips any candidate cut inside the quoted TARGET span
(_quoted_target_span, computed once per directive and shifted into each wrap
iteration's local coordinates). A reason="..." attribute value is deliberately
left wrappable at its own word boundaries -- that is this module's entire
purpose, and folding always reconstructs the exact original value regardless of
where the split lands, so nothing is lost by leaving it wrappable. Every existing
T-0991/T-4179/T-4477 case, plus round-trip and idempotence, stays green.

Left open: while implementing, found that a quoted target immediately followed
by a bare "# noqa: E501" trailing suffix on the same physical line fails to
parse (_parse_attrs' tail-comment regex needs a preceding space the quoted-
target regex's own \s* already consumed) -- filed as T-5270 since the
fix lives in src/frob/graph/dsl.py, outside this leaf's declared scope.
...
```
 ...
 src/frob/gates/_fmt_directives.py                  |   154 +-
 ...
 uv.lock                                            |     2 +-
 2919 files changed, 351424 insertions(+), 15576 deletions(-)
```

### Evidence
- `tests/test_gates_fmt_directives.py::TestQuotedTargetNeverSplitByWrap::test_pre_change_word_boundary_cut_would_have_split_the_quoted_target` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestQuotedTargetNeverSplitByWrap::test_quoted_target_is_never_split_across_physical_lines` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestUnbreakableSingleNodeIdStillUnsplittable::test_single_long_node_id_produces_one_noqa_suffixed_line` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestCanonicalizeTextIdempotentTwice::test_second_format_paths_run_reports_zero_changes` (pytest node id, verified passing when recorded)
