...
## THIS LEAF: policy (`policy`/`forbid`/`confine`/`mediate`/`at`/`require`/`arg`/`enables`/`rationale`/`component`/`use`/`call`/`import`) -- _policy.py: lexical/AST rules over Python source evaluated by a refinement-monotonicity checker, not graph facts. Read alongside T-4951, which wires forbid call/forbid import enforcement, and T-3920 item 1 (a consumer gerrymandered globs because they could not find `confine`).
