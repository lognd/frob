...
Real functional bug, not just a coverage-test nitpick: `--clean-sweep-worktrees`
and `--ticket-wait-s` parsed into argparse's Namespace correctly, but
src/frob/app/_config_external.py never listed either field in
`_BOOL_FLAGS`/`_FLOAT_FIELDS` (or `_AD_HOC_FORWARDED_FIELDS`), so
`_build_external_config_kwargs` never carried the parsed value into
AppConfig -- both fields always kept their class defaults
(`False`/`None`) regardless of what the user passed on the CLI, even
though downstream code (src/frob/app/clean_runner.py:116,
src/frob/app/ticket_runner/__init__.py:924) already reads
`cfg.clean_sweep_worktrees`/`cfg.ticket_wait_s` as if the flag worked.

Fix: added `clean_sweep_worktrees` to `_BOOL_FLAGS` and `ticket_wait_s`
to `_FLOAT_FIELDS` (matching their real AppConfig field types).

Measured: tests/unit/test_app_config_flag_coverage.py and
tests/unit/test_flag_coverage_gate.py (16 tests total) pass clean.
...
```
 ...
 uv.lock                                            |     2 +-
 2858 files changed, 297562 insertions(+), 15115 deletions(-)
```
