...
body_changes:
- mode: append
  reason: BUG002 cannot get a real repro for a win32-only code branch on this Linux
    host
  actor: logan
  at: '2026-09-23'
  old_length: 1938
...
anchor_reason: null
land_commit: null
worktree: /home/logan/projects/frob/.claude/worktrees/t-5384
branch: t-5384
---
...
frob:waive BUG002 reason="this fix only adds a sys.platform==\"win32\" branch, dead code on the Linux CI/dev host this evidence runs on; a real before/after repro that exercises the changed branch is impossible here -- the fix was code-reviewed against Windows CreateProcess/PATHEXT semantics and against the pre-existing win32 PATHEXT resolver in gitio.run_argv (T-3799) instead, and the 19 existing tests in both files were re-run to confirm zero behavior change on this host (the branch is provably unreached)."