...
Confirmed root cause: _write_fake_dotnet/_write_fake_unity (and their
crashing variants) wrote a #!/bin/sh script for the fake dotnet/Unity
test double; Windows has no shebang interpreter and CreateProcess only
auto-appends .exe (not the rest of PATHEXT) to an extensionless argv[0],
so the fixture was unexecutable there, surfacing as
TestingError.DotnetRunFailed instead of the test's expected outcome.

Fix is platform-branched, not a blanket rewrite: dotnet already has a
win32 PATHEXT-aware resolver in gitio.run_argv (_resolve_win32_executable,
T-3799) that finds a bare "dotnet" -> "dotnet.cmd" via shutil.which, so
writing dotnet.cmd on win32 is sufficient there. Unity's fixture instead
returns its own path directly via UNITY_PATH (never resolved through
run_argv's bare-name lookup), so its win32 branch must itself be named
Unity.cmd -- the returned path IS what gets spawned. Both branches emit
CRLF batch-file equivalents of the POSIX shell logic (parse args, copy
the named static fixture to the requested output path, exit with the
given code). Verified all 19 tests in both files still pass unchanged
on this Linux host (the sys.platform=="win32" branch is dead code here,
by construction) -- could not execute-verify the Windows branch itself
on this host, so the fix is code-reviewed against Windows CreateProcess/
PATHEXT semantics, not test-run on Windows.
...
```
 ...
 uv.lock                                            |     2 +-
 3920 files changed, 577846 insertions(+), 23425 deletions(-)
```
...
- `tests/unit/test_dotnet_runner.py::TestRunDotnetTests::test_requested_id_missing_from_results_is_err` (pytest node id, verified passing when recorded)
- `tests/unit/test_unity_batchmode.py::TestRunUnityBatchmode::test_maps_passing_and_failing_ids` (pytest node id, verified passing when recorded)
- `tests/unit/test_unity_batchmode.py::TestRunUnityBatchmode::test_requested_id_missing_from_results_is_err` (pytest node id, verified passing when recorded)
