...
scripts/fleet_status.py::_ticket_readiness_lines carried two production-side
`frob:tests` directives citing tests/unit/coordinator_suite/test_fleet_report.py's
TestPrintTicketReadiness methods, but that same test class already carries its
own `frob:tests scripts/fleet_status.py::_ticket_readiness_lines` directives --
a reverse binding for the identical (test, symbol) pair. TEST010 (T-4710)
flags this as redundant and recommends deleting the production-side copy,
which is what frob-arch's ARCH001/ARCH103 scan was surfacing as a
"ticket_readiness" finding on frob's own repo. Fix: delete the two
production-side frob:tests lines, keeping the test-side declaration (the
binding is preserved once, not lost).
...
```
 ...
 uv.lock                                            |     2 +-
 3897 files changed, 545377 insertions(+), 23246 deletions(-)
```
