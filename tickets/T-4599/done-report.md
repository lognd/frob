...
`frob ticket land` had a silent ~111s span between rapid `--files` scoping
and the worktree auto-sync onto main with no phase-transition logging, so a
stalled land looked identical to a slow-but-healthy one from the log alone.
Added phase-marker logging around the auto-rebase/worktree-sync and
post-land sweep-dispatch entry points so each transition is now visible.
Merged dev (T-4607's already-landed `.git/`-metadata-path fix in
`_rapid_sweep.py`/`_fix_engine_sync.py`, identical in substance to this
worktree's own earlier T-4599 copy of the same fix) by taking dev's
already-landed wording and dropping the duplicate.
...
```
 ...
 tickets/T-draft-5481fcad/ticket.md                 |    27 +
 tickets/T-draft-707b4040/ticket.md                 |    31 +
 tickets/T-draft-fd7b1d91/ticket.md                 |    28 +
 ...
 uv.lock                                            |     2 +-
 2787 files changed, 269843 insertions(+), 14905 deletions(-)
```

### Evidence
- `tests/unit/test_land_auto_rebase.py::TestAutoSyncWorktreeOntoMain::test_logs_a_phase_marker_before_starting_the_merge` (pytest node id, verified passing when recorded)
- `tests/unit/test_land_cmd_drain_wiring.py::TestPostLandSweepDispatchPhaseMarker::test_entry_marker_logged_unconditionally` (pytest node id, verified passing when recorded)
- `tests/unit/test_land_cmd_drain_wiring.py::TestPostLandSweepDispatchPhaseMarker::test_entry_marker_logged_even_on_dry_run` (pytest node id, verified passing when recorded)
