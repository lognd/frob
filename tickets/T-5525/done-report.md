...
TODO001 (and every other diff-dependent gate) hard-failed with a load
failure whenever `git merge-base HEAD main` had no local `main` ref to
resolve against -- exactly the shape a shallow CI checkout produces
(`actions/checkout` default depth fetches origin/main as a remote-tracking
ref, never a local branch). Fixed `_merge_base` to retry against
`origin/<base>` when the first attempt fails and `base` has no `/` in it
already (so an explicit remote ref is never double-tried). Added two
tests: one that reproduces the shallow-checkout shape directly (a
remote-tracking ref exists, the local branch does not) and asserts the
diff still resolves, and one confirming an already-remote-qualified bad
ref still surfaces GitFailed (no silent second spawn).
...
```
 ...
 tickets/T-5525/ticket.md                           |    74 +
 tickets/T-5526/ticket.md                           |    37 +
 tickets/T-5527/ticket.md                           |    81 +
 tickets/T-5528/ticket.md                           |    82 +
 tickets/T-draft-0f8e3bc0/ticket.md                 |    70 +
 tickets/T-draft-f728e1be/ticket.md                 |    51 +
 ...
 uv.lock                                            |   265 +-
 4629 files changed, 857549 insertions(+), 23559 deletions(-)
```

### Evidence
- `tests/test_gitio.py::TestWorkingDiff::test_falls_back_to_origin_base_when_local_ref_is_absent` (pytest node id, verified passing when recorded)
- `tests/test_gitio.py::TestWorkingDiff::test_no_fallback_when_base_already_names_a_remote` (pytest node id, verified passing when recorded)
