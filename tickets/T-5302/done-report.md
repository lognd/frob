...
Every WEBSEC/COMPLY/A11Y/SEO/WEBPERF rule leaf this epic adds (T-5301's
reserved rule ids) needs one shared answer before it does any real work:
is this repo a web app at all? frob.webapp.detect_frameworks gives that
answer with a pure file-presence/manifest-sniff check, no tree-sitter
parse -- a plain Python CLI repo returns the empty frozenset and every
downstream rule short-circuits to "not relevant", per the owner
directive verbatim.

frob.sql ships as an empty stub in the same leaf, not a follow-up,
because SYS100/layering would otherwise refuse every sibling leaf that
imports frob.webapp or frob.sql before this leaf lands. Both packages
are registered as strata capabilities (folded into the existing `core`
node, same shape as `perf`/`policy`) and as arch.layering leaf layers
(gates/app may import from them) in this same commit.

Positive-control fixtures cover all nine recognized frameworks plus a
plain-Python-CLI negative control; all eleven asserts pass.
...
```
 ...
 src/frob/webapp/_detect.py                         |   207 +
 ...
 tests/unit/test_webapp_detect.py                   |    50 +
 ...
 uv.lock                                            |     2 +-
 3961 files changed, 604998 insertions(+), 23464 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_missing_root_is_empty` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[nextjs-nextjs]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[vite-vite]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[django-django]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[flask-flask]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[fastapi-fastapi]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[rails-rails]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[laravel-laravel]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[sveltekit-sveltekit]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_positive_control[astro-astro]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_detect.py::test_detect_frameworks_plain_python_cli_is_empty` (pytest node id, verified passing when recorded)
