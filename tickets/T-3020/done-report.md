...
T-3020 -- Register frob.narrative as a strata component; close its
SELFAUDIT001/SYS003 waivers

PREMISE CHECK (important): the ticket described `frob.narrative` as
having "no strata component/node of its own at all" and asked to
register one plus a `cli -> narrative` flow, mirroring `frob.refactor`.
Before doing that work, I checked `design/frob.strata` directly and
found T-3029 had ALREADY landed a full `node narrative : trusted { ... }`
(with `fs.read`/`fs.write` via `src/frob/narrative/_cli.py`) plus
`flow f_t3029_cli_narrative : cli -> narrative`,
`flow f_t3029_gates_narrative : gates -> narrative`, and
`flow f_t3029_testsuite_narrative : testsuite -> narrative` -- all
committed after T-3020's own description was written. My first attempt
(adding a second `node narrative` + a duplicate `cli -> narrative` flow)
correctly failed `frob check`'s SYS004 ("Two facts of the same kind
share an id") the instant I ran it, which is exactly how this stale
premise surfaced -- reverted that attempt (`git checkout -- design/
frob.strata`) rather than force a workaround. This ticket's actual
remaining work was therefore just: remove the two now-dead waivers, and
prove they are genuinely dead (no live SELFAUDIT001/SYS003 finding
reappears), not silently forgiven.

WHAT changed:

- src/frob/gates/_narrative_blocks.py
  Removed the `frob:waive SELFAUDIT001` directive on
  `narrative_blocks_gate`'s `full.read_text(...)` call -- its `fs.read`
  is already declared on the `gates` strata node's `may "fs.read"`
  via-list (that via-list already names this exact file). Added
  `frob:ticket T-3020` and a second `frob:tests` binding.

- src/frob/__main__.py
  Removed the `frob:waive SYS003` directive on `_dispatch_narrative`'s
  `from frob.narrative._cli import ...` -- the `cli -> narrative` flow
  it names already exists (T-3029's `f_t3029_cli_narrative`). Added
  `frob:ticket T-3020` and a `frob:tests` binding.

- docs/commands/narrative.md
  AFFECT001 requires `narrative_blocks_gate`'s affects()-closure doc to
  move in the same diff that changes the function's body -- added a
  paragraph under "NARR001 (the detector)" recording why the
  SELFAUDIT001 waiver is gone (T-3029's prior node registration).

- tests/test_narrative_blocks.py (new class `TestT3020WaiversRemoved`)
  Two regression tests asserting neither
  `src/frob/gates/_narrative_blocks.py` nor `src/frob/__main__.py`'s
  source contains the string `frob:waive SELFAUDIT001` /
  `frob:waive SYS003` respectively -- a drift lock against either
  waiver silently regrowing.

WHY: both waivers' own `reason=`/`follow_up="T-3020"` text said they
were blocked purely by `design/frob.strata`'s lease state at the time
(T-2989 held it through T-3014's own work window) -- not that the
underlying capability gap was permanent. T-3029 closed that gap in the
meantime by registering the `narrative` node and its flows for an
unrelated reason (its own SYS102/SYS103/SYS106 findings on
`_cli.py`), so by the time this ticket's agent picked up the waivers,
removing them was a pure cleanup with no new strata modeling required.

How the acceptance criterion is proven:
[1] "GIVEN frob.narrative's fs.read/fs.write and its cli -> narrative
    import WHEN frob check --only sys/selfaudit runs THEN neither the
    SELFAUDIT001 waiver on narrative_blocks_gate nor the SYS003 waiver
    on _dispatch_narrative exists in source, and both remain unnecessary
    because design/frob.strata's narrative node and cli -> narrative
    flow (T-3029) declare the capability directly" -- proven by:
      ...
      tests/system/test_frob_self_model.py::TestFrobSelfModel::test_sys_gate_zero_violations
        (the real, whole-repo `frob.graph.build_graph` + `frob.gates.
        sys_gate` pass CI runs -- proves the removal produces zero SYS
        violations, not just that the waiver text is gone)

Test run: PYTHONPATH=<worktree>/src python -m pytest
tests/test_narrative_blocks.py -q -> SUITE-RESULT: exitstatus=0
collected=7 failed=0 (5 pre-existing + 2 new)
Also ran tests/system/test_frob_self_model.py together with
tests/test_narrative_blocks.py earlier in the drive: 17/17 passed.

ruff check/format: clean on all 3 touched source/test files.

Gate check: frob check --only gates --files
src/frob/gates/_narrative_blocks.py,src/frob/__main__.py,docs/commands/
narrative.md,tests/test_narrative_blocks.py --base dev -- iterated 3
times as real findings surfaced and were fixed in place:
  1st pass (before removing waivers, with a duplicate node/flow attempt
    still applied): SYS004 fatal ("Two facts of the same kind share an
    id") -- caused by my own premature node/flow duplication; reverted
    design/frob.strata entirely rather than force past it.
  2nd pass (waivers removed, no design/frob.strata edit): gate:AFFECT
    FAIL (1 error) -- narrative_blocks_gate's body changed but its
    affects()-closure doc was untouched; fixed by editing
    docs/commands/narrative.md.
  3rd pass (doc fixed, regression test added): gate:AFFECT and gate:SYS
    both clean (not in the FAIL list); the only new finding was a
    DOCARCH001 WARNING (gate:DOCARCH still shows "pass" -- warn-only) on
    the new test class's docstring, fixed by rewording it to state
    utility rather than change-narrative, in the final commit.
  No SELFAUDIT001 or SYS003 finding appears anywhere in any of the 3
  runs' full output for these files. All FAIL rows in each run's
  per-category gate summary are the same pre-existing repo-wide debt
  seen across every other ticket's check this session, unrelated to
  this diff.

Evidence: bound (frob ticket accept + 3x frob ticket evidence
--accepts 1 --base-ref dev), all 3 node ids collected in the rebuilt
pytest collection cache (14449 node ids).

Commits (branch t-3020, dev merged in cleanly -- T-4553 had landed on
dev in the meantime, confirmed present, no conflicts on any file this
ticket touched):
  b7448c528 fix(gates): remove stale SELFAUDIT001/SYS003 waivers on frob.narrative
  06dd48c3a test(narrative): lock the removed SELFAUDIT001/SYS003 waivers
  b1cddf8f3 docs(test): reword TestT3020WaiversRemoved's docstring to state utility
  (plus scope/accept/evidence ticket-ledger commits)

Final HEAD: b1cddf8f3033bf8a934d16d1bafce693befab6b0

Filed: none. The premise mismatch (node already registered by T-3029)
was caught and corrected within this ticket's own work, not filed
separately -- no new gap was discovered, an already-closed one was
simply not yet reflected in this ticket's own stale description.
...
```
 ...
 src/frob/gates/_narrative_blocks.py                |  14 +-
...
### Evidence
- `tests/test_narrative_blocks.py::TestT3020WaiversRemoved::test_narrative_blocks_gate_has_no_selfaudit001_waiver` (pytest node id, verified passing when recorded)
- `tests/test_narrative_blocks.py::TestT3020WaiversRemoved::test_dispatch_narrative_has_no_sys003_waiver` (pytest node id, verified passing when recorded)
