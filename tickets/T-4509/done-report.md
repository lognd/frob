...
Unity support (T-4501 editor detection, T-4508 batchmode evidence, T-4517
csharp collection, the asmdef strata component model) had no end-to-end
proof any of it actually composes against a real Unity checkout. Added a
fixture project (`tests/fixtures/unity_sample/`, two asmdefs, a
MonoBehaviour with a coroutine, an Editor-only script, NUnit [Test] and
[UnityTest] methods) plus `tests/system/test_unity_e2e.py` exercising
scaffold + asmdef-node matching, dead-code exemption of lifecycle/
coroutine methods, expected capability findings (net/fs-write/exec/eval),
a clean `frob vet` run, and NUnit+UnityTest collection. Merged the shared
`docs/guides/unity.md` (dev already carries T-4501/T-4508's sections as
a placeholder naming T-4509 as "its own future addition") by appending
this ticket's own section set rather than overwriting dev's.
...
```
 ...
 .claude/hooks/frob-suggest.py                      |    77 +-
 ...
 tests/test_hook_frob_suggest.py                    |   216 +-
 ...
 tickets/T-5124/ticket.md                           |    75 +
 tickets/T-5125/ticket.md                           |    72 +
 tickets/T-5126/ticket.md                           |    84 +
 ...
 tickets/T-5136/ticket.md                           |    56 +
 tickets/T-5137/ticket.md                           |    63 +
 tickets/T-5138/ticket.md                           |    59 +
 ...
 tickets/T-draft-06eaaa96/ticket.md                 |    48 +
 tickets/T-draft-330aa06d/ticket.md                 |    31 +
 ...
 uv.lock                                            |     2 +-
 2792 files changed, 270335 insertions(+), 14905 deletions(-)
```

### Evidence
- `tests/system/test_unity_e2e.py::TestUnityScaffoldAndAsmdefNodes::test_scaffold_succeeds_and_nodes_match_the_two_asmdefs` (pytest node id, verified passing when recorded)
- `tests/system/test_unity_e2e.py::TestMonoBehaviourAndCoroutineAreNotDeadCode::test_dead_symbols_gate_reports_zero_findings` (pytest node id, verified passing when recorded)
- `tests/system/test_unity_e2e.py::TestExpectedCapabilityFindings::test_runtime_script_finds_net_fs_write_and_exec` (pytest node id, verified passing when recorded)
- `tests/system/test_unity_e2e.py::TestExpectedCapabilityFindings::test_editor_script_finds_eval_and_fs_write_tagged_editor_only` (pytest node id, verified passing when recorded)
- `tests/system/test_unity_e2e.py::TestExpectedCapabilityFindings::test_frob_vet_cli_runs_cleanly_against_the_fixture` (pytest node id, verified passing when recorded)
- `tests/system/test_unity_e2e.py::TestNUnitAndUnityTestCollection::test_both_test_attribute_families_collected` (pytest node id, verified passing when recorded)
