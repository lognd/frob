...
Why: T-4710 lints a redundant production-side frob:tests declaration (TEST010's
own generic frob:tests-flavored MalformedDirective catch-all) but never shipped
the Tier-A fix -- a detector nobody can act on mechanically is still manual
burn-down. This wires the fix so the finding is actually resolvable.

What: fix_test010_redundant_test_declaration matches the T-4710 finding's own
reason text and applies DELETE/MOVE/REFUSE per its already-computed remedy.
MOVE rewrites the relocated line's own target from the test symbol to the
production symbol (the site changed, so the grammar's orientation convention
requires it) -- still the same (src, target) pair, never invented. Registered
in TIER_A_HANDLERS and _KNOWN_RULE_FIXABILITY. Verified end-to-end through the
real production entry points (build_graph, test_gate, apply_tier_a_fixes), not
the handler called directly, and the MOVE case's edge set is asserted
byte-for-byte identical before/after.

Left open: none in scope. Pre-existing, unrelated repo drift (test_every_
emitted_rule_literal_is_known failing on ~20 other rule ids already missing
from _KNOWN_GATE_RULES on dev, plus several TEST002-009 test_gate failures)
confirmed present on dev before this change -- not introduced here.
...
```
 ...
 src/frob/app/explore_runner.py                     |    49 +-
 ...
 src/frob/gates/_fix_engine_text.py                 |   548 +-
 ...
 tests/gates_suite/test_fix_engine.py               |   124 +
 ...
 tests/test_gates_fix_engine.py                     |   142 +
 tests/test_gates_fmt_directives.py                 |   295 +-
 ...
 uv.lock                                            |     2 +-
 2985 files changed, 397106 insertions(+), 15915 deletions(-)
```

### Evidence
- `tests/test_gates_fix_engine.py::TestFixTest010RedundantTestDeclaration::test_delete_case_fires_test010_and_fix_removes_the_line` (pytest node id, verified passing when recorded)
- `tests/test_gates_fix_engine.py::TestFixTest010RedundantTestDeclaration::test_move_case_fires_test010_and_fix_relocates_the_line` (pytest node id, verified passing when recorded)
- `tests/test_gates_fix_engine.py::TestFixTest010RedundantTestDeclaration::test_dangling_target_refuses_rather_than_guessing` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_fix_engine.py::TestFixEngineTierABatch2::test_tier_a_handlers_dict_covers_every_batch_rule` (pytest node id, verified passing when recorded)
