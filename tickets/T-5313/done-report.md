...
Every A11Y rule the web-app epic still owes (missing alt, missing aria-label,
skipped heading levels, missing <html lang>) needs the same html/vue/jsx
tree-sitter query shapes; this ticket lands the one shared query library so
no individual rule module hand-rolls its own node walk. Kept as a leaf-layer
Node-in/Result-out module (no frob.lang import) because [arch.layering]
still declares webapp = [] -- callers already allowed to import both lang
and webapp do the raw_tree() call and hand this module the parsed tree.
Positive and negative fixture controls exist per grammar family (html, jsx,
vue) and per query shape, plus an unsupported-language negative case.
...
```
 ...
 tests/unit/test_webapp_a11y_substrate.py           |   151 +
 ...
 uv.lock                                            |     2 +-
 3979 files changed, 613909 insertions(+), 23464 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_a11y_substrate.py::test_elements_with_attribute_html_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_elements_missing_attribute_html_negative_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_elements_with_attribute_jsx_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_elements_missing_attribute_jsx_negative_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_heading_sequence_vue_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_heading_sequence_vue_negative_control_skips_a_level` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_html_lang_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_html_lang_negative_control_missing_attribute` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_elements_with_attribute_unsupported_language_errors` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_attribute_queries_unsupported_language_errors[elements_with_attribute-args0]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_substrate.py::test_attribute_queries_unsupported_language_errors[elements_missing_attribute-args1]` (pytest node id, verified passing when recorded)
