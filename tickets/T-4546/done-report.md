...
flatten the remaining single-child verb groups: agent env, worktree
sweep (in _core.py) and narrative move (frob/narrative/_cli.py + _root.py)

### WHAT changed, per file

- src/frob/app/agent_runner.py: `_build_agent_parser` now sets
  `agent_command` default to `"env"` (help text updated to say so); new
  private `_normalize_agent_argv(argv)` inserts the implied `env` token
  ahead of `argv` when missing (leaves an explicit `env`/help flag
  alone); `run()` calls `parser.parse_args(_normalize_agent_argv(argv))`.

- src/frob/_cli_parsers/_core.py: `_add_agent_parser` (the `--help`-only
  discovery tree, real dispatch bypasses it entirely per its own
  docstring) mirrors the same default + updated help/description text,
  WITHOUT duplicating `env`'s `path` positional onto the group parser
  (see WHY below for why that specific mirroring technique doesn't work
  here). `_add_worktree_parser` is UNCHANGED (see the "worktree exclusion"
  note below).

- src/frob/narrative/_cli.py: `add_narrative_parser`'s `narrative_sub`
  is no longer `required=True`; `p.set_defaults(narrative_subcommand=
  "move")`; help/description text updated to document the flattening.

- src/frob/__main__.py: `_dispatch_narrative` now normalizes `argv[1:]`
  through a new private `_normalize_narrative_argv` (same shape as
  `_normalize_agent_argv`) before parsing, inserting the implied `move`
  token when it is missing. `_normalize_narrative_argv` lives HERE, not
  in `frob.narrative._cli` (see WHY below).

- tests/test_worktree_guard.py: `TestAgentRunnerEnv.test_unrecognized_
  subcommand_falls_through_to_usage_error` (T-1400) asserted the EXACT
  OPPOSITE of this ticket's new contract (bare `frob agent` used to exit
  1 with a usage error printed to stderr) -- replaced with
  `test_bare_invocation_defaults_to_env`, which asserts bare `frob agent`
  now prints the same `export FROB_WORKTREE=...` line `frob agent env`
  does.

- tests/unit/test_cli_single_child_groups.py: new `TestAgentGroupFlattened`
  (4 tests) and `TestNarrativeGroupFlattened` (3 tests); module docstring
  rewritten to explain why `agent`/`narrative` are tested at their real
  dispatch layer (`agent_runner`/`__main__`) rather than only against
  `_build_parser()` (which is real dispatch for `claude`/`natives` but
  help-discovery-only for `agent`/`narrative`/`worktree`), and why
  `worktree` is excluded.

### WHY

T-4522 flattened `claude sync` and `natives build`. `agent env` and
`narrative move` are the two remaining groups that were STILL genuinely
single-child. `worktree sweep` is NOT flattened: real dispatch
(`frob.app.worktree_runner._build_worktree_parser`) has grown a `remove`
and a `release-lease` child alongside `sweep` since this ticket was
filed (both post-date the ticket's own body, which was written against
the OLD state) -- it is no longer a single-child group, and defaulting
bare `frob worktree` to `sweep` (a worktree-REMOVING action) would
silently pick one of three very different actions on the user's behalf.
This is called out explicitly in the ticket title's own parenthetical
("worktree sweep (in _core.py)") which is now stale; verified by reading
`frob.app.worktree_runner.run`'s actual 3-way dispatch before touching
anything, per the "verify the premise" discipline.

`agent`/`narrative`'s real dispatch bypasses the standard `_build_parser()`
tree entirely: `__main__._dispatch` special-cases both (`agent` ->
`frob.app.agent_runner.run(argv)`; `narrative` -> `_dispatch_narrative`,
which builds its OWN fresh parser via `frob.narrative._cli.
add_narrative_parser`). `_core.py`'s `_add_agent_parser` and `_root.py`'s
narrative registration are `--help`-discovery-only trees, unlike
`claude`/`natives` where `_build_parser()` IS the real dispatch path.

T-4522's own mirroring technique -- add the child's flags directly onto
the group parser (`natives_p.add_argument("--path", ...)`,
`claude_p.add_argument("--check", ...)`) -- works only for OPTIONAL
FLAGS. `env`'s `path` and `move`'s `file`/`line` are POSITIONAL
arguments; mirroring a bare positional directly onto a parser that also
has `add_subparsers()` collides with the subparsers action's own
positional slot -- argparse tries to match the first token against the
subcommand choices FIRST, so `frob agent /some/worktree` would fail with
`invalid choice: '/some/worktree'` (verified empirically with a small
`argparse` repro before committing to this approach; the failure mode
is silent-until-invoked, not a syntax error, so it is easy to ship
broken). The fix instead normalizes `argv` BEFORE `parse_args` is ever
called -- insert the implied child token when the first token isn't
already the child name or a help flag -- at the real dispatch boundary
each verb's caller owns (`agent_runner.run`, `__main__._dispatch_narrative`).

`_normalize_narrative_argv` lives in `__main__.py`, not `frob.narrative
._cli` alongside `add_narrative_parser`/`run_narrative_command`: it
started out in `_cli.py` as a new PUBLIC export (its only caller,
`_dispatch_narrative`, is in a different module/node), which tripped
SELFAUDIT001 SYS110 (a public symbol not declared in the `narrative`
node's `attr interface=[...]` list in `design/frob.strata`) -- fixing
that properly means adding it to that interface list, but
`design/frob.strata` is LIVE-leased by T-4111 and the scope-lease guard
refused adding it. Moving the helper into `__main__.py` (already the
`cli` node, already `_dispatch_narrative`'s own module) as a PRIVATE
function needs no design-file interface change at all, and is arguably
the more honest home for it anyway: it is purely an argv-shape adapter
for this one dispatch call site, not part of narrative's own migration
logic.

### How the acceptance criterion is proven

Criterion: "GIVEN frob agent, frob worktree, frob narrative WHEN invoked
without a subverb THEN each runs what its single child ran, the two-word
spelling stays as a documented alias, and tests/unit/test_cli_single_
child_groups.py covers all five groups."

- `frob worktree` is deliberately excluded (see WHY) -- it is no longer
  a single-child group, so the premise does not hold for it; this is
  documented in the test module's own docstring, not silently dropped.
- `frob agent`/`frob narrative`: covered by `TestAgentGroupFlattened`/
  `TestNarrativeGroupFlattened` (7 tests total) plus
  `TestAgentRunnerEnv.test_bare_invocation_defaults_to_env` in
  tests/test_worktree_guard.py -- each asserts the flat form and the
  two-word form resolve to IDENTICAL dest values (or, for
  `test_run_dispatches_bare_invocation_to_env`, identical real
  `_run_env` calls), and each has a `test_help_notes_alias` asserting
  "T-4546" appears in `--help` output, matching `claude`/`natives`'
  T-4522 precedent exactly.
- Manually verified end to end through the real `python -m frob` entry
  point: `frob agent` and `frob agent env .` produce byte-identical
  `export FROB_WORKTREE=...`/`export FROB_AGENT=1` output.
...
All 79 tests across tests/unit/test_cli_single_child_groups.py,
tests/test_worktree_guard.py, tests/test_narrative_migrate.py,
tests/test_narrative_blocks.py pass locally (PYTHONPATH=<worktree>/src
pytest, serial). ruff check/format and `ty check` clean on every touched
file.

### Pre-ready checklist results

1. `frob check --only sys --files <all touched src files>`: 0 SYS/
   SELFAUDIT findings attributable to my files after the T-4546 T-1
   fix-up commit (initially caught its own SYS110 -- `normalize_
   narrative_argv` public-but-undeclared-in-interface -- fixed by moving
   it private into `__main__.py`, see WHY). The 1 remaining SELFAUDIT001
   error is T-4563's own pre-existing SYS111 pending-auto-accept state
   (unrelated, present before this ticket too).
2. `frob check --only arch --files <all touched src files>`: 0 errors,
   19 warnings, same baseline as before my change (this check was run
   before the `_normalize_narrative_argv` move into `__main__.py`; a
   pre-move run flagged an "abstraction-opportunity" NOTE, not a
   warning/error, that `_normalize_agent_argv`/`normalize_narrative_argv`
   shared a `(list[str]) -> list[str]` signature -- left as two separate
   private, single-caller helpers in two different CLI subsystems on
   purpose; unifying two one-line functions across modules for a shared
   signature alone would be a needless new coupling).
3. `frob check --only coverage --files <all touched src files>`: 0
   COV00x errors attributable to my files after adding the `frob:ticket
   T-4546` edges COV002 flagged on `_dispatch_narrative` and the new
   `TestAgentRunnerEnv`/`test_bare_invocation_defaults_to_env` symbols.
   Remaining COV/DRIFT/DOCARCH findings in the logs (`_LANG_CHOICES`,
   `main`, `TestSweepWorktreesLiveProcess`) are pre-existing, on symbols
   this ticket never touched (T-3233's/other tickets' territory).
4. `ruff check`/`ruff format --check` and `ty check`: clean on all 6
   touched files.
5. Cross-ticket: `git diff --name-only dev...HEAD` = __main__.py,
   _cli_parsers/_core.py, app/agent_runner.py, narrative/_cli.py,
   test_worktree_guard.py, test_cli_single_child_groups.py (plus ticket
   files). None leased by another in-progress ticket
   (`grep -l <path> .git/frob-leases/*.json` empty for all six, checked
   from ROOT).
6. kind=ux, not bug -- BUG002 repro-designation N/A.
7. tickets/T-4546/ticket.md's `id:` == T-4546; no stray draft dirs
   (T-5127 below is a genuinely new draft, not a promotion).
8. Evidence bound AFTER the last code commit (d72d04800) -- the only
   commit after evidence binding is the ticket-ledger's own evidence
   record, not a code change.

### Commits (this worktree, dev..HEAD)

- 9008a44d3 feat(cli): flatten agent/narrative single-child verb groups (T-4546)
- d72d04800 fix(cli): move narrative argv normalization to __main__, fix COV gates
- fa7a29b53 chore(tickets): record evidence for T-4546
(plus several chore(tickets) scope/unblock/start-transition commits)

### Filed (out-of-scope discovery, kind=bug)

- T-5127 "ops natives group leaf missing --path flag: T-4522/
  T-4520 landing-order drift" -- `tests/unit/test_cli_group_parity.py::
  TestOpsGroupParity::test_every_ops_leaf_matches_its_flat_twin[natives]`
  fails on dev (pre-existing, unrelated to this ticket: `natives`/`ops
  natives` are T-4522's/T-4520's own territory, `_misc.py` is not in
  T-4546's scope and I never touched it). Confirmed the failure via
  `git diff --name-only dev...HEAD` containing none of the involved
  files, and via `pytest.. -k natives` failing identically before any of
  my commits. Verify this promotes to a real id on main before citing it
  further.

### Scope notes for the coordinator

- Ticket's original scope (`_core.py`, `narrative/_cli.py`, `_root.py`,
  the test file) covered only the HELP-discovery trees for agent/
  worktree and undersold what narrative's real dispatch needed. Added,
  via `frob ticket scope --add` (none leased elsewhere at add time):
  `src/frob/app/agent_runner.py` and `src/frob/app/worktree_runner.py`
  (real dispatch parsers, distinct from `_core.py`'s help-only trees),
  `src/frob/__main__.py` (owns `_dispatch_narrative`'s raw-argv boundary,
  and now `_normalize_narrative_argv`), and `tests/test_worktree_guard.py`
  (T-1400's test asserted the exact opposite of this ticket's new
  contract and had to be updated).
- `src/frob/app/worktree_runner.py` was added to scope but NOT edited --
  left in place after discovering `worktree` is no longer single-child
  (see WHY); flagging this so the coordinator doesn't read the scope
  list as claiming worktree work that was not done.
- Wanted to add `design/frob.strata` to scope (to declare `normalize_
  narrative_argv` in the narrative node's `interface=` list) but it is
  LIVE-leased by T-4111 -- refused by the scope-lease guard. Worked
  around this by keeping the helper private inside `__main__.py` instead
  of exporting it from `frob.narrative._cli` (see WHY) -- no design-file
  edit needed at all, and arguably the better home for it regardless.
- No tests skipped. No waivers added.
...
```
 ...
 tests/test_worktree_guard.py                       |  31 +-
 ...
 tickets/T-4853/ticket.md                           |  84 ++
 tickets/T-4855/ticket.md                           |  84 ++
 tickets/T-4856/ticket.md                           |  85 ++
 ...
 tickets/T-4914/ticket.md                 | 125 +++
 tickets/T-4915/done-report.md            |  38 +
 tickets/T-4915/ticket.md                 | 345 ++++++++
 tickets/T-4916/ticket.md                 |  87 ++
 tickets/T-4917/ticket.md                 |  29 +
 tickets/T-4918/ticket.md                 |  99 +++
 tickets/T-draft-1f0f55cb/ticket.md                 |  65 ++
 tickets/T-4919/ticket.md                 |  99 +++
 tickets/T-4920/ticket.md                 | 123 +++
 tickets/T-4921/ticket.md                 |  29 +
 tickets/T-4922/ticket.md                 |  34 +
 tickets/T-draft-36c347fe/ticket.md                 |  68 ++
 tickets/T-4923/ticket.md                 | 113 +++
 tickets/T-4924/ticket.md                 |  31 +
 tickets/T-4925/ticket.md                 |  99 +++
 tickets/T-4926/ticket.md                 |  53 ++
 tickets/T-4927/ticket.md                 |  99 +++
 tickets/T-4928/ticket.md                 | 107 +++
 tickets/T-4929/ticket.md                 | 106 +++
 tickets/T-4930/ticket.md                 |  52 ++
 tickets/T-4931/ticket.md                 |  52 ++
 tickets/T-4932/ticket.md                 |  35 +
 tickets/T-4933/ticket.md                 |  45 +
 tickets/T-4934/ticket.md                 |  74 ++
 tickets/T-4935/ticket.md                 | 120 +++
 tickets/T-4936/ticket.md                 |  77 ++
 tickets/T-4937/ticket.md                 |  61 ++
 tickets/T-4938/ticket.md                 |  78 ++
 tickets/T-5127/ticket.md                 |  29 +
 tickets/T-4940/ticket.md                 |  37 +
 tickets/T-4941/ticket.md                 |  37 +
 tickets/T-4942/ticket.md                 |  99 +++
 tickets/T-4943/ticket.md                 |  99 +++
 tickets/T-4944/ticket.md                 |  99 +++
 tickets/T-4945/ticket.md                 | 100 +++
 tickets/T-4946/ticket.md                 |  50 ++
 tickets/T-4947/ticket.md                 |  68 ++
 tickets/T-4948/ticket.md                 | 106 +++
 tickets/T-4949/ticket.md                 |  73 ++
 ...
 tickets/archive/T-0971/ticket.md                   |  28 +-
 ...
 uv.lock                                            |   2 +-
 970 files changed, 81582 insertions(+), 2540 deletions(-)
```

### Evidence
- `tests/unit/test_cli_single_child_groups.py::TestAgentGroupFlattened::test_normalize_inserts_implied_env` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestAgentGroupFlattened::test_bare_agent_defaults_to_env` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestAgentGroupFlattened::test_run_dispatches_bare_invocation_to_env` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestAgentGroupFlattened::test_help_notes_alias` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestNarrativeGroupFlattened::test_normalize_inserts_implied_move` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestNarrativeGroupFlattened::test_bare_narrative_defaults_to_move` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_single_child_groups.py::TestNarrativeGroupFlattened::test_help_notes_alias` (pytest node id, verified passing when recorded)
- `tests/test_worktree_guard.py::TestAgentRunnerEnv::test_bare_invocation_defaults_to_env` (pytest node id, verified passing when recorded)
