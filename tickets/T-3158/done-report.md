...
DRIFT001 (scripts/fleet_status.py::worktrees_touching_ticket) was
GENUINELY LIVE: `frob check` on main reproduced it identically. Root
cause was real content drift, not a digest-noise false positive: T-3128
added a third dispatch branch to worktrees_touching_ticket (a worktree
that structurally started no ticket at all also gets the weaker
scope-only check), but docs/guides/coordinator-scripts.md's T-2665
paragraph still described only a two-way dispatch ("started this
ticket" vs "everything else"). Fixed by adding a T-3128 paragraph
documenting the third branch, then frob ack'd after re-verifying the
full function body against the anchor text.

DOC006 (tickets/T-3155/ticket.md) did NOT reproduce: a fresh `frob
check` run against main found zero DOC006 findings anywhere in that
file. Every path/file reference the ticket body cites
(src/frob/gates/__init__.py, src/frob/app/ticket_runner/_close_cmd.py,
src/frob/tickets/__init__.py, and both test files) resolves as a
tracked file, confirmed via `git ls-files --error-unmatch`, and all
existed already at the sweep's own commit (6f04de4c8990). The sweep's
own attribution engine found this UNATTRIBUTED (no candidate commits) --
consistent with pre-existing/stale-baseline residue, not a genuinely
new finding the T-3139 land caused. Stated explicitly per the ticket's
own disposition instructions: this identity is closed as pre-existing
residue the rolling baseline had not recorded, not fixed (nothing to
fix -- it does not reproduce).
