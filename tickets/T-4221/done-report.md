...
T-4221 land repair -- COV002 frob:tests on time_stable_offset_s / time_stable_gate

WHAT changed:
- src/frob/gates/_inv.py: added `# frob:tests` directives above the two
  public symbols COV002 flagged as missing them:
  - time_stable_offset_s (line 590): bound to
    tests/gates_suite/test_invariant.py::TestTimeStableGate.test_fails_once_clock_advances_past_horizon
    -- the only test whose spawned fixture module actually imports and
    calls time_stable_offset_s() (the other TestTimeStableGate cases
    exercise time_stable_gate's dispatch but their synthetic fixtures do
    not call this function). Kept public: its own docstring documents it
    as the function external test authors are meant to call, so making
    it private would break its documented contract, not just satisfy the
    gate.
  - time_stable_gate (line 713): bound to all four TestTimeStableGate
    cases (test_fails_once_clock_advances_past_horizon,
    test_stays_quiet_when_still_passing_at_horizon,
    test_baseline_failure_is_skipped_not_double_reported,
    test_no_time_stable_anchor_is_silent), which already existed and
    covered the function but had no frob:tests edge pointing back to it.
- Merged dev (clean, no conflicts: dev only added/edited ticket dirs
  unrelated to this file).

WHY: land was refused by COV002 -- both public symbols had frob:doc but
no frob:tests edge.

Proof:
- PYTHONPATH=.../src python -m pytest tests/gates_suite/test_invariant.py -k TimeStable -q
  SUITE-RESULT: exitstatus=0 collected=4 failed=0
- Evidence rebound to all 9 pre-existing node ids via `frob ticket
  evidence T-4221 ... --base-ref dev` AFTER the final commit (idempotent,
  content unchanged, HEAD stayed at 344cf7b69 -- confirmed via git status
  --short clean and git log -1).

Commits:
- (merge commit) Merge branch 'dev' into t-4221
- 344cf7b691ddded72cee1b2dc93987e61ef05fc1 fix(gates): add frob:tests to
  time_stable_offset_s and time_stable_gate (HEAD)

## Pre-READY checks

frob check --only coverage --files src/frob/gates/_inv.py --base dev:
  FAIL gate:COV (11 errors, all pre-existing/waived, none on line 590 or
  713 of _inv.py -- the remaining COV007 hits on _inv.py are on
  DIFFERENT, already-waived private symbols such as
  _DOC_INVARIANT_MARKER_RE, _inv003_doc_violations, etc., untouched by
  this fix) / FAIL gate:DRIFT, gate:DSL, gate:TODO (unrelated, other
  files) / pass gate:DOCARCH, gate:PLACE, gate:PROFILE, gate:WAIVE / pass
  claude-config-drift. Zero COV002 findings on time_stable_offset_s or
  time_stable_gate in this run -- confirmed no line in the raw output
  names either symbol.

ruff check src/frob/gates/_inv.py: All checks passed!
ty check src/frob/gates/_inv.py: All checks passed!
...
```
 ...
 tickets/T-4221/done-report.md                      | 735 +++++++++++++++++++
 ...
 uv.lock                                            |   2 +-
 634 files changed, 53316 insertions(+), 2062 deletions(-)
```

### Evidence
- `tests/gates_suite/test_invariant.py::TestTimeStableGate::test_fails_once_clock_advances_past_horizon` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_invariant.py::TestTimeStableGate::test_stays_quiet_when_still_passing_at_horizon` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_invariant.py::TestTimeStableGate::test_baseline_failure_is_skipped_not_double_reported` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_invariant.py::TestTimeStableGate::test_no_time_stable_anchor_is_silent` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl_invariant_property.py::TestTimeStableAttrs::test_valid_kind_and_horizon_always_parses_together` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl_invariant_property.py::TestTimeStableAttrs::test_kind_with_no_horizon_is_malformed` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl_invariant_property.py::TestTimeStableAttrs::test_horizon_with_no_kind_is_malformed` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl_invariant_property.py::TestTimeStableAttrs::test_unknown_kind_is_malformed` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl_invariant_property.py::TestTimeStableAttrs::test_malformed_horizon_is_rejected` (pytest node id, verified passing when recorded)
