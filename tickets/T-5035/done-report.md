...
CI run 35476139324 on dev: 4 land-lock guard tests simulated a held lock
from WITHIN the test process's own pid, but T-2406's self-pid exclusion
now correctly treats a same-pid holder as self-held, not foreign, so the
guards under test stopped refusing. Fixed each to hold the lock from a
real, separate `python3 -c` subprocess instead, polling a sibling
`.ready` marker rather than a fixed sleep.

Also found (and fixed, in-scope): T-3612 moved `refuse_if_land_in_
progress`'s DEFAULT (non-`whole_land`) probe from `land.lock` onto the
bare, pid-less `tickets.lock` ledger-splice lock -- so the reconcile/
set_parent/set_priority guard tests were locking the wrong file
regardless of pid; they now hold `TICKETS_LEDGER_LOCK_REL`, matching
what `_ledger_splice_flock_probe` actually reads. `test_drain.py`'s
failure was unrelated to both: its `_probe_land_once` fake was missing
the `whole_land` keyword-only parameter real callers now forward.
...
```
 ...
 uv.lock                                            |     2 +-
 2786 files changed, 270020 insertions(+), 14961 deletions(-)
```
...
- `tests/test_tickets_parent.py::TestSetParentLandInProgressGuard::test_refuses_and_writes_nothing_while_land_lock_held` (pytest node id, verified passing when recorded)
- `tests/test_tickets_priority.py::TestSetPriorityLandInProgressGuard::test_refuses_and_writes_nothing_while_land_lock_held` (pytest node id, verified passing when recorded)
