...
T-4760: Derived wrappers -- scaffold apply regenerates Makefile and make.bat
as managed blocks of run calls, plus a drift gate

WHAT changed (per file)

- src/frob/scaffold/_managed.py
  - `_marker_begin`/`_marker_end`/`_extract_region` now take a `target=`
    keyword and pick the comment token by extension (`::` for `.bat`,
    `#` otherwise) -- a `.bat` file cannot use `#` as a comment (it is
    an unknown command and aborts the script).
  - New `_defines_stamp` (does a Makefile already assign a `STAMP`
    variable) and a guard on `_apply_text_block`: the `makefile-core-shim`
    block is skipped (reported, not silently applied) the FIRST time it
    would be inserted into a Makefile with no `STAMP` -- fixes the
    measured cpp defect (applying `core: $(STAMP)` to a Makefile with no
    STAMP expanded to an unconditional prerequisite, no venv to gate a
    native build behind). Guarded so it only fires on first insertion,
    not on every later re-check of an already-shimmed file (which
    legitimately mentions `$(STAMP)` without assigning it).
  - New `_wrapper_entry_names(root)`: every `[commands]` entry declared
    in `root`'s `frob.toml`, unioned with the four native-default names
    (`test`/`lint`/`format`/`check`).
  - New `_makefile_wrapper_block_content`/`_makebat_wrapper_block_content`:
    render the wrapper-target managed-block body for each wrapper file,
    one `<name>:` target / `if "%1"=="<name>"` branch per
    `_wrapper_entry_names(root)`, each a single `frob run <name>` call.
  - `apply_managed_blocks` now also builds and applies these two blocks
    (`makefile-wrapper-targets` on `Makefile`, `makebat-wrapper-targets`
    on `make.bat`) through the same idempotent `_apply_text_block`
    machinery the static blocks use -- `make.bat` did not exist as a
    concept before this ticket; `apply` creates it fresh via the
    existing "absent -> create" path.

- src/frob/gates/_wrapper_drift.py (new)
  - `wrapper_drift_gate(root)`: WRAP001 (a wrapper target/branch expands
    more than one command inline), WRAP002 (a wrapper target/branch
    names an entry that is not declared in `[commands]` and is not a
    native default), WRAP003 (Makefile and make.bat managed-block name
    sets differ). Reuses `frob.scaffold._managed._wrapper_entry_names`
    for the known-name set (no duplicated logic) and `_extract_region`
    for parsing the managed-block text. No-op (empty tuple) on a
    project that has never run `frob scaffold apply` -- it checks an
    existing artefact for staleness, it does not require one to exist.

- src/frob/scaffold/data/shared/cpp/Makefile.j2
  - Kept `configure`/`build`/`release`/`install` (genuine cmake
    bootstrap). Removed the old `format`/`lint`/`check` hand-written
    targets and the 15-line `publish` target (re-implemented the
    version-bump/commit-message rule in shell -- a release verb belongs
    in `frob release`, not a second home in make). Added the
    `makefile-wrapper-targets` managed block, pre-baked with the same
    content `apply` would generate with no `[commands]` table yet
    declared (check/format/lint/test).

- src/frob/scaffold/data/shared/python/Makefile.j2
  - Left UNCHANGED from its prior content. The audit (SCAFFOLD-AUDIT.md
    5.1) already calls this file the reference implementation (it never
    shipped a duplicated command sequence to begin with -- `frob
    format`/`frob check` are named directly, not wrapped), and
    `tests/unit/test_scaffold_project.py::test_scaffolded_docs_make_targets_exist_in_makefile`
    (leased by T-4629, out of this ticket's scope) actively asserts the
    T-3400 doctrine this file already follows. Adding a wrapper block
    here would both duplicate `frob check`/`frob format` under a second
    name and break that protected test for no behavioral gain.

- src/frob/scaffold/data/types/pybind11-library/Makefile.j2
  - `develop` now runs `uv sync` (was a raw `pip install -e ".[dev]"`,
    forbidden by refs/python.md). The old `build` target ran `pip build`,
    which is not a real pip subcommand and could never have run --
    removed rather than carried forward broken. Added the wrapper block.

- src/frob/scaffold/data/types/pyo3-library/Makefile.j2
  - Kept `install`/`develop`/`build` (uv sync / maturin, genuine
    bootstrap). Removed the old back-to-back
    `frob check --stamp-coverage; frob check` `check` target (exactly
    the PRE001/SCOPE001 trap docs/commands/scaffold.md warns against) and
    the standalone format/lint/typecheck/test/coverage targets. Added
    the wrapper block.

- src/frob/scaffold/data/types/web-app/Makefile.j2
  - Kept `install`/`build` (npm install / npm run build). Removed the
    old 5-step inline `check` target and the standalone
    format/lint/typecheck/test/coverage targets. Added the wrapper block.

- docs/commands/scaffold.md
  - Documents the two new managed blocks (`makefile-wrapper-targets`/
    `makebat-wrapper-targets`), the make.bat creation path, the
    STAMP-gating fix on the core-shim, and a new "The wrapper-drift gate"
    section naming WRAP001/002/003 and the T-4910 wiring deferral.

- tests/unit/test_wrapper_drift.py (new)
  - 12 tests covering: wrapper generation with/without a declared
    `[commands]` table, idempotent re-apply, Makefile/make.bat target-set
    equality, the STAMP-gating fix (both the skip and the still-applies
    case), and the drift gate's three rules plus its no-managed-block and
    `uv run frob run` acceptance cases.

WHY

`[commands]`/`frob run` (T-4759) gives every project workflow command
exactly one declared home. Before this ticket, four of five Makefile
templates (cpp/pybind11/pyo3/web-app) still hand-authored the FULL
command sequences frob was supposed to own -- including one broken
target that could never have run (pybind11's `pip build`) and one that
re-implemented `frob check --stamp-coverage`'s own documented footgun
inline (pyo3). Wrapper targets are now generated, single-line
delegations, so the sequence has exactly one place it can drift: the
`[commands]` table itself. The drift gate is the standing check that
keeps a hand-edited wrapper file from silently re-acquiring an inlined
sequence.

HOW each acceptance criterion is proven

[1] Given a rendered project whose Makefile target body expands two
    steps inline, when frob check runs, then the drift gate reports it
    and names the target.
    -> tests/unit/test_wrapper_drift.py::TestWrapperDriftGate::test_inline_sequence_in_target_body_is_wrap001

[2] Given a Makefile target for an entry removed from [commands], when
    frob check runs, then it is reported.
    -> tests/unit/test_wrapper_drift.py::TestWrapperDriftGate::test_target_for_removed_commands_entry_is_wrap002

[3] Given scaffold apply run twice, when the second run completes, then
    the wrapper files are byte-identical to after the first.
    -> tests/unit/test_wrapper_drift.py::TestApplyGeneratesWrapperBlocks::test_second_apply_is_byte_identical

[4] Given a rendered project, when the generated Makefile and make.bat
    target sets are compared, then they are equal.
...
- tests/unit/test_wrapper_drift.py (12 tests, all pass)
- tests/unit/test_scaffold_managed.py (regression, all pass)
- tests/unit/test_scaffold_natives_shim.py (regression, all pass)
- tests/unit/test_scaffold_project.py (regression, all pass -- including
  test_scaffolded_docs_make_targets_exist_in_makefile, the T-3400 test
  that motivated leaving shared/python/Makefile.j2 untouched)
- tests/unit/test_run_commands.py (T-4759 regression, all pass)
- tests/system/test_cli_scaffold_apply.py (end-to-end CLI, all pass)

Commits: 6d43be572539b309fe7fa405b076a4c570082e14 (feature),
23a6938a510750ac4f1943c1ca59303e02d68832 (DSL/COV gate fixes on the
first commit -- comments with frob:ticket T-4760 plus trailing prose on
the same line read as malformed attribute syntax; five changed symbols
needed the frob:ticket directive COV002 requires). Evidence-binding
verbs added four more self-committing "chore(tickets)" commits on top;
final worktree HEAD: 17e29d3c99836686d6ed46cf01d2849dc4c3b5f6.

Filed: T-4910 (promoted from T-4910) -- wire wrapper_drift_gate
into frob check's `_ALL_GATES` pipeline (src/frob/gates/__init__.py) and
add make.bat.j2 manifest entries to scaffold new's per-type render
manifest (src/frob/scaffold/project.py). Both files were leased by
T-3962 at land time, outside this ticket's declared scope. Until T-4910
lands: (a) the drift gate exists and is fully tested but does not yet
run as part of `frob check`; (b) make.bat is created lazily by the
first `frob scaffold apply` run (already supported -- `_apply_text_block`
creates a missing target file from nothing) rather than shipped at
`frob scaffold new` time.

Scope note: tests/unit/test_scaffold_managed.py needed one behavioral
accommodation for the STAMP-gating fix (the new "already inserted"
guard, see WHAT above) -- resolved WITHOUT touching that file (not in
scope; not leased, but the design was adjusted instead of requesting
scope) so its existing assertions keep passing unmodified.

## Pre-READY checks

All run with `.venv/bin/frob check --only <stage> --files <9 touched
files> --base dev`, one at a time, nice -n 10:

- `--only sys`: FAIL overall on gate:DRIFT (6 errors, all pre-existing,
  none of them my files -- src/frob/app/ticket_runner/_rapid_sweep.py,
  src/frob/gates/invariants.py, src/frob/tickets/_evidence.py, all
  already waived), gate:DSL (1 error, tests/test_app.py:387, pre-
  existing, unrelated), gate:SELFAUDIT (4 errors: 2 are a genuine new
  finding -- src/frob/gates/_wrapper_drift.py's Path.read_text calls
  are an undeclared `fs.read` capability on the `gates` design/frob.strata
  node's via-list; fixing it means editing design/frob.strata and
  docs/design/registry/capability-via-ratchet.lock.json, BOTH leased by
  T-4112/T-4113 at land time -- filed as part of T-4910's scope rather
  than silently left broken; the other 2 are pre-existing/unrelated --
  src/frob/app/run_runner.py's `exec` capability from the already-merged
  T-4759, and a pending testsuite via-ratchet auto-accept note that only
  a land's own composed-tree run resolves). Zero findings on any of my
  files from gate:DOCARCH/PROFILE/WAIVE. After the DSL/COV fix commit,
  gate:DSL dropped from 3 errors (2 mine) to 1 (0 mine, pre-existing).
- `--only arch`: pass -- `frob-arch 22 warnings (36 waived), 549
  suggestions`. Two suggestions land on my files (wrapper_drift_gate is
  119 lines against a 60-line suggestion threshold; _managed.py crossed
  the 800-line large-file suggestion threshold) -- both SUGGESTIONS, not
  ARCH001/LARGE001 errors; overall stage is `pass`.
- `--only coverage`: FAIL overall pre-fix (16 errors: 6 were mine --
  COV002 on 5 changed symbols plus one). After adding the missing
  `frob:ticket T-4760` directives, re-run shows 10 errors, ZERO
  attributable to my files (the only COV hits left on my files are
  COV007, and all four are already `[waived: T-0871 ...]`, pre-existing
  waivers, not new errors).
- `ruff check` on all 9 touched files: `All checks passed!` (both
  before and after the DSL/COV fix commit).
- `ty check src/frob/scaffold/_managed.py src/frob/gates/_wrapper_drift.py`:
  `All checks passed!`

Cross-ticket scope note: `git diff --name-only dev...HEAD` also lists
docs/commands/run.md, src/frob/__main__.py, src/frob/_cli_parsers/_run.py,
src/frob/_cli_parsers/__init__.py, src/frob/app/run_runner.py,
src/frob/policy/_models.py, tests/unit/test_run_commands.py, and
tickets/T-4759/ticket.md -- these are ALL T-4759's own content, present
because this worktree merged branch t-4759 per this story's dispatch
plan (T-4759 READY and queued to land, wave-2 builds on it). None of
those files were touched by this ticket's own commits (git log -p on
23a6938a5/6d43be572 shows only the 9 files in T-4760's own lease); they
arrived via the merge commit and land with T-4759 first in the
coordinator's queue, then this ticket's own diff on top. This ticket's
own lease (.git/frob-leases/T-4760.json) covers exactly:
src/frob/scaffold/_managed.py, src/frob/gates/_wrapper_drift.py, the 5
Makefile.j2 paths, docs/commands/scaffold.md, and
tests/unit/test_wrapper_drift.py -- no cross-lease file was touched by
this ticket. The two files a full wiring fix would also need
(design/frob.strata plus docs/design/registry/capability-via-ratchet.lock.json,
and separately src/frob/gates/__init__.py / src/frob/scaffold/project.py)
were deliberately left alone because they are leased by
T-4112/T-4113/T-3962; that gap is filed as T-4910 instead.

No kind=bug repro designation needed (this ticket is kind=feature).
No tickets/<dir>/ mismatches introduced (only tickets/T-4760/ticket.md
touched, via the ledger verbs themselves, id: T-4760 matches its dir).
...
```
 ...
 src/frob/gates/_wrapper_drift.py                   |  234 ++
 ...
 uv.lock                                            |    2 +-
 2391 files changed, 168062 insertions(+), 9891 deletions(-)
```

### Evidence
- `tests/unit/test_wrapper_drift.py::TestWrapperDriftGate::test_inline_sequence_in_target_body_is_wrap001` (pytest node id, verified passing when recorded)
- `tests/unit/test_wrapper_drift.py::TestWrapperDriftGate::test_target_for_removed_commands_entry_is_wrap002` (pytest node id, verified passing when recorded)
- `tests/unit/test_wrapper_drift.py::TestApplyGeneratesWrapperBlocks::test_second_apply_is_byte_identical` (pytest node id, verified passing when recorded)
- `tests/unit/test_wrapper_drift.py::TestApplyGeneratesWrapperBlocks::test_makefile_and_makebat_target_sets_are_equal` (pytest node id, verified passing when recorded)
