...
T-4516 is a story ticket (no formal acceptance list -- attempted `--accepts 1`,
got AcceptanceIndexOutOfRange, confirming plain evidence binding is correct
here, same as T-4518/T-4506). Its two leaves already landed; this closes the
story rather than adding new code.

Item -> landed leaf:
- Collect NUnit [Test]/[TestCase] and [UnityTest] methods as frob:tests-bindable
  node ids, including parameterized-case collapse -> T-4517
  (src/frob/testing/_collect_csharp.py)
- dotnet test runner + Unity batchmode XML results parser, crash-vs-failure
  distinction -> T-4508 (src/frob/testing/_runners.py)
- Post-land COV002 sweep residue on _collect_csharp.py/_support.py fixed and
  landed separately as T-4561 (also merged onto dev before this close).

Verified on dev (after merging dev into this worktree, which pulled in both
T-4518 and T-4506's lands) by rerunning the leaves' own test files directly:
26 passed across tests/test_testing.py -k csharp, tests/unit/test_dotnet_runner.py,
tests/unit/test_unity_batchmode.py, and T-4561's tests/unit/test_collect_csharp.py.
...
```
 ...
 src/frob/app/ticket_runner/_lifecycle.py           |   417 +-
 src/frob/app/ticket_runner/_mutate.py              |   155 +-
 ...
 src/frob/gates/__init__.py                         |   426 +-
 ...
 src/frob/tickets/_models.py                        |   358 +-
 ...
 src/frob/tickets/_new_renumber.py                  |   171 +-
 ...
 src/frob/tickets/_setters.py                       |   272 +-
 ...
 tests/ticket_land_suite/test_claim_close.py        |    34 +-
 ...
 tests/ticket_land_suite/test_land_plan.py          |    22 +-
 ...
 uv.lock                                            |     2 +-
 3711 files changed, 468517 insertions(+), 22020 deletions(-)
```
...
- `tests/unit/test_dotnet_runner.py::TestRunDotnetTests::test_maps_passing_and_failing_ids` (pytest node id, verified passing when recorded)
- `tests/unit/test_dotnet_runner.py::TestRunDotnetTests::test_crash_with_no_results_file_is_run_failed` (pytest node id, verified passing when recorded)
- `tests/unit/test_unity_batchmode.py::TestParseUnityBatchmodeXml::test_parses_nested_test_suites_into_fqn_result_map` (pytest node id, verified passing when recorded)
- `tests/unit/test_unity_batchmode.py::TestRunUnityBatchmode::test_crash_with_no_results_file_is_run_failed_distinct_from_a_test_failure` (pytest node id, verified passing when recorded)
