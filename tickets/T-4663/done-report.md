...
Wires the already-real [arch.layering] schema and frob.arch._layering
checker into frob check as ARCH104 RED -- T-0620 shipped the checker
inert (declared but never called), so a kernel layering violation
(ledger < leases < land < app, gates independent) passed silently.
Positive control: test_upward_import_is_arch10x_red plants an upward
import from the ledger layer into the land layer and asserts frob check
now reports it as ARCH104 red; it fails on dev (nothing calls the
checker) and passes here. test_layering_job_reports_edges_checked
proves the job actually walked a nonzero edge count on a clean tree,
guarding against the same silent-zero failure mode in the other
direction. docs/modules/arch.md's DIP layering section now describes
the live kernel contract instead of an inert example.
...
```
 ...
 src/frob/arch/_layering.py                         |   110 +-
 ...
 tests/unit/test_layering_gate.py                   |   135 +
 ...
 uv.lock                                            |     2 +-
 2939 files changed, 358871 insertions(+), 15606 deletions(-)
```

### Evidence
- `tests/unit/test_layering_gate.py::test_upward_import_is_arch10x_red` (pytest node id, verified passing when recorded)
- `tests/unit/test_layering_gate.py::test_layering_job_reports_edges_checked` (pytest node id, verified passing when recorded)
- `tests/unit/test_layering_gate.py::test_no_declared_layering_config_is_not_a_violation` (pytest node id, verified passing when recorded)
- `tests/unit/test_layering_gate.py::test_allowed_edge_across_declared_layers_is_not_arch104` (pytest node id, verified passing when recorded)
