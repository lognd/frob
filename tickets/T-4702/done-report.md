...
Regenerated docs/commands/ from the LIVE argparse tree (T-4690/T-4692/T-4695/
T-4696/T-4698's final CLI surface) and added a coverage test that keeps the
two in sync going forward instead of relying on a one-time manual pass.

## What changed
- src/frob/docs/_command_pages.py (new): live_top_level_verbs (non-suppressed
  choices), all_registered_verbs (every choice, live+deprecated-but-still-
  dispatchable), generate_command_page, sync_missing_command_pages -- never
  overwrites an existing hand-written page, only fills genuine gaps.
- src/frob/app/docs_runner.py, src/frob/_cli_parsers/_core.py,
  src/frob/app/config.py, src/frob/app/_config_external.py: new
  `frob docs --sync-command-pages` verb wired through the standard
  AppConfig plumbing.
- Ran it: generated 22 stub pages (ack, agent, claude, clean, coverage,
  doctor, explore, fleet, graph, mutate, natives, perf, pool, process,
  profile, registry, serve, status, test, verify, vet, worktree) for verbs
  that had no docs/commands page at all.
- tests/unit/test_docs_commands_coverage.py (new): walks the real argparse
  tree against the real docs/commands/ directory in both directions
  (acceptance[1]), plus a synthetic positive control proving the assertion
  fires both ways, independent of repo state (acceptance[2]).
  `_RAW_DISPATCH_ONLY_VERBS = {"run"}` and `_NON_VERB_DOC_STEMS =
  {"cli-vocabulary"}` are the two narrow, documented exceptions:
  - `run` has no argparse subparser at all (raw-argv dispatch, T-0355's own
    `build` precedent) so it can never appear in any argparse-derived set,
    yet its page is real and hand-maintained.
  - `cli-vocabulary.md` documents a CONCEPT, not one verb by that name.
- docs/design/cli-regrouping.md: added a lead paragraph recording that the
  `quality`/`design`/`ops` verb groups this doc originally justified were
  REVERTED by T-4690/T-4687 (deprecation shims, sunset 2026-12-01), not kept
  as permanent aliases -- the doc's own historical justification no longer
  describes the current surface, so it needed a correction note rather than
  silent staleness.

## Acceptance[3]: stale spellings in ~/.claude/refs/frob.md (file NOT edited,
owner-owned)

Found via `grep -n` of every `frob <verb...>`-shaped span in the file against
the live argparse tree (`all_registered_verbs`) and the T-4690/T-4695
deprecation/regroup history:

- `~/.claude/refs/frob.md:7-10` (the "Verb groups" paragraph):
  - `` `explore` (map/outline/xref/docs) `` is stale -- T-4695 folded
    gitlog/stats/graph-query/graph-why/graph-affects/debt/deprecated into
    `explore` too. Replacement: `` `explore` (map/outline/xref/docs/gitlog/
    stats/graph-query/graph-why/graph-affects/debt/deprecated) ``.
  - `` `quality` (check/test/dup/arch/bind/cycle/mutate/perf) ``, `` `design`
    (sys/registry/docs/graph/exports) ``, and `` `ops`
    (release/natives/doctor/clean/fleet/deploy/scaffold/gitlog/stats) `` are
    stale -- per docs/design/cli-regrouping.md's own T-4690/T-4687 REVERTED
    note, all three groups are DEPRECATED (deprecation shims, sunset
    2026-12-01), not permanent verb groupings; each of their member verbs
    is only reachable standalone today. Replacement: drop the `quality`/
    `design`/`ops` parenthetical breakdowns entirely (or mark them
    "deprecated, sunset 2026-12-01") and note the members are standalone
    verbs.
  - Within the `ops` breakdown specifically, `gitlog`/`stats` are doubly
    stale: T-4695 also moved them under `explore`, so even as a historical
    description of `ops` they now double-list verbs `explore` claims.

No other `frob <verb...>` span in the file names a removed, renamed, or
regrouped spelling; the rest (`frob check`, `frob test`, `frob verify
status`, `frob pool`, `frob release stamp`, `frob vet`, `frob doctor`,
`frob natives build`, `frob claude sync`, `frob sync-skills`, `frob
profile`, `frob graph build`, `frob ticket doable/work/evidence/done-
report/close/land/brief`, `frob ack path`, `frob clean`, `frob agent env`)
all still resolve against the live tree unchanged. "frob bugs to
FROBLEMS.md" (line 151) is prose about defects in frob, not a CLI
invocation, and is not a finding.

## Tests
tests/unit/test_docs_commands_coverage.py -- 5/5 passing (2 real-repo
coverage tests + 3 positive-control tests).

## Scope
docs/commands/ticket.md deliberately excluded/deferred (leased by T-5132
at the time of this ticket).

## T-5132 regression fix absorbed into this land (coordinator directive)

`frob ticket start`/`work` began refusing every unsized ticket after
T-5132 landed, and `--unsized-ack REASON` was silently dropped: the new
`AppConfig` fields `ticket_unsized_ack`/`ticket_points_value` were missing
from `src/frob/app/_config_external.py`'s `_STRING_FIELDS` allowlist, and
`ticket_points`/`ticket_tokens_in`/`ticket_tokens_out`/
`ticket_tokens_cache_read` were missing from `_INT_FIELDS` -- same silent-
drop shape T-2004's own comment in this file already documents for
`ticket_anchor_reason`/`sys_threats_boundary`. Fixed by adding all six to
their correct tuples (first attempt misplaced two of them in `_PATH_FIELDS`
and four in `_FLOAT_FIELDS` by matching the wrong nearby block; caught by
`from_external`'s own pydantic validation and corrected before commit).
Added `TestFromExternal.test_unsized_ack_reason_reaches_cfg` and
`.test_ticket_points_int_field_reaches_cfg` to
tests/unit/test_app_config_from_external_t1276.py.

Searched for T-draft-e4547e60 (tickets/, .frob/prework, .frob/verify-drain,
.frob/journal, .frob/land-queue, .frob/tickets/*.lock) -- no such draft
ticket exists anywhere in this checkout at the time of this fix, so there
is nothing to absorb or drop with --absorbed-by; flagging this back to the
coordinator rather than guessing.

## Cross-ticket carry (coordinator-approved, option c)

This land carries the pre-land Tier-A fix of the tickets_ledger
`frob:tests` continuation in design/frob.strata (T-5280's scope) -- a
one-line directive canonicalization (removes a stale `// frob:tests \`
line-continuation marker), applied automatically by the unscoped pre-land
Tier-A pass and landed via `--allow-cross-ticket` per coordinator
direction rather than reset out again. Recorded by the coordinator as a
land-engine gap (unscoped Tier-A pre-land pass) on the T-5106 story.

## SELFAUDIT001 fix

Pre-land unscoped sweep found `core::fs.write` undeclared for
`src/frob/docs/_command_pages.py::sync_missing_command_pages`'s new write
site. Declared it on the `core` node in design/frob.strata and bumped
`core::fs.write`'s ratchet ceiling 31->32 in
docs/design/registry/capability-via-ratchet.lock.json in the same diff.
...
```
 ...
 frob-core/src/exact_regions.rs                     |     1 +
 frob-core/src/r3.rs                                |     1 +
 ...
 src/frob/app/_config_external.py                   |    56 +-
 ...
 src/frob/app/ticket_runner/__init__.py             |   164 +-
 src/frob/app/ticket_runner/_close_cmd.py           |    42 +-
 src/frob/app/ticket_runner/_land_cmd.py            |   768 +-
 ...
 src/frob/gates/_fix_engine_text.py                 |   571 +-
 ...
 .../ProjectSettings/ProjectVersion.txt             |     2 +
 tests/gates/test_comment_placement.py              |     1 +
 tests/gates/test_docarch_structural.py             |   215 +
 tests/gates/test_docptr.py                         |   127 +
 tests/gates/test_docstring_archaeology.py          |     1 +
 tests/gates/test_mutation_evidence_err_branches.py |     1 +
 tests/gates/test_rule_id_scan_branches.py          |    61 +-
 tests/gates/test_scan_timeout_enforcement.py       |    32 -
 tests/gates/test_scope_symref_helpers.py           |     1 +
 tests/gates/test_tdd_order.py                      |    10 +
 tests/gates_suite/test_claim_lint.py               |   165 +
 tests/gates_suite/test_compliance.py               |    83 +-
 tests/gates_suite/test_config_path_defaults.py     |   147 +
 tests/gates_suite/test_coverage.py                 |   360 +-
 tests/gates_suite/test_debt.py                     |     5 +
 tests/gates_suite/test_docblocks_refs_cov009.py    |   128 +
 tests/gates_suite/test_exhaust_burndown_t3861.py   |    54 +
 tests/gates_suite/test_fix_engine.py               |   129 +
 tests/gates_suite/test_forbid_rules.py             |   199 +
 tests/gates_suite/test_guard_closure.py            |   231 +
 tests/gates_suite/test_invariant.py                |   254 +
 tests/gates_suite/test_prework.py                  |    12 +-
 tests/gates_suite/test_protocol.py                 |    73 +-
 tests/gates_suite/test_route_response_model.py     |   159 +
 tests/gates_suite/test_run.py                      |    56 +-
 tests/gates_suite/test_severity_overrides_pin.py   |    13 +-
 tests/gates_suite/test_sys.py                      |   121 +-
 tests/gates_suite/test_sys_assume_template.py      |   281 +
 tests/gates_suite/test_sys_rule_liveness.py        |   319 +
 tests/gates_suite/test_test_gate.py                |   158 +-
 tests/gates_suite/test_tick.py                     |    80 +-
 tests/gates_suite/test_tick_dead_worktree.py       |   159 +
 tests/gates_suite/test_waive.py                    |    61 +-
 tests/gates_suite/test_wire.py                     |    46 +-
 tests/integration/test_interfaces.py               |     1 +
 tests/narrative/test_bulk.py                       |   255 +
 tests/system/conftest.py                           |    16 +-
 tests/system/test_artifact_smoke.py                |    29 +-
 tests/system/test_cli_check.py                     |    53 +-
 tests/system/test_cli_doctor.py                    |     6 +-
 tests/system/test_cli_native_missing.py            |     1 +
 tests/system/test_cli_scaffold_apply.py            |    36 +
 tests/system/test_cli_sys_audit.py                 |     8 +-
 tests/system/test_cli_ticket.py                    |    23 +-
 ...
 tests/system/test_scaffold_pool.py                 |     1 +
 tests/system/test_scaffold_pool_cli.py             |     1 +
 tests/system/test_scaffold_unity_project_cli.py    |   100 +
 tests/system/test_unity_e2e.py                     |   256 +
 tests/test_ack_worktree_lease.py                   |     2 +
 tests/test_app.py                                  |     4 +-
 tests/test_app_config.py                           |     5 +
 tests/test_app_daemon_proxy.py                     |    60 +-
 tests/test_arch_gate.py                            |    26 +-
 ...
 tests/test_check_runner.py                         |   114 +-
 tests/test_ci_report.py                            |     1 +
 tests/test_ci_validity.py                          |     2 +
 tests/test_ci_workflow_job_summary.py              |     8 -
 tests/test_ci_workflow_matrix.py                   |    54 +-
 tests/test_clean.py                                |     6 +
 tests/test_coverage.py                             |    47 +-
 tests/test_coverage_wait_shared.py                 |    33 +-
 tests/test_dataset_construct.py                    |   109 +
 tests/test_debt_runner.py                          |     2 +
 tests/test_decisions.py                            |     1 +
 tests/test_deprecated_runner.py                    |     1 +
 tests/test_docblocks_gate.py                       |     3 +
 tests/test_docenum_gate.py                         |    41 +
 tests/test_docptr_gate.py                          |    78 +-
 tests/test_dup.py                                  |     1 +
 tests/test_evidence_integrity.py                   |   113 +-
 tests/test_excludes.py                             |    91 +-
 tests/test_gate_cache.py                           |    64 +-
 tests/test_gates_directive_stack.py                |   288 +
 tests/test_gates_empty_diff_close.py               |   174 +-
 tests/test_gates_fix_engine.py                     |   218 +-
 tests/test_gates_fmt_directives.py                 |   296 +-
 tests/test_gates_milestone.py                      |     9 +
 tests/test_gates_mutation_evidence.py              |    34 +-
 tests/test_gates_ratchet.py                        |    54 +
 tests/test_gates_suppress.py                       |    48 +-
 tests/test_gates_vmodel.py                         |     2 +
 tests/test_ghio.py                                 |     2 +
 tests/test_gitio.py                                |    58 +
 tests/test_graph.py                                |   104 +-
 tests/test_graph_affects.py                        |    54 +
 tests/test_graph_reach.py                          |     1 +
 tests/test_hook_dispatch_telemetry.py              |     1 +
 tests/test_hook_frob_suggest.py                    |   269 +-
 tests/test_hook_frob_timeout_guard.py              |    65 +-
 tests/test_hook_root_write_guard.py                |   129 +-
 tests/test_hook_sync_claude_config.py              |   193 +
 tests/test_land_verify_claims_outcome.py           |    32 +-
 tests/test_lang.py                                 |   176 +-
 tests/test_lang_conformance_gate.py                |    84 +-
 tests/test_lang_support.py                         |    11 +-
 tests/test_mutate.py                               |     1 +
 tests/test_mutate_journal.py                       |    31 +-
 tests/test_narrative_blocks.py                     |    28 +
 tests/test_narrative_migrate.py                    |     3 +
 tests/test_perf.py                                 |    27 +-
 tests/test_pii_provenance_trust_identity.py        |   181 +
 tests/test_pii_structural_gate.py                  |   144 +-
 tests/test_policy.py                               |   196 +
 tests/test_pool_runner.py                          |     1 +
 tests/test_refactor.py                             |   314 +-
 tests/test_refs_gate.py                            |    61 +-
 tests/test_registry_corpus.py                      |     7 +-
 tests/test_registry_exhaustiveness.py              |    13 +
 tests/test_registry_models.py                      |     2 +
 tests/test_registry_staleness.py                   |    13 +-
 tests/test_release.py                              |    80 +-
 tests/test_release_worktree_lease.py               |     1 +
 tests/test_scaffold_worktree_lease_hook.py         |    30 +-
 tests/test_secrets_gate.py                         |     1 +
 tests/test_serve_daemon.py                         |    53 +-
 tests/test_serve_events.py                         |    18 +-
 tests/test_serve_leases.py                         |    29 +-
 tests/test_serve_socket.py                         |    44 +-
 tests/test_serve_tools_daemon_bypass.py            |     1 +
 tests/test_serve_watch.py                          |     8 +-
 tests/test_stats_agentic.py                        |     2 +
 tests/test_status.py                               |     3 +
 tests/test_testing.py                              |   108 +-
 tests/test_tick013_gate.py                         |     1 +
 tests/test_ticket_evidence.py                      |    41 +-
 tests/test_ticket_journal.py                       |     5 +
 tests/test_ticket_land_proof_claims.py             |    18 +-
 tests/test_ticket_leases.py                        |   464 +-
 tests/test_ticket_leases_cross_worktree.py         |   191 +-
 tests/test_ticket_lifecycle.py                     |     1 +
 tests/test_ticket_reconcile.py                     |   163 +-
 tests/test_ticket_runner_archive_force.py          |     1 +
 tests/test_ticket_store_stale_snapshot.py          |    19 +-
 tests/test_ticket_work_and_land_finish.py          |   250 +-
 tests/test_tickets.py                              |   246 +-
 tests/test_tickets_acceptance.py                   |    34 +-
 ...
 tests/test_tickets_gate_claim_evidence.py          |     6 +-
 tests/test_tickets_lease.py                        |    44 +-
 tests/test_tickets_leases.py                       |    24 +-
 tests/test_tickets_ledger_concurrency.py           |     1 +
 tests/test_tickets_live_tracker.py                 |    34 +-
 tests/test_tickets_migration.py                    |   126 +-
 tests/test_tickets_milestone_runs_last.py          |     4 +
 tests/test_tickets_milestone_sort.py               |     5 +
 tests/test_tickets_mutation_evidence.py            |     5 +
 tests/test_tickets_new_gate_rule_acceptance.py     |     3 +
 tests/test_tickets_organization.py                 |    24 +-
 tests/test_tickets_parent.py                       |   293 +-
 tests/test_tickets_points.py                       |   238 +
 tests/test_tickets_priority.py                     |   114 +-
 tests/test_tickets_registry_files.py               |   210 +
 tests/test_tickets_review.py                       |     1 +
 tests/test_tickets_scope_mutation.py               |    69 +-
 tests/test_tickets_triage_dates.py                 |     8 +-
 tests/test_tickets_velocity.py                     |    68 +-
 tests/test_todo_fmt_gate.py                        |     4 +
 tests/test_vet_containment.py                      |     1 +
 tests/test_waive_gate.py                           |   156 +-
 tests/test_walk_lint_gate.py                       |    12 +
 tests/test_worktree_guard.py                       |    54 +-
 tests/test_worktree_pythonpath.py                  |    10 -
 tests/ticket_land_suite/test_claim_close.py        |     6 +-
 tests/ticket_land_suite/test_draft.py              |    79 +
 tests/ticket_land_suite/test_land_core.py          |     9 +-
 tests/ticket_land_suite/test_land_lock.py          |   133 +-
 .../test_land_proof_unmeasured.py                  |   108 +
 .../ticket_land_suite/test_land_reaps_worktree.py  |    97 +
 tests/ticket_land_suite/test_ledger_splice.py      |    30 +-
 tests/ticket_land_suite/test_push.py               |    13 +-
 tests/ticket_land_suite/test_verify_intent.py      |    85 +-
 tests/tickets/__init__.py                          |     0
 tests/tickets/test_land_squash.py                  |   104 +
 tests/unit/arch_suite/test_abstraction.py          |     2 +
 tests/unit/arch_suite/test_complexity.py           |    10 +-
 tests/unit/arch_suite/test_concurrency.py          |    52 +-
 tests/unit/arch_suite/test_dispatch.py             |     8 +-
 tests/unit/arch_suite/test_lang_adapters.py        |    14 +-
 tests/unit/arch_suite/test_logging.py              |     6 +
 tests/unit/arch_suite/test_lsp.py                  |     2 +
 tests/unit/arch_suite/test_misc.py                 |    10 +
 tests/unit/arch_suite/test_smells.py               |     2 +
 tests/unit/arch_suite/test_type_design.py          |     8 +
 tests/unit/coordinator_suite/test_check_summary.py |     1 +
 ...
 .../unit/coordinator_suite/test_fleet_worktrees.py |    56 +-
 tests/unit/coordinator_suite/test_verify_lands.py  |     2 +
 tests/unit/fleet/test_manifest.py                  |     2 +
 tests/unit/fleet/test_route.py                     |     1 +
 tests/unit/fleet/test_status.py                    |     7 +-
 .../unit/gates/test_cov002_strata_declarations.py  |   131 +
 tests/unit/gates/test_deprecated_baseline.py       |    26 +-
 tests/unit/gates/test_detector_scope.py            |    12 +-
 ...
 tests/unit/gates/test_ffi_boundary_path_shape.py   |    18 +-
 tests/unit/gates/test_lexical_selfcheck.py         |    21 +-
 tests/unit/gates/test_lock_producer.py             |     1 +
 tests/unit/gates/test_negexist.py                  |     6 +
 tests/unit/gates/test_pkg_resources.py             |    13 +-
 tests/unit/gates/test_port_selfcheck.py            |     7 +
 tests/unit/gates/test_profile_boundary.py          |     2 +
 .../gates/test_profile_boundary_subject_count.py   |     7 +-
 tests/unit/gates/test_version_coupling.py          |     6 +
 tests/unit/gates/test_wire001_cli_dest_semantic.py |    24 +-
 tests/unit/graph/test_dsl.py                       |   470 +-
 ...
 tests/unit/perf/test_advisories.py                 |     3 +
 tests/unit/perf/test_cache_effects.py              |    89 +
 tests/unit/perf/test_collectors.py                 |     1 +
 tests/unit/perf/test_dup_spawn.py                  |    14 +-
 tests/unit/perf/test_effect_summaries.py           |    19 +-
 tests/unit/perf/test_harness_sampling.py           |     2 +
 tests/unit/perf/test_hotgraph.py                   |     3 +
 tests/unit/perf/test_hotpath_smells.py             |    25 +-
 tests/unit/perf/test_loop_effects.py               |     3 +
 tests/unit/perf/test_loop_variant.py               |    87 +
 tests/unit/perf/test_ratchet.py                    |     3 +
 tests/unit/perf/test_serial_pools.py               |     1 +
 tests/unit/perf/test_sketch_store.py               |     7 +
 tests/unit/rapid_sweep_suite/test_attribution.py   |    33 +-
 tests/unit/rapid_sweep_suite/test_baseline.py      |    27 +-
 tests/unit/rapid_sweep_suite/test_commit.py        |    28 +-
 tests/unit/rapid_sweep_suite/test_dispose.py       |    62 +-
 tests/unit/rapid_sweep_suite/test_filing.py        |    78 +-
 tests/unit/rapid_sweep_suite/test_sweep_run.py     |    35 +-
 tests/unit/rapid_sweep_suite/test_window.py        |   442 +
 ...
 tests/unit/strata/test_access.py                   |     1 +
 tests/unit/strata/test_admit_phase_wiring.py       |   241 +
 tests/unit/strata/test_audit.py                    |     9 +-
 tests/unit/strata/test_backpressure.py             |     1 +
 tests/unit/strata/test_bootstrap.py                |     1 +
 tests/unit/strata/test_capacity_projection.py      |     1 +
 tests/unit/strata/test_circuit_breaker.py          |     1 +
 tests/unit/strata/test_claims.py                   |    13 +-
 tests/unit/strata/test_claims_overdue.py           |   110 +
 tests/unit/strata/test_clock_ordering.py           |     1 +
 tests/unit/strata/test_contention.py               |    60 +-
 tests/unit/strata/test_crash.py                    |     1 +
 tests/unit/strata/test_cve_fingerprint.py          |    16 +-
 tests/unit/strata/test_cve_fingerprint_scan.py     |     4 +
 tests/unit/strata/test_delivery_semantics.py       |     1 +
 tests/unit/strata/test_demand.py                   |     1 +
 tests/unit/strata/test_design_load.py              |     8 +-
 tests/unit/strata/test_distributed_txn.py          |     1 +
 tests/unit/strata/test_effects.py                  |    64 +-
 tests/unit/strata/test_elaborate.py                |     1 +
 tests/unit/strata/test_facts.py                    |    24 +-
 tests/unit/strata/test_fallback.py                 |     1 +
 tests/unit/strata/test_fragments.py                |    59 +-
 tests/unit/strata/test_host_isolation.py           |     1 +
 tests/unit/strata/test_inbound_rate.py             |   133 +
 tests/unit/strata/test_interactive_cost.py         |     1 +
 tests/unit/strata/test_kernel_doc_extensions.py    |    67 +
 tests/unit/strata/test_message_schema.py           |     1 +
 tests/unit/strata/test_mode_conformance.py         |    66 +-
 tests/unit/strata/test_multifile.py                |     1 +
 tests/unit/strata/test_mutation_audit.py           |     1 +
 tests/unit/strata/test_native_staleness.py         |    71 +-
 tests/unit/strata/test_native_test.py              |     1 +
 tests/unit/strata/test_obligation_proof.py         |     1 +
 tests/unit/strata/test_observability.py            |     1 +
 tests/unit/strata/test_outbound_destination.py     |   179 +
 tests/unit/strata/test_packs_analyzable_warning.py |    87 +
 tests/unit/strata/test_parse.py                    |    13 +-
 tests/unit/strata/test_policy.py                   |     2 +
 tests/unit/strata/test_process_bounds.py           |     1 +
 tests/unit/strata/test_reliability.py              |     1 +
 tests/unit/strata/test_retry.py                    |     1 +
 tests/unit/strata/test_selfconform.py              |   462 +-
 tests/unit/strata/test_shared_state.py             |     1 +
 tests/unit/strata/test_shrink.py                   |     7 +-
 tests/unit/strata/test_slo.py                      |    16 +-
 tests/unit/strata/test_spof.py                     |     1 +
 tests/unit/strata/test_ssot.py                     |     1 +
 tests/unit/strata/test_starvation.py               |     1 +
 tests/unit/strata/test_strata_core_gil.py          |     7 +-
 tests/unit/strata/test_strata_scan_cache.py        |   207 +
 tests/unit/strata/test_supply_chain_boot.py        |     1 +
 tests/unit/strata/test_sync_depth.py               |    10 +-
 tests/unit/strata/test_sync_may.py                 |     2 +
 .../unit/strata/test_sys107_via_scope_advisory.py  |     1 +
 tests/unit/strata/test_threat.py                   |    68 +-
 tests/unit/strata/test_txn.py                      |     1 +
 tests/unit/strata/test_unity_asmdef.py             |   162 +
 tests/unit/strata/test_vmodel_authoring.py         |    41 +-
 tests/unit/test_ack_runner.py                      |     4 +
 tests/unit/test_app_config_flag_coverage.py        |     1 +
 tests/unit/test_app_config_from_external_t1276.py  |    46 +-
 ...t_app_config_pyproject_root_t_draft_1f1ae69b.py |    59 +
 tests/unit/test_app_runners.py                     |    25 +
 tests/unit/test_app_runners_batch5.py              |    10 +
 tests/unit/test_app_runners_batch6.py              |    40 +-
 tests/unit/test_app_runners_batch7.py              |   190 +-
 tests/unit/test_app_runners_json_guard_t2492.py    |     7 +-
 tests/unit/test_app_runners_process.py             |     4 +
 .../unit/test_app_runners_t0875_leaf_collision.py  |     5 +-
 .../test_app_runners_t0976_mutation_evidence.py    |     1 +
 tests/unit/test_app_runners_t1738_wave.py          |     1 +
 .../unit/test_app_runners_t1822_already_landed.py  |     1 +
 tests/unit/test_app_sys_capacity.py                |     2 +
 tests/unit/test_arch_ocp.py                        |     1 +
 tests/unit/test_arch_srp.py                        |    39 +-
 tests/unit/test_artifact_smoke_script.py           |     7 +-
 tests/unit/test_bind.py                            |     1 +
 tests/unit/test_branch_stranded_work_analysis.py   |     1 +
 tests/unit/test_capability_native.py               |     2 +
 tests/unit/test_check.py                           |    66 +-
 tests/unit/test_check_budget.py                    |    51 +-
 ...
 tests/unit/test_daemon_proxy_error_paths_t1457.py  |    65 +-
 tests/unit/test_design_invariants.py               |   136 +-
 ...
 tests/unit/test_doctor_runner_t1276.py             |   104 +-
 tests/unit/test_done_report_check_scope.py         |   177 +
 tests/unit/test_dotnet_runner.py                   |   182 +
 tests/unit/test_draft_finalize_attachments.py      |     3 +
 tests/unit/test_dup.py                             |    16 +-
 tests/unit/test_dup_cache.py                       |    16 +-
 tests/unit/test_dup_legacy_cpp.py                  |    41 +-
 tests/unit/test_dup_legacy_py.py                   |    10 +-
 .../test_explore_runner_parse_artifact_cache.py    |    55 +
 tests/unit/test_explore_verb.py                    |   130 +
 tests/unit/test_exports.py                         |     4 +
 ...
 tests/unit/test_land_auto_rebase.py                |    31 +
 tests/unit/test_land_cas_ledger_retry.py           |   313 +
 tests/unit/test_land_cmd_drain_wiring.py           |    62 +
 tests/unit/test_land_cmd_quarantine.py             |    62 +-
 tests/unit/test_land_compose.py                    |    13 +-
 tests/unit/test_land_cross_ticket_leakage.py       |     2 +
 tests/unit/test_land_default_queue.py              |   170 +
 .../test_land_dirty_main_orphaned_ticket_t2026.py  |    21 +-
 tests/unit/test_land_duplicate_ticket_id.py        |     1 +
 tests/unit/test_land_finish_guard.py               |     1 +
 tests/unit/test_land_format_gate.py                |     1 +
 ...
 tests/unit/test_land_parity_gate.py                |     3 +
 tests/unit/test_land_phase_elapsed_logging.py      |     1 +
 tests/unit/test_land_queue.py                      |   114 +
 tests/unit/test_land_release_coherence.py          |     1 +
 tests/unit/test_land_release_out_of_tree.py        |     1 +
 tests/unit/test_land_sibling_regression.py         |    47 +-
 tests/unit/test_land_splice_test_then_impl.py      |    19 +-
 tests/unit/test_land_squash_residue_reclaim.py     |    20 +-
 tests/unit/test_land_stackdump.py                  |   323 +
 tests/unit/test_land_stage_flip.py                 |    12 +-
 .../test_land_verify_claim_divergence_sentinel.py  |    13 +-
 tests/unit/test_lang_kotlin.py                     |     5 +
 tests/unit/test_lang_parse_guard.py                |    26 +-
 tests/unit/test_lang_project_detect.py             |   110 +
 tests/unit/test_lang_strata.py                     |    26 +-
 tests/unit/test_layering_gate.py                   |   140 +
 tests/unit/test_lease_lifecycle.py                 |   180 +
 tests/unit/test_leases_staleness_perf.py           |   310 +
 tests/unit/test_ledger_store_api.py                |   239 +
 tests/unit/test_lifecycle_work_base.py             |   200 +
 tests/unit/test_logging_module.py                  |    28 +-
 tests/unit/test_logging_quiet.py                   |    11 +-
 tests/unit/test_main_entry.py                      |    37 +-
 tests/unit/test_makefile_coverage.py               |    61 +-
 tests/unit/test_map.py                             |     1 +
 tests/unit/test_memo.py                            |    16 +-
 tests/unit/test_mutation_sweep_queue.py            |     4 +
 .../test_new_ticket_over_broad_scope_warning.py    |     2 +
 .../unit/test_new_ticket_scope_overlap_warning.py  |    17 +-
 tests/unit/test_outline.py                         |     1 +
 tests/unit/test_parse_runner_direct.py             |     5 +
 tests/unit/test_policy_weakening_gate.py           |    27 +-
 tests/unit/test_post_publish_lock_window.py        |   226 +
 tests/unit/test_process.py                         |    23 +-
 tests/unit/test_process_lock.py                    |    52 +-
 tests/unit/test_process_pid_liveness.py            |    24 +-
 tests/unit/test_process_reap.py                    |    25 +-
 tests/unit/test_process_tty.py                     |    16 +-
 tests/unit/test_profile.py                         |     6 +
 tests/unit/test_profile_runner.py                  |     2 +
 tests/unit/test_project_tool.py                    |     1 +
 tests/unit/test_pyfmt_runner.py                    |    15 +-
 tests/unit/test_pyproject_data_memoization.py      |   165 +
 tests/unit/test_pytest_spawn.py                    |     3 +
 tests/unit/test_pytest_spawn_env_wiring.py         |     1 +
 tests/unit/test_rapid_debt.py                      |     1 +
 tests/unit/test_rel002_dev_suffix.py               |   114 +
 tests/unit/test_release_stamp_guard.py             |     1 +
 tests/unit/test_release_workflow_gate.py           |    32 +-
 tests/unit/test_reporting_t1648_remainder.py       |     1 +
 .../test_reporting_t3285_fenced_subheadings.py     |     1 +
 tests/unit/test_require_python.py                  |     1 +
 tests/unit/test_run_commands.py                    |   180 +
 ...
 tests/unit/test_scaffold_unity_project.py          |   134 +
 .../unit/test_scope_closure_declared_scope_only.py |   193 +
 tests/unit/test_skills_sync.py                     |    23 +-
 tests/unit/test_stackdump.py                       |     1 +
 ...
 .../test_sync_claude_config_stale_guard_t3408.py   |     3 +
 tests/unit/test_telemetry_verb_recording.py        |   200 +
 tests/unit/test_ticket_anchor_cli.py               |     1 +
 tests/unit/test_ticket_cli_surface.py              |   182 +
 tests/unit/test_ticket_close_bug002_t1427.py       |     2 +
 tests/unit/test_ticket_close_bug002_t1438.py       |     1 +
 tests/unit/test_ticket_new_phase_progress.py       |    49 +
 tests/unit/test_ticket_new_related.py              |    11 +-
 .../unit/test_ticket_runner_base_forward_t4105.py  |     9 +-
 tests/unit/test_ticket_runner_designate_repro.py   |     2 +
 tests/unit/test_ticket_runner_gate_findings.py     |   107 +-
 tests/unit/test_ticket_runner_land_cmd_flags.py    |     5 +-
 tests/unit/test_ticket_runner_land_release.py      |    28 +-
 tests/unit/test_ticket_runner_ledger_mirror.py     |    76 +-
 tests/unit/test_ticket_runner_venv_sync_t3320.py   |     1 +
 tests/unit/test_ticket_set.py                      |   240 +
 tests/unit/test_ticket_store.py                    |    95 +-
 tests/unit/test_ticket_subverb_tail.py             |   250 +
 tests/unit/test_ticket_verbs_wait.py               |   190 +
 tests/unit/test_tool_absent_parser_reconcile.py    |     5 +
 tests/unit/test_unity_batchmode.py                 |   195 +
 tests/unit/test_unlanded_branch_work.py            |   104 +-
 tests/unit/test_verify_language_buckets.py         |    24 -
 tests/unit/test_verify_release_ci_status.py        |     7 +
 tests/unit/test_version_guard.py                   |     3 +
 tests/unit/test_waive_audit_runner.py              |     1 +
 tests/unit/test_waive_audit_watermark.py           |    10 +-
 tests/unit/test_wrapper_drift.py                   |   160 +
 tests/unit/test_xref.py                            |   119 +
 tests/unit/testing/test_stability.py               |     1 +
 tests/unit/tickets/__init__.py                     |     4 +
 tests/unit/tickets/test_start_transition_ledger.py |   146 +
 tests/unit/verify/test_attribution.py              |     5 +
 tests/unit/verify/test_backpressure.py             |    17 +-
 tests/unit/verify/test_bisect.py                   |     1 +
 tests/unit/verify/test_drain.py                    |     8 +-
 tests/unit/verify/test_quarantine.py               |    78 +
 tests/unit/verify/test_selection.py                |     2 +
 tests/unit/verify/test_verify_runner.py            |    46 +
 tests/unit/verify/test_worker.py                   |    35 +-
 tests/unit/vet/test_bare_toolchain.py              |     2 +
 tests/unit/vet/test_capability_modes.py            |    15 +-
 tests/unit/vet/test_taint.py                       |     2 +
 tests/vet_suite/test_advisories.py                 |   354 +-
 ...
 tests/vet_suite/test_lockfile.py                   |     1 +
 tests/vet_suite/test_opaque_indirection.py         |     1 +
 tests/vet_suite/test_scan_tree.py                  |    16 +-
 ...
 tickets/T-4702/done-report.md                      |  3168 +++
 tickets/T-4702/ticket.md                           |   522 +
 ...
 tickets/T-5178/ticket.md                           |    41 +
 tickets/T-5179/ticket.md                           |    33 +
 tickets/T-5180/ticket.md                           |    36 +
 tickets/T-5181/ticket.md                           |    31 +
 tickets/T-5182/ticket.md                           |    30 +
 ...
 tickets/T-5263/ticket.md                           |    40 +
 tickets/T-5264/ticket.md                           |   568 +
 tickets/T-5265/ticket.md                           |    58 +
 tickets/T-5266/ticket.md                           |    33 +
 tickets/T-5267/ticket.md                           |    32 +
 tickets/T-5268/ticket.md                           |    30 +
 tickets/T-5269/ticket.md                           |    29 +
 tickets/T-5270/ticket.md                           |    30 +
 tickets/T-5271/ticket.md                           |    31 +
 tickets/T-5272/ticket.md                           |    30 +
 tickets/T-5273/ticket.md                           |    31 +
 tickets/T-5274/ticket.md                           |    31 +
 tickets/T-5275/ticket.md                           |    30 +
 tickets/T-5276/ticket.md                           |    43 +
 tickets/T-5277/ticket.md                           |    32 +
 tickets/T-5278/ticket.md                           |    37 +
 tickets/T-5279/ticket.md                           |    37 +
 tickets/T-5280/done-report.md                      |  3025 +++
 tickets/T-5280/ticket.md                           |    47 +
 tickets/T-5281/ticket.md                           |    37 +
 tickets/T-5282/ticket.md                           |    43 +
 tickets/T-5283/ticket.md                           |    36 +
 tickets/T-5284/ticket.md                           |    35 +
 ...
 uv.lock                                            |     2 +-
 3248 files changed, 422767 insertions(+), 16492 deletions(-)
```

### Evidence
- `tests/unit/test_docs_commands_coverage.py::TestLiveSurfaceMatchesDocsCommands::test_every_live_verb_has_a_doc_page` (pytest node id, verified passing when recorded)
- `tests/unit/test_docs_commands_coverage.py::TestLiveSurfaceMatchesDocsCommands::test_every_doc_page_names_a_registered_verb` (pytest node id, verified passing when recorded)
- `tests/unit/test_docs_commands_coverage.py::TestAssertionFiresInBothDirections::test_planted_verb_with_no_doc_is_caught` (pytest node id, verified passing when recorded)
- `tests/unit/test_docs_commands_coverage.py::TestAssertionFiresInBothDirections::test_planted_doc_with_no_verb_is_caught` (pytest node id, verified passing when recorded)
- `tests/unit/test_docs_commands_coverage.py::TestSyncMissingCommandPages::test_generate_command_page_renders_help_text_and_usage` (pytest node id, verified passing when recorded)
- `tests/unit/test_docs_commands_coverage.py::TestSyncMissingCommandPages::test_sync_writes_only_missing_pages_and_never_overwrites` (pytest node id, verified passing when recorded)
- `tests/unit/test_app_config_from_external_t1276.py::TestFromExternal::test_unsized_ack_reason_reaches_cfg` (pytest node id, verified passing when recorded)
- `tests/unit/test_app_config_from_external_t1276.py::TestFromExternal::test_ticket_points_int_field_reaches_cfg` (pytest node id, verified passing when recorded)
- `cmd:bash -c "set -x; grep -q \"~/.claude/refs/frob.md:7-10\" tickets/T-4702/done-report.md && grep -q \"Replacement:\" tickets/T-4702/done-report.md && ! git diff --name-only dev...HEAD -- \"**/refs/frob.md\" | grep -q . && echo OK-acceptance3" exit=0 sha256=a74fa0920442` (cmd evidence, exit=0)
