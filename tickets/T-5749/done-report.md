...
Ledger tiers (A1) needed a milestone tier and a story-flavour concept before
the closer/roll-up leaves (A2-A4, B1-B6) can build on top of them. Added
TicketTier.MILESTONE and StoryFlavour (user_story|quality_objective), with a
real model_validator on both Ticket and TicketSpec enforcing flavour is only
legal on tier=story -- strict at both construction and ledger-load time since
this is a brand-new field with no historical ledger rows to stay lenient for
(unlike milestone/points). Docs anchor updated in the same change so the
...
```
 ...
 docs/design/registry/check-coverage.yaml           |  1467 +-
 ...
 docs/modules/tickets-data-storage.md               |   185 +-
 ...
 src/frob/tickets/_models.py                        |   581 +-
 ...
 tickets/T-draft-4ad886c1/ticket.md                 |   124 +
 tickets/T-draft-90b33f19/ticket.md                 |    81 +
 tickets/T-draft-c099f096/ticket.md                 |   124 +
 tickets/T-draft-df99eb2d/ticket.md                 |    83 +
 ...
 uv.lock                                            |   265 +-
 4715 files changed, 876255 insertions(+), 23563 deletions(-)
```

### Evidence
- `tests/test_tickets.py::TestStoryFlavour::test_flavour_round_trips_on_story` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestStoryFlavour::test_flavour_rejected_on_non_story_tier` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestStoryFlavour::test_milestone_tier_value_exists` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestStoryFlavour::test_flavour_none_legal_on_any_tier` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestStoryFlavour::test_ticket_spec_rejects_flavour_on_non_story_tier` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestStoryFlavour::test_ticket_spec_accepts_flavour_on_story_tier` (pytest node id, verified passing when recorded)
