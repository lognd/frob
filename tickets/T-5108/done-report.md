...
frob narrative move's default (no --keep-file) mode was deleting every line
in a moved comment run, including frob:doc/frob:waive/frob:invariant
directives sitting next to the prose -- silently dropping graph edges,
waivers, and invariants with no gate firing, a structural risk since T-4694's
now-landed Tier-A auto-fix calls the same move path unattended across
hundreds of runs. Added _directive_keep_lines (src/frob/narrative/_cli.py),
which auto-detects a frob:<verb> directive lead line plus its
backslash-continuation payload inside the extent and always keeps it in
place, merged with any explicit --keep-file lines, before migrate_block
runs. Positive control (prose + frob:doc + frob:waive + multi-line
frob:invariant) proves all three directives survive byte-for-byte and none
of the prose does; a second test replicates the agent's own diff-based
detection recipe as a regression guard.
...
```
 ...
 tickets/T-draft-ea3eeb0c/ticket.md                 |    61 +
 ...
 uv.lock                                            |     2 +-
 3736 files changed, 491962 insertions(+), 22345 deletions(-)
```

### Evidence
- `tests/test_narrative_migrate.py::TestDirectiveLinesPreserved::test_directive_keep_lines_finds_lead_and_continuation_lines` (pytest node id, verified passing when recorded)
- `tests/test_narrative_migrate.py::TestDirectiveLinesPreserved::test_positive_control_keeps_directives_moves_only_prose` (pytest node id, verified passing when recorded)
- `tests/test_narrative_migrate.py::TestDirectiveLinesPreserved::test_diff_of_moved_file_removes_no_directive_line` (pytest node id, verified passing when recorded)
