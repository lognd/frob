...
Owner directive of 2026-09-20 removes ticket prose from everything a user
reads. Measured before: 274 T-#### citations in help=/description=/epilog=
argparse string literals under src/frob/_cli_parsers/ (scripts/
count_ticket_citations.py --scope help), plus 7239 in docs/**/*.md prose
(--scope docs, excluding docs/audits, docs/design/registry, and frob:
directive-grammar example lines).

Help-string half: scripts/strip_help_citations.py (AST-precise, only
touches help=/description=/epilog= literals and the one help-returning
helper _deprecated_skip_help -- never a docstring/comment, T-4691's scope)
drove the count to zero: after = 0. Verified via a fresh `frob --help`
tree capture and count_ticket_citations.py --scope help == 0.

Docs half: measured 7239 before; cleaned the 24 docs/commands+guides pages
that each carried exactly one bare citation (mechanical, safe strip,
after = 7215). The remaining bulk (docs/modules/gates.md alone: 1279) is
citations woven into gate-history narrative prose across large module/
strata/guide docs -- unsafe to strip mechanically at this budget without
risking real provenance loss; filed T-5345 as the follow-up
epic scoped to docs/modules/*.md, docs/strata/*.md, docs/guides/*.md.

Positive control: tests/unit/coordinator_suite/test_count_ticket_citations.py
plants a T-1234 in a synthetic help= string and asserts find_help_citations
reports it (and that a docstring/comment citation is NOT reported).
...
```
 ...
 scripts/count_ticket_citations.py                  |   165 +
 scripts/fleet_status.py                            |  1183 +-
 scripts/measure_evidence_reach.py                  |    17 -
 scripts/strip_help_citations.py                    |   231 +
 ...
 src/frob/_cli_parsers/_ticket/_closeout.py         |    47 +-
 ...
 src/frob/tickets/_leases.py                        |   828 +-
 src/frob/tickets/_live_tracker.py                  |    42 +-
 src/frob/tickets/_models.py                        |   425 +-
 ...
 .../test_count_ticket_citations.py                 |   170 +
 ...
 .../coordinator_suite/test_strip_help_citations.py |   164 +
 ...
 tickets/T-5134/done-report.md                      |  3788 ++++
 tickets/T-5134/ticket.md                           |   294 +
 ...
 tickets/T-5293/ticket.md                           |    63 +
 tickets/T-5294/ticket.md                           |    65 +
 ...
 uv.lock                                            |     2 +-
 3753 files changed, 492963 insertions(+), 22586 deletions(-)
```

### Evidence
- `tests/unit/coordinator_suite/test_count_ticket_citations.py::TestFindHelpCitations::test_positive_control_plants_a_citation_the_detector_must_report` (pytest node id, verified passing when recorded)
- `tests/unit/coordinator_suite/test_count_ticket_citations.py::TestFindDocsCitations::test_prose_citation_is_reported` (pytest node id, verified passing when recorded)
- `tests/unit/coordinator_suite/test_count_ticket_citations.py::TestFindDocsCitations::test_directive_grammar_example_is_exempt` (pytest node id, verified passing when recorded)
