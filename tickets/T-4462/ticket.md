...
body_changes:
- mode: append
  reason: record BUG002 waiver for CI-config-only fix
  actor: logan
  at: '2026-09-13'
  old_length: 1906
  new_length: 2060
...
frob:waive BUG002 reason="CI-config change, only measurable on the runner (run 34752551520 shows the OpenError; the fix is shell: bash + RUNNER_TEMP)"
