...
- src/frob/graph/affects.py
  - New `_ref_file(ref)` helper: strips a code symref's `::qualname`
    suffix AND a doc anchor's `#anchor` suffix, whichever is present.
  - `scope_doc_code_gaps` now calls `_ref_file` at every split site
    that computes a "file portion" of a ref (the `scoped_symbols`
    filter, the `doc_file` computed from `_doc_targets_for`'s targets,
    and both the `doc_file`/`code_file` pair computed per `DOC`/
    `DESCRIBES` edge), instead of the previous ad hoc `ref.split("::",
    1)[0]` / `target.split("#", 1)[0]` mix that only ever stripped ONE
    of the two possible suffixes depending on which side of the edge
    the ref came from.

- tests/test_graph_affects.py
  - `TestScopeDocCodeGaps.test_scoping_the_whole_doc_file_subsumes_
    its_own_anchors`: THE REPRO -- a `DESCRIBES` edge whose "code" side
    is ANOTHER anchor in the SAME doc file (`docs/x.md#foo` describes
    `docs/x.md#bar`). Before the fix, `code_file` was computed as
    `edge.target.split("::", 1)[0]` -- a no-op on an anchor string (no
    `::` present), so it stayed `'docs/x.md#bar'` and never matched a
    scope entry of bare `'docs/x.md'`, firing a spurious gap even
    though the WHOLE FILE was in scope. This is the exact shape T-3412
    was filed against (272 closure warnings on one `--add` of a
    self-cross-referencing guide).
  - `TestScopeDocCodeGaps.test_scoping_the_whole_doc_file_still_flags_
    a_genuinely_unscoped_anchor`: control -- the SAME edge shape but
    the target anchor is in a DIFFERENT doc file (`docs/y.md#bar`)
    must still be flagged. Proves the fix subsumes a doc file's OWN
    anchors without blanket-suppressing genuinely cross-file gaps.

### WHY

`scope_doc_code_gaps` (the function `frob ticket new`/`frob ticket
scope`'s closure-warning CLI surface calls, per
`_scope_closure_warnings` in `src/frob/app/ticket_runner/_new.py`)
walks `frob:doc`/`frob:describes` edges and, for each edge, computes
which FILE each side of the edge lives in so it can check that file
against the declared scope. The "file portion" extraction was
inconsistent: a doc anchor's own side was always stripped via
`split("#", 1)[0]`, but the OTHER side of the SAME edge was only ever
stripped via `split("::", 1)[0]` -- correct when that side is a code
symref (`a.py::foo`), but a no-op when that side is ITSELF a doc
anchor (`docs/x.md#bar`, as happens whenever one doc section
cross-references another, including within the SAME file). The
un-stripped `#anchor` suffix then never matched a scope glob covering
just the bare file, so scoping the whole doc file still produced one
closure warning per anchor within it -- unfollowable at scale (one
`--add` per anchor) and, per the ticket's own filing, training
operators to ignore closure warnings entirely.

Original scope was `src/frob/tickets/_scope.py`; the actual bug (and
its fix) live in `src/frob/graph/affects.py::scope_doc_code_gaps`,
which `_scope_closure_warnings` (`src/frob/app/ticket_runner/_new.py`)
calls -- `_scope.py` itself does not compute anchor/file matching at
all. Scope was widened via `frob ticket scope --add` (see Scope
section below).

### Evidence / test node ids

- tests/test_graph_affects.py::TestScopeDocCodeGaps::test_scoping_the_whole_doc_file_subsumes_its_own_anchors
  (bound; designated + checked repro against 5f67a51ef, base-ref dev)
- tests/test_graph_affects.py::TestScopeDocCodeGaps::test_scoping_the_whole_doc_file_still_flags_a_genuinely_unscoped_anchor
  (bound)

No `--accepts` index used: T-3412's own "ACCEPTANCE" is prose, not the
structured `[N]`-indexed list `frob ticket evidence --accepts` targets.

Full `tests/test_graph_affects.py` green: `PYTHONPATH=.../src python -m
pytest tests/test_graph_affects.py -q` -> `collected=14 failed=0`. No
regression in the other 12 pre-existing tests in that file.

### Commit shas (worktree branch t-3412)

- 5f67a51ef -- test(graph): repro T-3412 -- scoping a doc file doesn't subsume its anchors (repro ALONE, against pre-fix code -- this is the --designate-repro/--check-repro base-ref)
- 7d23b297d -- fix(graph): scope closure lets a scoped doc FILE cover its own anchors (the fix, reapplied on top of the isolated repro commit)
- d6080cb5e -- chore(tickets): record evidence for T-3412 (frob-generated evidence-binding commit)

### Gates run

- `ruff check` / `ruff format --check` on both touched .py files: clean.
- `ty check` on both touched .py files: clean.
- `frob check --only coverage --files src/frob/graph/affects.py --files tests/test_graph_affects.py --base dev`: no COV/DRIFT/DSL/TODO finding attributable to either touched file (gate:DOCARCH is WARN-severity and passes at 0 errors; the new test's own docstring citing T-3412 trips a DOCARCH001 WARNING, same posture as the two pre-existing warnings already on `affects`/`caller_dependent_files` in this same file -- non-blocking, not a new pattern).
- `frob check --only arch --files src/frob/graph/affects.py --files tests/test_graph_affects.py --base dev`: pass, no new ARCH001/LARGE001 (only repo-wide pattern-recommendation noise, not attributable to these files).

### Scope

Original declared scope (`src/frob/tickets/_scope.py`) did not contain
the real defect. Widened via `frob ticket scope --add` (no conflicting
lease found on either path):
- `src/frob/graph/affects.py` (the actual `scope_doc_code_gaps` bug/fix)
- `tests/test_graph_affects.py` (its existing test file)

`docs/modules/graph.md#scope-closure-t-0998` already documents the
INTENDED semantics correctly ("scoping the whole file" is the stated
design) -- this is a pure bug fix restoring code to already-documented
behavior, so no doc edit was needed.
...
```
 ...
 uv.lock                                            |    2 +-
 2148 files changed, 126047 insertions(+), 5521 deletions(-)
```

### Evidence
- `tests/test_graph_affects.py::TestScopeDocCodeGaps::test_scoping_the_whole_doc_file_subsumes_its_own_anchors` (pytest node id, verified passing when recorded)
- `tests/test_graph_affects.py::TestScopeDocCodeGaps::test_scoping_the_whole_doc_file_still_flags_a_genuinely_unscoped_anchor` (pytest node id, verified passing when recorded)
