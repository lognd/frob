...
CI run 35951365410 failed test_scan_directory_fingerprints_excludes_the_
catalog_itself: FP-DESERIALIZE-YAML-001 fired on a self-scan of frob's
own source tree. Root-caused directly: src/frob/webapp/_websec_deser.py
(T-5309, a new WEBSEC109-116 sink detector) names yaml.load/eval/exec/
os.system as the shapes it scans FOR elsewhere -- never performing them
itself -- the exact self-match class _SELF_PATTERN_SUFFIXES already
excludes _cve_fingerprint.py/_dangerous_ops_*.py for, but this new file
was never added. Added it. Verified against the whole test_fingerprint.py
file (60/60 pass) plus test_capability_scan_python.py (83/83, no
regressions from the new exclusion entry).
...
```
 ...
 uv.lock                                            |   265 +-
 4535 files changed, 790004 insertions(+), 23544 deletions(-)
```

### Evidence
- `tests/vet_suite/test_fingerprint.py::TestFingerprintScan::test_scan_directory_fingerprints_excludes_the_catalog_itself` (pytest node id, verified passing when recorded)
