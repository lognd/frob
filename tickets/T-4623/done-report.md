...
## T-4623: Clean docstrings across 24 misc test files (DOCARCH001)

### WHAT changed

Rewrote 26 flagged docstrings across 24 test files under tests/ so each
states WHAT the test proves and HOW, instead of ticket-archaeology
narrative (prior-attempt history, what a superseded approach got wrong,
which policy replaced which). No behaviour changes -- docstrings only.
Added a `# frob:ticket T-4623` directive immediately above every changed
class/def so COV002 tracks the edit against this open ticket (several
of these symbols carried only a stale directive to an already-closed
ticket, e.g. `test_tickets_lease.py`'s `# frob:ticket T-2771`).

Files touched (all 24 in ticket scope):
tests/test_gates_fmt_directives.py, tests/test_coverage.py,
tests/test_waive_gate.py, tests/test_tickets_velocity.py,
tests/test_tickets_triage_dates.py, tests/test_tickets_parent.py,
tests/test_tickets_organization.py, tests/test_tickets_live_tracker.py,
tests/test_tickets_leases.py, tests/test_tickets_lease.py,
tests/test_tickets_collision.py, tests/test_tickets_cmd_evidence.py,
tests/test_ticket_store_stale_snapshot.py, tests/test_ticket_reconcile.py,
tests/test_ticket_land_proof_claims.py, tests/test_ticket_evidence.py,
tests/test_serve_daemon.py, tests/test_scaffold_worktree_lease_hook.py,
tests/test_release.py, tests/test_lang_support.py, tests/test_excludes.py,
tests/test_evidence_integrity.py, tests/test_cache_gate.py,
tests/test_arch_gate.py

tests/test_worktree_guard.py (1 DOCARCH001 finding) was deliberately
NOT touched per the ticket body -- it is leased by in-progress T-4546.
Confirmed via `grep -l "tests/test_worktree_guard.py" .git/frob-leases/*.json`
still resolving to T-4546's lease at the time of this work; not filed as
a separate ticket per the ticket body's own instruction to "wait" (the
ticket that will pick it up is T-4546's own successor once it lands, or
a fresh scoped ticket once that lease clears -- left for the coordinator
to sequence, no ticket filed since the body did not ask for one here).

### WHY

Doctrine T-2994 (debloat sprint v0.534.0): code and docs carry UTILITY,
tickets carry NARRATIVE. DOCARCH001 flags a docstring that cites a
ticket AND reads as change-narrative (what a prior attempt got wrong)
rather than utility (how/when to reuse the code). No narrative worth
preserving needed moving into another ticket's body -- every removed
docstring was retelling the SAME ticket already named in this ticket's
own scope list (the ticket that made the original change), and that
ticket's own body already carries the incident/history; nothing was an
orphaned fact that would be lost. No `frob ticket body --append` calls
were needed as a result.

### Measurement (scoped, not repo-wide)

Direct measurement via `frob.gates._docstring_archaeology.docarch001_violations`
run against exactly the 24 ticket-scope files (frob's own `--only gates`
stage scans repo-wide internally and filters by --files only in its
report, and under this session's fleet load a scoped `--only gates`
invocation took ~50 min; the direct call is the same code path the gate
uses, filtered client-side to the 24 files, verified below against the
official `frob check --only gates` run for a second confirmation):

BEFORE (initial per-file re-measure, done before touching code):
  26 findings across 24 files (one 27th finding, in tests/test_worktree_guard.py,
  is out of scope per above).

AFTER (same direct measurement, post-edit):
  TOTAL 0

CONFIRMED via the official gate, `frob check --only gates --files <the
24 files> --base dev` (full invocation, ~50min under fleet load,
output saved at
/tmp/claude-1000/.../scratchpad/t4623_measure2.txt):
  `grep "DOCARCH001" t4623_measure2.txt` restricted to the 24 files: 0 hits.
  (gate:DOCARCH tool summary: "0 errors, 523 warnings" repo-wide --
  the 523 are pre-existing findings in OTHER files outside this
  ticket's scope, confirmed by filtering the raw output to only the
  24 scoped filenames.)

### Acceptance criterion [1]

"Given a frob check scoped to these 24 files, when DOCARCH001 is
measured, then the combined finding count is 0" -- proven by both the
direct measurement (0/0) and the official `frob check --only gates`
run filtered to these files (0 hits).

### Evidence bound (after the last commit)

- tests/test_waive_gate.py::TestWaive007RealRepo::test_zero_findings_on_real_repo
  (exercises the edited TestWaive007RealRepo docstring's own test,
  confirms no regression)
- tests/gates/test_docstring_archaeology.py::TestDocarch001Violations::test_ticket_plus_narrative_wording_warns
  (the DOCARCH001 gate's own positive-control test, proving the
  detector this ticket's measurement relies on still fires correctly)

Both bound via:
  frob ticket evidence T-4623 <node-id> --accepts 1 --base-ref dev

### Full touched-set pytest run

`PYTHONPATH=$(pwd)/src PYTEST_XDIST_AUTO_NUM_WORKERS=1 python -m pytest`
over all 24 touched files hit a pre-existing fleet-load stall
(`test_waive_gate.py::TestWaive006RealRepo::test_zero_errors_on_real_repo`
timed out at 1280s under 8+ concurrent `frob check` processes on this
host, unrelated to TestWaive007 which this ticket touched). Re-run in
isolation once load allowed:

  tests/test_waive_gate.py::TestWaive007RealRepo  ->  1 passed
  tests/gates/test_docstring_archaeology.py::TestDocarch001Violations  -> 6 passed

## Pre-READY checks

`frob check --only sys --files <the 24 files> --base dev`:
  0 SELFAUDIT001 findings attributable to any of the 24 files (the run's
  SELFAUDIT001/DRIFT/DSL findings are all in unrelated src/ files
  outside this ticket's scope -- confirmed by grepping the raw output
  for the 24 filenames).

`frob check --only arch --files <the 24 files> --base dev`:
  Tool summary: `pass  frob-arch  19 warnings (36 waived), 544 suggestions`.
  No ARCH001/LARGE001 on any of the 24 files (the only per-file hits
  against our files are pre-existing "high-coupling" (import-count)
  WARN-severity suggestions on test_coverage.py/test_evidence_integrity.py/
  test_tickets_cmd_evidence.py/test_waive_gate.py, unrelated to docstring
  text and unchanged by this edit).

`frob check --only coverage --files <the 24 files> --base dev`:
  0 COV002 findings on any of the 24 files (confirmed after adding the
  `# frob:ticket T-4623` directives; grepped the raw output for the 24
  filenames -- zero hits).

`ruff check <the 24 files>`: All checks passed! (5 pre-existing
`# noqa` warnings in test_gates_fmt_directives.py at unrelated lines
242/265/697/817/949 -- not touched by this edit, not new).

`ty check <the 24 files>`: All checks passed!

### Cross-ticket

`git diff --name-only dev...HEAD` in this worktree: exactly the 24
scoped files plus tickets/T-4623/ticket.md (evidence-record commit).
None of the 24 files are leased by any OTHER in-progress ticket
(checked `.git/frob-leases/*.json` at ROOT, excluding T-4623.json/
T-4624.json).

### Commit / HEAD

Code commit: 3a60d9b51 "docs(tests): rewrite DOCARCH001 docstrings to
state test utility"
Evidence-record commits (auto-committed by `frob ticket evidence`):
4d5703be5, ff36da8e9 (final)
HEAD: ff36da8e934ce799dc46f912be2b375ec9a29cea

### Filed

none (no out-of-scope work discovered; test_worktree_guard.py's
DOCARCH001 finding is explicitly deferred by the ticket body itself,
not a new discovery).
...
```
 ...
 tests/test_evidence_integrity.py                   |   13 +-
 ...
 tests/test_release.py                              |    9 +-
 tests/test_scaffold_worktree_lease_hook.py         |   20 +-
 tests/test_serve_daemon.py                         |   10 +-
 tests/test_testing.py                              |  106 +-
 tests/test_ticket_evidence.py                      |   31 +-
 tests/test_ticket_land_proof_claims.py             |   12 +-
 tests/test_ticket_leases.py                        |  452 ++--
 tests/test_ticket_reconcile.py                     |   11 +-
 ...
 tests/test_tickets_collision.py                    |   13 +-
 tests/test_tickets_lease.py                        |    8 +-
 tests/test_tickets_leases.py                       |    9 +-
 tests/test_tickets_live_tracker.py                 |   14 +-
 tests/test_tickets_migration.py                    |  121 +-
 tests/test_tickets_organization.py                 |    9 +-
 ...
 uv.lock                                            |    2 +-
 2182 files changed, 133227 insertions(+), 6001 deletions(-)
```
