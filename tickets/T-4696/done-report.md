...
Nine ticket field-setters (priority/kind/component/tier/milestone/sprint
plus label/accept/body) doubled frob ticket's CLI surface for zero new
capability. Folds the six genuine single-value setters into frob ticket
set <id> <field> <value>, dispatching to the same frob.tickets.set_*
library functions their standalone verbs already called -- no validation
re-derived, no new mechanism. label/accept/body stay separate: each
measured to not be a one-token field write (label is a repeatable
add/remove list; accept has three modes plus --criterion-file; body has
--append-file/--set-file), the same file-streaming disqualifier the
ticket's own body rule names, generalized here. Each folded flat verb
keeps working through a deprecation shim (sunset 2026-12-01) wrapped at
the dispatch-table level, so the pre-existing _mutate.py implementations
(outside this ticket's scope) needed no edits.
...
```
 ...
 tickets/T-4696/done-report.md                      |  2991 +++
 ...
 uv.lock                                            |     2 +-
 2966 files changed, 379555 insertions(+), 15892 deletions(-)
```

### Evidence
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_priority` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_kind` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_component` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_tier` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_milestone` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestSetRoundTrip::test_sprint` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestDeprecatedShimByteIdentical::test_priority_shim_matches_set` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_set.py::TestCitationSweep::test_no_markdown_code_fence_recommends_a_deleted_field_setter` (pytest node id, verified passing when recorded)
