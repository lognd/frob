...
Root cause: `_other_ticket_holding_live_lease` (src/frob/gates/_fix_engine_scope.py)
called `scope_matches`, which unconditionally appends T-4650's
DEFAULT_REGISTRY_FILES glob set to EVERY ticket's effective scope
(design/frob.strata among them). That made every registry file read as
"held" by ANY other in-progress ticket regardless of that ticket's real
declared or live-narrowed scope, defeating this exact module's
"narrowed live lease wins over stale declared scope" precedence
(T-2328) for the one file class it mattered most for.

Fix: `_other_ticket_holding_live_lease` now only counts a registry-file
match as genuinely held when the other ticket's OWN effective scope
(via `_scope_globs`, without the implicit registry addition) actually
names it -- the blanket, everyone-gets-it addition alone no longer
counts. Non-registry files and genuine declared/live overlaps on
registry files (e.g. a ticket scoped to `design/**`) are unaffected.

Measured: the previously-failing test now passes; the full
tests/gates_suite/test_fix_engine.py file (75 tests) passes clean
after the fix, confirming no regression to the sibling lease/scope/
revert tests that also use design/frob.strata as their fixture file.

`frob check --only gates` (even scoped with --files to just the two
touched files) did not complete within a 580s budget under this
session's heavy fleet contention -- this matches the already-filed,
known T-5153 ("frob check: the sys stage takes 1268s of a 1900s root
check and --files scoping does not skip it") -- so this evidence
relies on the targeted pytest run above plus the land dry-run rather
...
```
 ...
 src/frob/app/_config_external.py                   |    23 +
 ...
 src/frob/app/ticket_runner/_land_cmd.py            |   604 +-
 ...
 src/frob/gates/_docarch_structural.py              |   358 +
 ...
 tickets/T-5214/ticket.md                           |    30 +
 tickets/T-5215/ticket.md                           |    31 +
 tickets/T-5217/ticket.md                           |    31 +
 tickets/T-5219/ticket.md                           |    32 +
 tickets/T-5222/ticket.md                           |    31 +
 tickets/T-5228/ticket.md                           |    36 +
 tickets/T-5229/ticket.md                           |    32 +
 tickets/T-5231/ticket.md                           |    43 +
 tickets/T-5238/ticket.md                           |    30 +
 tickets/T-5240/ticket.md                           |    30 +
 tickets/T-5242/done-report.md                      |  2821 +++
 tickets/T-5242/ticket.md                           |    43 +
 tickets/T-draft-0a4d0959/ticket.md                 |    45 +
 tickets/T-draft-5464244a/ticket.md                 |    31 +
 ...
 uv.lock                                            |     2 +-
 2831 files changed, 287494 insertions(+), 15059 deletions(-)
```

### Evidence
- `tests/gates_suite/test_fix_engine.py::TestFixEngineScopeLease::test_narrowed_live_lease_wins_over_stale_declared_scope` (pytest node id, verified passing when recorded)
