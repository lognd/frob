...
T-4113: H3-3 -- outbound destination constraint + rate lint (SYS114/SYS115)
Worktree: /home/logan/projects/frob/.claude/worktrees/t-4112 (shared with T-4112)
HEAD sha: c4ceeeca25078a0700c997ac008a2d5aa14f311d
Code commit: 1e22371ec289a397ff88003db21156d94c548827
Evidence commit: c4ceeeca25078a0700c997ac008a2d5aa14f311d

RULE-ID DEVIATION FROM THE TICKET, FLAGGED EXPLICITLY: the ticket suggested
SYS111 (destination) / SYS112 (rate). Both were ALREADY LIVE rules
(T-1977's capability-ratchet check / T-2523's ambient-capability-reason
check, src/frob/gates/_sys_selfaudit.py) -- a real id collision, only
found once I grepped the actual rule-string usage rather than trusting
the ticket's suggestion. Used the next free pair instead: SYS114
(destination-constraint) / SYS115 (missing-rate lint).

WHAT changed, per file
- src/frob/strata/_outbound_destination.py (new): SYS114 fires when an
  outbound flow's `dst` node is foreign-trust (`Node.trust == "foreign"`,
  `_models.py::TRUST`'s bottom rung, this repo's "outside this system's
  trust perimeter" spelling) and the `src` node's bound code
  (`_code_binding.py::bind_code`) has no config-field-bound host
  constraint at an outbound-call-shaped site (`_CONFIG_BOUND_HOST_RE`:
  `get/post/put/request/connect(config.FIELD` vs. a quoted literal).
  Deny-by-default: absence of the proof fires, regardless of whether a
  hardcoded literal is ALSO present or the node has no bound code at all.
  SYS115 (implemented as the ticket's own "cheaper stepping stone"): among
  one node's own outbound-to-foreign flows, a flow with no `rate` while a
  SIBLING outbound-to-foreign flow (same `src`) already declares one.
  Both rules walk the same flow population (`_outbound_foreign_flows`),
  live in one module per the ticket's own "share a home" instruction.
- tests/unit/strata/test_outbound_destination.py (new): 7 unit tests --
  SYS114 must-fire (hardcoded literal host, no config binding),
  must-stay-quiet (config-bound host), must-stay-quiet (internal/non-
  foreign flow), waiver-suppression; SYS115 must-fire (missing rate with
  a sibling that has one), must-stay-quiet (every flow has a rate),
  must-stay-quiet (lone flow, no sibling to contrast against).
- tests/unit/strata/fixtures/outbound_destination/{hardcoded,configured,
  config}.py (new): synthetic stub source files matching the ticket's
  own fixture spec -- hardcoded.py's call site hardcodes a literal host,
  configured.py's reads from config.MEDIA_HOST_URL. SYNTHETIC FIXTURE,
  EXPLICITLY FLAGGED: frob makes no outbound network call to a foreign
  host as part of its own operation (its one foreign-touching flow,
  f_registry_fetch : registry -> vet, is the OPPOSITE direction -- src
  foreign, modeling a response flowing IN), so this coverage is entirely
  hand-built, never drawn from design/frob.strata.
- src/frob/strata/_waive.py: added "SYS114"/"SYS115" to
  MULTI_INSTANCE_WAIVER_FAMILIES (per-flow multi-instance, same shape as
  REL200/REL201/REL303).
- src/frob/strata/__init__.py: exported SYS_UNCONSTRAINED_DESTINATION,
  SYS_MISSING_OUTBOUND_RATE, OutboundDestinationReport,
  OutboundDestinationViolation, check_outbound_destination,
  check_outbound_rate.
- src/frob/app/sys_runner.py: wired both entrypoints into `frob sys
  audit`, folding their findings into the SAME combined ReliabilityReport
  T-4112 already extended (field-identical violation shape) -- one
  printer, no new report/printer pair.
- docs/strata/threat.md: new "Outbound destination + rate (SYS114/
  SYS115, T-4113)" section (`#sys11x-outbound-destination-obligation-
  t-4113` anchor), cross-referencing the SYS100/SYS101 file-granular gap
  this closes and the fixture-note disclosure.
- docs/design/registry/check-coverage.yaml: added CHK-GATE-SYS114 and
  CHK-GATE-SYS115.

How each acceptance criterion is proven
- (a) destination-constraint (SYS114): must-fire proven by
  test_hardcoded_literal_host_fires; must-stay-quiet proven by
  test_config_bound_host_stays_quiet (exact fixture shapes the ticket
  specified: a literal-host call site vs. a config-field-bound one).
  test_internal_flow_never_fires proves the rule is scoped to foreign-
  dst flows only. test_waived_finding_is_suppressed proves the explicit-
  waive escape hatch works.
- (b) missing-rate lint (SYS115): must-fire proven by
  test_missing_rate_with_sibling_rate_fires (a sibling flow from the same
  src DOES declare a rate); must-stay-quiet proven by
  test_every_flow_with_rate_stays_quiet. test_lone_outbound_flow_with_no_
  sibling_stays_quiet proves the "narrower/cheaper, not a bare-absence
  obligation" scoping the ticket's own framing calls for.
- Fixture synthetic, explicitly flagged: stated in the module docstring,
  the test-file docstring, and the new threat.md section.

Test node ids (evidence-bound, T-4113)
...
- ty check: clean.
- Targeted pytest: 144 passed, 0 failed (see T-4112's Done report for
  the full run list, same invocation covers both tickets).
- `frob check --only arch --files <touched>`: pass.

KNOWN GAP / BLOCKING CROSS-TICKET DEPENDENCY, explicitly flagged:
`frob check --only sys --files <touched> --base dev` reports 4
SELFAUDIT001 findings attributable to this ticket's new files:
  - src/frob/strata/_outbound_destination.py:180 -- `fs.read`
    (read_text() on the bound-file proof pass, same operation
    `_reliability.py::_files_evidence_timeout` already does) undeclared
    on node `stratamod`.
  - tests/unit/strata/fixtures/outbound_destination/{hardcoded,
    configured}.py -- `net.connect` (the `requests.get(...)` needle
    literal the fixture deliberately contains, never executed) undeclared
    on node `testsuite`.
The fix is a `via`-grant addition to design/frob.strata (both grants'
exact text drafted and verified locally, then reverted) -- BUT
`frob ticket scope T-4112 --add design/frob.strata` / `T-4113 --add
design/frob.strata` were BOTH refused: `ScopeLeaseConflict: requested
--add glob overlaps a path leased by another in-progress ticket
(T-4111)`. Per the standing brief ("refused if leased elsewhere ->
record in your report, do not touch it"), design/frob.strata was left
untouched. The coordinator needs to either sequence this ticket's land
after T-4111 releases design/frob.strata, or have T-4111's own worktree
add the via-grant (drafted text below for reuse):

  In `node testsuite`'s block, alongside the existing `may "net.connect"
  via "tests/test_refs_gate.py";`:
    may "net.connect" via
      "tests/unit/strata/fixtures/outbound_destination/hardcoded.py",
      "tests/unit/strata/fixtures/outbound_destination/configured.py";

  In `node stratamod`'s `may "fs.read" via ...` list, add
  "src/frob/strata/_outbound_destination.py" alongside the existing
  "src/frob/strata/_reliability.py" entry (identical proof-against-code
  read_text() shape).

Filed: none (no out-of-scope work found beyond the design/frob.strata
lease conflict above, which is a sequencing note, not a new defect).

Gates: frob check --only arch/coverage clean for this ticket's files;
frob check --only sys reports the 4 SELFAUDIT001 findings above, blocked
on T-4111's live lease on design/frob.strata -- not waived, not fixed,
explicitly reported for the coordinator to sequence.

## UPDATE 2026-09-19 (T-4221 lease released -- registry work completed)

T-4221 landed on dev (f27804db4), releasing its lease on design/frob.strata
and docs/design/registry/capability-via-ratchet.lock.json. The KNOWN
GAP / BLOCKING CROSS-TICKET DEPENDENCY section above is now resolved.
Worktree /home/logan/projects/frob/.claude/worktrees/t-4112 (shared with
T-4112), code commit df4d54b92:

- `git merge dev --no-edit`: clean, no conflicts.
- `frob ticket scope T-4113 --add design/frob.strata --add
  docs/design/registry/capability-via-ratchet.lock.json --add
  docs/design/registry/check-coverage.yaml --reason "leases released"`:
  succeeded.
- design/frob.strata: added the two via grants drafted in this file's
  original report, exactly as drafted:
  - `stratamod` node's `may "fs.read" via ...` list: added
    "src/frob/strata/_outbound_destination.py".
  - `testsuite` node: added a new `may "net.connect" via
    "tests/unit/strata/fixtures/outbound_destination/hardcoded.py",
    "tests/unit/strata/fixtures/outbound_destination/configured.py";`
    grant, with a comment explaining the needle-literal-never-executed
    shape.
  Also added `frob:ticket T-4113` to both node headers (COV002 fired on
  both: "node changed with no frob:ticket edge to an open ticket").
- docs/design/registry/capability-via-ratchet.lock.json: bumped
  `entries."stratamod::fs.read"` 12 -> 13 and
  `entries."testsuite::net.connect"` 4 -> 6, with reasons naming T-4113
  and the new via-list sites (see T-4112's why-file for a note on this
  file's stale duplicate top-level keys, not touched).
- src/frob/strata/_waive.py: added `frob:ticket T-4112` / `frob:ticket
  T-4113` above `MULTI_INSTANCE_WAIVER_FAMILIES` (COV002, same
  no-frob:ticket-edge finding as the strata nodes).

The 4 SELFAUDIT001 findings from the original report (2 fs.read on
stratamod, 2 net.connect on testsuite -- printed as 2 aggregate findings
by `frob check`, one per (node, atom) pair) are now gone; re-measured at
zero in a fresh `--only sys` run (see T-4112's why-file "Pre-READY checks"
section for the pasted output, which covers both tickets' shared files).

gates.md's REL303/SYS114/SYS115 rows: NOT added. `docs/modules/gates.md`
is leased by a DIFFERENT in-progress ticket, T-draft-a62505d4
("Registry-file class: append-shared, not whole-file leases", worktree
t-draft-aefc4511) -- not the T-4111 this file's original report named
(T-4111 has since landed and the lease moved on). Per the standing brief,
left untouched. The coordinator should sequence the gates.md SYS114/
SYS115 rows after T-draft-a62505d4 releases that lease.

Evidence: re-bound (idempotent, same 7 node ids, now current against
commit df4d54b92).

Filed: none new this pass.
...
```
 ...
 .../registry/capability-via-ratchet.lock.json      |   93 +-
 docs/design/registry/check-coverage.yaml           |   52 +-
 ...
 src/frob/strata/_inbound_rate.py                   |  232 ++
 src/frob/strata/_outbound_destination.py           |  362 +++
 ...
 tests/unit/strata/test_inbound_rate.py             |  132 ++
 tests/unit/strata/test_mode_conformance.py         |   65 +-
 tests/unit/strata/test_native_staleness.py         |   66 +-
 tests/unit/strata/test_outbound_destination.py     |  171 ++
 ...
 uv.lock                                            |    2 +-
 2174 files changed, 136645 insertions(+), 5792 deletions(-)
```

### Evidence
- `tests/unit/strata/test_outbound_destination.py::TestOutboundDestinationConstraint::test_hardcoded_literal_host_fires` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundDestinationConstraint::test_config_bound_host_stays_quiet` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundDestinationConstraint::test_internal_flow_never_fires` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundDestinationConstraint::test_waived_finding_is_suppressed` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundRateLint::test_missing_rate_with_sibling_rate_fires` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundRateLint::test_every_flow_with_rate_stays_quiet` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_outbound_destination.py::TestOutboundRateLint::test_lone_outbound_flow_with_no_sibling_stays_quiet` (pytest node id, verified passing when recorded)
