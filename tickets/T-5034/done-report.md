...
Root cause confirmed by direct instrumentation (temporary print statements,
reverted): `_required_release_bump` called `_ticket_runner._graph_snapshot(root)`,
whose cache-or-build fallback trusts a `load_graph(cache)` HIT as-is with
no staleness check against `root`'s current tree. `root` here is the
temporary squash worktree (`/tmp/frob-land-squash-*/wt`) `land()` builds
the new commit onto. That worktree's `.frob/cache.db` already existed
(and was a hit) from an EARLIER stage of the SAME land pipeline (a
covers-scope/gate check that also builds a graph snapshot on this same
squash worktree, before the squash-apply writes the new file(s) onto it)
-- so `diff_class` read a stale, EMPTY snapshot (0 symbols) even though
the squashed tree genuinely added a new public function, and computed
BumpClass.NONE instead of MINOR.

Fix: call `frob.graph.build_graph` directly instead of the cache-trusting
`_graph_snapshot` helper. `build_graph` incrementally reconciles the
cache against the CURRENT file state (mtime/hash staleness checks,
`_prune_stale_cache`) rather than blindly trusting whatever a prior
land-pipeline stage already cached there, so the snapshot this bump
computation reads is always current as of this call.

Measured: tests/ticket_land_suite/test_release.py::TestRealCallbackStaleWorktreeManifest::test_stale_worktree_manifest_still_lands_main_plus_one
now passes (was failing on dev tip: CHANGELOG.md stayed at 0.183.0
instead of computing the real 0.184.0 minor bump). Full regression
sweep: tests/ticket_land_suite/test_release.py (36 tests),
tests/tickets/test_land_squash.py, tests/unit/test_land_squash_stage.py,
...
tests/unit/test_land_cmd_backpressure.py -- 95 tests total, 0 failed.
...
```
 ...
 uv.lock                                            |     2 +-
 2974 files changed, 386665 insertions(+), 15912 deletions(-)
```
