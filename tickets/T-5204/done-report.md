...
T-5138 acceptance [5]/DESIGN item 7 was out of T-5138's own declared scope
(doctor.py not in it). T-5269 was an exact duplicate of this ticket and was
dropped, absorbed into T-5204. This ticket wires the missing piece: frob
doctor compares installed ruff/ty/mypy (PyPI) and eslint (npm) versions
against each registry's latest published release, cached 24h under
.frob/tool-version-lag-cache.json, and reports a finding once a tool is 2+
minor versions behind. Every network call is a typani Result
(LintToolLagError.Unavailable/UnparseableResponse) gated by net_enabled()
(the FROB_DISABLE_NET kill switch, same T-0822 posture frob.vet._osv
already uses); every test monkeypatches the fetcher or pre-seeds the cache,
so the suite never touches the real network. clippy is disclosed residue:
it ships as a rustup component with no standalone PyPI/npm/crates release,
so there is no registry query for it -- not silently guessed at.
...
```
 ...
 tickets/T-5285/ticket.md                           |    47 +
 tickets/T-5286/ticket.md                           |    48 +
 ...
 uv.lock                                            |     2 +-
 3251 files changed, 423887 insertions(+), 16943 deletions(-)
```

### Evidence
- `tests/unit/test_doctor.py::TestLintToolVersionLag::test_stale_tool_is_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLintToolVersionLag::test_fresh_tool_is_not_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLintToolVersionLag::test_missing_tool_is_skipped` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLintToolVersionLag::test_fetch_false_serves_only_from_cache` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLintToolVersionLag::test_fresh_cache_entry_skips_the_network` (pytest node id, verified passing when recorded)
