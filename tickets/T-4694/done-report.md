...
Registered frob narrative move's migrate_block/set_body engine as DOCARCH002
check 2's Tier-A --fix handler so frob check --fix moves a flagged citation
run automatically instead of only printing the remedy. The ledger write
happens before the file write (T-2994 constraint 1: a half-applied fix never
loses narrative from both places), reuses set_body's T-2678 archived-ticket
routing (constraint 3) and migrate_block's own marker check for idempotency
(constraint 4), and docs/commands/narrative.md now discloses that the fix
always moves the whole cited run rather than attempting the load-bearing/
archaeology split (constraint 2). All 5 acceptance criteria bound to tests
in tests/narrative/test_docarch002_fix.py.
...
```
 ...
 src/frob/gates/_fix_engine.py                      |   414 +-
 ...
 tests/gates_suite/test_fix_engine.py               |   132 +
 ...
 uv.lock                                            |     2 +-
 3262 files changed, 425306 insertions(+), 16944 deletions(-)
```

### Evidence
- `tests/narrative/test_docarch002_fix.py::TestFixDocarch002NarrativeMove::test_positive_control_moves_prose_and_leaves_pointer` (pytest node id, verified passing when recorded)
- `tests/narrative/test_docarch002_fix.py::TestFixDocarch002NarrativeMove::test_second_run_is_idempotent` (pytest node id, verified passing when recorded)
- `tests/narrative/test_docarch002_fix.py::TestFixDocarch002NarrativeMove::test_archived_ticket_writes_archive_path_and_ticket_list_stays_clean` (pytest node id, verified passing when recorded)
- `tests/narrative/test_docarch002_fix.py::TestFixDocarch002NarrativeMove::test_negative_control_directive_citation_is_untouched` (pytest node id, verified passing when recorded)
- `tests/narrative/test_docarch002_fix.py::test_docs_disclose_the_whole_block_move_limitation` (pytest node id, verified passing when recorded)
