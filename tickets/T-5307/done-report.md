...
T-5307 extends SEC005's taint_gate with frob.webapp._websec_sinks, a
framework-gated WEBSEC101-106 registry covering the DOM/template XSS sink
corpus T-5141 names: JS/TS/JSX innerHTML/outerHTML/document.write/
insertAdjacentHTML and dangerouslySetInnerHTML via tree-sitter raw_tree,
Vue v-html via a text-regex over the template_element subtree, and
Jinja/Django/Rails autoescape-bypass sinks via text-regex (no tree-sitter
grammar for those config shapes). Folded into taint_gate's existing
Violation output rather than a second gate registration, since it is the
same source-reaches-sink-with-no-sanitizer-hop shape SEC005 already uses,
and reuses T-5302's detect_frameworks short-circuit so a non-web repo
pays only one cheap detect call. One positive + one negative fixture per
rule proves both the firing and the clean (literal/sanitized) case.
...
```
 ...
 tests/unit/test_websec_sinks.py                    |    92 +
 ...
 uv.lock                                            |     2 +-
 3995 files changed, 614080 insertions(+), 23467 deletions(-)
```

### Evidence
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc101_positive-WEBSEC101-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc101_negative-WEBSEC101-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc102_positive-WEBSEC102-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc102_negative-WEBSEC102-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc103_positive-WEBSEC103-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc103_negative-WEBSEC103-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc104_positive-WEBSEC104-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc104_negative-WEBSEC104-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc105_positive-WEBSEC105-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc105_negative-WEBSEC105-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc106_positive-WEBSEC106-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_fixture[webesc106_negative-WEBSEC106-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::test_websec_sink_findings_no_framework_short_circuits` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_sinks.py::TestTaintGateWebsecExtension::test_taint_gate_emits_websec_violation` (pytest node id, verified passing when recorded)
