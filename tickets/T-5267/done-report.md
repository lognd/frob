...
T-5139 landed the doctor.py tool registry (relevant_tool_findings) unwired,
because src/frob/check/*.py and src/frob/gates/__init__.py both carried
other tickets' live leases at that ticket's time. This ticket closes that
gap: TOOL001/TOOL002 wire relevant_tool_findings into frob.gates so a
relevant-and-missing/failed tool's served rules go loudly UNMEASURED and
fail the run (Severity.ERROR, a deliberate deviation from UNRESOLVED's
usual never-fails posture, per T-5139's own design), overridable via
[tool_registry].allow_missing in frob.toml or frob:waive TOOL001/TOOL002.
TOOL003 is the DUP/ARCH regrowth guard for a bare shutil.which(...) call
outside the registry. Both gates ride the existing per-family gate:TOOL
stage every rule family already gets, so frob check/vet/test/ticket land
(which all resolve through frob.check's own family/exit-code path) render
and fail on it with no bespoke summary code needed.

The dedicated `--allow-missing-tool NAME --reason` CLI flag T-5139's
design named needs src/frob/_cli_parsers/_check.py and
src/frob/app/ticket_runner/_land_cmd.py, both outside this ticket's
declared scope -- filed as follow-up residue, not silently dropped.
...
```
 ...
 src/frob/gates/__init__.py                         |   500 +-
 ...
 uv.lock                                            |     2 +-
 3250 files changed, 423756 insertions(+), 16943 deletions(-)
```

### Evidence
- `tests/unit/gates/test_tool_registry_gate.py::TestToolRegistryGate::test_missing_relevant_tool_is_tool001` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestToolRegistryGate::test_failed_relevant_tool_is_tool002` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestToolRegistryGate::test_allow_missing_tool_suppresses_tool001` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestToolRegistryGate::test_no_findings_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestBareShutilWhichGate::test_flags_bare_shutil_which` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestBareShutilWhichGate::test_registry_module_itself_is_exempt` (pytest node id, verified passing when recorded)
- `tests/unit/gates/test_tool_registry_gate.py::TestBareShutilWhichGate::test_clean_file_reports_nothing` (pytest node id, verified passing when recorded)
