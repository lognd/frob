...
CI red on dev tip: changelog.d/T-4759.md was a stray fragment left over
from a T-4759 land that got requeued afterward (ticket state is now
"queued", not "done"), tripping the "every changelog fragment belongs to
a done ticket" invariant. Deleted the stray fragment rather than editing
any ledger file. test_release.py is fully green (71/71) after the fix.
...
```
 ...
 uv.lock                                            |     2 +-
 3260 files changed, 424741 insertions(+), 16943 deletions(-)
```
