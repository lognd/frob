...
strata-core/src/parse/grammar_core.rs
  - New `Parser::parse_dotted_path` helper: `IDENT ("." IDENT)*`, used by
    `module`, `import`, and the `accepts ... from` clause. `*` is not a
    lexable character anywhere in this grammar (lexer.rs's symbol set),
    so `import a.*` fails at the LEXER, before any parser rule runs.
  - `ModuleAst` gained three fields, all defaulting empty/None so every
    pre-existing file is unaffected: `imports: Vec<Value>`,
    `exports: Vec<Value>`, `layer: Option<i64>`.
  - `parse_module` now parses a dotted module name (`module a.b;`) instead
    of a single ident; the zero-`.` case is byte-identical to before.

strata-core/src/parse/grammar_module.rs (new file)
  - `parse_import`: `import a.b as c;` -- dotted path + mandatory alias.
    Trailing `;` optional (this grammar has no top-level `;` terminator
    anywhere else, but the ticket's own examples use one).
  - `parse_export`: `export { node X; channel Y; label Z; }`. Absent or
    empty block both mean "fully private module" (D-M1) and are
    indistinguishable in the AST.
  - `parse_layer`: `layer N;` -- D-M9's hierarchy-position declaration
    (owner ruling 2026-09-19 19:30, tickets/T-draft-0bcabfa4: hierarchy,
    import up only, flows declared by the lower module, accept down by
    reference). Lower number = higher in the hierarchy (platform = 0). At
    most once per file (duplicate is a parse error).
  - `check_accepts_are_not_aliases`: post-parse pass over the whole file
    -- refuses `accepts FLOW from X;` when X exactly matches one of this
    file's own `import ... as X;` aliases, since D-M9's whole point is
    that `accepts` names the peer module BY REFERENCE, never through a
    local alias.

strata-core/src/parse/grammar_node.rs
  - `parse_node` gained the `accepts FLOW from MODULE;` clause (same
    clause-loop position as `may`/`carries`/`waive`), pushed into a new
    per-node `accepts: Vec<{flow, from}>` field.

strata-core/src/parse/grammar_policy.rs
  - `parse_program` dispatch: `layer` (requires a module first, refused
    inside a fragment), `import`/`export` (same posture), and a final
    `check_accepts_are_not_aliases` call before returning the parsed AST.

strata-core/src/parse/mod.rs
  - 11 new colocated unit tests (see Evidence below), including two that
    embed the REAL design/frob.strata and all 7 design/litmus/*.strata
    files at compile time (`include_str!`) and assert they still parse
    with empty imports/exports/layer.

docs/strata/surface.md
  - New "Import/export grammar (T-5125, parse-only)" subsection
    under "Module system": full grammar sketch, `layer N;`'s rationale
    (chosen over "module X above Y, Z;" per the ticket's own menu -- a
    single rank number answers both direction questions the linker needs
    without hand-listing every pairwise edge), the accepts-by-reference
    rule, and the alias-rejection rule enforced at parse time.
  - Updated the `Module` AST field-list bullet to note the two new JSON
    fields and that binding them onto the pydantic model is
    T-draft-9d041fdf's job, not this ticket's.

WHY
---
T-5125 is the grammar-only first leaf of the module-system
epic (T-4662/T-draft-0a0c7b43). The owner's D-M9 ruling (recorded
verbatim in tickets/T-draft-0bcabfa4, superseding an earlier draft of
the same decision I had initially recorded) fixed import direction by
an explicit module hierarchy rather than accepts-by-reference alone:
modules rank themselves via `layer N;`, imports only go to a lower
number, flows are declared by the lower module, and `accepts` is the
upper module's downward, reference-only acknowledgment. This ticket
implements exactly the grammar surface that ruling requires and nothing
past parse-only -- name resolution, the elaborator's private-by-default
binding, and the linker's layer-ordering/cycle enforcement are
T-draft-9d041fdf and T-draft-27d3ece1's jobs.

How each acceptance criterion is proven
----------------------------------------
[1] import/export/accepts round-trip into the AST with dotted paths and
    aliases:
    strata-core/src/parse/mod.rs::tests::parses_import_export_and_accepts

[2] `import tickets.*;` (or any wildcard/re-export) is a syntax error:
    strata-core/src/parse/mod.rs::tests::import_wildcard_is_a_syntax_error
    (fails at the LEXER with "unexpected character '*'", confirmed by
    hand against the compiled extension too -- see Manual verification.)

[3] design/frob.strata and all 7 design/litmus/*.strata parse unchanged:
...
[4] the D-M9 hierarchy declaration form is part of the grammar (at most
    once per file):
...
[5] `accepts` takes a module reference, never an import alias:
...
Full cargo test suite for the crate: 222 passed, 0 failed (all 209
pre-existing tests unchanged plus 13 new ones -- 11 for this ticket's
grammar, 2 more than the 9 bound above since two are pure sanity checks
not tied to a numbered acceptance criterion: `parses_dotted_module_name`,
`layer_is_absent_by_default`, `empty_export_block_is_a_fully_private_module`,
`import_requires_alias`).

Manual verification (Rust build with maturin is expected but maturin is
not installed in this sandbox -- see Pre-READY notes): built the
extension-module cdylib directly with `cargo build --release --features
extension-module` (PYO3_PYTHON pinned to the project .venv's Python
3.11.15, LD_LIBRARY_PATH pointed at the matching libpython3.11.so from
~/.local/share/uv/python), loaded it as `strata_core` from a scratch
directory, and confirmed by hand:
  - `import a.*` -> `{"err": {"message": "unexpected character '*'", ...}}`
  - the full import/export/accepts/layer example round-trips into the
    exact expected JSON shape
  - all 8 real files (design/frob.strata + 7 litmus files) parse with
    `imports: [] exports: [] layer: null`
The pydantic `Module` model (src/frob/strata/_ast.py, out of this
ticket's scope) has no `extra="forbid"` on any model, confirmed by
inspection, so the new JSON keys are silently ignored by every existing
Python caller until T-draft-9d041fdf binds them -- no downstream break.

Commits (all on branch t-draft-1f0f55cb)
-----------------------------------------
65f7adad4 feat(strata-core): parse-only import/export/accepts module grammar
a179cddf5 feat(strata-core): add D-M9 hierarchy layer declaration and alias check
17e19ff95 chore(strata-core): bind parse_node's accepts clause to an open ticket
34edd21c2 fix(strata-core): move frob:ticket directive onto its own comment line
ef95a6928 test(strata-core): parse the real design/frob.strata and litmus files
Current HEAD after evidence binding: 59002f5c6

Note on worktree history: this ticket's first four commits were
originally made in worktree t-draft-ea93df7b before the coordinator
identified that worktree had been silently repointed to another
ticket's branch (T-4688) mid-session. The coordinator rescued the work
onto branch `rescue-T-5125`; I created this ticket's own
worktree via `frob ticket work T-5125` from ROOT and
cherry-picked all 4 commits cleanly (no conflicts), then continued
(added the design/frob.strata + litmus positive-control tests) here.

Filed: none (no out-of-scope work discovered; the D-M9 spelling
question the ticket's owner-decision addendum left open -- `layer N`
vs. `module X above Y, Z;` -- was resolved in-scope by picking `layer N`
and documenting the choice in surface.md, since STRATA-MODULES.md
explicitly asked the grammar leaf to "pick one, document it").

## Pre-READY checks

`frob check --only sys --files strata-core/src/parse/grammar_core.rs --files strata-core/src/parse/grammar_node.rs --files strata-core/src/parse/grammar_policy.rs --files strata-core/src/parse/grammar_module.rs --files strata-core/src/parse/mod.rs --files docs/strata/surface.md --base dev`:
  pass gate:DOCARCH (0 errors), FAIL gate:DRIFT (6 errors, all pre-existing/unrelated,
  e.g. src/frob/tickets/_land_squash.py, src/frob/gates/invariants.py -- none in my
  files), FAIL gate:DSL (1 error: tests/test_app.py:387 pre-existing '# noqa: E501'
  directive, not mine), pass gate:PROFILE, pass gate:WAIVE, pass gate:SELFAUDIT
  (0 errors -- the only SELFAUDIT001 seen in an earlier run, design:1 testsuite
  via-list growth, is unrelated to Rust/docs files and did not recur in the final
  run). Zero findings attributable to strata-core/** or docs/strata/surface.md.

`frob check --only arch --files <same six files> --base dev`:
  pass frob-arch (21 warnings, 546 suggestions, all pre-existing repo-wide
  pattern-recommendations). Two large-file notices (strata-core/src/lib.rs 1168
  lines, strata-core/src/parse/mod.rs 2368 lines, mostly pre-existing colocated
  tests per that file's own docstring) did not fail the gate.

`frob check --only coverage --files <same six files> --base dev`:
  FAIL gate:COV (10 errors, all pre-existing in src/frob/vet/**, unrelated to my
  files), FAIL gate:DRIFT/DSL/TODO (all pre-existing elsewhere: src/frob/perf/*,
  src/frob/strata/_secrets.py, tests/test_app.py). The one COV002 that WAS mine
  (strata-core/src/parse/grammar_node.rs::Parser.parse_node changed with no
  frob:ticket edge to an OPEN ticket -- the file's existing directive named
  closed T-1627) was fixed by adding `frob:ticket T-5125` directly on
  parse_node (commit 17e19ff95, syntax fixed in 34edd21c2) and does not recur.

`ruff check` / `ty check`: not applicable -- no Python files touched
(git diff --name-only dev...HEAD confirms zero .py files in this ticket's diff).

`cargo test --release` (strata-core, PYO3_PYTHON pinned + LD_LIBRARY_PATH set):
...
```
 ...
 uv.lock                                            |    2 +-
 2238 files changed, 145332 insertions(+), 7513 deletions(-)
```

### Evidence
- `strata-core/src/parse/mod.rs::tests::parses_import_export_and_accepts` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::import_wildcard_is_a_syntax_error` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::design_frob_strata_still_parses_after_the_module_grammar_change` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::all_seven_litmus_files_still_parse_after_the_module_grammar_change` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::parses_layer_declaration` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::duplicate_layer_statement_is_a_parse_error` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::accepts_with_an_import_alias_is_a_parse_error` (pytest node id, verified passing when recorded)
- `strata-core/src/parse/mod.rs::tests::accepts_from_a_non_aliased_dotted_path_still_parses` (pytest node id, verified passing when recorded)
