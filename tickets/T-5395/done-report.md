...
IMPORTANT CORRECTION vs. the original framing: the CI failure was NEVER
about the 249 reserved WEBSEC/COMPLY/A11Y/SEO/WEBPERF/SQL rule ids from
T-5301. tests/gates_suite/test_sys_rule_liveness.py's own
_LIVENESS_FAMILIES tuple only covers ("SYS", "SELFAUDIT", "REL", "PII",
"THREAT", "VMOD", "CLAIM") -- WEBSEC/COMPLY/A11Y/SEO/WEBPERF/SQL are not
in that tuple at all and were never going to be checked by this test,
reserved or not, so there was no PERF015-018/T-5136-style reservation
exemption to find or add (checked: no such mechanism exists anywhere in
this file or in _waive.py's own WEBSEC reservation comment, which only
talks about frob:waive round-tripping through the known-rule-id check,
a completely different gate).

The REAL failure on re-run: 8 real, recently-registered rule ids in the
covered families had no _LIVENESS_FIXTURES entry: CLAIM001, PII005,
PII013, REL303, SYS114, SYS115, SYS116, SYS117. Every one of them
already has an existing, passing test elsewhere in this repo that plants
the exact violation and asserts the rule fires (found via git grep, no
new fixtures written) -- added the 8 mapping entries, no rule code
touched. All 4 tests in tests/gates_suite/test_sys_rule_liveness.py
pass.

Hand-checked the two things the dry-run does not measure (T-draft-
c7aa1ed2): no new file-reading module (single test-file dict edit only,
no design/frob.strata gap); no docs touched at all.
...
```
 ...
 uv.lock                                            |     2 +-
 3981 files changed, 626174 insertions(+), 23491 deletions(-)
```
