...
Adds T-3004 section 7's multi-level invariants: Invariant gains optional
level/evidence_levels fields (schema-only, L1) so a declared invariant
can name its V-model artifact level and tag individual evidence entries
with the level they were verified at, then INVLVL001 (L2, new id per
owner review since INV003 already exists) flags a tagged entry whose
level does not match the invariant's paired test level. Both additions
are strictly optional so no existing invariants/*.md file is affected.
Not yet wired into frob check's dispatch table: src/frob/gates/__init__.py
and _waive.py were held by a concurrent T-3010 scope lease throughout
this ticket's work; filed T-5759 to finish the registration
once that lease frees. Verified with the new gate's 5 unit tests plus the
full existing INV-family suite (86 tests total, all passing).
...
```
 ...
 src/frob/gates/_invariant_level.py                 |    95 +
 ...
 src/frob/gates/invariants.py                       |   130 +-
 src/frob/ghio.py                                   |    17 -
 ...
 tests/gates/test_invariant_level.py                |    96 +
 ...
 uv.lock                                            |   265 +-
 4414 files changed, 763875 insertions(+), 23542 deletions(-)
```

### Evidence
- `tests/gates/test_invariant_level.py::TestInvariantLevelGate::test_quiet_when_no_level_declared` (pytest node id, verified passing when recorded)
- `tests/gates/test_invariant_level.py::TestInvariantLevelGate::test_fires_on_a_level_mismatch` (pytest node id, verified passing when recorded)
- `tests/gates/test_invariant_level.py::TestInvariantLevelGate::test_quiet_when_verified_at_the_paired_level` (pytest node id, verified passing when recorded)
- `tests/gates/test_invariant_level.py::TestInvariantLevelGate::test_quiet_for_untagged_evidence` (pytest node id, verified passing when recorded)
- `tests/gates/test_invariant_level.py::TestInvariantLevelGate::test_multiple_invariants_only_mismatched_one_fires` (pytest node id, verified passing when recorded)
