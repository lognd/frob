...
Deletes the four duplicate CLI surfaces T-4690 targets (quality/design/ops
group verbs, outline/map/xref/docs-search explore mirrors, fmt, whereis,
verify status/fleet status) through one shared deprecation-shim mechanism
(frob._cli_parsers._shims.announce_shim, frob.app.app._DEPRECATED_SPELLINGS)
instead of N bespoke ones, so the CLI stops growing duplicate names with no
removal path. explore survives as the one group verb per the 2026-09-19
coordinator amendment; every other deleted/deprecated spelling still works,
with a stderr notice, through its 2026-12-01 sunset window, then fails
loudly. The one flat mirror already unreachable before this ticket
(docs-search) is deleted outright rather than shimmed, since there was no
...
```
 ...
 tickets/T-4690/done-report.md                      |  2829 +++
 ...
 uv.lock                                            |     2 +-
 2880 files changed, 313925 insertions(+), 15306 deletions(-)
```

### Evidence
- `tests/unit/test_cli_shims.py::TestIsPastSunset::test_before_sunset_is_false` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestIsPastSunset::test_on_sunset_is_false` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestIsPastSunset::test_after_sunset_is_true` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestAnnounceShim::test_before_sunset_prints_notice_and_returns` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestAnnounceShim::test_after_sunset_exits_nonzero` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestAnnounceShim::test_never_writes_to_stdout` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestDeprecatedSpellingsTable::test_every_deleted_group_verb_is_covered` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestDeprecatedSpellingsTable::test_every_deleted_explore_mirror_is_covered` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestDeprecatedSpellingsTable::test_verify_status_and_fleet_status_redirect_to_top_level_status` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestSuppressedFromUsageLine::test_deleted_names_absent_from_usage_choices` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestSuppressedFromUsageLine::test_explore_survives_in_usage_choices` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestPrintWhereis::test_plain_output_names_the_live_executable` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestPrintWhereis::test_json_output_is_parseable` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestCitationSweep::test_no_markdown_code_fence_recommends_a_deleted_group_verb` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_group_parity.py::TestExploreGroupParity::test_docs_search_has_no_flat_twin` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_group_parity.py::TestExploreGroupParity::test_every_explore_leaf_matches_its_flat_twin[xref]` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [1] replace: 'Given the built argparse tree, when frob --help runs after this ticket, then explore, quality, design, ops, fmt, docs, whereis and the verify status / fleet status spellings are absent from the usage line' -> 'Given the built argparse tree, when frob --help runs after this ticket, then explore SURVIVES in the usage line while its standalone mirrors outline, map, xref and docs-search are absent, and quality, design, ops, fmt, docs, whereis and the verify status / fleet status spellings are absent' (reason: 2026-09-19 coordinator review: explore is the surviving verb, not a deletion target; removing the delete-then-rebuild cycle between this ticket and T-4695; logan, 2026-09-19)
