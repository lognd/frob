...
T-5324's a11y_findings implemented (root, frameworks), but the real T-5323
a11y_gate discovers and calls every hook as a11y_findings(ctx, frameworks)
once per already-parsed file -- a TypeError on any repo with a detected
framework. Reworks the module to the per-file hook contract: the
statement page is matched by ctx.file against a candidate table, and
section findings fire only on that file's own ctx. Renumbers the
statement rule ids off A11Y107-114 (which collided with T-5323's own
A11Y101-115 structure rules) onto the unclaimed A11Y116-A11Y128 band.
Drops the "no statement page anywhere" precondition finding: an earlier
Path.cwd()-approximated version of that check measurably produced a false
positive on an unrelated file in a sibling hook's own gate test, so it is
documented as a deliberate known gap instead. Adds a real end-to-end test
that runs frob.gates._a11y_gate.a11y_gate() over a throwaway git repo and
asserts it does not raise and the rule fires on a planted fixture.
...
```
 ...
 uv.lock                                            |     2 +-
 4275 files changed, 721918 insertions(+), 23508 deletions(-)
```
...
- `tests/unit/test_webapp_a11y_statement.py::test_a11y_findings_non_statement_file_is_always_quiet` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_statement.py::test_locate_statement_page_explicit_root` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_statement.py::test_a11y_findings_no_frameworks_short_circuits_to_empty` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_statement.py::test_a11y_gate_does_not_crash_and_fires_statement_rule` (pytest node id, verified passing when recorded)
