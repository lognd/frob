...
WHAT changed:
- docs/strata/kernel.md: deleted the false sentence at line 29 ("no other
  kernel extension exists or is planned"). Reworded the "Conditional flows"
  section heading to drop the "the one extension" claim and added a pointer
  to the new "Kernel extensions beyond the six primitives" section. Added
  that new section naming all eight domains measured non-sugar in
  scratchpad/STRATA-KEYWORDS.md (code binding, capability via-lists,
  waivers, entity/architecture, vmodel, policy, host/ACL, kerberos), each
  with a Law-1 record stating what crosses into the prover (nothing, for
  domains 3-8; only derived tier-2 facts for domain 1; only the `may` atom
  itself for domain 2) versus what stays Python-side/non-Datalog.
- tests/unit/strata/test_kernel_doc_extensions.py (new): three tests --
  (1) asserts the false "no other kernel extension" sentence is gone,
  (2) asserts all eight domain markers are named in the doc,
  (3) asserts a "Law-1 record" appears at least once per domain (8+).

WHY: T-4681/SF-21 measured docs/strata/kernel.md:29 as false -- ~79 of 139
strata keywords are non-sugar across eight domains the six-primitive
charter did not name. T-4681's decision keeps kernel.md as the spec but
requires it name these domains as deliberate extensions rather than deny
them, each with a law-1 record per charter law 1 ("the prover never learns
a domain word"). This leaf is documentation-only: it does not decide
whether any domain should later be desugared into the six primitives
(that is the eight per-domain decision leaves T-5033/5037/5074/5076/5077/
5078/5079/5080, blocked by the module-system story).

How each acceptance criterion is proven:
- [0] test_false_no_other_extension_claim_is_gone (false sentence removed)
  + test_all_eight_extension_domains_are_named (all eight domains present)
  together prove: the false sentence is gone AND all eight domains are
  named. Verified failing at HEAD c8f56ef10 before the doc edit (positive
  control: ran the same 3 tests against `git show HEAD:docs/strata/
  kernel.md` restored in place -- all 3 failed, 0 found).
- [1] test_law_one_record_present_for_each_domain proves each domain
  carries a law-1 record (>= 8 "Law-1 record" occurrences, one per
  domain, matched via regex tolerant of the doc's line-wrapping).
- [2] Not independently testable (a documentation completeness/scope
  claim); satisfied by construction -- the new section documents the
  eight domains exactly as measured in scratchpad/STRATA-KEYWORDS.md
  without proposing or performing any desugaring, matching the ticket's
  explicit SCOPE NOTE.

Test node ids (bound via `frob ticket evidence`, all --base-ref dev):
- tests/unit/strata/test_kernel_doc_extensions.py::test_false_no_other_extension_claim_is_gone (accepts 1, 3)
- tests/unit/strata/test_kernel_doc_extensions.py::test_all_eight_extension_domains_are_named (accepts 1, 2, 3)
- tests/unit/strata/test_kernel_doc_extensions.py::test_law_one_record_present_for_each_domain (accepts 1, 2)

Filed: none (no out-of-scope discoveries).

Commit shas (branch t-4952):
- 22fa2a333 docs(strata): correct kernel.md's false no-other-extension claim
- evidence-binding commits (chore(tickets): record evidence for T-4952)
  follow it; the last is the HEAD reported in the READY line.

## Pre-READY checks
- `frob check --only sys --files docs/strata/kernel.md --files tests/unit/strata/test_kernel_doc_extensions.py --base dev`: gate:DOCARCH pass, gate:PROFILE pass, gate:WAIVE pass; gate:DRIFT/gate:DSL errors present are all pre-existing, already-waived, and attributed to unrelated files (src/frob/app/ticket_runner/_rapid_sweep.py, src/frob/gates/invariants.py, src/frob/tickets/_evidence.py) -- none attributable to docs/strata/kernel.md or the new test file. Zero SELFAUDIT001 on my files.
- `frob check --only arch --files docs/strata/kernel.md --files tests/unit/strata/test_kernel_doc_extensions.py --base dev`: pass (frob-arch 21 warnings (36 waived), 546 suggestions; no ARCH001/LARGE001 on my files).
- `frob check --only coverage --files docs/strata/kernel.md --files tests/unit/strata/test_kernel_doc_extensions.py --base dev`: the only kernel.md-referencing findings are pre-existing waived COV007s on src/frob/strata/_claims.py and _facts.py helpers named in kernel.md's Capacity semantics section (unrelated to my edit); zero COV002 on the new test file's symbols.
- `ruff check tests/unit/strata/test_kernel_doc_extensions.py`: All checks passed!
- `ty check tests/unit/strata/test_kernel_doc_extensions.py`: All checks passed!
- `cd /home/logan/projects/frob/.claude/worktrees/t-4952 && nice -n 10 /home/logan/projects/frob/.venv/bin/frob ticket land T-4952 --dry-run --worktree /home/logan/projects/frob/.claude/worktrees/t-4952`:
  `land T-4952: DRY RUN clean -- merged=True wip_committed=False (would finalize/close/squash-apply/commit onto /home/logan/projects/frob)` -- zero ERROR lines.

HEAD sha (branch t-4952): 50fbb3d5592e5f9882f222d0927ccf3d1d53c8a7
...
```
 ...
 tickets/T-4952/done-report.md                      |   80 +
 ...
 uv.lock                                            |    2 +-
 2442 files changed, 182998 insertions(+), 12091 deletions(-)
```

### Evidence
- `tests/unit/strata/test_kernel_doc_extensions.py::test_law_one_record_present_for_each_domain` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_kernel_doc_extensions.py::test_all_eight_extension_domains_are_named` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_kernel_doc_extensions.py::test_false_no_other_extension_claim_is_gone` (pytest node id, verified passing when recorded)
