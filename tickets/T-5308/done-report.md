...
Adds WEBSEC117-122 (header/URL/log injection, WebSocket origin/WSS enforcement), the header/URL/log leaf of the T-5140 web-app epic, reusing T-5307's framework-detection contract and taint-pass shape.
Wiring uses a new generic discovery mechanism in taint_gate (pkgutil/importlib over frob.webapp._websec_* modules exposing websec_findings) instead of a hand-edit, since both direct-edit files were leased by concurrent siblings (T-5311, T-5395) at scope time; T-5311 requested this mechanism once its own lease freed.
Positive control proven by TestTaintGateDiscovery calling the real taint_gate() against a synthetic Django repo: frob's own checkout is not a detected web framework, so a self-check of this repo cannot show the finding.
...
```
 ...
 tests/unit/test_websec_headers_log.py              |   200 +
 ...
 uv.lock                                            |     2 +-
 4146 files changed, 677339 insertions(+), 23504 deletions(-)
```

### Evidence
- `tests/unit/test_websec_headers_log.py::test_websec_headers_log_findings_fixture` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_headers_log.py::test_websec_headers_log_findings_no_framework_short_circuits` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_headers_log.py::test_websec_findings_hook_emits_gate_violation` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_headers_log.py::test_websec_findings_hook_empty_frameworks_short_circuits` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_headers_log.py::TestTaintGateDiscovery::test_taint_gate_emits_websec117_violation` (pytest node id, verified passing when recorded)
- `tests/unit/test_websec_headers_log.py::TestTaintGateDiscovery::test_discovery_finds_a_planted_fake_module` (pytest node id, verified passing when recorded)
