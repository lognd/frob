...
No verb could remove a ticket attachment; the only prior path was a
hand-edited ledger, which the ticket-ledger-gotchas lesson forbids.
Added `frob ticket attach <id> --remove PATH` / `--remove-all`
(frob.tickets._reporting_attachments.remove_attachment): deletes the
attachment file, drops its Attachment record, and persists via the same
lease/write discipline attach() already uses, so the existing
single-ticket-id auto-commit dispatch covers the commit unchanged.
Refuses (AttachRemoveError.AttachmentInDoneReport) when the targeted
path is quoted in the ticket's done-report (v2 file or legacy body
prose) so a closed ticket's cited evidence can't be silently deleted.
New error family lives beside the module that raises it
(AttachRemoveError, sibling to TicketError/ClipboardError) rather than
adding a TicketError member, since src/frob/tickets/_models.py is
outside this ticket's scope (T-5133 holds its lease for the worktree's
lifetime).
...
```
 ...
 tickets/T-5151/done-report.md                      |  3281 +++
 ...
 tickets/T-5316/ticket.md                           |    36 +
 tickets/T-5317/ticket.md                           |    36 +
 ...
 uv.lock                                            |     2 +-
 3393 files changed, 454034 insertions(+), 17451 deletions(-)
```

### Evidence
- `tests/test_tickets.py::TestRemoveAttachment::test_removes_file_and_ledger_record` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestRemoveAttachment::test_matches_by_basename` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestRemoveAttachment::test_remove_all` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestRemoveAttachment::test_no_matching_path_is_err` (pytest node id, verified passing when recorded)
- `tests/test_tickets.py::TestRemoveAttachment::test_refuses_when_cited_in_done_report` (pytest node id, verified passing when recorded)
