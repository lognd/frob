...
This is T-WEBSUB-3 of the T-5140 web-app-lint epic: a pure registry leaf
that reserves rule-id space so downstream story leaves (T-5141..T-5148)
can ship without immediately colliding with the known-rule-id gate, and
so `frob:waive` on an id whose check hasn't landed yet fails soft
(no-op) instead of loud (UnknownRuleId). Doing this now, ahead of the
story leaves, means no future story has to touch _KNOWN_GATE_RULES /
check-coverage.yaml / frob.toml at all -- it can go straight to writing
the check and flipping the placeholder doc row.

LAUNCH's severity mapped to "advisory" only after confirming T-5304 had
actually landed on dev (Severity.ADVISORY + [gates.severity] "advisory"
parsing both present in _waive.py); the ticket started twice because the
first `ticket start` collided with T-5304's own live lease on frob.toml.
...
```
 ...
 tickets/T-5300/ticket.md                           |   293 +
 tickets/T-5301/ticket.md                           |   194 +
 tickets/T-5302/ticket.md                           |   213 +
 tickets/T-5303/ticket.md                           |   339 +
 ...
 uv.lock                                            |     2 +-
 3908 files changed, 567520 insertions(+), 23422 deletions(-)
```
...
- `tests/gates_suite/test_coverage.py::TestCoverageGate::test_waive002_reserved_websec_id_round_trips_clean` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_coverage.py::TestCoverageGate::test_waive002_flags_unknown_rule_id_as_ineffective` (pytest node id, verified passing when recorded)
