...
Why: T-4703 measured 1,712 runs of 3+ consecutive directive lines (largest 73)
before leaf 1 removed the reverse frob:tests copy. This leaf is the stack lint
(name the remaining stacks) plus the Tier-A fix that collapses them using
leaf 2's multi-target grammar, sequenced after leaves 1-3 so it measures
post-cleanup stacks, not ones about to be deleted or reoriented.

What: frob.gates._directive_stack.stack_lint_violations groups edges by
(file, src) and flags a symbol carrying threshold-or-more DISTINCT origins
(a multi-target line counts once, matching how a human reads physical
lines). fix_dstack001_merge merges same-kind frob:tests/frob:doc runs into
one multi-target header per kind, in FIRST-SEEN order (interleaved
doc/tests/doc collapses to one doc header then one tests header). Both are
conservative: any other kind, a non-default per-target attribute (frob:tests'
own kind="unit" default is recognized and collapsed safely), or a
non-contiguous stack is left untouched rather than guessed at. Idempotent by
construction -- the merged form drops below threshold, so a second run finds
nothing. DSTACK001 registered in _KNOWN_GATE_RULES and docs/modules/gates.md.
...
declared scope; frob.toml threshold wiring was blocked by a lease collision
with the concurrently in-progress T-4663. Filed as T-draft-ced04135.
...
```
 ...
 src/frob/gates/_directive_stack.py                 |   123 +
 ...
 src/frob/gates/_fix_engine_text.py                 |   266 +-
 ...
 tickets/T-draft-83d6c01e/ticket.md                 |    30 +
 tickets/T-draft-ced04135/ticket.md                 |    31 +
 ...
 uv.lock                                            |     2 +-
 2942 files changed, 365732 insertions(+), 15589 deletions(-)
```

### Evidence
- `tests/test_gates_directive_stack.py::TestStackThresholdOffByOne::test_n_minus_one_is_not_a_finding` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestStackThresholdOffByOne::test_n_is_exactly_one_finding` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestOnlyStackedSymbolFires::test_stacked_symbol_fires_alongside_a_scattered_run_of_the_same_length` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestMultiTargetLineCountsOnce::test_multi_target_edges_sharing_an_origin_count_as_one_line` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestDstack001MergeFix::test_interleaved_doc_tests_doc_collapses_to_one_doc_then_one_tests` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestDstack001MergeFix::test_tests_doc_tests_puts_tests_first_not_a_hardcoded_kind_order` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestDstack001MergeFix::test_merge_is_idempotent` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestDstack001MergeFix::test_only_paths_scoping_leaves_an_unlisted_file_untouched` (pytest node id, verified passing when recorded)
- `tests/test_gates_directive_stack.py::TestDstack001MergeFix::test_non_default_attrs_leaves_the_stack_untouched` (pytest node id, verified passing when recorded)
