...
Adds the mechanical classification primitive curated landing needs:
classify_ticket_commits/_is_bookkeeping_commit partition a ticket
branch's own commit history into real-work vs pure ledger-bookkeeping
(tickets/**, .frob/**) commits, in commit order, with no git I/O of its
own. Unwired from the live land path, mirroring the exact "prove the
mechanical primitive in isolation, wire it later" precedent T-3546's
classify_test_then_impl_paths already set in the sibling _land_splice.py
module -- deciding what a caller does with the split (squash bookkeeping
into one commit, keep real-work commits individually, refuse an
out-of-range count) is real follow-up integration work. Documented in a
new docs/guides/landing.md, cross-linked from agent-playbook.md's guides
list. Verified with 9 new unit tests covering both helpers' must-fire/
must-stay-quiet shapes plus the empty/all-real/all-bookkeeping edges.
...
```
 ...
 src/frob/tickets/_land_squash.py                   |   581 +-
 ...
 tests/unit/tickets/test_land_squash.py             |    92 +
 ...
 uv.lock                                            |   265 +-
 4534 files changed, 789890 insertions(+), 23545 deletions(-)
```

### Evidence
- `tests/unit/tickets/test_land_squash.py::TestIsBookkeepingCommit::test_pure_ticket_ledger_paths_are_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestIsBookkeepingCommit::test_pure_frob_cache_paths_are_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestIsBookkeepingCommit::test_mixed_ledger_and_real_paths_is_not_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestIsBookkeepingCommit::test_pure_source_change_is_not_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestIsBookkeepingCommit::test_empty_path_set_is_not_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestClassifyTicketCommits::test_partitions_real_work_from_bookkeeping_preserving_order` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestClassifyTicketCommits::test_empty_input_returns_two_empty_tuples` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestClassifyTicketCommits::test_all_real_work_no_bookkeeping` (pytest node id, verified passing when recorded)
- `tests/unit/tickets/test_land_squash.py::TestClassifyTicketCommits::test_all_bookkeeping_no_real_work` (pytest node id, verified passing when recorded)
