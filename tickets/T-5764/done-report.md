...
frob's future LAYOUT review gate (T-5747 story) needs to validate crunk's
gallery manifest without importing crunk (webapp is a leaf layer with no
cross-repo dependency). This vendors a byte-pinned copy of crunk's
gallery-manifest.v1.json JSON Schema plus a mirror pydantic model so
frob can load and validate a manifest, and implements the owner-decided
byte-hash staleness rule (any source/fixture-props change re-opens
review) as a plain comparison helper. Tests round-trip crunk's own
fixture manifest, assert parity on the missing-source_hash rejection,
and diff the vendored schema against crunk's live file when a sibling
checkout is present, so drift between the two repos is caught here.
...
```
 ...
 tickets/T-5751/ticket.md                           |    59 +
 tickets/T-5755/ticket.md                           |    54 +
 tickets/T-5756/ticket.md                           |    55 +
 ...
 tickets/T-5762/ticket.md                           |    55 +
 tickets/T-5763/ticket.md                           |    57 +
 tickets/T-5764/ticket.md                           |    93 +
 tickets/T-5765/ticket.md                           |    62 +
 ...
 uv.lock                                            |   265 +-
 4721 files changed, 881435 insertions(+), 23562 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_gallery_schema.py::test_fixture_manifest_round_trips` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_missing_source_hash_rejected` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_load_gallery_manifest_not_found` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_load_gallery_manifest_malformed` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_entry_is_stale_on_hash_change` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_entry_is_stale_false_when_unchanged` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_gallery_schema.py::test_vendored_schema_matches_crunk_source` (pytest node id, verified passing when recorded)
