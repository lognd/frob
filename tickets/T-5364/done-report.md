...
facts about a route's <head> (title, meta/link tags, JSON-LD), and
re-querying the DOM per rule would multiply parse cost by the number of
rules in the family. This ticket adds the one shared walk:
extract_page_metadata normalizes one .html/.jsx route file's <head> (or
JSX <Head> wrapper, the Next.js convention) into a frozen PageMetadata,
reusing T-5302's framework detection contract and T-5300's html/jsx
grammar wiring through frob.lang.raw_tree rather than re-detecting
frameworks or standing up a second tree-sitter parser. build_duplicate_
title_index folds a route table's worth of PageMetadata into the
cross-route duplicate-title index every downstream SEO rule reuses
instead of re-scanning the table itself, matching the ticket's planted
duplicate-title fixture pair.
...
```
 ...
 src/frob/webapp/_seo_substrate.py                  |   419 +
 ...
 tests/unit/test_webapp_seo_substrate.py            |   100 +
 ...
 uv.lock                                            |     2 +-
 3977 files changed, 613950 insertions(+), 23464 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_seo_substrate.py::test_extract_page_metadata_html_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_extract_page_metadata_jsx_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_extract_page_metadata_no_head_html_is_negative_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_extract_page_metadata_no_head_jsx_is_negative_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_extract_page_metadata_unsupported_extension_is_negative_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_build_duplicate_title_index_positive_control` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_seo_substrate.py::test_build_duplicate_title_index_no_duplicates_is_negative_control` (pytest node id, verified passing when recorded)
