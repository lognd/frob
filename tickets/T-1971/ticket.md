...
## Drop reason
- 2026-08-10: User decision 2026-08-10: do nothing in the sibling repos. All 8 frob-wired siblings are still on ledger v1 with uncommitted tickets.md diffs; 7 are pure-append residue generated by the stale 0.184.0 global binary (which predated auto-commit-after-write), and lithos additionally has 223/3 in tickets.md plus 59 modified Rust files indicating live work. Migrating requires first triaging someone else's uncommitted diffs in their own repos, which the user declined. The global frob install has been repaired (0.184.0 -> 0.448.0, reinstalled from the primary checkout after uv tool upgrade failed against a deleted agent worktree), so the residue-generating root cause is fixed going forward. Reopen only if the siblings are wanted on v2.
