---
id: T-5086
title: 'migration: split module gates out of design/frob.strata into design/gates.strata
...
acceptance:
- text: Given the land, when it completes, then design/gates.strata exists with its
...
- text: 'Given the repository at this commit, when the strata checker runs, then it
  ...
  evidence: []
- text: Given every cross-module flow involving gates, when the linker runs, then
  ...
  evidence: []
- text: Given the templated-assume gate, when it runs against design/gates.strata,
...
land_commit: null
---
Migration leaf for module `gates` (members: gates, checker, verify, security). Hub LAST: fan-in 8, the widest surface; bidirectional with app, graph, platform, strata, tickets and vet -- every remaining cycle decision lands here.

One-time reviewed rewrite, by hand, module by module (owner decision D-M6:
there is NO a sys-split tool (rejected) tool). Write design/gates.strata with:
  - `module gates` header (dotted path form);
