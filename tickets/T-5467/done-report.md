...
CI run 35951365410 caught test_real_repo_source_tree_is_fully_registered
reporting frob.tickets and frob.webapp as unfaceted: both contain a
language-name string literal (per unfaceted_packages's AST scan) with no
LANGUAGE_SENSITIVE_PACKAGES entry to account for it. frob.tickets' only
hit is Pydantic's model_dump(mode="python"), a serialization-mode
literal that happens to collide with the language name, not a real
per-language branch. frob.webapp's hits are symmetric per-language
membership/dispatch checks (HTML/CSS/JSX family frozensets, an
extractor-dispatch dict, a python-only guard) routing an already-
detected language to the right extraction function, the same posture
frob.app's own AGNOSTIC entry documents. Both packages added as
...
```
 ...
 uv.lock                                            |   265 +-
 4407 files changed, 763149 insertions(+), 23539 deletions(-)
```
