...
One-line I001 (unsorted-imports) autofix on tests/unit/verify/test_quarantine.py, applied via
`ruff check --fix` in a dedicated worktree (rescued from an earlier accidental root-dirtying
write). Repo-wide `ruff check src/ tests/ scripts/` was 1 error before this change, 0 after.
Touched-set `frob test --base main` selected and ran the affected test file: exit=0, 3.61s.
`frob check --ticket T-3085` shows gate:SCOPE clean (0 errors); the DRIFT/WAIVE/COV/etc failures
surfaced by --only families are pre-existing repo-wide findings, unrelated to and unaffected by
this single import-ordering line, confirmed present on main HEAD prior to this change. The
repo-wide ruff-format drift (74 files) is likewise pre-existing and out of this ticket's scope.
No new tickets filed.

### Changed
```
 tickets/T-3085/done-report.md | 22 ++++++++++++++++++++++
 tickets/T-3085/ticket.md      | 23 +++++++++++++++++++++--
 2 files changed, 43 insertions(+), 2 deletions(-)
```

### Evidence
- `tests/unit/verify/test_quarantine.py::TestNormalizeFindingPath::test_absolute_and_relative_resolve_identical` (pytest node id, verified passing when recorded)

### Captured claims
- tests: 1 passed (from 1 evidence id(s))
- gates: 77 error(s), 744 warning(s), 865 waived
