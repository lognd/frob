...
- tests/unit/test_conftest_sigbreak_faulthandler.py
  Rewrote TestSigbreakFaultHandlerCrossPlatformSafety's class docstring
  and all 6 of its method docstrings. They cited T-3577's revert
  narrative ("Superseded (T-3577): the function under test no longer
  exists"); now state the utility fact plainly ("No-op skip: the covered
  function is not present in tests/conftest.py"), keeping the reference
  as a plain reason with no "used to"/"superseded" wording.

- tests/unit/test_makefile_coverage.py
  Rewrote 5 flagged class docstrings (TestCoverageRecipeDelegates
  ToFrobCoverageFull, TestPreviouslyZeroModulesNowAttributeInThe
  CommittedLock, TestCoverageRecipeReconcilesStaleLeasesBeforeDoctor,
  TestInstallToolUsesServeExtraPackageSpecNotUnsupportedFlag) and 1
  method docstring (test_frob_format_exits_nonzero_on_an_unfixable_
  syntax_error) to state what each asserts about the Makefile recipe
  body, with a one-line "See T-#### for the design rationale" pointer
  instead of narrating the prior shell-block/flag history.

- tests/unit/test_ticket_runner_gate_findings.py
  Rewrote 8 flagged docstrings: TestCheckGateFindingsFn,
  TestPythonForTree (class + its
  test_check_gates_summary_fn_spawns_the_tree_venv_python method),
  TestSharedCheckSpawnFn, TestParseErrorFindingsFromJsonDropsBlank
  Identity (class + its test_blank_identity_diagnostic_is_dropped_not_
  added method), TestErrorFindingIdentityOffFileAnchors (class + its
  test_must_fire_new_selfaudit001_finding_not_deduped_against_unrelated_
  one method). All now state the assertion made rather than the T-1055
  PLACE001/incident narrative behind each.

- tests/unit/test_graph_cache.py
  Rewrote 7 flagged class docstrings (TestEmptyRowGuard,
  TestRecreateNeverExposesASchemaIncompleteDb, TestLockBackoff,
  TestHandleIdentity) and 3 method docstrings
  (test_path_never_absent_during_recreate,
  test_run_with_stale_reconnect_recovers_from_bare_database_error,
  test_run_with_stale_reconnect_recovers_from_interface_error) to state
  the invariant/behavior asserted, dropping the CI-incident-round
  narrative and "used to"/"the old" phrasing DOCARCH001 keys off.
  Also added a `# frob:ticket T-4419` directive alongside each existing
  historical ticket ref on these 7 symbols (COV002 requires an
  open-ticket edge on any changed symbol; the pre-existing refs (T-4018,
  T-3607, T-3623/T-3700, T-3706, T-3733, T-3654, T-3669) are historical
  provenance and stayed in place).

- tests/unit/strata/test_threat.py
  Rewrote 5 flagged docstrings: TestDischargeChokepointShape's
  test_noflow_from_a_specific_foreign_trust_node_discharges method,
  TestFlowCompletenessGap (class + its
  test_no_foreign_node_anywhere_still_discharges_by_absence method),
  TestEvalFiresCwe94 class, TestCheckEffectCompleteness's
  test_effect_on_a_file_absent_from_owner_does_not_crash method. All now
  state what each fixture proves about the strata threat-discharge
  machinery instead of the G2/G3/G8 audit-finding narrative.

No behaviour change anywhere: docstrings only. `# frob:tests` /
`# frob:ticket` directive placement is otherwise unchanged.

### WHY

T-2994 doctrine: code and docs carry UTILITY, tickets carry NARRATIVE.
T-4419's original denominator (166, measured 2026-09-11) was stale; a
direct `docarch001_violations(root)` call today (2026-09-19) measured
182 findings across tests/unit. Rather than take the whole 182 (the
sibling T-4421 blew its estimate 10x taking exactly that shape), the
ticket was re-measured per subdirectory, recorded in the ticket body,
then split file-granular into 6 clusters of ~30-33 findings each (no
file split across a cluster boundary). T-4419 itself was narrowed
(`frob ticket scope --add`, `--reason "narrow to cluster 0..."`) to
cluster 0 (32 findings, 5 files); the other 5 clusters were filed as
child tickets (kind=docs, parent T-4419, `--ack-related` since they are
deliberately similar splits, not duplicates):

  T-4632  cluster 1 (32 findings, 9 files)
  T-4630  cluster 2 (33 findings, 16 files)
  T-4628  cluster 3 (33 findings, 21 files)
  T-4631  cluster 4 (33 findings, 33 files, all weight-1)
  T-4629  cluster 5 (14 findings, 14 files, all weight-1)

Two files were excluded from every cluster because they are leased by
other in-progress tickets (checked via `grep -l <path>
/home/logan/projects/frob/.git/frob-leases/*.json` before filing):
  tests/unit/verify/test_verify_runner.py (1 finding) -- leased T-3082
  tests/unit/strata/test_selfconform.py (4 findings) -- leased T-draft-213c1cfd
These 5 findings are recorded as deferred in cluster 5's body and in
T-4419's own body, not dropped.

T-4419's acceptance[1] was stale (still named the full tests/unit
denominator after the narrowing); amended via `frob ticket accept
--amend 1` to name the 5 cluster-0 files explicitly, with a --reason
audit trail.

### How each acceptance criterion is proven

acceptance[1] (amended): "Given a scoped frob check on the 5 files in
cluster 0 ..., when DOCARCH001 is measured, then its finding count for
those files is 0."

Proof: a direct `docarch001_violations(root)` call filtered to the 5
cluster-0 files returns 0 findings (was 32 before this change) --
verified after every edit, and again as the final check after the
frob:ticket-directive fix-up commit. The gate's own unit-test suite
(tests/gates/test_docstring_archaeology.py::TestDocarch001Violations,
15 tests, exercising the exact `docarch001_violations`/`_is_archaeology`
code path this measurement used) is bound as evidence for the acceptance
criterion, since no new test file is in this ticket's scope.

Regression proof (not bound as acceptance evidence, run and green):
`PYTHONPATH=$(pwd)/src PYTEST_XDIST_AUTO_NUM_WORKERS=1 python -m pytest
tests/unit/test_ticket_runner_gate_findings.py
tests/unit/test_conftest_sigbreak_faulthandler.py
tests/unit/test_graph_cache.py tests/unit/strata/test_threat.py
tests/unit/test_makefile_coverage.py -p no:cacheprovider -q`
-> SUITE-RESULT: exitstatus=0 collected=252 failed=0 (docstring-only
edit, no behavior touched).

### Pre-READY checklist evidence

1. `frob check --only sys --no-cache --files <5 touched files> --base
   dev`: 0 SELFAUDIT001 findings attributable to any of the 5 files.
   (Command exits 1 overall on pre-existing unrelated DRIFT001/DSL001
   findings in src/frob/tickets/_evidence.py etc. -- not attributable to
   this change.)
2. `frob check --only arch --no-cache --files <5 touched files> --base
   dev`: exit 0, `pass frob-arch 19 warnings (36 waived), 542
   suggestions` -- no ARCH001/LARGE001 on any of the 5 files.
3. `frob check --only coverage --no-cache --files <5 touched files>
   --base dev`: 0 COV002 findings on any of the 5 files (the initial run
   found 8 COV002s on tests/unit/test_graph_cache.py -- fixed by adding
   `# frob:ticket T-4419` directives, second commit, re-measured to 0).
   Remaining COV/DRIFT/DSL/TODO FAILs in that same invocation are all in
   unrelated files (src/frob/vet/*, src/frob/app/ticket_runner/
   _rapid_sweep.py, src/frob/gates/invariants.py,
   src/frob/tickets/_evidence.py, tests/unit/test_ticket_store.py) --
   pre-existing repo state untouched by this ticket.
4. ruff check + ruff format --check on all 5 files: clean ("All checks
   passed!" / "5 files already formatted"). ty check on all 5 files:
   "All checks passed!"
5. Cross-ticket: `git diff --name-only dev...HEAD` covers exactly the 5
   files in T-4419's own lease (`.git/frob-leases/T-4419.json`); none
   overlap another in-progress ticket's lease.
6. Not applicable: kind=docs, not bug/security.
7. Not applicable: no tickets/<dir>/ promotions touched in this
   worktree.
8. Evidence bound after the last content commit (1a082578d, the
   frob:ticket-directive fix). The two `chore(tickets): record evidence
   for T-4419` commits (9d7833e97, 1b3eaad9a) are the evidence-binding
   tool's own ledger writes, not content changes.
...
### Commit shas (this worktree, branch t-4419)

e239b99d7  chore(tickets): body T-4419                          (re-measurement recorded)
86fe9b32d  chore(tickets): record T-4419 start transition
1feb69e8e  docs(tests): clean DOCARCH001 change-narrative from cluster 0 docstrings
1a082578d  docs(tests): bind T-4419 frob:ticket edge on changed graph-cache docstrings
832e0c616  chore(tickets): accept T-4419                        (acceptance[1] amended)
9d7833e97  chore(tickets): record evidence for T-4419
1b3eaad9a  chore(tickets): record evidence for T-4419
HEAD -> 1b3eaad9aa325347b6973076c3889923187bbbfe

### Filed

T-4632, T-4630, T-4628, T-4631,
T-4629 (children of T-4419, clusters 1-5, kind=docs).
...
```
 ...
 tests/unit/test_graph_cache.py                     |   97 +-
 ...
 tests/unit/test_ticket_runner_gate_findings.py     |   76 +-
 ...
 uv.lock                                            |    2 +-
 2159 files changed, 130721 insertions(+), 5717 deletions(-)
```
...
- `tests/gates/test_docstring_archaeology.py::TestDocarch001Violations::test_bare_ticket_reference_stays_quiet` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [1] replace: 'Given a full frob check on tests/unit, when DOCARCH001 is measured, then its finding count for tests/unit is 0' -> 'Given a scoped frob check on the 5 files in cluster 0 (tests/unit/test_ticket_runner_gate_findings.py, tests/unit/test_conftest_sigbreak_faulthandler.py, tests/unit/test_graph_cache.py, tests/unit/strata/test_threat.py, tests/unit/test_makefile_coverage.py), when DOCARCH001 is measured, then its finding count for those files is 0' (reason: narrowed scope to cluster 0 per the split plan; the other 5 clusters (177 findings) are covered by child tickets; logan, 2026-09-19)
