...
Disclosed by the T-4553 implementer: build_call_graph only ever recorded
PRIVATE callee edges (T-0841's shared-graph safety), so caller_dependent_
files (the rapid land's --files dependents walk) found callers of a
changed private symbol only, silently omitting every caller of a changed
PUBLIC API.

Fix, matching the owner's "never narrow the shared callee graph for one
consumer" lesson: build_call_graph gained include_public_callees (opt-in,
default False, requires verify_imports=True) which ADDS public-callee
edges on top of its existing private-only set -- frob.dup/frob.gates'
shared consumers of CallGraph.calls are unaffected by default. Added
frob.graph.affects.public_caller_dependent_files, a caller_dependent_
files sibling that builds this opt-in graph and resolves public-symbol
callers strictly through import bindings (from A import f / A.f), reusing
the same import-verified candidate filter that already guards T-2188's
private-callee hazard -- never a bare repo-wide short-name match.

Wiring _land_cmd.py's _rapid_caller_dependents to actually call the new
function end to end is out of T-4560's declared/implicit scope (that file
is not in either); filed T-draft-a7b77421 for that follow-up.
...
```
 ...
 tickets/T-draft-a7b77421/ticket.md                 |    29 +
 ...
 uv.lock                                            |     2 +-
 2803 files changed, 275273 insertions(+), 15056 deletions(-)
```

### Evidence
- `tests/unit/test_check_scoped_files.py::TestPublicCallerDependentFiles::test_public_callee_caller_is_found_through_import_binding` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_scoped_files.py::TestPublicCallerDependentFiles::test_same_named_private_helpers_in_different_modules_stay_unlinked` (pytest node id, verified passing when recorded)
