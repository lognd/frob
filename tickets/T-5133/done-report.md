...
Sprint and milestone had collapsed onto one meaning: sprint carried the
version on 592/691 open tickets, milestone was null or mixed-format on
the rest. Shipped the mechanical fix in two halves: going forward,
validate_sprint refuses a semver-shaped sprint label (--semver-sprint-ack
override) and sprint_shape_warning nudges a calendar/numbered label
toward a goal name, both matching the owner's same-day amendment (a
sprint is a goal you can say in three words, not a version or a
calendar week). Backward, frob ticket sprint migrate is a one-shot,
idempotent repair moving every semver-shaped sprint onto milestone
(keeping an existing conflicting milestone and logging it, never
overwriting) and normalizing v-prefixed milestones it encounters.
Re-slicing the ~600 open tickets into goal-named sprint labels (step 3
of the ticket body) is data-content work, not mechanical migration --
cut from scope and filed as a follow-up (see Filed).
...
```
 ...
 docs/modules/tickets-data-storage.md               |   152 +
 ...
 src/frob/app/ticket_runner/_mutate.py              |   171 +-
 ...
 tests/test_gates_fix_engine.py                     |   214 +-
 ...
 tests/test_tickets_sprint_migrate.py               |   183 +
 ...
 tickets/T-5133/done-report.md                      |  3040 +++
 ...
 tickets/T-5280/ticket.md                           |    44 +
 tickets/T-5281/ticket.md                           |    37 +
 tickets/T-draft-4df380d9/ticket.md                 |    35 +
 ...
 uv.lock                                            |     2 +-
 3010 files changed, 414987 insertions(+), 16040 deletions(-)
```

### Evidence
- `tests/test_tickets_sprint_migrate.py::TestValidateSprint::test_semver_shaped_refused` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestValidateSprint::test_calendar_shaped_warns_not_refuses` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestValidateSprint::test_goal_label_accepted` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestValidateSprint::test_semver_ack_bypasses_refusal` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestSprintShapeWarning::test_calendar_shaped_warns` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestSprintShapeWarning::test_goal_label_silent` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestMigrateSprintToMilestone::test_migrates_unmilestoned_ticket` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestMigrateSprintToMilestone::test_conflict_keeps_existing_milestone` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestMigrateSprintToMilestone::test_normalizes_v_prefixed_milestone` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestMigrateSprintToMilestone::test_idempotent_second_run_is_a_noop` (pytest node id, verified passing when recorded)
- `tests/test_tickets_sprint_migrate.py::TestBoardGroupsByMilestoneSemverOrder::test_board_orders_tickets_by_milestone_semver_not_lexical` (pytest node id, verified passing when recorded)
