...
Why: T-4703 measured 5,663 of 30,915 directive lines carrying a noqa suffix
(18%); most exist only because the pre-T-4712 wrapper's word-boundary cut
made a directive line unwrappable in shapes leaves 1-3 have since fixed. This
leaf is the noqa-strip lint plus fix, sequenced last so it measures
post-narrowing residue, not stale noise.

What: noqa_strip_violations (FMT002) flags a directive line's noqa suffix
as no-longer-needed when removing it still fits the file's own resolved
line length. fix_fmt002_noqa_strip strips it via strip_needless_noqa_paths,
a narrow sibling of canonicalize_text that ONLY ever rewrites an
already-single-physical-line run into another single line -- it never
wraps or unwraps into a different line count, so it cannot re-trip T-1987's
ARCH001 regression (rewrapping a noqa-suppressed line into several lines
changed enclosing-function line counts on two real lands, T-1970/T-1968).
_NOQA_SUFFIX_RE's escape hatch stays available for the genuine residue (a
single token wider than the limit on its own) -- _rewrite_directive_run's
existing unconditional leave-alone behavior is untouched. Idempotent by
construction: a stripped line is never a candidate a second pass finds
again. Registered FMT002 in _KNOWN_GATE_RULES, frob.toml (warn), and
docs/modules/gates.md.

Left open: TIER_A_HANDLERS registration (_fix_engine.py) and the
violation-collection wiring (gates/__init__.py) are outside this leaf's
declared scope, same class of gap T-4713's own DSTACK001 follow-up hit.
Filed as T-5275.
...
```
 ...
 uv.lock                                            |     2 +-
 2975 files changed, 387215 insertions(+), 15909 deletions(-)
```

### Evidence
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaText::test_strips_a_noqa_that_no_longer_fits_the_line` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaText::test_load_bearing_noqa_is_left_untouched` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaText::test_bare_noqa_with_no_code_is_also_stripped_when_it_now_fits` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaText::test_idempotent_second_pass_is_a_no_op` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaText::test_never_changes_physical_line_count_reconstructing_t1970_shape` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestNoqaStripViolations::test_flags_a_directive_whose_noqa_no_longer_fits_the_reason` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestNoqaStripViolations::test_load_bearing_noqa_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestNoqaStripViolations::test_clean_file_with_no_noqa_at_all_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaPaths::test_check_mode_reports_without_writing` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestStripNeedlessNoqaPaths::test_second_run_reports_zero_changes` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestFixFmt002NoqaStrip::test_strips_and_is_idempotent` (pytest node id, verified passing when recorded)
- `tests/test_gates_fmt_directives.py::TestFixFmt002NoqaStrip::test_only_paths_scoping_leaves_an_unlisted_file_untouched` (pytest node id, verified passing when recorded)
