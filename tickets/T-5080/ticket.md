...
## THIS LEAF: kerberos (`realm`/`kdc`/`spn`/`delegation`/`target`/`direction`/`transitive`/`trusts`, plus `gmsa`/`sudoers` at the edges) -- _elaborate.py:175-181 and _krb.py: a Kerberos trust graph, a THIRD graph beside Node/Flow and vmodel. SF-09 recorded the whole std.krb surface as unused (archive/T-0262), so this domain carries the strongest "was it ever a good idea" question of the eight -- which under T-4678 is a question to ANSWER, not a licence to delete.
