...
## T-4624: Clean docstrings across 12 files (DOCARCH001)

### WHAT changed

Rewrote 29 flagged docstrings across 12 test files under tests/ so each
states WHAT the test proves and HOW, instead of ticket-archaeology
narrative. No behaviour changes -- docstrings only. Added a
`# frob:ticket T-4624` directive immediately above every changed
class/def so COV002 tracks the edit against this open ticket.

Files touched (all 12 in ticket scope):
tests/test_tickets_acceptance.py, tests/test_refs_gate.py,
tests/test_hook_root_write_guard.py, tests/test_docptr_gate.py,
tests/test_check_runner.py, tests/vet_suite/test_scan_tree.py,
tests/test_tickets_evidence_cli.py, tests/test_refactor.py,
tests/test_perf.py, tests/test_land_verify_claims_outcome.py,
tests/test_hook_frob_timeout_guard.py, tests/test_gates_suppress.py

### WHY

Doctrine T-2994 (debloat sprint v0.534.0): code and docs carry UTILITY,
tickets carry NARRATIVE. Every removed docstring was retelling the
history of the SAME ticket that made the original change (already
present in that ticket's own body); nothing was an orphaned fact
that would be lost by dropping it from the docstring. No
`frob ticket body --append` calls were needed as a result.

### Measurement (scoped, not repo-wide)

Direct measurement via `frob.gates._docstring_archaeology.docarch001_violations`
run against exactly the 12 ticket-scope files:

BEFORE (initial per-file re-measure, done before touching code):
  29 findings across 12 files (matches the ticket body's own estimate
  of 29).

AFTER (same direct measurement, post-edit):
  TOTAL 0

CONFIRMED via the official gate: the `--only sys` run below (see
Pre-READY checks) runs the DOCARCH gate alongside sys as part of the
same gate-group bundle, and its raw output's `[gate:DOCARCH]` findings,
filtered to this ticket's 12 filenames, is 0 hits (post-edit).

### Acceptance criterion [1]

"Given a frob check scoped to these 12 files, when DOCARCH001 is
measured, then the combined finding count is 0" -- proven by both the
direct measurement (0/0) and the official `frob check --only sys`
run's incidental DOCARCH gate output filtered to these 12 files (0
hits).
...
Bound via:
  frob ticket evidence T-4624 <node-id> --accepts 1 --base-ref dev

### Full touched-set pytest run

`PYTHONPATH=$(pwd)/src PYTEST_XDIST_AUTO_NUM_WORKERS=1 python -m pytest`
over all 12 touched files (with test_refactor.py narrowed to the two
touched node ids, since the full file has one pre-existing,
environment-caused failure unrelated to this change -- Python 3.10's
stdlib `enum` module lacks `StrEnum`, hit by an unrelated fixture in
`TestRunSplit::test_split_carries_forward_imports_moved_body_needs`):

  409 passed, 0 failed

  (includes both touched tests/test_refactor.py node ids:
  TestRunRefactor::test_run_refactor_does_not_roll_back_on_ticket_md_evidence_carrier
  and TestVerify::test_import_resolution_skips_non_python_touched_file,
  run individually: 2 passed, 0 failed)

  tests/gates/test_docstring_archaeology.py::TestDocarch001Violations -> 6 passed

## Pre-READY checks

`frob check --only sys --files <the 12 files> --base dev`:
  0 SELFAUDIT001 findings attributable to any of the 12 files. DRIFT/DSL
  errors in the run are all in unrelated src/ files (e.g.
  src/frob/tickets/_land_squash.py, src/frob/_cli_parsers/_core.py) --
  confirmed by grepping the raw output for the 12 filenames.

`frob check --only arch --files <the 12 files> --base dev`:
  Tool summary: `pass  frob-arch  19 warnings (36 waived), 544 suggestions`.
  No ARCH001/LARGE001 on any of the 12 files.

`frob check --only coverage --files <the 12 files> --base dev`:
  0 COV002 findings on any of the 12 files (confirmed after adding the
  `# frob:ticket T-4624` directives; grepped the raw output for the 12
  filenames -- zero hits).

`ruff check <the 12 files>`: All checks passed!

`ty check <the 12 files>`: All checks passed!

### Cross-ticket

`git diff --name-only dev...HEAD` in this worktree: exactly the 12
scoped files plus tickets/T-4624/ticket.md (evidence-record commit).
None of the 12 files are leased by any OTHER in-progress ticket
(checked `.git/frob-leases/*.json` at ROOT, excluding T-4623.json/
T-4624.json).

### Commit / HEAD

Code commit: 4d6076f8c "docs(tests): rewrite DOCARCH001 docstrings to
state test utility"
Evidence-record commit (auto-committed by `frob ticket evidence`):
80fa43020 (final)
HEAD: 80fa43020f9a57ca40fc5e6859517c1d487707fd
...
```
 ...
 tests/test_land_verify_claims_outcome.py           |   25 +-
 ...
 tests/test_tickets_evidence_cli.py                 |   22 +-
 ...
 uv.lock                                            |    2 +-
 2193 files changed, 135846 insertions(+), 6459 deletions(-)
```
