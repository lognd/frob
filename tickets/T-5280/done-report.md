...
T-5132 landed `frob ticket points`/`frob ticket tokens` without registering
them in `_ledger_mirror.LEDGER_VERB_STRATEGY` (T-2603) -- that file was not
in T-5132's declared scope, so every real invocation of either verb crashed
with "has no LEDGER_VERB_STRATEGY entry". Registered both as
GENERIC_COMMIT_MIRRORED, the same strategy `milestone`/`priority` already
use. A second, related gap (the new --points/--unsized-ack/--tokens-* CLI
fields are also missing from `_config_external.py`'s `from_external`
allowlist, so those values are silently dropped even once the crash above
is fixed) could NOT be fixed in this same ticket -- that file is leased by
T-4702; see the Done report's Filed/Gates for the follow-up.
...
```
 ...
 src/frob/app/ticket_runner/_ledger_mirror.py       |    54 +-
 ...
 src/frob/gates/_fix_engine_text.py                 |   340 +-
 ...
 tickets/T-draft-ac05cce9/done-report.md            |  3023 +++
 tickets/T-draft-ac05cce9/ticket.md                 |    47 +
 tickets/T-draft-e4547e60/ticket.md                 |    36 +
 ...
 uv.lock                                            |     2 +-
 3004 files changed, 410791 insertions(+), 15987 deletions(-)
```

### Evidence
- `tests/unit/test_ticket_runner_ledger_mirror.py::TestAutoCommitDispatchCoversEveryStrategy::test_every_strategy_member_is_covered` (pytest node id, verified passing when recorded)
