...
Adds the squawk migration-safety adapter so the SQL family covers
migration-level anti-patterns (NOT-NULL without default,
index-without-CONCURRENTLY, lock-taking rewrites) alongside sqlfluff's
statement-level rules. Registers squawk into T-5335's own
REQUIRED_FOR_FAMILY machinery, reusing sql_relevance verbatim as its
relevance predicate rather than inventing a narrower migrations-only
predicate, per the owner's stated posture that SQL tools are
required-for-the-family with no never-fail flag. Parsing is kept pure
and separate from the subprocess spawn so it is fully testable via
canned squawk JSON fixtures without the real binary installed.
...
```
 ...
 uv.lock                                            |   265 +-
 4360 files changed, 747266 insertions(+), 23534 deletions(-)
```

### Evidence
- `tests/unit/sql/test_squawk_adapter.py::test_parse_squawk_json_not_null_no_default` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_squawk_adapter.py::test_parse_squawk_json_index_without_concurrently` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_squawk_adapter.py::test_parse_squawk_json_empty_array_is_no_findings` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_squawk_adapter.py::test_parse_squawk_json_malformed_text_is_no_findings` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_squawk_adapter.py::test_parse_squawk_json_missing_messages_falls_back_to_help` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_squawk_adapter.py::test_squawk_findings_absent_binary_is_empty_not_raising` (pytest node id, verified passing when recorded)
