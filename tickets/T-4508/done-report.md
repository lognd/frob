...
T-4508 land repair -- 18 rename/rename ledger conflicts + SELFAUDIT001 ceiling drift

WHAT changed:
- The branch carried its own stale renumbers of drafts that dev had
  since promoted to different real ids, causing 18 rename/rename
  conflicts under tickets/ on merge (T-4615..T-4627-shaped and six
  T-draft-* dirs). No merge was actually in progress at the time I
  picked this up (worktree was already clean at commit 7054de2fa +
  two prior id-line fixups), so per the recipe I:
  1. `git checkout dev -- tickets/` -- took dev's ledger wholesale.
  2. `git checkout HEAD -- tickets/T-4508/` -- restored this ticket's
     own directory (done-report.md, state: done) on top.
  3. Verified every tickets/<dir>/ticket.md `id:` field matches its
     directory name (script check, zero mismatches).
  4. Confirmed the 4 T-draft-* dirs left in the tree
     (213c1cfd/5658939f/a62505d4/ede38ca6) are the SAME ones dev itself
     still carries (git grep -l on their titles resolved inside dev's
     own tree) -- not stale leftovers, so nothing further to delete.
  5. Committed with FROB_LAND_INTERNAL=1
     ("chore(tickets): sync ledger to dev, keep T-4508's own state").
  6. `git merge dev --no-edit` -- now clean (auto-merged CHANGELOG.md,
     design/frob.strata, docs/design/registry/capability-via-ratchet.lock.json,
     no conflicts).

- Re-ran `frob check --only sys --files src/frob/testing/_dotnet_runner.py
  --files tests/unit/test_dotnet_runner.py --base dev` per the
  coordinator's instruction: SELFAUDIT001 fired because dev's merged-in
  tests (T-4111/T-4524 landed on dev in the interim) moved the
  testsuite-glob via-list counts past this ticket's last-measured
  ceilings: testsuite::env.read 50 -> 51, testsuite::fs.write 538 -> 540.
  Bumped docs/design/registry/capability-via-ratchet.lock.json's two
  entries to the new measured counts with a T-4508-attributed reason
  (matching this repo's existing ceiling-bump convention), fixed a
  dropped trailing newline the JSON rewrite introduced, committed
  separately ("chore(registry): bump testsuite via-ratchet ceilings
  after dev merge").

Proof:
- `frob check --only sys --files src/frob/testing/_dotnet_runner.py
  --files tests/unit/test_dotnet_runner.py --files
  docs/design/registry/capability-via-ratchet.lock.json --base dev`:
  gate:SELFAUDIT no longer appears in the FAIL list (previously "2
  errors"), confirming the ceiling bump cleared it. Remaining FAILs
  (gate:DRIFT 5 errors all pre-existing-waived, gate:DSL 1 error on
  tests/test_app.py:387, unrelated to this ticket's files) are the same
  repo-wide pre-existing findings seen on the other repairs in this
  session.
- PYTHONPATH=.../src python -m pytest tests/unit/test_dotnet_runner.py -q
  SUITE-RESULT: exitstatus=0 collected=10 failed=0
- ruff check / ty check on src/frob/testing/_dotnet_runner.py and
  tests/unit/test_dotnet_runner.py: All checks passed! (both tools).
- Evidence rebound to the ticket's existing 4 node ids via `frob ticket
  evidence T-4508 ... --base-ref dev` AFTER the final commit (no-op,
  content already correct, HEAD stayed at e52456cf7).
- `git diff --name-only dev...HEAD` cross-checked against
  /home/logan/projects/frob/.git/frob-leases/*.json: no other
  in-progress ticket leases any of the touched paths (T-4615..T-4620 are
  this branch's own new ticket filings not yet present on dev at all --
  additions, not conflicts).

Commits:
- 7054de2fa merge dev into worktree for landing T-4508 (pre-existing)
- ba00f2e8c / 2fd79877b (pre-existing stale-id-line fixups)
- (new) chore(tickets): sync ledger to dev, keep T-4508's own state
- (new) Merge branch 'dev' into t-4508 (clean)
- e52456cf7 chore(registry): bump testsuite via-ratchet ceilings after
  dev merge (HEAD)

## Pre-READY checks

frob check --only sys --files src/frob/testing/_dotnet_runner.py --files
tests/unit/test_dotnet_runner.py --files
docs/design/registry/capability-via-ratchet.lock.json --base dev:
  pass gate:DOCARCH, gate:PROFILE, gate:WAIVE, claude-config-drift.
  FAIL gate:DRIFT (5 errors, all pre-existing/waived, unrelated files),
  FAIL gate:DSL (1 error, tests/test_app.py:387, pre-existing, unrelated
  file). gate:SELFAUDIT is CLEAN (no longer in the FAIL list) after the
  ceiling bump -- this was the specific finding this repair targeted.

ruff check src/frob/testing/_dotnet_runner.py tests/unit/test_dotnet_runner.py:
  All checks passed!

ty check src/frob/testing/_dotnet_runner.py tests/unit/test_dotnet_runner.py:
  All checks passed!

(No --only arch / --only coverage re-run requested for this ticket by
the coordinator's brief -- only the --only sys ceiling check was named;
the underlying .py files were not modified in this repair, only the
ledger and the ratchet lock file.)
...
```
 ...
 tickets/T-4508/done-report.md                      | 629 ++++++++++++++++
 ...
 uv.lock                                            |   2 +-
 587 files changed, 48630 insertions(+), 1952 deletions(-)
```
