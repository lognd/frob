...
Implements A11Y129-135 (redundant entry SC 3.3.7, accessible authentication
SC 3.3.8, contrast SC 1.4.3) as frob.webapp._a11y_forms_contrast, the third
of three sibling A11Y substrate leaves the T-5140 web-app epic needed
before T-5323's gate discovery could wire it up.

After merging T-5323's land, the module's a11y_findings hook was rewritten
to match the actual landed per-file A11yFileContext shape
(a11y_findings(ctx, frameworks)) rather than this ticket's earlier
(root, frameworks) plan sketch -- redundant-entry/accessible-auth now read
straight off ctx.source, contrast walks ctx.root directly with no re-parse.
A real end-to-end positive control now calls frob.gates._a11y_gate.a11y_gate
over a planted git repo and asserts A11Y131 fires through the live gate,
proving the hook is discovered and wired, not merely catalogued.

Known gap recorded in the module doc: a11y_gate's own tracked-file walk
excludes .css/.scss, so A11Y133-135 are fully implemented and unit-tested
directly but cannot fire through the live gate until a follow-up widens
that walk -- out of this ticket's scope since src/frob/gates/** is
off-limits here.
...
```
 ...
 src/frob/webapp/_a11y_forms_contrast.py            |   659 +
 ...
 tests/unit/test_webapp_a11y_forms_contrast.py      |   151 +
 ...
 tickets/T-5322/done-report.md                      |  4160 ++++
 ...
 uv.lock                                            |     2 +-
 4229 files changed, 706492 insertions(+), 23508 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_contrast_ratio_black_on_white_is_21_to_1` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_contrast_ratio_is_symmetric` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_empty_frameworks_short_circuits` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y129_positive-violation.html-A11Y129-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y129_negative-clean.html-A11Y129-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y130_positive-violation.html-A11Y130-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y130_negative-clean.html-A11Y130-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y131_positive-violation.html-A11Y131-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y131_negative-clean.html-A11Y131-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y132_positive-violation.html-A11Y132-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y132_negative-clean.html-A11Y132-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y133_positive-violation.css-A11Y133-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y133_negative-clean.css-A11Y133-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y134_positive-violation.css-A11Y134-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y134_negative-clean.css-A11Y134-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y135_positive-violation.scss-A11Y135-True]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_findings_fixture[a11y135_negative-clean.scss-A11Y135-False]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_a11y_forms_contrast.py::test_a11y_gate_discovers_forms_contrast_hook_end_to_end` (pytest node id, verified passing when recorded)
