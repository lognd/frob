...
Implements PERF015-018, the T-5135 audit's missing lint rules, per the
owner directive "perf finding ships as the fix plus a lint detector":

PERF015/PERF016 (frob.perf._loop_variant): the negation of PERF008 --
fires on a loop-body call reaching a spawn/fs-walk effect whose
arguments carry the loop's own variable, the exact shape PERF008 is
exempt from by construction. PERF016 is the unconditional, spawn-only
WARN subset (every git-spawn H2-H5/M7 shape in T-5135); PERF015 is the
broader advisory rule, gated by an iteration-source-name heuristic.

PERF017/PERF018 (frob.perf._cache_effects): PERF017 fires when only one
returning branch of an if/else calls a cache-write-shaped helper (the H2
success-only-cache-write shape); PERF018 fires when a hoisted expensive
value is recomputed by a transitive callee inside a loop instead of
being threaded through (the H4 discarded-hoist shape).

Each rule carries a positive-control test that plants its audit shape
and asserts the finding fires; all 8 new tests plus the existing 164
perf-suite tests pass.

Descoped: frob.toml severities, docs/modules/gates.md rows, and
_KNOWN_GATE_RULES registration -- frob.toml and docs/modules/gates.md
both carried live cross-worktree leases (T-5138, T-5121) at start;
_KNOWN_GATE_RULES was never in T-5136's declared scope. Filed as
T-5202.
...
```
 ...
 tickets/T-5136/done-report.md                      |  2836 +++
 ...
 uv.lock                                            |     2 +-
 2918 files changed, 345334 insertions(+), 15538 deletions(-)
```

### Evidence
- `tests/unit/perf/test_loop_variant.py::TestPerf016::test_git_spawn_with_loop_variable_pathspec_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_loop_variant.py::TestPerf016::test_loop_invariant_spawn_is_not_flagged_by_perf016` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_loop_variant.py::TestPerf015::test_loop_variant_ticket_id_spawn_is_flagged_advisory` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_loop_variant.py::TestPerf015::test_loop_variant_call_without_iteration_source_name_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_cache_effects.py::TestPerf017::test_success_only_cache_write_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_cache_effects.py::TestPerf017::test_both_branches_writing_cache_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_cache_effects.py::TestPerf018::test_hoisted_value_recomputed_in_loop_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/perf/test_cache_effects.py::TestPerf018::test_hoisted_value_threaded_through_is_not_flagged` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [4] remove: removed 'given [[perf.heavy]] populated, when frob check runs on dev, then PERF007 reports at least one finding instead of zero' (reason: criterion 4 (PERF007 fires once [[perf.heavy]] is populated) requires editing frob.toml, which carried a live cross-worktree lease (T-5138) when T-5136 started and was removed from scope; filed as T-5202; logan, 2026-09-21)
