...
- src/frob/strata/_elaborate.py
  - Added `_validate_boundary_phase_admit(boundary)`: validates that a
    declared `admit { rate_limit ...; max_size ... }` block's quantities
    carry the right dimension (`rate_limit` -> "rate", `max_size` ->
    "size"), returning `Err(StrataError.UnitMismatch)` otherwise. Wired
    into `_validate_one_boundary_phases`'s fixed fail-closed order
    alongside the existing parse/effect/record/refuse checks, and into
    `_validate_boundary_phases`'s docstring.
  - Added `_admit_bound_claims_for(boundary)` and
    `_elaborate_admit_bound_claims(module)`: desugar a declared `admit`
    block's `rate_limit`/`max_size` into `BoundClaim` kernel facts
    (`Bound(rate, boundary) <= rate_limit`, `Bound(size, boundary) <=
    max_size`), `target` = the boundary id, `assumed=True`,
    `required_rung=Rung.L1`. `Bound` is one of the six kernel primitives
    (`_models.py::BoundClaim`) so this is desugaring under charter law 1,
    not a kernel extension -- no `_models.py` change was needed or made.
  - Wired `_elaborate_admit_bound_claims` into `_build_base_kernel_model`'s
    `claims=` tuple, alongside the existing `module.claims` elaboration.
  - Added `# frob:ticket T-4911` on every new/changed function
    (`_validate_boundary_phase_admit`, `_validate_one_boundary_phases`,
    `_admit_bound_claims_for`, `_elaborate_admit_bound_claims`,
    `_validate_boundary_phases`, `_build_base_kernel_model`) to satisfy
    COV002.

- src/frob/strata/_backpressure.py
  - Added `_admit_ceiling_boundary_ids(model)`: maps a node id to the
    boundary id whose inbound flow (`Flow.dst == node.id`) feeds it.
  - Added `_declared_admit_ceiling(model, node_id)`: True iff that node's
    inbound boundary carries a `BoundClaim` over `Metric.RATE` or
    `Metric.SIZE` (i.e. a declared `admit` block was elaborated for it).
  - `_unproven_bounded_intake_violations` (REL261) now checks
    `_declared_admit_ceiling` FIRST: if the node's inbound boundary
    declares an admit ceiling, the obligation is proven outright (INFO
    log "DECLARED", REL261 does not fire), skipping the
    `_BOUNDED_INTAKE_TOKEN_RE` regex path entirely for that node. The
    regex path is UNCHANGED and still runs, with an INFO log "GUESSED",
    for every node with no declared admit block -- nothing was deleted,
    per the ticket's actual "declared beats guessed, not delete the
    regex" requirement (the acceptance criteria explicitly require the
    converse control to keep passing).
  - Expanded the module docstring with a T-4911 paragraph explaining the
    declared-over-guessed preference and the silent-zero INFO-logging
    requirement.

- tests/unit/strata/test_admit_phase_wiring.py (new)
  - `TestAdmitPhaseValidation`: valid admit rate+size elaborates; wrong
    dimension on `rate_limit` and on `max_size` each fail closed with
    `StrataError.UnitMismatch` (acceptance 1).
  - `TestAdmitBoundDesugaring`: a declared `rate_limit` emits exactly one
    `BoundClaim(metric=RATE, target=<boundary id>, limit=<quantity>)`; no
    admit block emits no `BoundClaim` at all.
  - `TestBackpressureDeclaredCeiling`:
    - `test_declared_admit_ceiling_discharges_with_no_code_token`: THE
      POSITIVE CONTROL. A `consumer`+`bounded_intake` node whose bound
      code has NO bounded-intake token: with no admit block declared,
      REL261 fires (proves the regex-only path would have failed this
      node, i.e. this control genuinely exercises the gap). Adding a
      `BoundClaim(RATE)` on the inbound boundary (what
      `_elaborate_admit_bound_claims` would produce for a real `admit
      rate_limit`) makes REL261 NOT fire -- the declared ceiling changed
      the backpressure verdict (acceptance 2).
    - `test_no_admit_block_still_uses_regex_fallback`: THE CONVERSE
      CONTROL. No admit block declared, real bounded-intake token present
      in bound code -> REL261 does not fire via the (unmodified) regex
      path, proving the regex path was not deleted (acceptance 3).

WHY: T-4911's audit found `admit`/`rate_limit`/`max_size`/`judge` parsed
and modelled (`AdmitPhase`, `_ast.py:341`) but read by nothing, while
`_backpressure.py` independently (and only approximately) re-derives the
same "is intake bounded" answer with a source-text regex whose own
comment concedes it "is not a claim the matched token bounds the SAME
queue the node models." Wiring `admit` through elaboration into
`BoundClaim` kernel facts, then preferring that declared fact in
`_backpressure.py`, replaces a guess with the model's own precise answer
wherever a model author bothers to declare one, without removing the
regex fallback for models that do not. `judge` is left OUT OF SCOPE per
the ticket body (needs a `Flow(condition=phase)`-shaped design decision,
not a wiring change) -- no follow-up ticket filed since the ticket body
already tracks this decision explicitly.

Acceptance criteria proof (test node ids, all under
tests/unit/strata/test_admit_phase_wiring.py):
  1. TestAdmitPhaseValidation::test_valid_admit_rate_and_size_elaborates
     (+ ::test_admit_rate_limit_with_wrong_dimension_fails_closed /
     ::test_admit_max_size_with_wrong_dimension_fails_closed for the
     fail-closed half)
  2. TestBackpressureDeclaredCeiling::
     test_declared_admit_ceiling_discharges_with_no_code_token
  3. TestBackpressureDeclaredCeiling::
     test_no_admit_block_still_uses_regex_fallback

Filed: none. `judge` phase wiring is explicitly out of scope per the
ticket body itself, which already records the open design question
(Flow(condition=phase) vs. folding into effect) -- no new ticket needed.

Commits (worktree /home/logan/projects/frob/.claude/worktrees/t-4911,
branch t-4911):
  85df60972 feat(strata): wire the boundary admit block into elaborate and backpressure
  147cba55c..6e5061965 chore(tickets): record evidence for T-4911 (4 evidence-bind commits,
    one per acceptance criterion, all after 85df60972)

## Pre-READY checks

`frob check --only sys --files src/frob/strata/_elaborate.py --files src/frob/strata/_backpressure.py --files tests/unit/strata/test_admit_phase_wiring.py --base dev`:
  FAIL overall (gate:DRIFT 6 errors/5 waived, gate:DSL 1 error, gate:SELFAUDIT 1 error) --
  but grep across the full output for `_elaborate.py`/`_backpressure.py`/`test_admit_phase_wiring`
  shows ZERO hits among the errors; every reported error is pre-existing baseline noise in
  unrelated files (tests/unit/test_ticket_runner_*.py DOCARCH001 warnings, tests/test_app.py
  DSL001, etc.), none attributable to this ticket's files.

`frob check --only arch --files src/frob/strata/_elaborate.py --files src/frob/strata/_backpressure.py --files tests/unit/strata/test_admit_phase_wiring.py --base dev`:
  pass  frob-arch  21 warnings (36 waived), 547 suggestions -- 0 ARCH001/LARGE001, no findings
  in our 3 files (all findings are pattern-recommendation notes on unrelated pre-existing files).

`frob check --only coverage --files src/frob/strata/_elaborate.py --files src/frob/strata/_backpressure.py --files tests/unit/strata/test_admit_phase_wiring.py --base dev`:
  Before adding `frob:ticket T-4911` directives: 6 COV002 findings on our new/changed
  _elaborate.py functions. After adding the directives and recommitting: re-ran the same
  command -- 0 hits for `_elaborate.py`/`_backpressure.py`/`test_admit_phase_wiring` among
  the remaining 10 COV errors (all pre-existing, in src/frob/vet/**, src/frob/verify/**,
  etc.). COV002 clean for this ticket's files.

`ruff check src/frob/strata/_elaborate.py src/frob/strata/_backpressure.py tests/unit/strata/test_admit_phase_wiring.py`:
  All checks passed! (one E501 fixed by wrapping the generator expression; ruff format
  applied once to the new test file, then re-verified clean)

`ty check src/frob/strata/_elaborate.py src/frob/strata/_backpressure.py tests/unit/strata/test_admit_phase_wiring.py`:
  All checks passed! (fixed 2 unresolved-attribute diagnostics on the test's ClaimBody
  union by re-asserting `isinstance(body, BoundClaim)` after list-comprehension filtering)

`cd /home/logan/projects/frob/.claude/worktrees/t-4911 && nice -n 10 /home/logan/projects/frob/.venv/bin/frob ticket land T-4911 --dry-run --worktree /home/logan/projects/frob/.claude/worktrees/t-4911`:
  Tail (only ERROR lines in the whole run):
    ERROR: land: T-4911 cannot land -- missing evidence or a Done report; record evidence
    (`frob ticket evidence T-4911 <node-id>...` ...) and add a '## Done report' section
    under T-4911's entry in tickets.md, then retry `frob ticket land T-4911`
    ERROR: ticket land failed: NotCloseable: ticket is missing evidence or a Done report
  NOTE for the coordinator: this is the EXPECTED shape of a clean dry-run at the finisher
  stage under this repo's close-dance split (BRIEF.md close dance: finisher does steps 1-2
  -- commit + bind evidence -- coordinator does steps 3-4 -- write the Done report into
  tickets.md and close). Evidence is bound (4/4 acceptance criteria, all 4 node ids present
  in the ticket's `evidence:` list, verified via `tickets/T-4911/ticket.md`, bound AFTER the
  last code commit 85df60972). The dry-run's only two ERROR lines are both instances of the
  same NotCloseable check, which fires solely because no `## Done report` section exists yet
  in tickets.md -- BRIEF.md explicitly forbids the finisher from running
  `frob ticket done-report`/`close` (that is the coordinator's step 3). No other ERROR line
  appears anywhere in the dry-run output (pre-land fmt/Tier-A/RENDER001/profile/land.lock-wait
  lines above it are all WARNING/INFO, not ERROR). All gate/test/lint/type checks above this
  section are independently clean for this ticket's files.
...
```
 ...
 tests/unit/strata/test_admit_phase_wiring.py       |  248 ++
 ...
 tickets/T-4718/done-report.md                      | 2656 ++++++++++++++++++++
 ...
 tickets/T-4952/done-report.md                      | 2518 +++++++++++++++++++
 ...
 uv.lock                                            |    2 +-
 2446 files changed, 185850 insertions(+), 12098 deletions(-)
```

### Evidence
- `tests/unit/strata/test_admit_phase_wiring.py::TestAdmitPhaseValidation::test_valid_admit_rate_and_size_elaborates` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_admit_phase_wiring.py::TestAdmitPhaseValidation::test_admit_rate_limit_with_wrong_dimension_fails_closed` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_admit_phase_wiring.py::TestBackpressureDeclaredCeiling::test_declared_admit_ceiling_discharges_with_no_code_token` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_admit_phase_wiring.py::TestBackpressureDeclaredCeiling::test_no_admit_block_still_uses_regex_fallback` (pytest node id, verified passing when recorded)
