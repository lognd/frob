...
Batches the redundant load_queue calls the T-5135 perf audit measured on
`frob ticket close` and `frob ticket work --cluster` (M4/M5, lower
priority, kept out of T-5135 to preserve its fix order).

Before: `_close` (src/frob/app/ticket_runner/_close_cmd.py) spawned
load_queue up to 2x between fresh_ticket load and transition() -- once
inside `_undisclosed_remainder_reason` (disclosure check) and again for
the blocked_by guard -- with no ledger mutation between them. `_work_
cluster` (src/frob/app/ticket_runner/_lifecycle.py) spawned load_queue
2x -- its own call to compute `members`, then a second inside
`_refuse_on_cluster_scope_conflict` immediately after, same ledger
state.

After: both call sites now share one load_queue result, threaded via a
new optional `queue_result` parameter (default None preserves the old
self-loading behavior for every other caller) on
`_undisclosed_remainder_reason` and `_refuse_on_cluster_scope_conflict`.
Call count per invocation: close disclosure+blocked_by path 2 -> 1;
work --cluster scope-conflict path 2 -> 1. Loads that legitimately
straddle a ledger mutation (the pre/post-evidence re-load in `_close`,
the per-member reload in `_start_cluster_members` after each member's
own commit) are untouched -- merging those would read stale state.
...
```
 ...
 uv.lock                                            |     2 +-
 2887 files changed, 325033 insertions(+), 15270 deletions(-)
```
...
- `tests/unit/test_close_t1648_remainder.py::TestRemainderDisclosureGuard::test_allows_when_filed_ticket_is_open` (pytest node id, verified passing when recorded)
