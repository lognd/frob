...
T-4660: post-publish sweep must never hold .frob/derived.lock across a full check

## WHAT changed

- src/frob/app/ticket_runner/_rapid_sweep.py
  - Added `_SNAPSHOT_DIR_REL`, `_SNAPSHOT_WORKTREE_TIMEOUT_S` constants.
  - Added `_snapshot_worktree(root, commit_sha)`: a context manager that
    checks `commit_sha` out into a throwaway `git worktree` under
    `.frob/rapid-sweep-snapshots/`, yields the snapshot path (or `None`
    on any failure -- bad commit, not a git checkout, wedged git), and
    always removes the worktree + directory on exit.
  - Added `_run_full_check_in_snapshot(root, final_id, commit_sha)`:
    the new call site for the sweep's full unscoped check. It runs the
    check against the snapshot (falling back to `root` only when the
    snapshot could not be created -- a path a real land's checkout never
    takes) by delegating to `_land_cmd._unscoped_error_findings`
    UNCHANGED (same argv/env/timeout/parsing semantics, same mock seam
    every existing test in tests/unit/rapid_sweep_suite/ already uses)
    with `effective_root` swapped for `root`.
  - `_measure_fresh_sweep_state` now calls `_run_full_check_in_snapshot`
    instead of calling `_unscoped_error_findings(root, ..., full=True)`
    directly.

- docs/modules/tickets-verify-sweep.md
  - New "### Bounded lock window: the check never holds root's own lock
    (T-4660)" subsection under "Deferred post-land sweep", documenting
    the measured incident, the snapshot-isolation fix, and the
    second (orphaned-child) incident filed separately as T-4686.

- tests/unit/test_post_publish_lock_window.py (new file)
  - `TestSnapshotWorktree`: 3 tests on `_snapshot_worktree` itself
    (yields a real detached checkout of the commit; removes the
    worktree on exit; yields `None` for an unresolvable commit).
  - `test_post_publish_never_holds_derived_lock_across_a_check`: spawns
    a REAL subprocess running `run_deferred_post_land_sweep` with
    `_land_cmd._unscoped_error_findings` monkeypatched (in that child)
    to record which `root` it was actually called with and hold THAT
    root's own `derived_state_lock` (SHARED) for 1.5s -- then asserts
    (a) the recorded root is NOT the live checkout, and (b) a land-side
    EXCLUSIVE `derived_state_lock` acquire on the live checkout,
    attempted while the simulated check is still holding its own lock,
    completes in well under a second.
  - `test_next_land_not_blocked_by_previous_sweep`: same shape, framed
    as the "next land" acceptance criterion.

## WHY

Measured today 15:36 (this ticket's own body): `sweep-async T-4588`
spawned `python -m frob check --json` that ran 29 minutes holding the
root `derived.lock` READ (SHARED), and the next `frob ticket land`
blocked 25 minutes waiting for WRITE (EXCLUSIVE); killing the sweep
freed the land within 20 seconds. Root cause: the sweep's full check ran
directly against the shared checkout, so `frob check`'s own
`derived_state_lock(root, exclusive=False)` (held for its entire run)
was the SAME lock file a land's EXCLUSIVE acquire needs. Routing the
check through a throwaway git-worktree snapshot instead means the
check's own lock acquisition lands on a DIFFERENT file entirely -- the
land never contends with it, not "bounded", genuinely absent.

## Acceptance criteria proof

[1] Given a land that has just published, when post-publish runs, then
    no single .frob/derived.lock hold exceeds a bounded ceiling and no
    hold spans a full check invocation.
    -> tests/unit/test_post_publish_lock_window.py::test_post_publish_never_holds_derived_lock_across_a_check

[2] POSITIVE CONTROL: instruments the lock and asserts the check-owned
    hold excludes the live root's lock, and the land acquire is fast.
    -> tests/unit/test_post_publish_lock_window.py::test_post_publish_never_holds_derived_lock_across_a_check

[3] Given post-publish completes, when the next land starts, then it
    acquires derived.lock without waiting on the previous land's sweep.
    -> tests/unit/test_post_publish_lock_window.py::test_next_land_not_blocked_by_previous_sweep

[4] docs/modules/tickets-verify-sweep.md documents the bounded-hold rule
    and the ceiling, updated in this same change.
    -> docs/modules/tickets-verify-sweep.md, "Bounded lock window" section
    -> tests/unit/test_post_publish_lock_window.py::TestSnapshotWorktree::test_yields_a_detached_checkout_of_the_commit (bound as representative evidence; the doc change itself is reviewable in the diff)

## Second incident (orphaned check child) -- NOT closed here

The coordinator's brief also asked for the sweep child to die with its
parent, with a positive control test. Attempted during this ticket: a
SIGTERM/SIGINT handler on the sweep worker that kills its own process
group needs a genuinely NEW `process-control` capability declared on
src/frob/app/ticket_runner/_rapid_sweep.py in design/frob.strata's `cli`
node (SYS100 self-audit gate). `design/frob.strata` is currently leased
by another in-progress ticket (T-4112):

  $ frob ticket scope T-4660 --add design/frob.strata --reason "..."
  ERROR: tickets: T-4660 cannot lease an add glob: held by in-progress T-4112 (scope 'design/frob.strata')
  ERROR: scope change failed: ScopeLeaseConflict

Per the standing-rules brief ("Stay strictly inside the ticket's declared
scope ... refused if leased elsewhere -> record in your report, do not
touch it"), the process-control code (a `_die_with_children_on_terminate`
helper, its call site, and a third positive-control test) was written,
verified to work, then REMOVED rather than shipped with an unresolvable
SELFAUDIT001 (ERROR-tier) finding -- `Violation.file` for this gate
family is always the constant `design_dir` string, so a per-symbol
`frob:waive SELFAUDIT001` in the source file can never match it; only a
design/frob.strata edit (or a `waive "SYS100:..." ` clause in that same
leased file) can suppress it.

Filed as a follow-up: T-4686 "Orphaned check child after its sweep
parent dies" (kind=bug, scope src/frob/app/ticket_runner/_rapid_sweep.py,
blocked on nothing but the design/frob.strata lease held by T-4112).

## Files NOT touched (out of ticket scope, per T-4634/T-4572 note)

src/frob/tickets/_land.py, src/frob/tickets/_land_squash.py -- untouched.
design/frob.strata -- touched and reverted (see above); final diff does
not include it.

## Test node ids run

tests/unit/test_post_publish_lock_window.py -- 5/5 pass
tests/unit/rapid_sweep_suite/ (full package, touched-set regression) -- 211/211 pass

Commands:
  PYTHONPATH=<worktree>/src <venv>/bin/python -m pytest \
    tests/unit/rapid_sweep_suite/ tests/unit/test_post_publish_lock_window.py \
    -p no:cacheprovider -q
  -> SUITE-RESULT: exitstatus=0 collected=216 failed=0

## Commits

65745242a fix(tickets): bound the post-publish sweep's derived.lock hold window
03e80c6b1 fix(tickets): correct follow-up ticket id to its promoted T-4686
c57c453a5/42064cd16/29a0b5341 chore(tickets): record evidence for T-4660 (self-committing evidence writes)

## Pre-READY checks

frob check --only sys --files src/frob/app/ticket_runner/_rapid_sweep.py --files tests/unit/test_post_publish_lock_window.py --files docs/modules/tickets-verify-sweep.md --base dev
  -> gate:SELFAUDIT clean of any finding attributable to this ticket's files (the one remaining SELFAUDIT001, SYS111 testsuite-glob growth, is a documented land-time-only auto-accept, not a bare-check-fixable finding); gate:DSL's one remaining error is tests/test_app.py:387, untouched by this ticket.

frob check --only arch --files src/frob/app/ticket_runner/_rapid_sweep.py --files tests/unit/test_post_publish_lock_window.py --files docs/modules/tickets-verify-sweep.md --base dev
  -> pass  frob-arch   19 warnings (36 waived), 546 suggestions  (repo-wide pattern-recommendation noise, none new to these files; no ARCH001/LARGE001 on touched files)

frob check --only coverage --files src/frob/app/ticket_runner/_rapid_sweep.py --files tests/unit/test_post_publish_lock_window.py --files docs/modules/tickets-verify-sweep.md --base dev
  -> zero COV002/COV003 findings on any new symbol in this diff (the two COV002 findings raised on the first pass, on `_SNAPSHOT_DIR_REL`/`_SNAPSHOT_WORKTREE_TIMEOUT_S` for missing frob:ticket edges, were fixed by adding `# frob:ticket T-4660` directly above each); the remaining gate:COV errors/warnings are pre-existing, waived findings on symbols this ticket did not touch.

ruff check src/frob/app/ticket_runner/_rapid_sweep.py tests/unit/test_post_publish_lock_window.py
  -> All checks passed!

ty check src/frob/app/ticket_runner/_rapid_sweep.py tests/unit/test_post_publish_lock_window.py
  -> All checks passed!

## Scope refusals / waivers / skipped tests

- design/frob.strata: ScopeLeaseConflict against T-4112 (see "Second
  incident" section above) -- not touched, follow-up filed as T-4686.
- No `frob:waive` added anywhere in this diff.
- No test skipped.

READY T-4660 /home/logan/projects/frob/.claude/worktrees/t-4660 29a0b5341d5928db2086fe970891ad38355f31cc
...
```
 ...
 tickets/T-5036/ticket.md                           |  32 +
 tickets/T-5037/ticket.md                           |  75 ++
 tickets/T-5038/ticket.md                 | 125 +++
 tickets/T-5039/done-report.md            |  38 +
 tickets/T-5039/ticket.md                 | 345 ++++++++
 tickets/T-5040/ticket.md                 |  87 ++
 tickets/T-5041/ticket.md                 |  29 +
 tickets/T-5042/ticket.md                 |  99 +++
 tickets/T-draft-1f0f55cb/ticket.md                 |  65 ++
 tickets/T-5043/ticket.md                 |  99 +++
 tickets/T-5044/ticket.md                 | 123 +++
 tickets/T-5045/ticket.md                 |  36 +
 tickets/T-5046/ticket.md                 |  34 +
 tickets/T-draft-36c347fe/ticket.md                 |  68 ++
 tickets/T-5047/ticket.md                 |  38 +
 tickets/T-5048/ticket.md                 |  99 +++
 tickets/T-5049/ticket.md                 |  60 ++
 tickets/T-5050/ticket.md                 |  99 +++
 tickets/T-5051/ticket.md                 | 107 +++
 tickets/T-5052/ticket.md                 |  30 +
 tickets/T-5053/ticket.md                 |  33 +
 tickets/T-5054/ticket.md                 | 106 +++
 tickets/T-5055/ticket.md                 |  52 ++
 tickets/T-5056/ticket.md                 |  52 ++
 tickets/T-5057/ticket.md                 |  35 +
 tickets/T-5058/ticket.md                 |  45 +
 tickets/T-5059/ticket.md                 |  74 ++
 tickets/T-5060/ticket.md                 | 120 +++
 tickets/T-draft-a38af1c4/ticket.md                 |  77 ++
 tickets/T-5061/ticket.md                 |  61 ++
 tickets/T-5062/ticket.md                 |  78 ++
 tickets/T-5063/ticket.md                 |  37 +
 tickets/T-5064/ticket.md                 |  37 +
 tickets/T-5065/ticket.md                 |  35 +
 tickets/T-5066/ticket.md                 |  99 +++
 tickets/T-5067/ticket.md                 |  99 +++
 tickets/T-5068/ticket.md                 |  99 +++
 tickets/T-5069/ticket.md                 | 100 +++
 tickets/T-5070/ticket.md                 |  50 ++
 tickets/T-5071/ticket.md                 |  68 ++
 tickets/T-5072/ticket.md                 | 106 +++
 tickets/T-5073/ticket.md                 |  73 ++
 ...
 tickets/archive/T-0328/ticket.md                   |  52 ++
 tickets/archive/T-0336/ticket.md                   |  18 +
 tickets/archive/T-0339/ticket.md                   |  52 ++
 tickets/archive/T-0364/ticket.md                   |  24 +
 tickets/archive/T-0377/ticket.md                   |  52 ++
 tickets/archive/T-0378/ticket.md                   |  52 ++
 tickets/archive/T-0379/ticket.md                   |  76 ++
 ...
 uv.lock                                            |   2 +-
 1208 files changed, 86556 insertions(+), 2945 deletions(-)
```

### Evidence
- `tests/unit/test_post_publish_lock_window.py::test_post_publish_never_holds_derived_lock_across_a_check` (pytest node id, verified passing when recorded)
- `tests/unit/test_post_publish_lock_window.py::test_next_land_not_blocked_by_previous_sweep` (pytest node id, verified passing when recorded)
- `tests/unit/test_post_publish_lock_window.py::TestSnapshotWorktree::test_yields_a_detached_checkout_of_the_commit` (pytest node id, verified passing when recorded)
