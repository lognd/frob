...
Measured 2026-09-20 (STRANDED.md): 38/54 in-progress tickets were abandoned by
dead agents while their worktree directories survived; orphaned_leases only
tests directory existence, never branch or process liveness, so nothing in
`frob check` ever caught this class.

Added TICK015 (`_tick015_requeue_dead_worktree`, frob.gates._tickets_gate):
ERROR per IN_PROGRESS ticket whose T-5120-stamped worktree/branch is judged
dead (path gone, branch gone, or no live process cwd'd into the worktree via
`scan_for_live_worktree_process`) -- requeues it (IN_PROGRESS -> QUEUED,
releasing its lease) and records a failure-log entry naming the dead
worktree, reusing `frob ticket fail`'s own record_failure+transition+
commit_ticket_ledger_change composition rather than inventing a second
requeue path. A ticket with no recorded worktree (pre-T-5120) is skipped.

Registered in _KNOWN_GATE_RULES and docs/modules/gates.md. frob.toml severity
override was left out of scope: TICK015's ERROR severity is hardcoded on the
Violation itself (matching TICK014's own precedent of no frob.toml entry),
and `frob.toml` itself is currently leased by another in-progress ticket
(T-5138), so no override table entry was added.
...
```
 ...
 tickets/T-5135/ticket.md                           |   546 +
 tickets/T-5136/ticket.md                           |    56 +
 tickets/T-5137/ticket.md                           |    63 +
 tickets/T-5138/ticket.md                           |    57 +
 ...
 uv.lock                                            |     2 +-
 2782 files changed, 269862 insertions(+), 14907 deletions(-)
```
...
- `tests/gates_suite/test_tick_dead_worktree.py::TestTick015DeadWorktreeRequeue::test_live_holder_is_untouched` (pytest node id, verified passing when recorded)
