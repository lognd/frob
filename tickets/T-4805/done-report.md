...
T-4805: SCOPE002 emits 587 scope-closure warnings on a ticket with EMPTY scope

WHAT CHANGED

src/frob/app/ticket_runner/_new.py (_scope_closure_warnings)
  - Root cause: _scope_closure_warnings resolved "is this site in scope"
    for each doc/test/private-helper gap through
    frob.tickets._models.scope_matches, which ALSO treats
    DEFAULT_REGISTRY_FILES (design/frob.strata, docs/design/registry/
    capability-via-ratchet.lock.json, docs/modules/gates.md, docs/design/
    registry/check-coverage.yaml) and LEDGER_PATH (tickets.md) as
    implicitly in scope -- correct for SCOPE001 gating, but wrong for
    scope-CLOSURE math: an EMPTY declared scope was still "matching"
    design/frob.strata, so scope_doc_code_gaps walked every frob:doc
    edge hanging off that hub file (587 warnings, none naming a file the
    ticket touches).
  - Fix: scope_tuple is checked for emptiness FIRST and returns () before
    even loading the graph snapshot (log at debug, per the engineering
    rules). For a non-empty scope, a new _declared_scope_only(symref)
    closure is built from frob.tickets._models._scope_globs(scope_tuple)
    with LEDGER_PATH stripped back out -- i.e. the SAME directory-glob
    expansion scope_matches uses, but without its implicit ledger/
    registry/CLI-wiring additions. Every gap returned by
    scope_doc_code_gaps / scope_test_gaps / scope_private_helper_gaps is
    now re-filtered through _declared_scope_only on its "already in
    scope" side (gap.scoped_site / helper_gap.caller) before being
    turned into a warning line. A scope that genuinely declares
    design/frob.strata still gets its closure warnings -- this is a
    filter, not a disable.
  - Bound the function to T-4805 directly (frob:ticket T-4805 alongside
    the pre-existing frob:ticket T-0998) since COV002 requires an open-
    ticket edge for the changed symbol.
  - Backticked a "frob:doc" mention in the new docstring prose that the
    DSL directive scanner was misparsing as a malformed directive
    (DSL001) -- matches the backtick convention the rest of the
    docstring already used for every other DSL-token mention.

tests/unit/test_scope_closure_declared_scope_only.py (new)
  - Stubs _graph_snapshot and the three T-0998 gap functions
    (scope_doc_code_gaps, scope_test_gaps, scope_private_helper_gaps) so
    each test controls exactly which raw (unfiltered) gaps reach the
    filter under test, independent of the real repo's current graph
    content.
  - TestEmptyScopeIsZeroWarnings: positive control -- an empty declared
    scope emits ZERO warnings even when the (stubbed) graph would
    otherwise report 587 design/frob.strata gaps; also asserts the graph
    snapshot is never even loaded for an empty scope, and that the empty-
    scope path logs at debug.
  - TestDeclaredScopeOnlyFiltering: negative control -- a three-file
    scope keeps only gaps whose scoped site is reachable from those three
    files (asserts a design/frob.strata gap and helper gap are dropped);
    regression control -- a scope that DOES declare design/frob.strata
    still gets its closure warnings, proving the fix filters rather than
    disables the check.

tickets/T-4805/ticket.md
  - Evidence bound to acceptance criteria 1 and 2; repro designated and
    verified (see below).

WHY

Filing any ticket with an empty or non-hub declared scope produced
hundreds of unactionable scope-closure warnings (measured 140, 345, 71,
587, 587, 624 across multiple tickets today), because closure math
leaked the SCOPE001-gating implicit-scope set into the closure filter.
Per the owner's guard-design lesson, a guard that cries wolf on every
invocation stops being read at all; per the scope-closure directive,
narrowing to the declared scope must PRESERVE bidirectional closure for
files actually in scope, not silence it -- hence the regression test
above rather than a blanket disable.

ACCEPTANCE CRITERIA

[1] "an empty declared scope emits ZERO scope-closure warnings"
...
[2] "a ticket declaring N specific files emits warnings ONLY for doc
    edges reachable from those N files -- and still are emitted for
    those"
...
TEST NODE IDS (all pass at HEAD, PYTHONPATH=<worktree>/src, pytest -p no:cacheprovider -q)
  tests/unit/test_scope_closure_declared_scope_only.py::TestEmptyScopeIsZeroWarnings::test_empty_scope_emits_zero_warnings_even_with_hub_file_gaps
...
BUG002 REPRO (kind=bug)
  Repro test: tests/unit/test_scope_closure_declared_scope_only.py::TestEmptyScopeIsZeroWarnings::test_empty_scope_emits_zero_warnings_even_with_hub_file_gaps
  Designated via `frob ticket evidence T-4805 --designate-repro ... --base-ref 25af4ef31`
    (25af4ef31 = the repro-test-only commit, added BEFORE the fix commit
    a76932b3e, per T-2021's own-worktree technique -- the ticket's fork
    point off dev predates the new test file entirely, so a fork-point
    base-ref would report TEST_ABSENT_AT_PARENT, not a real verdict).
  `--check-repro --base-ref 25af4ef31` result:
    "FAILED_AT_PARENT: ... genuinely fails at 25af4ef31... -- a real
    repro, this is what BUG002 wants"
  Commit shas:
    25af4ef31 test(tickets): add failing repro for scope-closure declared-scope-only
    a76932b3e fix(tickets): compute scope closure over declared scope only
    0a44af618 fix(tickets): backtick frob:doc mention, bind ticket directive (final)

SCOPE / LEASE NOTES
  Ticket's declared scope: src/frob/app/ticket_runner/_new.py,
  tests/unit/test_scope_closure_declared_scope_only.py. The fix also
  touches docs/modules/graph.md's "Scope closure (T-0998)" section in
  spirit (it documents the exact three gap functions this note applies
  to), but `frob ticket scope T-4805 --add docs/modules/graph.md` was
  REFUSED: "held by in-progress T-draft-ea93df7b (scope
  'docs/modules/graph*.md')". Per the BRIEF's scope-lease-conflict
  instruction, this file was left untouched rather than self-expanding
  scope onto a leased file. The coordinator may want to sequence a small
  T-draft-ea93df7b-or-successor doc note about the declared-scope-only
  filtering once that lease frees up, or land this ticket with
  --allow-cross-ticket if docs/modules/graph.md is otherwise ready.

## Pre-READY checks

`frob check --only sys --files src/frob/app/ticket_runner/_new.py --files tests/unit/test_scope_closure_declared_scope_only.py --base dev`:
  pass gate:DOCARCH (0 errors); FAIL gate:DRIFT (6 errors, all pre-existing/waived, none in my files); FAIL gate:DSL (1 error, not in my files -- grep confirms zero hits on _new.py/test_scope_closure_declared_scope_only.py in the DSL findings); pass gate:PROFILE; pass gate:WAIVE. Zero SELFAUDIT001/DSL001 attributable to my two touched files (confirmed by grep over the full tool output).

`frob check --only arch --files src/frob/app/ticket_runner/_new.py --files tests/unit/test_scope_closure_declared_scope_only.py --base dev`:
  pass frob-arch (22 warnings, 36 waived, 546 suggestions -- all pattern-recommendation notes on unrelated files, none on my two files, no ARCH001/LARGE001 anywhere in the run); pass claude-config-drift.

`frob check --only coverage --files src/frob/app/ticket_runner/_new.py --files tests/unit/test_scope_closure_declared_scope_only.py --base dev` (re-run after the final commit 0a44af618):
  Only my-file hit: [gate:COV] src/frob/app/ticket_runner/_new.py:1175 COV007 on _emit_scope_closure_warnings -- pre-existing, already waived (docs/design/cli-hygiene.md's Principle 4 anchor), not introduced by this change. No COV002/DSL001 on my files after the frob:ticket-T-4805 directive and backtick fixes (confirmed by grep: only the one pre-existing waived COV007 line matches _new.py; zero matches for the test file).

`ruff check src/frob/app/ticket_runner/_new.py tests/unit/test_scope_closure_declared_scope_only.py`: All checks passed!

`ty check src/frob/app/ticket_runner/_new.py tests/unit/test_scope_closure_declared_scope_only.py`: 1 diagnostic, pre-existing and unrelated (line 423, a documented cross-platform `ty: ignore[unused-ignore-comment]` suppression on `_resolve_new_acceptance`'s call site that predates this change and is win32-target-specific per its own comment).
...
```
 ...
 src/frob/app/ticket_runner/_new.py                 |  72 +-
 ...
 uv.lock                                            |   2 +-
 1014 files changed, 82396 insertions(+), 2554 deletions(-)
```

### Evidence
- `tests/unit/test_scope_closure_declared_scope_only.py::TestEmptyScopeIsZeroWarnings::test_empty_scope_emits_zero_warnings_even_with_hub_file_gaps` (pytest node id, verified passing when recorded)
- `tests/unit/test_scope_closure_declared_scope_only.py::TestDeclaredScopeOnlyFiltering::test_three_file_scope_keeps_only_matching_gaps` (pytest node id, verified passing when recorded)
- `tests/unit/test_scope_closure_declared_scope_only.py::TestDeclaredScopeOnlyFiltering::test_scope_actually_covering_the_hub_file_still_gets_its_warnings` (pytest node id, verified passing when recorded)
