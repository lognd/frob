...
src/frob/worktrees/_disposable_sweep.py had its own duplicate
os.kill(pid, 0) liveness probe (_pid_is_alive) instead of reusing the
one sanctioned implementation PLATFORM002's own _ALLOWLIST names
(src/frob/process/_pid_liveness.py::pid_alive, T-3018/T-3003/T-3191).
Both implementations shared the exact same EPERM-is-alive precedent
(T-4437), confirming this was a real duplicate, not a distinct case
PLATFORM002 misidentified.

Fix: _pid_is_alive now delegates to the sanctioned pid_alive() instead
of calling os.kill directly -- one implementation, cross-platform-safe
(pid_alive also handles the win32 branch this module's own POSIX-only
duplicate never did).

Measured: tests/unit/test_clean_worktrees_sweep.py and
tests/unit/gates/test_win32_kill_signal.py (12 tests total) pass clean.
...
```
 ...
 uv.lock                                            |     2 +-
 2855 files changed, 294631 insertions(+), 15113 deletions(-)
```
