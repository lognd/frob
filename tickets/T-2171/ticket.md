...
## Drop reason
- 2026-08-11: duplicate of T-2170: the coordinator disposed these same 3 findings (ARCH103/COV001/TEST001 on reclaim_orphaned_squash_residue) to T-2170, which is actively wiring that function into land() (in-progress@t-2170) and will naturally add its doc/test edges at the call site -- fixing here in parallel would race a live worktree on the same function. Re-measured at the real path (uv run frob check --only gates --json via scripts/check_summary.py): all 3 identities still reproduce, confirming this is not stale, just superseded by T-2170's own in-flight fix.
