...
tests/unit/test_ticket_2691_doc006.py's two "backticked future verb is
flagged" tests asserted a pre-T-5126 behavior: T-5126 later, deliberately,
made DOC006 skip CLI-pointer resolution for any ticket's own body
regardless of state (docstring on _is_ticket_body_doc, pinned by
tests/gates/test_docptr.py::TestDoc006TicketBodyCliPointerSkip), so a
ticket body backtick-quoting a not-yet-existing CLI verb is intentionally
NOT flagged today. The two T-2691/T-2742 tests were never updated when
T-5126 landed and rotted into asserting the opposite of current, intended
behavior. Inverted both assertions (found == []) and updated their
docstrings to record the supersession instead of silently deleting the
regression coverage. tests/unit/test_ticket_2691_doc006.py is fully green
(5/5); the T-5126 CliPointer tests still pass unchanged.
...
```
 ...
 tickets/T-draft-d18ef73b/ticket.md                 |    37 +
 ...
 uv.lock                                            |     2 +-
 3368 files changed, 435771 insertions(+), 17344 deletions(-)
```
