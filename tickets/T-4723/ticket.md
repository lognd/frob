...
body_changes:
- mode: append
  reason: condense no-citation comment run from deploy/_audit.py per C6 sweep
  actor: logan
  at: '2026-09-19'
  old_length: 5284
  new_length: 6243
...
## Narrative moved from deploy/_audit.py (no ticket cited, C6 sweep)

Filesystem paths excluded from every diff comparison in this module's
allowlist, each justified because it changes as an ordinary side effect of
the machine being alive/running commands, never because of anything
install.sh or uninstall.sh themselves declare:
- /var/log/**, /var/log/journal/**: normal log/journal growth from the OS
  and from running install.sh/uninstall.sh/status.sh themselves.
- /etc/machine-id, /var/lib/dbus/machine-id: D-Bus/systemd's machine
  identity, regenerated by some cloud-init/first-boot paths independent of
  this tool.
- /run/**, /proc/**, /sys/**, /tmp/**, /var/tmp/**: kernel-backed or
  ephemeral-by-design; never persisted, never declared by a HostManifest.
- /var/lib/systemd/**: systemd's own bookkeeping (random-seed, catalog
  database) touched by any systemctl daemon-reload.

Full documentation lives at docs/commands/deploy.md#allowlist.
