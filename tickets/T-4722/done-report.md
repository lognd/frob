...
T-4722 -- Source narrative C3: tickets/ (land pipeline, models, store) -- Done report

## Changed

src/frob/tickets/_land.py         -- 10 comment runs split (see below)
src/frob/tickets/_models.py       -- 1 comment run split
src/frob/tickets/_reporting.py    -- 1 comment run split
src/frob/tickets/_land_release.py -- 3 comment runs split
src/frob/tickets/_live_tracker.py -- 1 comment run split into 4 ticket targets
src/frob/tickets/_store.py        -- 1 comment run split
src/frob/tickets/_land_finalize.py -- 1 comment run split
src/frob/tickets/_mutation_evidence.py -- 1 comment run split

The other 11 files in the ticket's declared scope
(_archive.py, _land_git_ops.py, _unlanded.py, _accept.py, _flow.py,
_land_compose.py, _land_queue.py, _land_squash.py, _land_verify.py,
_setters.py, _worktree_sweep.py) were measured at 15+ line runs in the
ticket brief, but every one of those runs turned out to be a single
`frob:waive ... reason="..."` directive block (a multi-line quoted
reason attribute), which DOCARCH002's own spec exempts "by syntax, not
by wording" -- so none of them needed migration. Verified by re-scanning
every file with a directive-aware run-finder (scratchpad script) before
and after: zero non-directive runs over 12 lines existed in those 11
files to begin with.

Ticket bodies appended (via `frob narrative move`, condensed to the
decision + its why, never a verbatim dump):
T-0577, T-4243, T-1932, T-1003, T-0754, T-2064 (folded into T-0754's own
citation), T-4634, T-3618, T-4312, T-1132, T-2638, T-1805, T-1760,
T-4257 (x2), T-0605, T-1559, T-1633, T-3496, T-0821, T-1727, T-0458.

All are archived tickets except T-4243/T-4312/T-4634 (active) --
proving the archived-ticket write path (T-2994 constraint 3) succeeded
on the very first move (T-0577) before any batch ran.

## Why

Per T-2994's doctrine, code carries utility and tickets carry
narrative. Every comment run over 12 consecutive `#` lines in this
cluster's files was judged line-by-line: the load-bearing part (an
invariant, a platform quirk, why the obvious approach fails) stayed in
the code as a short comment plus a `# see T-#### for the history behind
this` pointer; the historical part (which ticket superseded which,
what a prior attempt got wrong, measured incident detail) moved into
that block's own cited ticket body, condensed to the decision and its
measured reason. No block in this cluster cited zero tickets, so
nothing was added to T-4722's own body (acceptance [2] is vacuously
satisfied for this cluster -- every run already carried a citation).

## Acceptance criteria

[0] Zero comment runs longer than 12 consecutive `#` lines remain in
the 19 scoped files.
  PROOF: `python3 find_runs2.py <all 19 files>` (directive-aware,
  excludes `frob:`/`noqa`/`type:`/`ruff:`/`mypy:` lead lines per
  DOCARCH002's syntax exemption) returns zero hits after the change.

[1] Every block that cited a ticket has its narrative readable in that
ticket body, moved never deleted.
  PROOF: each `frob narrative move` call's own stdout ("T-#### body
  append: N -> M chars: <text>") shown inline above; spot-checked
  T-0577, T-4243, T-1932, T-4312 bodies directly via `frob ticket show`
  during the session.

[2] Every block that cited no ticket has its narrative in this cluster
ticket's body.
  N/A for this cluster -- every qualifying run already cited at least
  one ticket.

[3] `git diff -w` over non-comment lines is empty; comments only.
  PROOF: `git diff -w -- src/frob/tickets/` filtered to non-comment
  added/removed lines returns 0 lines (script run this session).
  `git diff --stat` shows only the 8 touched .py files plus ticket.md
  bodies.

[4] `frob ticket list` exits 0 after every batch.
  PROOF: checked after the T-0577 move (first ever archived-ticket
  write, proving T-2994 constraint 3), after the T-1760/T-4257 batch,
  after the _live_tracker.py 4-way split, and again after the final
  commit -- exit 0 every time (session transcript).

## Evidence bound (--accepts 1..5, --base-ref dev)

1. tests/unit/test_land_lock_liveness.py::TestLandLockSurvivesSigkilledHolder::test_land_lock_reclaims_promptly_after_sigkill
2. tests/unit/test_ticket_store.py::TestSlugify::test_lowercases_and_hyphenates
3. tests/unit/test_reporting_t1648_remainder.py::TestDisclosureShapedLanguage::test_reworded_heading_still_flagged_structurally
4. tests/unit/test_app_runners_t0976_mutation_evidence.py::TestCollectStacksViaSamplerArgvStripping::test_non_marker_first_arg_is_not_stripped
5. tests/unit/test_land_release_coherence.py::TestReadWorkingVersions::test_reads_pyproject_version_from_disk

Also ran (not bound, extra confidence on the no-behavior-change claim):
tests/unit/test_land_lock_liveness.py, tests/unit/test_land_release_coherence.py,
tests/unit/test_ticket_store.py, tests/unit/test_app_runners_t0976_mutation_evidence.py,
tests/unit/test_reporting_t1648_remainder.py, tests/unit/test_reporting_t3285_fenced_subheadings.py
-- 148 tests total, 0 failures.

## Scope change mid-flight

Coordinator asked (critical land-blocker T-3324 fix needs the file):
`src/frob/tickets/_land_squash.py` was removed from T-4722's scope via
`frob ticket scope T-4722 --remove src/frob/tickets/_land_squash.py
--reason "released to the critical DOC006/T-3324 fix; its comment runs
go to the ninth narrative cluster"`. This file was never edited by
T-4722 (verified: `git diff dev -- src/frob/tickets/_land_squash.py`
was empty before the removal, and `find_runs2.py` found zero
non-directive runs over 12 lines in it, so nothing was lost). The scope
removal is mirrored onto main (`tickets/T-4722/ticket.md` scope no
longer lists it; ROOT's working tree is clean on that file). Its
comment-run migration (if any survives after DOCARCH002 lands) is
deferred to the ninth narrative cluster T-4772 names for excluded
files.

## Commits

a3244bde7 docs(tickets): move C3 comment-run narrative into cited tickets (T-4722)
  (plus several self-committing `chore(tickets): ...` commits from
  `frob narrative move` and `frob ticket evidence`, each ticket.md body
  append landing as its own commit per the tool's own transaction)
Final HEAD: 09b5da875fbbb4541daf90ff249af428c21dffe2

Evidence was bound AFTER the last source-code commit (a3244bde7); the
only commits after it are the tool's own ticket-ledger evidence writes,
which do not touch src/.

Note: `_land_finalize.py` and `_land.py` keep-fragments were followed
by a small manual comment-text edit (subject reattached after a split
line landed mid-sentence) -- comment-only, verified by the diff -w
check above; no separate commit was needed since these preceded the
single source commit.

## Pre-READY checks

`nice -n 10 frob check --only sys --files <8 touched .py> --base dev`:
exit 1 overall, but zero findings on any of the 8 touched files (all
findings were pre-existing DOCARCH001/WAIVE010 items in unrelated
tests/ and gates/ files outside this ticket's scope; no SELFAUDIT001
anywhere in the run).

`nice -n 10 frob check --only arch --files <8 touched .py> --base dev`:
`pass frob-arch 21 warnings (36 waived), 546 suggestions` -- no
ARCH001/LARGE001 findings on any touched file (all warnings are
repo-wide pattern-recommendation/anti-pattern-escape notes on files
outside this ticket's scope).

`nice -n 10 frob check --only coverage --files <8 touched .py> --base dev`:
exit 1 overall (`FAIL gate:COV 17 errors`), but zero COV002 (or any
COV) findings on any of the 8 touched files -- all 6 listed COV007
errors are pre-existing findings on src/frob/vet/_source.py and
src/frob/vet/_typosquat.py, outside this ticket's scope; this change
adds no new public symbols.

`ruff check <8 touched .py>`: All checks passed!

`ty check <8 touched .py>`: All checks passed!
...
```
 ...
 src/frob/tickets/_land_release.py                  |   60 +-
 ...
 src/frob/tickets/_reporting.py                     |    9 +-
 ...
 uv.lock                                            |    2 +-
 2216 files changed, 139281 insertions(+), 7150 deletions(-)
```
