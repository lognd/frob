...
body_changes:
- mode: append
  reason: 'T-4709: preserve SYS100/SYS104 retirement detail trimmed from _fix_engine.py'
  actor: logan
  at: '2026-09-19'
  old_length: 6813
  new_length: 7439
...
SYS100 (T-1531) used to be a pure `.strata` text rewrite (same category
as DOC007/DOC002/INV006-carry/FMT001/REG010/REL002) reusing the
`frob.strata._sync_may` writer -- deleted by T-2922/T-2920. T-1870:
SYS104 (the `interface=` sibling of this same category) is deliberately
NOT wired here any more -- deleted entirely, along with its writer and
every other `interface=` mutation path, per an explicit owner directive
that no code path may auto-update declared public-symbol surface.