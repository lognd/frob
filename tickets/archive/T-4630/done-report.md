...
-- T-4630 (child of T-4419, cluster 2)

Note: filed as T-4630, promoted to T-4630 by the same T-4524
land squash that promoted T-4419's other children (see T-4419's body).

### WHAT changed, per file (31 of the cluster's 33 findings)

- tests/unit/coordinator_suite/test_fleet_report.py: 2 docstrings
  (TestFleetStatusLarge001WaiverParses.test_waiver_still_suppresses_
  large001, TestOwnDocstringHasNoMalformedDirective class).
- tests/unit/gates/test_deprecated_baseline.py: 2 method docstrings
  (TestTighten.test_symbol_no_longer_deprecated_is_dropped,
  TestDepr005ViolationsGrowth.test_growth_beyond_baseline_fires_at_the_
  right_file_and_line).
- tests/unit/gates/test_wire001_cli_dest_semantic.py: 2 method
  docstrings (TestConfigExternalForwardedDestNames.test_comment_and_
  docstring_mentions_are_not_collected,
  TestWire001CliDestViolations.test_dest_mentioned_only_in_a_comment_
  is_flagged).
- tests/unit/perf/test_hotpath_smells.py: 2 method docstrings
  (TestPerf010YamlCLoader.test_does_not_fire_on_helper_loader_
  indirection, TestPerf011RepoScanInLoop.test_does_not_fire_when_
  earlier_loop_is_an_unrelated_genexpr).
- tests/unit/rapid_sweep_suite/test_attribution.py: 2 method
  docstrings (TestIdentitiesStillReproducing.test_failed_silent_tool_
  result_is_unmeasurable_not_zero,
  TestRevalidateDispatchableSweepTickets.test_terminal_ticket_is_not_
  selected_and_logs_no_invalid_transition).
- tests/unit/strata/test_contention.py: 2 method docstrings
  (TestOverlappingPath.test_common_arbitered_resource_still_fires_
  without_module, TestSharedStoreWrite.test_arbitered_store_still_
  fires_without_module).
- tests/unit/strata/test_cve_fingerprint.py: 2 class docstrings
  (TestXxeFingerprint, TestT0510Fingerprints).
- tests/unit/strata/test_effects.py: 2 docstrings
  (test_mutate_declares_every_real_effect_it_exercises method,
  TestStaleViaSymbol class).
- tests/unit/strata/test_facts.py: 2 docstrings
  (test_krb_no_transit_still_terminal_for_confidentiality_noflow
  method, TestBuildFactsNativeExtensionUnavailable class).
- tests/unit/strata/test_native_staleness.py: 2 method docstrings
  (TestStaleNatives.test_reproducible_rebuild_clears_the_content_
  digest_latch, TestSeedWorktreeNativeSourceMtimes.test_repo_side_
  untracked_file_does_not_block_seeding). The first needed a second
  pass after re-measurement still flagged it on "the prior" (incidental
  narrative-keyword match) -- reworded to "the recorded artifact".
- tests/unit/test_arch_srp.py: 2 docstrings
  (TestArch103WaiverStaysEffective class and its test_waiver_mechanism_
  resolves_a_genuine_arch103_by_exact_symbol method).
- tests/unit/test_conftest_stackdump.py: 2 docstrings
  (test_sessionfinish_stays_byte_for_byte_unchanged_when_terminal_
  already_at_column_zero method, TestStallAbortResultLines class).
- tests/unit/test_dup_legacy_cpp.py: 2 function docstrings
  (test_collect_locals_cpp_covers_bindings,
  test_collect_locals_cpp_param_folds_to_positional_token).
- tests/unit/test_gitattributes_merge.py: 2 docstrings
  (TestAttachmentCrlfSuppression class, its test_v1_flat_attachment_
  still_covered method).
- tests/unit/test_process_lock.py: 3 docstrings (TestPortableFlock
  class, TestSharedIdCounter class, its test_two_checkouts_with_
  divergent_views_never_collide method).

All rewritten to state what each test/fixture proves, with a "See
T-#### for the design rationale" pointer where useful, instead of
narrating prior incidents/investigations. No behaviour change;
`# frob:tests`/`# frob:ticket` directive placement otherwise unchanged.

One file's 2 findings were deferred, not fixed here:
tests/unit/test_cli_group_parity.py -- leased by in-progress
T-draft-5658939f (cli-regrouping parity fix). Removed from T-4630's
scope via `frob ticket scope --remove --reason` before starting.

### WHY

Continuation of T-4419's split plan (T-2994 doctrine). Cluster 2 (33
findings, 16 files, 31 of which are in this ticket's actual scope after
the leased-file removal) was one of 6 clusters cut from T-4419's 177
non-leased tests/unit findings.

### How the acceptance criterion is proven

acceptance[1]: "Given a scoped frob check on cluster 2 files, when
DOCARCH001 is measured, then its finding count for those files is 0."

Proof: a direct `docarch001_violations(root)` call filtered to the 15
files actually in T-4630's scope (test_cli_group_parity.py excluded,
leased elsewhere) returns 0 findings (was 31 before this change). The
gate's own unit-test suite
(tests/gates/test_docstring_archaeology.py::TestDocarch001Violations)
is bound as evidence.

Regression proof (not bound as acceptance evidence, run and green, via
a background nohup + PID-poll after this worktree's stale natives were
rebuilt with `frob natives build`):
`PYTHONPATH=$(pwd)/src PYTEST_XDIST_AUTO_NUM_WORKERS=1 python -m pytest
<15 files> -p no:cacheprovider -q` -> SUITE-RESULT: exitstatus=0
collected=387 failed=0.

### Pre-READY checklist evidence

1. `frob check --only sys --no-cache --files <15 files> --base dev`: 0
   unwaived SELFAUDIT001 findings on any of the 15 files (command exits
   1 overall on pre-existing unrelated DRIFT001/DSL001 findings, not
   attributable to this change).
2. `frob check --only arch --no-cache --files <15 files> --base dev`:
   exit 0, `pass frob-arch 19 warnings (36 waived), 542 suggestions` --
   no ARCH001/LARGE001 on any of the 15 files.
3. `frob check --only coverage --no-cache --files <15 files> --base
   dev`: 0 COV002 findings anywhere. 2 unwaived COV006 findings surface
   on tests/unit/test_arch_srp.py (call-graph reachability heuristic on
   `test_import_check_env_arch103_no_longer_fires_raw` and
   `test_git_head_sha_arch103_is_waived`'s `frob:tests` bindings) --
   verified pre-existing via `git diff dev -- tests/unit/test_arch_
   srp.py`: neither method's `# frob:tests` line changed in this diff
   (only the class docstring and a third method's docstring, on
   different lines, were touched), so this is not attributable to this
   change.
4. ruff check + ruff format --check on all 15 files: clean. ty check on
   all 15 files: "All checks passed!"
5. Cross-ticket: `git diff --name-only dev...HEAD` covers exactly the
   15 files in T-4630's own lease (.git/frob-leases/T-4630.json, which
   already excludes test_cli_group_parity.py after the scope removal);
...
7. Not applicable: no tickets/<dir>/ promotions touched in this
   worktree (T-4630 was already promoted by the T-4524 land squash
   before this worktree was created).
8. Evidence bound after the last content commit (675dca6df). The two
   `chore(tickets): record evidence for T-4630` commits are the
   evidence-binding tool's own ledger writes, not content changes.

### Scope refusal / deferral

`frob ticket start T-4630` initially refused: declared scope collided
with in-progress T-draft-5658939f's lease on
`tests/unit/test_cli_group_parity.py`. Resolved via `frob ticket scope
T-4630 --remove tests/unit/test_cli_group_parity.py --reason "leased by
in-progress T-draft-5658939f ..."` before starting. Its 2 DOCARCH001
findings are deferred, not dropped -- pick up once T-draft-5658939f's
lease releases.
...
### Commit shas (this worktree, branch t-4630)

675dca6df  docs(tests): clean DOCARCH001 change-narrative from cluster 2 docstrings
601917d36  chore(tickets): record evidence for T-4630
ec7f42aee  chore(tickets): record evidence for T-4630
HEAD -> ec7f42aee126d0dc6e99d9577144b1155e276739
...
```
 ...
 tests/unit/coordinator_suite/test_fleet_report.py  |   53 +-
 .../unit/coordinator_suite/test_fleet_worktrees.py |   54 +-
 tests/unit/gates/test_deprecated_baseline.py       |   19 +-
 ...
 tests/unit/test_conftest_stackdump.py              |   19 +-
 ...
 tests/unit/test_dup_legacy_cpp.py                  |   26 +-
 tests/unit/test_gitattributes_merge.py             |   27 +-
 ...
 tests/unit/test_process_lock.py                    |   40 +-
 ...
 uv.lock                                            |    2 +-
 1345 files changed, 97921 insertions(+), 3707 deletions(-)
```
