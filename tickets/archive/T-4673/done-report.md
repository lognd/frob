...
T-4673 -- SF-11: require_analyzable WARNs on every single design load

## What changed

src/frob/strata/_packs.py
  - Added module-level `_WARNED_ANALYZABLE_MODULES: set[str]` (process
    lifetime) with a one-line docstring citing SF-11 and the 570/45/12.7
    measurement.
  - `require_analyzable` now checks the module's name against that set
    before logging: first occurrence for a given module name still
    WARNs (and records the name); any subsequent auto-inject for the same
    module name in the same process logs at DEBUG instead, with the same
    message plus "(already warned this process)".

tests/unit/strata/test_packs_analyzable_warning.py (new)
  - TestAnalyzableWarningDeduped.test_load_design_ids_twice_warns_once:
    the ticket's literal positive control -- calls `load_design_ids` over
    frob's own design/ tree twice in one process and asserts at most one
    auto-inject WARNING record. Confirmed failing before the fix (2
    warnings, one per call) and passing after.
  - test_first_call_still_warns: asserts the FIRST auto-inject for a
    fresh module still WARNs -- guards against silently regressing to
    option (3) (DEBUG-only).
  - test_second_call_same_module_does_not_rewarn: asserts a second
    auto-inject for the SAME module name in the same process does not
    re-WARN, and that the module name lands in
    `_packs._WARNED_ANALYZABLE_MODULES`.
  Each test monkeypatches `_WARNED_ANALYZABLE_MODULES` to a fresh empty
  set first, so ordering/other tests in the same pytest-xdist worker
  cannot leak state between them.

## Why (ticket's chosen option)

Ticket text names three options and explicitly picks (2), "log ONCE per
process (or per module identity) instead of per call", specifically
because it stays scope-disjoint from design/frob.strata and the T-4598
lease/blocker that option (1) (declaring the pack in the model) would
require. This lands option (2), keyed on module identity so a WARNING
about a *different* module's missing pack is never suppressed by an
earlier warning for frob's own module. Option (1) remains open to the
owner as part of Story D's model decisions -- recorded here per the
ticket's instruction, not re-opened as a new ticket.

## Acceptance criteria proof

[1] "a test calling load_design_ids twice in one process asserts at most
    ONE such warning record -- a positive control that fails at HEAD
    c8f56ef10 with one per call."
    -> test_load_design_ids_twice_warns_once. Verified failing before the
    _packs.py edit (2 warnings from 2 load_design_ids calls over
    design/), passing after (<=1).

[2] "a second test asserts the FIRST warning is still emitted."
    -> test_first_call_still_warns.

Both bound as evidence below.

## Test node ids (evidence)

tests/unit/strata/test_packs_analyzable_warning.py::TestAnalyzableWarningDeduped::test_load_design_ids_twice_warns_once  (accepts=1)
tests/unit/strata/test_packs_analyzable_warning.py::TestAnalyzableWarningDeduped::test_first_call_still_warns  (accepts=2)

Full local run (all 3 new tests + existing tests/unit/strata/test_packs.py,
11 collected): exitstatus=0, failed=0.

## Commit

0cdacf140  fix(strata): dedupe require_analyzable auto-inject warning per process
  (t-4673 branch, worktree /home/logan/projects/frob/.claude/worktrees/t-4673)

Evidence bound via `frob ticket evidence T-4673 <node> --accepts N --base-ref dev`
AFTER this commit. Each `evidence` invocation made its own tool-generated
ledger commit (75fff0994, 1b637f474 -- "chore(tickets): record evidence for
T-4673"); no hand-authored code commit was made after evidence binding.
Worktree HEAD at report time: 1b637f474596263bafb10d32c2bcf24ef6d03da1.

## Pre-READY checks

- `frob check --only sys --files src/frob/strata/_packs.py --files tests/unit/strata/test_packs_analyzable_warning.py --base dev`:
  tool summary FAIL gate:DRIFT (6 errors, 5 waived) + FAIL gate:DSL (1 error) --
  grep of the full output for "_packs.py" and "test_packs_analyzable_warning"
  found ZERO matches; every reported finding is pre-existing repo-wide noise
  (DRIFT001 on _rapid_sweep.py/_evidence.py/invariants.py, a DSL finding
  elsewhere) unrelated to this ticket's files. No SELFAUDIT001/SYS finding
  attributable to _packs.py or the new test file.
- `frob check --only arch --files src/frob/strata/_packs.py --files tests/unit/strata/test_packs_analyzable_warning.py --base dev`:
  tool summary "pass frob-arch 20 warnings (36 waived), 546 suggestions" --
  none reference _packs.py or the new test file (grep: zero matches).
- `frob check --only coverage --files src/frob/strata/_packs.py --files tests/unit/strata/test_packs_analyzable_warning.py --base dev`:
  tool summary shows FAIL gate:COV/DRIFT/DSL/TODO counts, but grep of the
  full output for "_packs.py" and "test_packs_analyzable_warning" found
  ZERO matches -- every reported finding is pre-existing, unrelated to this
  ticket's files (vet/_*.py COV007 waivers, _rapid_sweep.py/_evidence.py
  DRIFT001, etc). No COV002 (missing frob:doc/frob:tests on a new public
  symbol) fired: `_WARNED_ANALYZABLE_MODULES` is private (leading
  underscore module + name), and `require_analyzable` (the only touched
  public symbol) already carries frob:doc + frob:tests and both are
  unchanged in this diff.
- `ruff check src/frob/strata/_packs.py tests/unit/strata/test_packs_analyzable_warning.py`:
  clean after one auto-fix (import sort in the new test file);
  `ruff format --check`: both files already formatted.
- `ty check src/frob/strata/_packs.py tests/unit/strata/test_packs_analyzable_warning.py`:
  "All checks passed!"

## Cross-ticket scope

`git diff --name-only dev...HEAD`: src/frob/strata/_packs.py,
tests/unit/strata/test_packs_analyzable_warning.py -- both are exactly
T-4673's declared scope (the second is a new file, not previously leased).
No grep hit for either path in any other .git/frob-leases/*.json at time
of this report.

## Filed

none (in-scope work only; no out-of-scope findings surfaced).
...
```
 ...
 uv.lock                                            |    2 +-
 1321 files changed, 94659 insertions(+), 3184 deletions(-)
```

### Evidence
- `tests/unit/strata/test_packs_analyzable_warning.py::TestAnalyzableWarningDeduped::test_load_design_ids_twice_warns_once` (pytest node id, verified passing when recorded)
- `tests/unit/strata/test_packs_analyzable_warning.py::TestAnalyzableWarningDeduped::test_first_call_still_warns` (pytest node id, verified passing when recorded)
