...
WHAT changed:
- Merged dev into t-4540 (commit d6c3b124b, then re-checked at HEAD 808a02ec2434760495f7f3e754f6b6fc0026c6b9).
- Always-take-dev files resolved with `git checkout dev -- <file>`:
  .frob-release.json, CHANGELOG.md, docs/design/registry/capability-via-ratchet.lock.json,
  pyproject.toml, uv.lock.
- design/frob.strata: took dev's side wholesale. Investigated the "real content conflict"
  first: `git diff $(git merge-base dev HEAD) HEAD -- design/frob.strata` showed only two
  hunks from T-4540's branch history (a stratamod fs.write via-list line adding
  `_effects.py`, tagged T-4495, and a testsuite env.read glob-collapse, also T-4495).
  Neither hunk is T-4540's own change (T-4540 is a REL002 dev-suffix fix, unrelated to
  strata capability declarations) -- both were stale artifacts from an old merge-base and
  dev's current file already contains the same T-4495 content (verified: dev's file has
  `_effects.py` in stratamod's fs.write via-list, and `may "env.read" via "tests/**"` in
  testsuite, i.e. T-4495 already landed on dev and superseded the branch's stale copy).
  `git diff dev -- design/frob.strata` after resolution is empty -- confirms nothing from
  T-4540's own diff needed re-applying to this file.
- tickets/: no textual conflicts; all renames/tickets git could resolve automatically
  matched dev's ids. Verified every tickets/<dir>/ticket.md's `id:` line matches its
  directory name (no mismatches found) and no stale T-4540-branch renumber or leftover
  draft ticket directory exists.
- No changes needed to src/frob/gates/_release.py, src/frob/gates/__init__.py, or
  tests/unit/test_rel002_dev_suffix.py themselves; T-4540's own REL002 dev-suffix fix
  (`_rel002_dev_ahead_of_stamp`, `_rel002_coherence_violations` update) merged cleanly
  with dev, verified present in the merged tree via
  `git diff dev -- src/frob/gates/_release.py src/frob/gates/__init__.py tests/unit/test_rel002_dev_suffix.py`.

WHY: dev had moved past the branch's merge-base with T-4495 (fs.write via-list collapse
and env.read glob collapse) landing after T-4540 branched; those hunks in T-4540's own
diff were leftover copies of already-superseded content, not T-4540's actual work, so the
correct resolution was dev's version with zero re-application.

Acceptance criteria proof:
- "GIVEN pyproject version 0.531.1.devN ... WHEN REL002 runs THEN it reports no finding"
  -> tests/unit/test_rel002_dev_suffix.py::test_dev_version_ahead_of_stamp_does_not_fire_rel002
- "GIVEN a FINAL version that does not match the stamp ... THEN it still fires exactly as
  today" -> tests/unit/test_rel002_dev_suffix.py::test_final_version_mismatch_still_fires_rel002
Both run green: `PYTHONPATH=$(pwd)/src python -m pytest tests/unit/test_rel002_dev_suffix.py -q`
-> SUITE-RESULT: exitstatus=0 collected=4 failed=0. Both recorded via
`frob ticket evidence T-4540 <node-id> --accepts <1|2> --base-ref dev`.

Gates: `frob check --only gates --ticket T-4540 --files src/frob/gates/_release.py
--files src/frob/gates/__init__.py --files tests/unit/test_rel002_dev_suffix.py --base dev`
completed (gate-summary: 62 errors, 4987 warnings, 984 waived; runtime ~15 min --
`tickets`=677s, `cross_ticket_leakage`=191s, `coverage`=229s, `sys`=103s dominate, matching
the documented ~8s-fixed-plus-heavy-stage cost model). The run's own
`gate:scope-note` states explicitly: "--ticket T-4540 scopes ONLY gate:SCOPE/gate:PREWORK
and the diff-driven checks inside gate:COV (COV002/TODO001) and gate:FMT/gate:AFFECT to
this ticket's touched set -- every OTHER gate family's counts above are REPO-WIDE, not
filtered to this ticket." Checked every failing gate family's individual findings for a
mention of src/frob/gates/_release.py, src/frob/gates/__init__.py's touched region (the new
`_rel002_dev_ahead_of_stamp` / updated `_rel002_coherence_violations`), or
tests/unit/test_rel002_dev_suffix.py: none of gate:ARCH/COV/CROSSTICKET/DOC/DRIFT/DSL/
LANG/MILE/PERF/REF/TICK/TODO/WIRE's errors name any of the three files or T-4540's added
symbols -- all are pre-existing repo-wide findings (e.g. ARCH001 length on unrelated
_land_release.py functions, COV006/COV007 on _land_cmd.py/_walk_zig.py/dsl.py, CROSSTICKET
findings elsewhere). gate:SELFAUDIT's one error is the same T-4563 testsuite-glob pending-
auto-accept note seen on T-4503's run, unrelated to T-4540. gate:SCOPE itself passed clean
(0 errors, 39 warnings -- the SCOPE002 warnings suggest widening T-4540's scope to
docs/modules/release.md and tests/test_release.py for doc/test-coverage closure on symbols
T-4540 did not touch, informational only, not required for this fix). Did not attempt to
fix the pre-existing repo-wide findings -- out of scope for this ticket and not new
regressions caused by this merge.

Commit shas: d6c3b124b (merge commit), HEAD=808a02ec2434760495f7f3e754f6b6fc0026c6b9.
...
```
 ...
 src/frob/gates/__init__.py                         | 257 +++++----
 ...
 tickets/T-4540/done-report.md                      | 125 +++++
 ...
 uv.lock                                            |   2 +-
 460 files changed, 30961 insertions(+), 1659 deletions(-)
```

### Evidence
- `tests/unit/test_rel002_dev_suffix.py::test_dev_version_ahead_of_stamp_does_not_fire_rel002` (pytest node id, verified passing when recorded)
- `tests/unit/test_rel002_dev_suffix.py::test_final_version_mismatch_still_fires_rel002` (pytest node id, verified passing when recorded)
