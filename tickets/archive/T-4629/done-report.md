...
: T-4629 (DOCARCH001 debloat cluster 5)

WHAT changed: rewrote the docstring on each DOCARCH001-flagged symbol
across all 14 of the ticket's scoped files to state WHAT the test proves
and HOW, replacing change-narrative prose ("used to", "before this fix",
"folded into", "the old X", etc.) with utility prose plus a bare
"(see T-####)" ticket reference, per docs/modules/docstrings.md's purpose
test (T-2994 doctrine: code/docs carry utility, tickets carry narrative).
No behavior change; docstrings only. Also added a `# frob:ticket T-4629`
directive to TestRunMultiplePaths in tests/unit/test_pyfmt_runner.py
(alongside the existing, now-closed T-3906 reference) after the coverage
gate flagged the changed symbol as lacking an edge to an OPEN ticket
(COV002) -- see PRE-READY FINDING below.

Files changed (14, the ticket's full scope -- both deferred/leased files
noted in the ticket body, tests/unit/verify/test_verify_runner.py (T-3082)
and tests/unit/strata/test_selfconform.py (T-4633), were already excluded
from scope by whoever split T-4419, so nothing to skip here):
...
PRE-READY FINDING (fixed): `frob check --only coverage` flagged
tests/unit/test_pyfmt_runner.py::TestRunMultiplePaths (COV002 -- "changed
with no frob:ticket edge to an open ticket") because the docstring edit
changed the symbol and its only existing frob:ticket reference, T-3906, is
now [done]. Fixed by adding a second `# frob:ticket T-4629` directive
alongside the existing T-3906 one, rather than discarding the original
provenance reference.

WORKTREE NOTE: `frob ticket work T-4629` initially failed with a git
worktree-add timeout under fleet load, then a follow-on ledger-commit
timeout left the worktree's index/working-tree checkout incomplete
(thousands of phantom `D`/`??` entries although the actual ledger commit
`9c496cecc` was fine in the shared .git). Fixed from inside the worktree
with `git checkout HEAD -- tickets/` (no uncommitted work existed yet, so
this only restored files the interrupted checkout never wrote); confirmed
clean afterward (`git status --short` -> only `uv.lock`).
...
MEASURE:
  Before: 14 DOCARCH001 findings across the 14 files above (measured via
  `frob.gates._docstring_archaeology.docarch001_violations(root)` filtered
  to this ticket's file set).
  After: 0 DOCARCH001 findings across the same 14 files.

ACCEPTANCE [1] proof: DOCARCH001 finding count for cluster-5 files is 0
after the rewrite; evidence bound to
...
test, all green (26 collected, 0 failed).
...
Test node ids (all pass, PYTHONPATH=src pytest -p no:cacheprovider -q):
  ...
  tests/unit/test_new_ticket_scope_overlap_warning.py::TestNonRelativeScopeDoesNotCrash (all methods)
  tests/unit/test_process_pid_liveness.py::TestKernel32PlatformGuard (all methods)
  tests/unit/test_pyfmt_runner.py::TestRunMultiplePaths (all methods)
  tests/unit/test_rel002_dev_suffix.py::test_dev_version_ahead_of_stamp_does_not_fire_rel002
  tests/unit/test_scaffold_natives_shim.py::TestMakefileCoreShimTemplate (all methods)
  tests/unit/test_scaffold_project.py::test_freshly_scaffolded_project_is_v2_must_fire
  tests/unit/test_ticket_new_related.py::TestPossibleEnforcementSymbolsRetargeted (all methods)
  tests/unit/test_waive_audit_watermark.py::TestSaveWatermark::test_creates_parent_dir_if_missing
  tests/unit/verify/test_backpressure.py::TestEffectiveProfileOrStandard (all methods)
  tests/unit/vet/test_capability_modes.py::TestExpandDeclaredKind::test_unwired_family_stays_coarse

## Pre-READY checks

`frob check --only sys --files <14 touched> --base dev`: pass (0
SELFAUDIT001/sys findings attributable to these files -- only pre-existing,
unrelated repo-wide findings, e.g. src/frob/vet/*.py COV007 waivers).

`frob check --only arch --files <14 touched> --base dev`: pass (no ARCH001
or LARGE001 on these files).

`frob check --only coverage --files <14 touched> --base dev`: initially
FOUND 1 real COV002 finding caused by this change
(tests/unit/test_pyfmt_runner.py::TestRunMultiplePaths cited only a
now-[done] ticket, T-3906, as its frob:ticket edge). FIXED by adding a
second `# frob:ticket T-4629` directive alongside the existing (now-closed)
T-3906 reference. Re-check confirmed 0 COV002 findings on this file
afterward (pytest re-run green: 1 collected, 0 failed).

`ruff check <14 touched>`: All checks passed!

`ty check <14 touched>`: 1 pre-existing error, unrelated to this diff --
tests/unit/verify/test_backpressure.py:133 `invalid-assignment` on
`settings.pre_commit_sweep_enabled = False` (line 133; this ticket's only
edit to that file is the class docstring at line ~144). Confirmed
pre-existing by `git diff` showing no change near line 133.

CPU BUDGET NOTE: per the 14:50 BRIEF update, all checks above were run one
at a time, `nice -n 10`'d, never three in parallel.

WORKTREE HYGIENE: uv.lock was left modified by `frob ticket work T-4629`'s
`uv sync` step (1431-line diff, unrelated dependency version bumps). Reset
with `git checkout HEAD -- uv.lock` before commit -- out of this ticket's
scope and not part of the diff landed.

Commit SHAs:
  7484eacb0 docs(tests): rewrite change-narrative docstrings to utility (DOCARCH001 cluster 5)
  000bc4baf chore(tickets): record evidence for T-4629 (ledger commit, auto-created by `frob ticket evidence`)

Evidence bound (after last content commit, per checklist item 8):
  frob ticket evidence T-4629 tests/gates/test_docstring_archaeology.py::TestDocarch001Wiring::test_fires_through_run_gates --accepts 1 --base-ref dev
...
```
 ...
 tests/unit/test_scaffold_project.py                |   10 +-
 ...
 tickets/T-4629/ticket.md                           |   49 +
 tickets/T-4630/done-report.md                      | 1508 +++++++++++++++++++
 tickets/T-4630/ticket.md                           |   59 +
 tickets/T-4631/done-report.md                      | 1577 ++++++++++++++++++++
 ...
 uv.lock                                            |    2 +-
 1412 files changed, 103124 insertions(+), 4652 deletions(-)
```
