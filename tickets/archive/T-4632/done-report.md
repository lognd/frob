...
-- T-4632 (child of T-4419, cluster 1)

Note: this ticket was filed as T-4632 and promoted to T-4632 by
an unrelated fleet land squash (T-4524, commit ade7d9db3) between filing
and start; T-4419's body was updated with the id mapping.

### WHAT changed, per file

- tests/unit/coordinator_suite/test_fleet_worktrees.py
  Rewrote 4 flagged docstrings: TestInProgressTicketScopeLeases.test_no_
  worktree_flagged_as_leak, TestWorktreesTouchingTicket.test_series_
  worktree_matches_sibling_ticket_via_start_transition,
  TestWorktreeContentClassificationLiveGit (class) and its
  test_non_conventionally_named_worktree_classifies_active_via_
  structural_ids method.

- tests/unit/graph/test_dsl_markdown_waive.py
  Rewrote 3 flagged class docstrings: TestWaiveReasonUnescapedQuoteIsLoud,
  TestBrokenDirectEdgeVerbIsLoud, TestChangelogMultiLineCodeSpanMention.

- tests/unit/rapid_sweep_suite/test_filing.py
  Rewrote 3 flagged method docstrings:
  TestRelativizeRegressionScopeFile.test_filed_ticket_scope_is_relative_
  end_to_end, TestRaiseQuarantineForRedBatch.test_closed_ticket_
  attribution_still_raises, TestFileRegressionTicket.test_duplicate_
  finding_disposes_to_declaring_ticket_instead_of_dropping.

- tests/unit/rapid_sweep_suite/test_sweep_run.py
  Rewrote 3 flagged docstrings: TestPersistBaseline (class),
  TestDeferredSweepRun.test_fresh_baseline_files_normally_no_new_noise,
  TestDeferredSweepSpawn.test_spawn_pins_frob_root_env_not_bare_os_
  environ.

- tests/unit/test_app_runners_batch7.py
  Rewrote 4 flagged method docstrings: TestTicketRenumber.test_dry_run_
  without_old_new_exits_1, TestTicketRenumber.test_whole_ledger_already_
  contiguous, TestTicketLand.test_land_success_prints_files,
  TestTicketStart.test_short_dissimilar_titles_are_not_flagged_as_
  related.

- tests/unit/test_check.py
  Rewrote 3 flagged class docstrings: TestDispatchCheckThreadsGate
  Selectors, TestRunGatesCacheWiring, TestGatesErrorResultTotalAbort.

- tests/unit/test_check_budget.py
  Rewrote 4 flagged method docstrings:
  TestDerivePostLandSweepBudget.test_derives_from_measured_timing_with_
  headroom, TestRunBudgetedCheck.test_budget_json_stdout_is_pure_
  parsable_json, TestRunBudgetedCheck.test_json_reports_universe_skip_
  despite_narrow_resume, TestRunBudgetedCheck.test_only_scoped_budget_
  runs_exactly_the_named_group.

- tests/unit/test_ticket_runner_ledger_mirror.py
  Rewrote 4 flagged docstrings: TestVerbStrategy.test_derived_sets_
  track_the_live_strategy_table, TestVerbStrategy.test_missing_raises,
  TestEvidenceRebindMirror.test_prior_scope_mirror_then_replace_does_
  not_leave_the_old_id_resurrectable, TestMirrorPreservesEvidence
  (class).

- tests/unit/test_ticket_store.py
  Rewrote 4 flagged method docstrings: TestWriteTicket.test_content_
  loss_refuses_by_default, TestWriteTicket.test_non_strict_opt_out_
  warns_loudly_instead_of_refusing, TestV2WriteTicket.test_write_all_
  v2_keeps_done_report_split_out, TestWriteArchivedTicket.test_v2_
  write_archived_ticket_keeps_done_report_split_out.

All 32 flagged docstrings (per T-4419's cluster 1 plan) now state what
each test/fixture proves, with a "See T-#### for the design rationale"
pointer where useful, instead of narrating prior incidents/attempts. No
behaviour change; `# frob:tests`/`# frob:ticket` directive placement
otherwise unchanged. Two rewrites needed a second pass after the first
re-measurement still flagged them on incidental narrative-keyword
overlap ("SUPERSEDED", "the old") unrelated to their intended meaning --
reworded to avoid the trigger words while keeping the same meaning.

### WHY

Continuation of T-4419's split plan (T-2994 doctrine: code/docs carry
utility, tickets carry narrative). Cluster 1 (32 findings, 9 files) was
one of 6 clusters cut from T-4419's 177 non-leased tests/unit findings;
see T-4419's body for the full per-file breakdown.

### How the acceptance criterion is proven

acceptance[1]: "Given a scoped frob check on cluster 1 files, when
DOCARCH001 is measured, then its finding count for those files is 0."

Proof: a direct `docarch001_violations(root)` call filtered to the 9
cluster-1 files returns 0 findings (was 32 before this change). The
gate's own unit-test suite
(tests/gates/test_docstring_archaeology.py::TestDocarch001Violations) is
bound as evidence, exercising the exact code path this measurement used.
...
<9 files> -p no:cacheprovider -q` -> SUITE-RESULT: exitstatus=0
collected=591 failed=0.

### Pre-READY checklist evidence

1. `frob check --only sys --no-cache --files <9 files> --base dev`: 0
   unwaived SELFAUDIT001 findings on any of the 9 files (command exits 1
   overall on pre-existing unrelated DRIFT001/DSL001 findings, same as
   T-4419's run -- not attributable to this change).
2. `frob check --only arch --no-cache --files <9 files> --base dev`:
   exit 0, `pass frob-arch 19 warnings (36 waived), 542 suggestions` --
   no ARCH001/LARGE001 on any of the 9 files.
3. `frob check --only coverage --no-cache --files <9 files> --base
   dev`: 0 COV002 findings on any of the 9 files (all 9 already carried
   an open frob:ticket edge from their prior work, so no directive
   addition was needed this time, unlike T-4419's test_graph_cache.py
   case). Remaining COV/DRIFT/DSL/TODO FAILs in that invocation are all
   pre-existing, unrelated findings (src/frob/vet/*, unrelated files,
   already-waived TODO001/PLACE001 on lines this change did not touch).
4. ruff check + ruff format --check on all 9 files: clean. ty check on
   all 9 files: "All checks passed!"
5. Cross-ticket: `git diff --name-only dev...HEAD` covers exactly the 9
   files in T-4632's own lease (.git/frob-leases/T-4632.json); verified
   byte-for-byte match against the lease file.
6. Not applicable: kind=docs, not bug/security.
7. Not applicable: no tickets/<dir>/ promotions touched in this
   worktree (T-4632 itself was already promoted by the T-4524 land
   squash before this worktree was created).
8. Evidence bound after the last content commit (5de038b8c). The two
   `chore(tickets): record evidence for T-4632` commits are the
...
### Commit shas (this worktree, branch t-4632)

5de038b8c  docs(tests): clean DOCARCH001 change-narrative from cluster 1 docstrings
5807db753  chore(tickets): record evidence for T-4632
0b0a98a7b  chore(tickets): record evidence for T-4632
HEAD -> 0b0a98a7b4fa411284998cccd36d4341cc884305
...
```
 ...
 .../unit/coordinator_suite/test_fleet_worktrees.py |   51 +-
 ...
 uv.lock                                            |    2 +-
 1329 files changed, 96267 insertions(+), 3447 deletions(-)
```
