...
body_changes:
- mode: append
  reason: 'T-1633: each attribute alternative is LEFT-ANCHORED by an explicit

    leading-character alternation rather than a lookbehind, because this

    pattern is handed to git grep -E (POSIX ERE), which has no lookbehind

    at all. Without the anchor, ticket= matched inside any longer

    identifier ending in it -- active_ticket=T-1582 in ordinary Done-report

    prose read as a live-tracker citation and refused a land.'
  actor: logan
  at: '2026-09-19'
  old_length: 2499
  new_length: 2976
...
<!-- narrative-moved:src/frob/tickets/_live_tracker.py:68:T-1633 -->
frob:ticket T-1633
