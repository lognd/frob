...
3. tests/gates_suite/test_sys.py::TestKnownGateRuleIds::test_every_emitted_rule_literal_is_known -- a rule id literal is constructed in src/frob/gates or src/frob/strata that is not in the known-rule registry. Every emitted rule must be registered (that registry is what WAIVE002/docs generation read).
