...
-- T-4628 (child of T-4419, cluster 3)

Note: filed as T-4628, promoted to T-4628 by the same T-4524
land squash that promoted T-4419's other children (see T-4419's body).
No scope collisions this time; `frob ticket work T-4628` started
cleanly against the full 21-file cluster.

### WHAT changed, per file (all 33 findings)

- tests/unit/arch_suite/test_concurrency.py: 1 method docstring
  (TestForkPoolHazards.test_self_join_deadlock_discharges_on_real_repo_
  vet_scan).
- tests/unit/arch_suite/test_dispatch.py: 1 class docstring
  (TestCollectDispatchRefs).
- tests/unit/arch_suite/test_lang_adapters.py: 1 method docstring
  (TestFourWayCrossLanguageEquivalence.test_derived_class_has_the_
  field_and_one_method).
- tests/unit/coordinator_suite/test_fleet_host_load.py: 1 method
  docstring (TestOrphanedForkserverCount.test_two_level_chain_with_
  dead_root_is_orphaned).
- tests/unit/coordinator_suite/test_fleet_land.py: 1 method docstring
  (TestLandProcessRows.test_a_land_in_a_different_repo_is_not_counted).
- tests/unit/gates/test_detector_scope.py: 1 method docstring
  (TestDetectorScope.test_tracked_gate_files_filters_to_detector_
  roots).
- tests/unit/gates/test_examined_sites.py: 1 class docstring
  (TestAttachExaminedSites).
- tests/unit/gates/test_exhaustive_handling_path_shape.py: 1 function
  docstring (test_exclude_glob_and_test_dir_are_honored_not_scanned_
  as_production).
- tests/unit/gates/test_ffi_boundary_path_shape.py: 1 function
  docstring (test_exclude_glob_and_test_dir_are_honored_not_scanned_
  as_production).
- tests/unit/test_land_dirty_main_orphaned_ticket_t2026.py: 2 method
  docstrings (TestCommitOrphanedNewTicketDirOnlyDrift.test_non_queued_
  orphan_is_never_auto_committed,
  TestRefuseIfMainDirtyOrphanedTicketHeal.test_genuinely_human_dirty_
  root_still_refuses).
- tests/unit/test_land_in_progress_window.py: 3 docstrings
  (TestLandInProgressWindowNarrowedToSplice.test_land_lock_held_but_
  tickets_lock_free_allows_the_write, TestWholeLandVerbClassification
  class) -- 2 findings; a 3rd COV002 surfaced afterward needing a
  `# frob:ticket T-4628` directive on the class and method (see below).
- tests/unit/test_land_verify_claim_divergence_sentinel.py: 2
  docstrings (TestQueue001CodedSentinelIsAlsoExcluded class, its
  test_queue001_coded_sentinel_does_not_refuse method).
- tests/unit/test_lang_strata.py: 2 method/class docstrings
  (TestParseStrata.test_publicness_is_derived_from_clearance_not_a_
  blanket_true, TestGrammarAuthoritativeSymbolsCorpusWide class).
- tests/unit/test_main_entry.py: 2 docstrings
  (TestHelpListsDirectDispatchVerbs class,
  TestGroupedHelpFormatter.test_no_help_text_breaks_inside_a_word).
- tests/unit/test_policy_weakening_gate.py: 2 docstrings
  (TestPolicyWeakeningGateFileIdentity class, its test_must_fire_two_
  weakenings_in_different_files_get_distinct_file_identities method).
- tests/unit/test_process_reap.py: 2 class docstrings
  (TestInstallSigtermReaper, TestIsLiveCheckProcess).
- tests/unit/test_release_workflow_gate.py: 2 method/class docstrings
  (TestCrossBuiltTargetsSkipImportSmoke.test_install_only_runs_inside_
  the_non_cross_branch, TestArtifactSmokeAarch64UsesNativeArmRunner
  class).
- tests/unit/test_skills_sync.py: 2 class docstrings
  (TestSkillsSyncRenderLint, TestMakefileRecipeDelegates).
- tests/unit/test_ticket_runner_land_release.py: 2 method/class
  docstrings (TestApplyReleaseBumpForLand.test_stamp_failure_
  propagates_instead_of_staging_stale_manifest,
  TestVerifyOneBucketPassingSpawnFailureIsUnmeasured class).
- tests/unit/test_unlanded_branch_work.py: 2 method/class docstrings
  (TestUnlandedBranchWork.test_real_ticket_id_inside_a_string_literal_
  is_not_flagged, TestBranchOwnChangedFilesConsolidation class).
- tests/unit/verify/test_worker.py: 2 method docstrings
  (TestRunCoalescedVerification.test_unfilable_finding_still_pins_the_
  watermark_on_the_next_wake,
  TestRunCoalescedVerification.test_partially_vanished_findings_still_
  pin_the_watermark).
...
except the T-4628 COV002 fix-up below. Every rewrite passed
re-measurement to 0 on the first attempt (no narrative-keyword
false-positive retries needed, unlike T-4630's two).

### WHY

Continuation of T-4419's split plan (T-2994 doctrine). Cluster 3 (33
findings, 21 files) was one of 6 clusters cut from T-4419's 177
non-leased tests/unit findings.

### How the acceptance criterion is proven

acceptance[1]: "Given a scoped frob check on cluster 3 files, when
DOCARCH001 is measured, then its finding count for those files is 0."

Proof: a direct `docarch001_violations(root)` call filtered to the 21
cluster-3 files returns 0 findings (was 33 before this change). The
...
a background nohup + PID-poll):
`PYTHONPATH=$(pwd)/src PYTEST_XDIST_AUTO_NUM_WORKERS=1 python -m pytest
<21 files> -p no:cacheprovider -q` -> SUITE-RESULT: exitstatus=0
collected=541 failed=0.

### Pre-READY checklist evidence

1. `frob check --only sys --no-cache --files <21 files> --base dev`: 0
   unwaived SELFAUDIT001 findings on any of the 21 files (command exits
   1 overall on pre-existing unrelated DRIFT001/DSL001 findings, not
   attributable to this change).
2. `frob check --only arch --no-cache --files <21 files> --base dev`:
   exit 0, `pass frob-arch 19 warnings (36 waived), 544 suggestions` --
   no ARCH001/LARGE001 on any of the 21 files.
3. `frob check --only coverage --no-cache --files <21 files> --base
   dev`: initial run found 3 COV002s on
   tests/unit/test_land_in_progress_window.py (TestLandInProgressWindow
   NarrowedToSplice class, its test_land_lock_held_but_tickets_lock_
   free_allows_the_write method, and TestWholeLandVerbClassification
   class) -- fixed by adding `# frob:ticket T-4628` directives
   alongside the existing T-3612 reference, second commit, re-measured
   to 0 on that file alone. No COV002 anywhere else.
4. ruff check + ruff format --check on all 21 files: clean. ty check on
   all 21 files: "All checks passed!"
5. Cross-ticket: `git diff --name-only dev...HEAD` covers exactly the
   21 files in T-4628's own lease (.git/frob-leases/T-4628.json) plus
   this ticket's own `tickets/T-4628/ticket.md` ledger file (the
   evidence-binding tool's own write, not a lease-scope file);
   verified byte-for-byte match on the content files.
6. Not applicable: kind=docs, not bug/security.
7. Not applicable: no tickets/<dir>/ promotions touched in this
   worktree (T-4628 was already promoted by the T-4524 land squash
   before this worktree was created).
8. Evidence bound after the last content commit (929ee1a1f, the
   frob:ticket-directive fix). The two `chore(tickets): record evidence
   for T-4628` commits are the evidence-binding tool's own ledger
   writes, not content changes.
...
### Commit shas (this worktree, branch t-4628)

0711e5e6f  docs(tests): clean DOCARCH001 change-narrative from cluster 3 docstrings
929ee1a1f  docs(tests): bind T-4628 frob:ticket edge on changed land-window docstrings
f63fa6bf8  chore(tickets): record evidence for T-4628
c849cf364  chore(tickets): record evidence for T-4628
HEAD -> c849cf36477c200c97a1746b13a902a19d963c02
...
```
 ...
 tests/unit/coordinator_suite/test_fleet_land.py    |   11 +-
 ...
 tests/unit/test_main_entry.py                      |   20 +-
 ...
 tests/unit/test_unlanded_branch_work.py            |   30 +-
 ...
 uv.lock                                            |    2 +-
 1367 files changed, 99684 insertions(+), 4103 deletions(-)
```
