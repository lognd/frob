...
frob:waive BUG002 reason="this bug-kind ticket's own diff-touched code (src/frob/gates/_dup_graph_schema.py) already landed on main as a disclosed passenger of T-2435's cross-ticket land (--allow-cross-ticket, same series worktree, T-1618) -- there is no separate parent commit where this fix is absent to reproduce a repro-fail against; the fix and the bound tests' passing state are already atomically merged into main's history"
