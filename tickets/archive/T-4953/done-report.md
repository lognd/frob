...
-- T-4953 (now fully proven -- supersedes the earlier partial report)

WHAT changed:
- src/frob/_cli_parsers/_quality.py: `_add_quality_parser` now also calls
  `_add_check_skip_unified_arg(check_p)` (imported alongside the existing
  `_add_check_skip_args`/`_add_check_scope_args`/`_add_check_selection_args`), and gained a
  `# frob:ticket T-4953` directive (COV002 required an edge to an open ticket; the pre-existing
  `# frob:ticket T-1567` alone no longer satisfied it once this function changed).
- tickets/T-4953/ticket.md (via `frob ticket accept --remove`): acceptance criterion [1]
  ("test_every_ops_leaf_matches_its_flat_twin[natives] passes") REMOVED with a reason, since it
  was mis-scoped into this ticket at filing time (this ticket's scope is `_quality.py` only) and
  its evidence had been wrongly bound to a test that does not cover the natives path. Split out
  into its own ticket (title: "Restore --skip flag parity between frob ops natives and its flat
  twin", filed against T-4806, BLOCKED on T-5129's live lease on `_ops.py`/`_misc.py`) -- see
  CI-BURNDOWN.md for that ticket's final id, which was still being minted by a severely
  contention-loaded `frob ticket new` at the time this report was written (see T-5075's diagnosis
  of why ticket-ledger operations are currently slow on this box).

WHY: CI run 35476139324 on dev failed tests/unit/test_cli_group_parity.py::TestQualityGroupParity::
test_every_quality_leaf_matches_its_flat_twin[check]: `frob quality check`'s parser was missing
the unified `--skip STAGE` flag (T-4524 form) that its flat twin `frob check` registers, because
the flat twin's builder (src/frob/_cli_parsers/_check.py, top-level `check_p`) calls both
`_add_check_skip_args` AND `_add_check_skip_unified_arg`, while the grouped builder only called
the first. Real src bug (classification (a)), fixed by calling the same helper the flat twin
calls. The `[natives]` half of the original CI failure list is a DIFFERENT pair of files
(`_ops.py`/`_misc.py`) with the same class of drift, correctly split into its own ticket rather
than left as an unprovable criterion on this one.

Acceptance criterion proof (now the ticket's only criterion):
    "test_every_quality_leaf_matches_its_flat_twin[check] passes" --
    ...
    -p no:cacheprovider -p no:xdist -q` -> `SUITE-RESULT: exitstatus=0 collected=8 failed=0`
    (re-verified after the frob:ticket-edge fixup commit, still 8/8 passing).

Evidence bound: tests/unit/test_cli_group_parity.py::TestQualityGroupParity::
test_every_quality_leaf_matches_its_flat_twin -- correctly and singly bound to the one remaining
criterion (confirmed via `frob ticket show T-4953` after the `--remove`: acceptance[1] carries this
evidence, acceptance_amendments[1] records the removal and its reason).

Filed: the ops/natives split-out ticket (id pending at report time, see CI-BURNDOWN.md's final
table for the resolved id -- filed against parent T-4806, BLOCKED on T-5129's lease).

Commits (in order):
- 935255989c876e1c3e59123b8dd7ce9f6369fdcf -- fix(cli): register the unified --skip arg on frob
  quality check
- 4318d83acfe8efa3f620fae3f8ca9f1d104c2425 -- evidence bind (initial, later corrected)
- a0f7fdaf5066e845171a1c0bb6bac6a52894d631 -- chore(cli): bind frob:ticket T-4953 edge on
  _add_quality_parser (fixes COV002)
- 8822a78e1f982d555c4f68ff8529d4d6362d35f6 -- chore(tickets): accept T-4953 (removed the
  mis-scoped [natives] criterion with a reason)

## Pre-READY checks

`frob check --only sys --files src/frob/_cli_parsers/_quality.py --base dev`: FAIL gate:DRIFT (6
errors) and FAIL gate:DSL (1 error) -- grepped the full log for my touched path: zero matches.
Pre-existing repo-wide baseline (same DRIFT001 seen across every ticket in this pass). Not
attributable.

`frob check --only arch --files src/frob/_cli_parsers/_quality.py --base dev`: pass (frob-arch 21
warnings/36 waived, 546 suggestions -- none naming my file); no ARCH001/LARGE001.

`frob check --only coverage --files src/frob/_cli_parsers/_quality.py --base dev`: FIRST run
(before the frob:ticket-edge fix) showed `COV002: src/frob/_cli_parsers/_quality.py::
_add_quality_parser changed with no frob:ticket edge to an open ticket` -- ATTRIBUTABLE, fixed by
adding `# frob:ticket T-4953`. RE-RUN after the fix: grepped the full log for my file, zero
matches (the COV002 finding is gone); remaining FAIL gate:COV (10 errors)/FAIL gate:TODO (5
errors) are the same pre-existing baseline unrelated files seen on every other ticket this pass.

`ruff check src/frob/_cli_parsers/_quality.py`: All checks passed!

`ty check src/frob/_cli_parsers/_quality.py`: All checks passed!

Cross-ticket scope: `git diff --name-only dev...HEAD` = src/frob/_cli_parsers/_quality.py (in my
declared scope) and tickets/T-4953/ticket.md (my own ticket's ledger file, expected). No other
in-progress ticket leases src/frob/_cli_parsers/_quality.py.
...
```
 ...
 src/frob/_cli_parsers/_quality.py                  |    3 +
 ...
 src/frob/app/ticket_runner/_rapid_sweep.py         |  840 ++++++-
 ...
 tickets/T-4660/done-report.md                      | 1376 ++++++++++
 tickets/T-4660/ticket.md                           |   68 +
 ...
 tickets/T-4689/done-report.md                      | 1029 ++++++++
 ...
 tickets/T-4709/done-report.md                      | 2621 ++++++++++++++++++++
 ...
 tickets/T-4806/done-report.md                      | 1285 ++++++++++
 ...
 uv.lock                                            |    2 +-
 2420 files changed, 177259 insertions(+), 11258 deletions(-)
```

### Evidence
- `tests/unit/test_cli_group_parity.py::TestQualityGroupParity::test_every_quality_leaf_matches_its_flat_twin` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [1] remove: removed 'test_every_ops_leaf_matches_its_flat_twin[natives] passes' (reason: split out into T-5057 (Restore --skip flag parity between frob ops natives and its flat twin), which is BLOCKED on T-5129's live lease on src/frob/_cli_parsers/_ops.py and _misc.py -- this ticket's own scope (_quality.py only) cannot satisfy this criterion, and its evidence was previously mis-bound to a test that does not cover the natives path; logan, 2026-09-19)
