...
body_changes:
- mode: append
  reason: 'T-1760 ROOT CAUSE this whole reset exists to close: none of

    CHANGELOG.md/.frob-release.json/pyproject.toml is protected by

    ticket.scope (_auto_resolve_out_of_scope_conflicts only fires on a

    genuine git CONFLICT, keep="ours"), and git merge --squash performs an

    ordinary clean 3-way merge on any file that does NOT conflict. A

    worktree branched before a sibling''s land already advanced these files

    carries its own (older) copies at whatever content they held at the
    ...
    root''s last real commit already declared. Measured on main across four

    consecutive lands (T-1692/T-1754/T-1755/T-1756): the version oscillated

    0.366.0 -> 0.365.0 -> 0.366.0 -> 0.365.0, each backward step exactly
...
<!-- narrative-moved:src/frob/tickets/_land_release.py:533:T-1760 -->
...
see T-1805 for the history behind this