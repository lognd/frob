## Done report

_load_parser_factory (via resolve_dotted_symbol) resolved a configured
[[docblocks.commands]].parser entry with a plain importlib.import_module,
which returns whatever the RUNNING land process already has imported --
not the merge-candidate worktree's own content. A same-diff new
subcommand + its own README.md row was refused forever because the
"live" registry the guard compared against was, by construction, one
commit stale.

Reproduced directly this session: added a regression test
(TestAssertDiffDoesNotAddNewFileLocalErrorsDoc005::test_a_same_diff_new_subcommand_does_not_falsely_refuse)
and confirmed it FAILS at the parent commit (pre-fix code re-applied via
a patch/checkout roundtrip) with the exact refusal text the ticket
describes, then passes with the fix.

Fix: added _load_parser_factory_from_root, which imports the configured
parser module fresh from the candidate root's own on-disk file
(importlib.util.spec_from_file_location under a process-unique name, so
it never touches sys.modules), and threaded it through _console_trees
and _apply_bypass_subtree_patches via an optional loader parameter.
Every existing whole-repo-gate caller keeps the prior in-process
default; only _land_cmd._doc005_checker now passes the root-rooted
loader.

Also updated the 3 pre-existing DOC005 land-time test fixtures to write
their own physical copy of tests/test_docblocks_gate.py inside the
temp repo (they previously relied on the real project's already-
imported tests package, which the fix's fresh-import no longer reads).

Gates: ty check and ruff format --check clean on both touched src files.
All 4 DOC005 land tests pass. Full `frob check --ticket T-2941` shows
only pre-existing repo-wide baseline noise (ruff-format on 82 untouched
files, ty diagnostics elsewhere, frob-cycle, frob-exports) plus one
unrelated pre-existing test failure
(TestBranchDriftGuard::test_branch_drift_before_final_commit_refuses_by_construction),
confirmed to reproduce identically with this ticket's diff fully
reverted.

Not fixed here (informational only, no code change): the land refusal
message still suggests `frob:waive DOC005 reason="..."` as an escape
hatch. Verified this DOES work at land time -- the land-time check
(_unwaived_file_local_error_findings) reads the waiver directly off
README.md's own text, independent of DSL001's whole-repo markdown-
waiver allowlist that excludes DOC005 -- so the "second, smaller bug"
the ticket flagged does not actually block a land; whether frob check's
own full-repo DOC005 run would re-flag the same waived row post-merge
is a separate question outside this ticket's scope.

### Changed
```
 src/frob/app/ticket_runner/_land_cmd.py   |  17 ++++-
 src/frob/gates/_docblocks_refs.py         | 118 ++++++++++++++++++++++++++++--
 tests/test_ticket_work_and_land_finish.py |  75 ++++++++++++++++++-
 3 files changed, 200 insertions(+), 10 deletions(-)
```

### Evidence
(no evidence recorded)

### Captured claims
- tests: 0 passed (from 0 evidence id(s))
- gates: 94 error(s), 873 warning(s), 874 waived
- error-findings: ARCH103@src/frob/app/_version_guard.py, ARCH103@src/frob/app/ticket_runner/_land_cmd.py, ARCH103@src/frob/refactor/_verify.py, COV001@.claude/hooks/frob-suggest.py, COV001@strata-core/src/graph/model.rs, COV001@strata-core/src/graph/query.rs, COV003@tickets/T-3181, COV007@.claude/hooks/frob-suggest.py, CYCLE001@src/frob/__init__.py, DOC001@docs/strata/graph.md, DOC002@src/frob/tickets/_leases.py, DOC005@README.md, DOC005@docs/modules/cli.md, DOC006@tickets/T-2962/ticket.md, DOC007@src/frob/app/check_runner.py, DOC007@src/frob/app/doctor_runner.py, DOC007@src/frob/check/_python.py, DOC007@src/frob/ci_report.py, DOC007@src/frob/ghio.py, DOC007@tests/unit/test_app_runners_batch6.py, DOC007@tests/unit/test_check.py, DOC007@tests/unit/test_close_blocked_by_guard.py, DOC007@tests/unit/test_doctor_runner_t1276.py, DOC007@tests/unit/test_logging_module.py, DOC007@tests/unit/test_reopen_ticket.py, DRIFT001@scripts/fleet_status.py, DRIFT001@src/frob/gates/__init__.py, DRIFT001@src/frob/tickets/_land_squash.py, DRIFT002@docs/modules/gates.md, DRIFT002@src/frob/app/check_runner.py, DRIFT002@src/frob/app/doctor_runner.py, DRIFT002@src/frob/check/_python.py, DRIFT002@src/frob/ci_report.py, DRIFT002@src/frob/ghio.py, DRIFT002@tests/unit/test_app_runners_batch6.py, DRIFT002@tests/unit/test_check.py, DRIFT002@tests/unit/test_close_blocked_by_guard.py, DRIFT002@tests/unit/test_doctor_runner_t1276.py, DRIFT002@tests/unit/test_logging_module.py, DRIFT002@tests/unit/test_reopen_ticket.py, FLAGCOV001@frob.toml, LARGE001@src/frob/__main__.py, LARGE001@src/frob/process/_reap.py, LARGE001@src/frob/stats/_agentic.py, LARGE001@strata-core/src/graph/vmodel.rs, LARGE001@strata-core/src/parse/grammar_core.rs, OPAQUE001@src/frob/app/ticket_runner/_land_cmd.py, PERF004@.claude/hooks/frob-suggest.py, PII012@src/frob/app/doctor_runner.py, PII012@src/frob/serve/_socketd.py, PII012@tests/unit/test_doctor_runner_t1276.py, PRE001@tickets/T-2941, REF002@docs/modules/ci_report.md, REF002@docs/modules/ci_validity.md, REF002@docs/modules/ghio.md, REF002@src/frob/tickets/_done_report.py, REG002@docs/design/registry/check-coverage.yaml, REG005@docs/design/registry/check-coverage.yaml, REG008@docs/design/registry/check-coverage.yaml, REL001@src/frob/__main__.py, REL001@src/frob/stats/_agentic.py, SEC110@.claude/hooks/frob-suggest.py, SEC110@src/frob/__main__.py, SEC110@src/frob/logging/logger.py, SEC110@tests/conftest.py, SEC110@tests/test_worktree_guard.py, SUPPRESS001@src/frob/app/_config_external.py, TICK004@tickets.md, TICK006@tickets.md, TICK011@tickets.md, WAIVE006@src/frob/gates/_rule_id_scan.py, WIRE001@src/frob/gates/_docblocks_refs.py, WIRE002@src/frob/gates/_tdd_order.py, unknown-argument@tests/unit/test_app_runners_process.py, unknown-argument@tests/unit/test_pytest_spawn_env_wiring.py, unresolved-attribute@scripts/fleet_status.py, unresolved-attribute@tests/system/test_fleet_status_ground_truth.py, unresolved-attribute@tests/test_app_daemon_proxy.py, unresolved-attribute@tests/test_coverage_wait_shared.py, unresolved-attribute@tests/test_serve_leases.py, unresolved-attribute@tests/test_serve_socket.py, unresolved-attribute@tests/test_ticket_land.py, unresolved-attribute@tests/test_ticket_leases.py, unresolved-attribute@tests/test_ticket_reconcile.py, unresolved-attribute@tests/test_tickets_parent.py, unresolved-attribute@tests/test_tickets_priority.py, unresolved-attribute@tests/unit/test_conftest_stackdump.py, unresolved-attribute@tests/unit/test_coordinator_scripts.py, unresolved-attribute@tests/unit/test_land_finish_guard.py, unresolved-attribute@tests/unit/test_land_lock_liveness.py, unresolved-attribute@tests/unit/test_process_lock.py, unresolved-attribute@tests/unit/test_rapid_sweep.py, unresolved-attribute@tests/unit/test_stackdump.py, unresolved-attribute@tests/unit/test_ticket_store.py
