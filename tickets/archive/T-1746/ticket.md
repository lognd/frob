...
DOCARCH001 cleanup note (T-4420): tests/gates_suite/test_wire.py::TestWireGate.test_test_helper_called_from_a_real_test_in_the_same_file_is_not_flagged's docstring used to say: 'T-1746: a helper called directly from a genuine test_* function in its OWN defining file now counts as reached -- the same-file test-fixture-reuse false positive this ticket exists for (T-1727's motivating case: a shared fixture two test classes in one file both call from real test_* methods). Superseded the pre-T-1746 assertion that same-file usage never counts.' Moved here; the test docstring now states only what it verifies.