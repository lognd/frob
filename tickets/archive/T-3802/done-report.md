...
-- T-3802 (update docs/modules/process.md for ProcessGuardError.SpawnFailed)

### WHAT changed, per file

- docs/modules/process.md
  - `ProcessGuardError` code snippet under "## Public API" now lists
    all three members (`ExecDisabled`, `Timeout`, `SpawnFailed`), not
    just `ExecDisabled`.
  - `guarded_subprocess_run`'s snippet comment now names all three
    `Err(...)` outcomes instead of only `Err(ExecDisabled)`.
  - New prose paragraph explaining the exception-to-Result mapping
    `guarded_subprocess_run` performs for each member: `ExecDisabled`
    (kill switch, never spawns), `Timeout` (T-3015, `subprocess.
    TimeoutExpired` caught), `SpawnFailed` (T-3797, `FileNotFoundError`/
    `PermissionError`/`NotADirectoryError` -- all `OSError` subclasses
    -- caught), and notes the `doctor.py::_probe_binary_version`
    caller dependency on this contract, matching the source docstring's
    own explanation.

- src/frob/process/_guard.py
  - Removed the `frob:waive AFFECT001` on `ProcessGuardError` whose
    entire reason text was "docs/modules/process.md update filed as
    follow-up T-3802" -- now obsolete since this ticket IS that
    follow-up landing.

### WHY

T-3797 added `Timeout` and `SpawnFailed` to `ProcessGuardError` and
updated `guarded_subprocess_run`'s own docstring, but scoped itself to
`src/frob/process/_guard.py` + `tests/unit/test_process_guard.py` only,
filing this ticket as the doc follow-up and leaving an AFFECT001 waiver
in place naming it. `docs/modules/process.md`'s `ProcessGuardError`
code snippet still showed only `ExecDisabled`, so a reader of the docs
alone would not know `Timeout`/`SpawnFailed` exist or what triggers
each, and `guarded_subprocess_run`'s own snippet still implied it only
ever returns `Err(ExecDisabled)`.

### Drift-check the anchor

Ran the real AFFECT001/AFFECT002 stale-doc gate
(`frob check --only affect_drift --files src/frob/process/_guard.py
--files docs/modules/process.md --base dev`) against the finished
change: no AFFECT001 finding for `ProcessGuardError` or its
`docs/modules/process.md#public-api` anchor, and no new DRIFT/DSL
finding attributable to either touched file (the DRIFT001 findings the
run reports are pre-existing, on unrelated files, already waived on
dev). Removing the AFFECT001 waiver on `ProcessGuardError` did not
reintroduce the finding it used to suppress -- confirming the doc is
now actually in sync, not just waived into silence.

### Evidence / test node ids

No new test needed -- this is a pure docs-drift fix with no new public
symbol to cover (the doc snippet gained lines, the source lost a
waiver comment; no code behavior changed). Bound the real docs/
stale-doc drift gate's own test suite as evidence per the coordinator's
brief ("evidence via the docs gate test"):
...
All 4 pass unchanged (`collected=4 failed=0`) -- this is the exact
mechanism (`affect_drift_gate`/AFFECT001) the drift-check above ran for
real against `docs/modules/process.md` and `ProcessGuardError`.

### Commit shas (worktree branch t-3802)

- 8ec16cbed -- docs(process): document Timeout and SpawnFailed on ProcessGuardError
- 704661373 -- chore(tickets): record evidence for T-3802 (frob-generated evidence-binding commit)

### Gates run

- `ruff check` / `ruff format --check` on src/frob/process/_guard.py: clean.
- `ty check` on src/frob/process/_guard.py: clean.
- `frob check --only coverage --files docs/modules/process.md --base dev`:
  the two COV findings the run reports against process.md are both
  pre-existing and already waived (unrelated symbols, `tool_no_output_
  result` and `_arm_forkserver_helper_pdeathsig_if_requested`), not
  introduced by this change.
- `frob check --only affect_drift --files src/frob/process/_guard.py
  --files docs/modules/process.md --base dev`: clean for both touched
  files (see "Drift-check the anchor" above).

### Scope

Original declared scope (`docs/modules/process.md`) plus, added via
`frob ticket scope --add` (no conflicting lease found):
- `src/frob/process/_guard.py` (removing the now-obsolete AFFECT001
  waiver that named this exact ticket)
...
```
 ...
 uv.lock                                            |    2 +-
 2152 files changed, 128334 insertions(+), 5526 deletions(-)
```

### Evidence
- `tests/test_gates_affect_drift.py::TestAffectDriftGate::test_no_closure_is_silent` (pytest node id, verified passing when recorded)
- `tests/test_gates_affect_drift.py::TestAffectDriftGate::test_stale_dependent_doc_flagged` (pytest node id, verified passing when recorded)
- `tests/test_gates_affect_drift.py::TestAffectDriftGate::test_stale_dependent_code_flagged` (pytest node id, verified passing when recorded)
- `tests/test_gates_affect_drift.py::TestAffectDriftGate::test_clean_when_closure_also_touched` (pytest node id, verified passing when recorded)
