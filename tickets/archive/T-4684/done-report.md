...
T-4684 -- cross-worktree lease release must not gate on the LOCAL
ticket's prior state
=============================================================================
(filed by me as T-4684 while working T-4659; auto-promoted to
T-4684 during T-4634's land, per the coordinator's instruction to unblock
and work it directly -- no real blocked_by edge existed, since my
original `frob ticket new` call recorded "blocked_by: T-4659" only as
body TEXT, not a real --blocked-by edge.)

WHAT changed, per file
-----------------------
src/frob/tickets/_evidence.py
  - _sync_cross_worktree_lease(): changed the release branch from
    `elif from_state is TicketState.IN_PROGRESS: release_lease(...)` to
    an unconditional `else: release_lease(...)` (i.e. release on every
    transition that does not ENTER IN_PROGRESS). release_lease is already
    idempotent (Ok(None) no-op when no lease file exists for the ticket
    id), so this changes nothing observable for a transition that never
    held a lease -- it only fixes the case where a lease exists but the
    CALLING checkout's own local ledger view never recorded the
    IN_PROGRESS transition that created it.
  - Added frob:ticket/frob:doc/frob:tests directives and an expanded
    docstring explaining the T-4684 root cause and fix.

tests/test_ticket_leases_cross_worktree.py
  - New test
    TestCrossWorktreeLeaseVisibility::test_drop_from_a_worktree_that_never_saw_in_progress_still_releases:
    files+plans a ticket on `repo` and commits, cuts a FRESH worktree
    from that commit, starts the ticket IN_PROGRESS there (uncommitted to
    `repo`), then drops the ticket FROM `repo` -- whose own local ledger
    view is still PLANNED -- and asserts the lease file is gone. This is
    a genuine, designated BUG002 repro (see below): it fails at the
    test-only commit and passes once the fix commit is applied.
  - Added `drop_ticket`, `_lease_path`, `leases_dir` to the existing
    import blocks (ruff --fix reordered them).

docs/modules/tickets-lifecycle.md
  - New "### Cross-worktree release is keyed on the lease file, not local
    state (T-4684)" subsection under "## Cross-worktree lease
    side-channel (T-0473)": explains the pre-T-4684 `from_state` gate,
    why it under-released across worktrees, and the T-3259 incident it
    caused.

WHY
---
This is the real root cause behind the T-3259 incident I found and
documented (but could not fix in-scope) while working T-4659: T-4659's
own scope was `src/frob/tickets/_leases.py` only, and the actual bug
lives in `frob.tickets._evidence._sync_cross_worktree_lease`, which reads
`ticket.state` from the CALLING checkout's own local view before the
transition. Because `tickets.md` is per-branch (T-0473's entire premise),
a ticket started `IN_PROGRESS` in worktree A, never merged back to
`main`, has a lease file `main`'s own ledger never saw created. Dropping/
closing/requeuing that ticket FROM `main` computed `from_state` as
whatever `main`'s stale local view said (`PLANNED` in the reproduced
case) -- never `IN_PROGRESS` -- so the release branch never fired and the
lease file survived. This is EXACTLY the observed T-3259 shape: a
coordinator dropping a ticket from the primary checkout that was worked
in a dispatched agent worktree.

Fix: stop asking "did the LOCAL view see IN_PROGRESS a moment ago" and
instead always attempt the release whenever the ticket is not entering
IN_PROGRESS. `release_lease` was already written to be a safe idempotent
no-op on a missing file, so this has zero cost/behavior change for the
overwhelming common case (no lease held) and closes the gap for the
cross-worktree case.

Positive control (BUG002-designated repro)
--------------------------------------------
Split into two commits specifically so a real pre-fix comparison ref
exists (T-2021's technique, since a squash-on-land repo makes post-land
repro verification against main impossible by construction, T-2025):
  1. 0628d450d -- test(tickets): repro for cross-worktree lease release
     gating bug (T-4684) -- the new test ALONE, confirmed by hand to fail
     against the pre-fix _evidence.py (ran `git checkout --
     src/frob/tickets/_evidence.py` to strip the uncommitted fix, re-ran
     the test, watched it fail with `assert not True` on the lease path,
     then reapplied the fix from a saved patch).
  2. 572b3a748 -- fix(tickets): release cross-worktree lease on lease
     existence, not local state (T-4684) -- the fix alone.

`frob ticket evidence T-4684 --designate-repro <node> --base-ref 0628d450d`
then `--check-repro` both report FAILED_AT_PARENT (a genuine repro, not
UNRESOLVED) -- BUG002 satisfied.

Test node ids bound as evidence
---------------------------------
- tests/test_ticket_leases_cross_worktree.py::TestCrossWorktreeLeaseVisibility::test_drop_from_a_worktree_that_never_saw_in_progress_still_releases
  (the designated repro/positive control)
- tests/test_ticket_leases_cross_worktree.py::TestCrossWorktreeLeaseVisibility::test_release_on_close_removes_the_lease
  (regression guard: the pre-existing same-worktree close path still
  releases correctly under the new unconditional-else logic)

Regression sweep (not bound as evidence, run to confirm no breakage)
-----------------------------------------------------------------------
PYTHONPATH=<worktree>/src python -m pytest \
  tests/test_ticket_leases_cross_worktree.py tests/test_ticket_leases.py \
  -p no:cacheprovider -q
=> SUITE-RESULT: exitstatus=0 collected=202 failed=0

## Pre-READY checks

`frob check --only sys --files src/frob/tickets/_evidence.py --files tests/test_ticket_leases_cross_worktree.py --files docs/modules/tickets-lifecycle.md --base dev`:
  gate-summary: '7 errors, 557 warnings, 0 unresolved, 5 waived
  [drift=0.22s, sys=79.77s]'. Every hit against _evidence.py is a
  pre-existing, already-waived DRIFT001/WAIVE010 finding on lines/symbols
  I did not touch (grepped for _evidence.py/test_ticket_leases_cross_worktree.py/
  tickets-lifecycle.md across every non-DOCARCH001 line: only pre-existing
  waived DRIFT001 on _done_transition_structural_guard/add_cmd_evidence,
  neither of which this diff touches).

`frob check --only arch --files src/frob/tickets/_evidence.py --files tests/test_ticket_leases_cross_worktree.py --base dev`:
  pass  frob-arch  19 warnings (36 waived), 546 suggestions. The
  god-module/large-file/high-coupling findings on _evidence.py are
  pre-existing (2804-line file already over threshold before this
  26-line diff); no new ARCH001/LARGE001.

`frob check --only coverage --files src/frob/tickets/_evidence.py --files tests/test_ticket_leases_cross_worktree.py --base dev`:
  tool summary: '22 errors, 685 warnings, 0 unresolved, 288 waived
  [coverage=10.90s, drift=0.11s]'. Zero COV002 hits anywhere in the run
  (grepped). `_sync_cross_worktree_lease` is a pre-existing private
  symbol, unaffected.

`ruff check src/frob/tickets/_evidence.py tests/test_ticket_leases_cross_worktree.py`:
  All checks passed! (one import-order fix applied via `ruff check --fix`
  to the test file's new imports)

`ruff format --check src/frob/tickets/_evidence.py tests/test_ticket_leases_cross_worktree.py`:
  2 files already formatted.

`ty check src/frob/tickets/_evidence.py tests/test_ticket_leases_cross_worktree.py`:
  All checks passed!

Cross-ticket scope note
------------------------
`git diff --name-only 309e6ed63...HEAD` (309e6ed63 is the worktree's
base) touches: src/frob/tickets/_evidence.py,
tests/test_ticket_leases_cross_worktree.py, docs/modules/tickets-lifecycle.md,
tickets/T-4684/ticket.md -- all inside this ticket's own lease
(.git/frob-leases/T-4684.json), matching its declared scope exactly.

Commits (in order)
-------------------
264305390 -- chore(tickets): record T-4684 start transition
0628d450d -- test(tickets): repro for cross-worktree lease release gating bug (T-4684)
572b3a748 -- fix(tickets): release cross-worktree lease on lease existence, not local state (T-4684)
8f7c02ad3, a6dac46ab -- chore(tickets): record evidence for T-4684
                        (auto-committed by `frob ticket evidence`,
                        AFTER the fix commit, per close-dance ordering)

HEAD: a6dac46ab8f8c472d96fcb169662edbb891b926e
...
```
 ...
 docs/modules/tickets-lifecycle.md                  |  27 +
 ...
 src/frob/tickets/_evidence.py                      |  31 +-
 ...
 tests/test_ticket_leases_cross_worktree.py         |  45 +
 ...
 tickets/T-4962/ticket.md                 |  34 +
 tickets/T-4965/ticket.md                 |  53 ++
 tickets/T-4972/ticket.md                 |  35 +
 ...
 uv.lock                                            |   2 +-
 747 files changed, 69052 insertions(+), 2263 deletions(-)
```

### Evidence
- `tests/test_ticket_leases_cross_worktree.py::TestCrossWorktreeLeaseVisibility::test_drop_from_a_worktree_that_never_saw_in_progress_still_releases` (pytest node id, verified passing when recorded)
