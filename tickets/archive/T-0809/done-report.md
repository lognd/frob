...
- `tests/unit/arch_suite/test_misc.py::TestProtocolSummaryEngine::test_leaf_resource_declarations_populate_acquired_released_escaped` (pytest node id, verified passing when recorded)
- `tests/unit/arch_suite/test_misc.py::TestProtocolSummaryEngine::test_resource_sets_join_transitively_through_a_caller` (pytest node id, verified passing when recorded)
- `tests/unit/arch_suite/test_misc.py::TestProtocolSummaryEngine::test_resource_sets_join_across_a_recursive_cluster` (pytest node id, verified passing when recorded)
