...
land_commit: null
---
The deferred post-land unscoped sweep (T-1684) for T-3985 at commit 79200c119c8d2c233a845cb8cf57120871f85496 found 1 new (rule, file) identit(ies) that were not present in the previous sweep's baseline.

T-1935: this is a count of DISTINCT (rule, file) IDENTITIES (1), not a raw finding count -- every finding sharing a (rule, file) pair collapses into ONE identity here (deliberately, so attribution and quarantine reason about "which files went red", not individual diagnostics). An independent re-measurement found 9 actual finding(s) across those 1 identit(ies).

New (rule, file) identit(ies) filed here:

- missing-argument  tests/unit/test_check_gates_summary.py
