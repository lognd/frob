...
# T-4718 -- why-file (READY)

Worktree: /home/logan/projects/frob/.claude/worktrees/t-4718 (branch t-4718)
HEAD: 8d911e0674816625383c42bbd280eecf817c38a8

## What changed (source files, comment-only)

20 files, all 20 in scope handled (18 files edited, 2 files -- _collect_kotlin.py
and _evasion_coverage.py -- inspected and found already under DOCARCH002's
12-line cap once frob:* directive lines and their `\`-continuations are
correctly excluded from the count; no changes needed there):

- src/frob/testing/_coverage_refresh.py (2 runs) -> T-3275, T-1677, T-2087
- src/frob/testing/_coverage_wait.py (1 run) -> T-1095, T-1126, T-1516
- src/frob/verify/_worker.py (3 runs) -> T-0907, T-1523, T-1694, T-2324,
  T-3222, T-3464, T-1688, T-1703, T-3886
- src/frob/vet/_capability_c.py (2 runs) -> T-0328, T-0339, T-0377, T-0378,
  T-0379
- src/frob/vet/_capability_core.py (5 runs) -> T-1223, T-2885, T-0414,
  T-1210, T-0244, T-0169, T-2358
- src/frob/vet/_capability_csharp.py (1 run) -> T-1600, T-4536
- src/frob/vet/_capability_kotlin.py (1 run) -> T-0328, T-0377, T-0378,
  T-0379, T-0662, T-0663, T-0664, T-0723
- src/frob/vet/_capability_modes.py (1 run) -> T-1075
- src/frob/vet/_capability_python.py (1 run) -> T-0209, T-0328, T-0378,
  T-1626
- src/frob/vet/_capability_registry/_dangerous_ops_python.py (2 runs) ->
  T-2457, T-2464, T-2479
- src/frob/vet/_capability_registry/_kinds.py (2 runs) -> T-2464, T-2906,
  T-3492, T-3493
- src/frob/vet/_capability_registry/_unity_api.py (1 run) -> T-0771,
  T-4514, T-4554
- src/frob/vet/_capability_rust.py (1 run) -> T-0328, T-0339, T-0377,
  T-0378, T-0661
- src/frob/vet/_capability_scan.py (6 runs) -> T-0151, T-0153, T-0158,
  T-0201, T-0253, T-0910, T-0729, T-0380, T-1659
- src/frob/vet/_capability_typescript_bindtable.py (1 run) -> T-0328,
  T-0337, T-0377, T-0378, T-0432, T-0660
- src/frob/vet/_obfuscation.py (2 runs) -> T-0208
- src/frob/vet/_scan.py (1 run) -> T-0794, T-3708
- src/frob/vet/_capability_registry/_opaque.py -- inspected, its one
  47-line run is entirely frob:doc/frob:enforces/frob:waive directive
  lines; already clean, no edit.

One follow-up commit: the comment condensing on _capability_scan.py left
`_SELF_PATTERN_SUFFIXES` (a changed constant) with only a `frob:ticket
T-0910` edge, and T-0910 is `[done]` -- COV002 requires an edge to an
OPEN ticket. Added `# frob:ticket T-4718` alongside it; re-checked
COV002 clears.

## Why each split was made the way it was

Same doctrine as T-4709/T-4770: the text that stays in code is the
load-bearing invariant/gotcha a reader modifying or reusing this code
needs (a resolution rule, a scope-shadowing discipline, a fail-closed
default, a perf-critical algorithmic property); the text that moved is
historical framing (measurement numbers, prior-round-rejected designs,
cross-reference chains to landed work, incident narratives, per-shape
ticket attribution). Full per-block reasoning is in each commit message
and in the condensed append text now living in the cited tickets'
bodies (each begins "T-4718 sweep (condensed from <file>:<lines>,
trimmed for DOCARCH002's 12-line cap)"). No no-ticket blocks existed in
this cluster's files -- acceptance criterion 3 is proven vacuously true
by verify_t4718_no_ticket_blocks.py, not skipped (every over-length
block in this cluster's files cited at least one ticket, confirmed by
the directive-aware scan run before any edits).

The two biggest blocks (_capability_scan.py's self-match-exclusion
evolution history spanning T-0151/T-0153/T-0158/T-0201/T-0253, and
_capability_typescript_bindtable.py's 130-line resolver docstring) each
needed multiple rounds of trimming to actually clear 12 lines --
condensing narrative-heavy prose to the essential mechanism cost more
lines than expected on the first pass; both were re-measured and
re-trimmed until the directive-aware scan reported zero.

## A real bug this session found and fixed in itself

The manual splice script computed a removed range as `lines[start-1:end]`
with `end` set to the LAST LINE OF THE COMMENT BLOCK -- but three times
I set `end` to the line number of a `frob:ticket`/`frob:doc` directive
that immediately FOLLOWED the block instead of the block's own last
content line, which silently deleted that directive (Python slicing
`lines[end:]` starts one line AFTER `end`, not at it). This tripped a
directive-count audit I ran across the whole scope (before/after count
of every `frob:(doc|enforces|tests|waive|invariant|ticket)` line,
comparing dev to the worktree) -- it is exactly the T-draft-cd9fd605
hazard the brief warned about, just triggered by a hand-rolled splice
instead of the `frob narrative move` tool's whole-run default. Found
and fixed 4 dropped directives (T-1694 in _worker.py, T-1223 in
_capability_core.py, `frob:doc docs/modules/vet.md#public-api` x2 and
`frob:ticket T-0910` in _capability_scan.py) across two follow-up
commits, then re-ran the audit clean.

## Move-never-delete / idempotency

No `frob narrative move` calls were used (same reasoning as T-4770: the
tool's `--keep-file` requires keep-lines to be an exact verbatim SUBSET
of the block's own original lines, which cannot express a rewritten/
condensed keep-block -- every block here needed genuine rewording to
fit 12 lines, not a literal line-subset). All edits were manual:
condense the in-code comment to a short paraphrase, append the FULL
VERBATIM original block text to every ticket the block cited (directive
`frob:ticket` lines AND prose-mentioned ticket ids both honored, matching
T-4770's convention), via `frob ticket body --append-file` from inside
the worktree. `frob ticket list` was run after multiple batches and
after the final state and exited 0 every time (also acceptance [5]'s
bound evidence).

## Pre-READY checks

`frob check --only sys --files <19 touched files> --base dev`:
  ...
  ticket's scope (same T-3799 frob.lock lease conflict as the T-4770
  cluster hit, and the T-4335/T-2521 AFFECT001 waiver on
  _rapid_sweep.py -- neither touched here). Zero SELFAUDIT001 anywhere
  in the run attributable to my files.

`frob check --only arch --files <19 touched files> --base dev`:
  pass frob-arch: 21 warnings (36 waived), 545 suggestions. No ARCH001,
  no LARGE001 on any touched file (grepped explicitly for both rule ids
  against every touched path -- zero hits).

`frob check --only coverage --files <19 touched files> --base dev`
  (run twice -- before and after the COV002 fix commit):
  First run: 1 real COV002 finding on a file I touched
  (_capability_scan.py::_SELF_PATTERN_SUFFIXES, changed with its only
  frob:ticket edge pointing at a done ticket). Fixed by adding a
  `# frob:ticket T-4718` directive. Second run (scoped to
  _capability_scan.py alone): zero COV002 findings. Remaining gate:COV
  errors in the full-scope run (COV001/COV006/COV007) are all on files
  outside this ticket's scope, pre-existing.

`ruff check <20 touched files>`: All checks passed!
`ruff format --check <20 touched files>`: 20 files already formatted.
`/home/logan/projects/frob/.venv/bin/ty check <20 touched files>`: All checks passed!

## Land dry-run

`cd /home/logan/projects/frob/.claude/worktrees/t-4718 && nice -n 10
/home/logan/projects/frob/.venv/bin/frob ticket land T-4718 --dry-run
--worktree /home/logan/projects/frob/.claude/worktrees/t-4718`:

Ran clean through fmt/Tier-A-fix preview, RENDER001/malformed-directive
warnings on files this ticket never touches (src/frob/check/__init__.py,
tests/test_app.py -- pre-existing, unrelated), profile=rapid detection,
and the 37-file rapid --files scope resolution (18 touched + 19
caller-dependent). It then reached the ONE ERROR this leaf cannot clear
itself, by the brief's own division of labor:

  ERROR: land: T-4718 cannot land -- missing evidence or a Done report;
  record evidence (...) and add a '## Done report' section under
  T-4718's entry in tickets.md, then retry `frob ticket land T-4718`

All 5 acceptance criteria already have bound evidence (`frob ticket show
T-4718` lists all five as `bound([...])`, not `UNBOUND`) -- the
remaining half of this refusal is specifically the missing '## Done
report' section, which this session's own standing brief explicitly
tells implementers NOT to write ("DO NOT run `frob ticket done-report`
or `close`... write the done-report body to
scratchpad/why-<id>.txt... Final message: READY"). This is therefore
the expected, structural stopping point for an implementer-stage dry
run on a docs-kind ticket in this workflow, not a defect in the sweep:
the coordinator adds the Done report and re-runs land for real. I am
flagging the exact ERROR line rather than working around the standing
prohibition.

## Cross-ticket lease check

`git diff --name-only dev...HEAD -- src docs tests` (source files only,
excluding this session's own `tickets/` ledger-mirror commits): all 19
edited files plus the untouched-but-in-scope _opaque.py and
_evasion_coverage.py/_collect_kotlin.py are inside T-4718's own lease
(`.git/frob-leases/T-4718.json`). No file is leased by another
in-progress ticket.

## Merge with dev

dev moved substantially during this session (117+ commits, mostly other
agents' ticket-ledger mirrors and sprints); merged with `git merge dev
--no-edit` once, no conflicts.

## Acceptance evidence (bound via --evidence-cmd, docs-kind rules)

- [1] zero runs >12 lines: verify_t4718_runs.py (directive-aware,
  handling `\`-continuations on frob:tests/frob:waive blocks) -- exit=0,
  CLEAN across all 20 files
- [2] ticket-cited narrative readable in cited ticket bodies:
  verify_t4718_narrative_moved.py checks all 60 cited tickets for a
  "T-4718" mention -- exit=0, CLEAN (all 60 present)
- [3] no-ticket blocks in cluster ticket body: verify_t4718_no_ticket_
  blocks.py -- exit=0, CLEAN (zero existed, proven not assumed, by the
  pre-edit directive-aware scan recording a ticket list for every
  offending block)
- [4] git diff -w non-comment-empty: verify_t4718_nonbehavior.sh --
  exit=0, CLEAN
- [5] frob ticket list exits 0: bound directly as the evidence command
  -- exit=0

All five bound via `frob ticket evidence T-4718 --evidence-cmd ... --accepts N
--base-ref dev`, after the last source-edit commit (the COV002 fix) -- no
commit follows the evidence binding.
...
```
 ...
 src/frob/testing/_coverage_wait.py                 |   35 +-
 ...
 tickets/T-4953/done-report.md                      | 2509 +++++++++++++++++++
 ...
 uv.lock                                            |    2 +-
 2437 files changed, 180086 insertions(+), 12057 deletions(-)
```

### Evidence
- `cmd:python3 /tmp/claude-1000/-home-logan-projects-frob/a42444f0-d505-4990-88ee-806296379a83/scratchpad/verify_t4718_runs.py /home/logan/projects/frob/.claude/worktrees/t-4718 exit=0 sha256=637994b743fb` (cmd evidence, exit=0)
- `cmd:python3 /tmp/claude-1000/-home-logan-projects-frob/a42444f0-d505-4990-88ee-806296379a83/scratchpad/verify_t4718_narrative_moved.py exit=0 sha256=8f99d52c0d4f` (cmd evidence, exit=0)
- `cmd:python3 /tmp/claude-1000/-home-logan-projects-frob/a42444f0-d505-4990-88ee-806296379a83/scratchpad/verify_t4718_no_ticket_blocks.py exit=0 sha256=5b40d073e950` (cmd evidence, exit=0)
- `cmd:bash /tmp/claude-1000/-home-logan-projects-frob/a42444f0-d505-4990-88ee-806296379a83/scratchpad/verify_t4718_nonbehavior.sh /home/logan/projects/frob/.claude/worktrees/t-4718 exit=0 sha256=c591e06b73c8` (cmd evidence, exit=0)
- `cmd:/home/logan/projects/frob/.venv/bin/frob ticket list exit=0 sha256=c9fa8659c073` (cmd evidence, exit=0)
