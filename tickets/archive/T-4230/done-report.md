...
T-4230 land repair -- entrypoint_coverage_violations ARCH001 split

WHAT changed:
- src/frob/gates/_coverage.py: `entrypoint_coverage_violations` had grown
  to 98 lines (parsing coverage.xml, joining classes, and per-module
  verdict logic all inline), past ARCH001's threshold. Split into:
  - `_entrypoint_coverage_hits(root, snapshot)` -> `(known_paths,
    hits_by_class_line) | None`: loads/parses coverage.xml via the
    module's existing private `_load_coverage_xml`/`_parse_classes`,
    returns `None` on missing/malformed input (same silent-on-
    unmeasured contract as before).
  - `_entrypoint_violation_for_module(root, rel_path, hits_by_class_line)`
    -> `Violation | None`: per-module COV010 verdict (guard-range lookup
    via `_main_guard_line_range`, hit-range check, Violation
    construction) -- identical logic, extracted verbatim.
  `entrypoint_coverage_violations` itself is now a thin loop calling
  both helpers; the `# frob:doc .../gate-time...`/`# frob:tests`/
  `# frob:todo` directive block above it (with its `# noqa: E501`
  markers) is untouched, kept exactly where it was.
  Both new helpers carry `# frob:ticket T-4230` (added after the first
  `--only coverage` run flagged COV002 on them).
- Merged dev (clean, no conflicts -- dev's changes were new ticket dirs
  unrelated to this file).

WHY: land was refused by ARCH001 on
src/frob/gates/_coverage.py::entrypoint_coverage_violations (98 lines,
long-AND-complex). Splitting the coverage.xml-parsing phase from the
per-module verdict phase is the natural decomposition (the function's
own two-phase shape: gather hits, then loop modules).

Proof:
- `awk` line-count check on the refactored function body: 45 lines
  (was 98), well under threshold.
- PYTHONPATH=.../src python -m pytest
  tests/gates_suite/test_coverage.py::TestEntrypointCoverage -q
  SUITE-RESULT: exitstatus=0 collected=5 failed=0
- `frob check --only arch --only coverage --files
  src/frob/gates/_coverage.py --base dev`: zero ARCH001/LARGE001 on this
  file (`frob-arch` reports only pre-existing god-module/large-file/
  abstraction-opportunity findings on the whole 1500+-line module, none
  attributable to the split function, and `pass frob-arch`). The two
  COV002 findings my split introduced (missing frob:ticket on the new
  helpers) were fixed in a follow-up commit and no longer appear.
  Two findings on this file remain and are PRE-EXISTING, unrelated to
  this split, and out of this ticket's scope (scope is
  src/frob/gates/_coverage.py only, not docs/modules/gates.md or the
  ticket ledger):
  - COV001 on `entrypoint_coverage_violations` itself: the existing
    `# frob:doc docs/modules/gates.md#cov010-entrypoint-coverage-t-4230`
    anchor line was already present, byte-identical, before this repair
    (confirmed via `git show abdf29d95:src/frob/gates/_coverage.py`,
    the pre-refactor commit) -- docs/modules/gates.md's actual heading
    slug does not match this hand-written anchor fragment. A doc-anchor
    fix belongs to a ticket scoped to docs/modules/gates.md, not this
    one.
  - TODO002 on the pre-existing `# frob:todo T-draft-50806633` directive:
    that draft ticket id does not resolve (`frob ticket show
    T-draft-50806633` -> no ticket), also unchanged since before this
    repair. Rebinding it needs either filing/finding the real successor
    ticket, out of this ARCH001-only repair's scope.
- ruff check / ruff format --check / ty check src/frob/gates/_coverage.py:
  all pass (ruff format needed one reformat after the split, applied and
  reverified clean).

Commits:
- (merge) Merge branch 'dev' into t-4230
- 5cc63f72f refactor(gates): split entrypoint_coverage_violations under
  ARCH001
- f37f7afcb fix(gates): add frob:ticket to new entrypoint-coverage
  helpers (HEAD)

## Pre-READY checks

frob check --only arch --only coverage --files src/frob/gates/_coverage.py --base dev:
  pass frob-arch (19 warnings, 36 waived, 542 suggestions -- zero
  ARCH001/LARGE001 attributable to entrypoint_coverage_violations or its
  two new helpers). FAIL gate:COV (13 errors -- the two pre-existing
  COV001/TODO002-adjacent findings above plus other files' pre-existing
  COV006 waived findings, none new from this split after the frob:ticket
  fix). FAIL gate:DRIFT (5, pre-existing/waived, other files). FAIL
  gate:DSL (1, tests/test_app.py:387, pre-existing, unrelated file).
  FAIL gate:TODO (5 errors, includes the pre-existing T-draft-50806633
  dangling reference noted above, unchanged by this repair). pass
  gate:DOCARCH, gate:PLACE, gate:PROFILE, gate:WAIVE, claude-config-drift.

ruff check src/frob/gates/_coverage.py: All checks passed!
ruff format --check src/frob/gates/_coverage.py: passed after one
  reformat (applied, then reverified clean)
...
```
 ...
 .../registry/capability-via-ratchet.lock.json      |  82 ++-
 ...
 tests/gates_suite/test_coverage.py                 | 123 ++++
 ...
 tickets/T-4230/done-report.md                      | 671 +++++++++++++++++
 ...
 uv.lock                                            |   2 +-
 615 files changed, 50199 insertions(+), 1971 deletions(-)
```

### Evidence
- `tests/gates_suite/test_coverage.py::TestEntrypointCoverage::test_uncovered_guard_fires_cov010` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_coverage.py::TestEntrypointCoverage::test_covered_guard_is_silent` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_coverage.py::TestEntrypointCoverage::test_module_with_no_guard_is_silent` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_coverage.py::TestEntrypointCoverage::test_missing_coverage_xml_is_silent` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_coverage.py::TestEntrypointCoverage::test_guard_detection_is_structural_not_lexical` (pytest node id, verified passing when recorded)
