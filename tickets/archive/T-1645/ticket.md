...
evidence_changes:
- old_node: tests/unit/test_app_runners_batch7.py::TestTicketStart::test_start_warns_on_over_broad_scope
  new_node: tests/unit/test_app_runners_batch7.py::TestTicketStart::test_start_over_broad_scope_ack_bypasses_refusal
  reason: T-1866 renamed test_start_warns_on_over_broad_scope to test_start_refuses_over_broad_scope
    because the behavior it exercises changed from warn-only to a hard refusal (start
    now refuses a mega-glob scope instead of merely warning). Rebinding T-1645s own
    evidence citation to the still-existing test that covers the same _lifecycle.py
    code path (the now-warn-only-for-acked-tickets _warn_scope_breadth_on_start),
    so T-1645 stays evidenced against real code rather than a deleted test name.
  actor: logan
  at: '2026-08-08'
threat: null
component: null
