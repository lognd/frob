...
body_changes:
- mode: append
  reason: 'T-4770: preserve false-positive measurement and follow-up plan trimmed
    from _wire.py'
  actor: logan
  at: '2026-09-19'
  old_length: 2761
...
T-4770 follow-up (condensed from _WIRE003_SCAN_GLOBS's docstring in
src/frob/gates/_wire.py, trimmed for DOCARCH002's 12-line cap): the
"wider scope" ask was T-1725's own, measured and found too imprecise
for ERROR-severity enforcement right now. False positives came from
ordinary doc prose mentioning "frob" near unrelated backtick-quoted
vocabulary (priority levels, board columns, config keys) -- see
docs/audits/wire003-repo-scan.md for the measured count this ticket's
Done report cites. The follow-up widening approach: a per-token
allowlist, or requiring `uv run frob`/`frob <verb>` as a stricter
anchor.