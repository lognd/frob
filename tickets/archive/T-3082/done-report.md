...
T-3082: quarantine.json persists on disk after clear; a stale cleared
record is byte-identical to a live one

WHAT changed (worktree /home/logan/projects/frob/.claude/worktrees/t-3082,
HEAD a72fc6835728903224861cb70cba440cd81022b4):

1. src/frob/verify/_quarantine.py
   - New `_QUARANTINE_STATUS_REL = ".frob/quarantine.status"` constant
     and `_STATUS_RAISED`/`_STATUS_CLEARED` string constants.
   - New private `_quarantine_status_path(root)` and
     `_write_quarantine_status(root, *, raised)`: writes a SECOND,
     trivial companion file containing exactly the bare word "raised"
     or "cleared" (plus newline) -- never JSON.
   - New public `quarantine_status_marker(root) -> str | None`: reads
     that marker back, `None` if it has never been written (matching
     `load_quarantine`'s own `Ok(None)`-for-fresh-store convention).
   - Wired `_write_quarantine_status` into all three of this module's
     mutation call sites, inside the same `_quarantine_lock_path`
     critical section as `_save_quarantine`, immediately after it:
     `raise_quarantine` (raised=True), `clear_quarantine` (raised=False),
     and `retire_unidentifiable_findings`'s own full-clear path
     (raised=False) -- its partial-retire path (a well-formed sibling
     still undisposed) deliberately does NOT touch the marker, since
     quarantine is still raised in that case.

2. tests/unit/verify/test_quarantine.py
   New `TestQuarantineStatusMarker` class (5 tests): never-raised is
   `None`, raised is "raised", cleared (via `clear_quarantine`) is
   "cleared" while `.frob/quarantine.json` itself keeps its full cleared
   record, cleared via `retire_unidentifiable_findings`'s full-clear
   path is also "cleared", and a partial retire (well-formed sibling
   still undisposed) leaves the marker unaffected.

3. src/frob/app/verify_runner.py
   - New `VerifyStatus.quarantine_status_marker: str | None` field.
   - `build_status` now calls `quarantine_status_marker(root)` and
     populates it, alongside the existing `quarantine_raised` (both
     come from the same call, never two independently-computed
     answers).
   - `_print_status_human`'s "clear" branch now names the marker
     explicitly (`"quarantine:       clear (cleared)"` vs. plain
     `"quarantine:       clear"` when never raised) -- this is the
     literal fix for "`frob verify status` cannot mistake stale for
     live": the CLI's own human-facing report now surfaces the same
     tombstone a direct disk read would show, rather than requiring an
     operator to independently `cat .frob/quarantine.json` or trust
     `quarantine_raised` alone.

4. tests/unit/verify/test_verify_runner.py
   Two new `TestBuildStatus` cases: `quarantine_status_marker` is
   `None` when nothing has ever been raised, and reflects "raised" then
   "cleared" through a real raise/clear cycle, cross-checked against
   `quarantine_raised` at each step.

WHY (design decision, per the ticket's own instruction to say why):
Chose option (b) from the ticket text -- "a companion
.frob/quarantine.status file" -- over option (a) (delete/move
`.frob/quarantine.json` on clear). `clear_quarantine`'s own docstring
and this module's header docstring both state the cleared record is
KEPT on disk as an audit trail, and dozens of existing tests
(`tests/unit/rapid_sweep_suite/test_dispose.py`,
`tests/unit/verify/test_verify_runner.py`'s own `TestDispose` cases)
assert `load_quarantine` returns the full cleared record (with
`cleared_at`/`cleared_reason`/`cleared_by` populated) immediately after
`clear_quarantine` -- a tested contract multiple call sites depend on,
per the ticket's own explicit warning not to regress it. Rather than
changing `.frob/quarantine.json`'s shape (which would require touching
every one of those callers/tests, well outside a single-file-scoped
bugfix), this adds a second, deliberately trivial file next to it that
answers "raised or cleared?" in one word -- the literal human-facing
footgun this ticket exists to fix (the incident: "read the file,
believed quarantine was raised, spent four failed disposal commands
before discovering it had already been cleared") is fixed by giving that
same direct-disk-read path a file that cannot lie by shape, without
touching the tested single-file persistence contract at all. The
coordinator's specific ask ("so `frob verify status` cannot mistake
stale for live") is additionally addressed by wiring the same marker
into the CLI's own status output (item 3 above) -- `frob verify status`
was already programmatically correct (`build_status`'s own `raised =
record is not None and record.cleared_at is None` never mistook a
cleared record for live), so this closes the human-readability gap in
its printed report too, not just on raw disk.

Acceptance criteria (added via `frob ticket accept`, none existed
before this ticket was worked):
  [1] GIVEN a raised quarantine is cleared (via clear_quarantine or
      retire_unidentifiable_findings) WHEN a caller reads
      .frob/quarantine.status directly THEN it reads 'cleared', never
      the byte-identical-in-shape stale record misread as live; the
      marker reads 'raised' only while genuinely raised.
      Evidence:
  ...
  [2] GIVEN frob verify status is run WHEN quarantine has been raised
      and then cleared THEN its output surfaces the same raised/cleared
      tombstone marker (quarantine_status_marker) that a direct disk
      read would show, never leaving the CLI's own report as the only
      place still ambiguous between stale and live.
      Evidence:
...
PROOF -- all 7 evidence node ids pass after merging dev twice (once
before adding the verify_runner.py wiring, once after T-4554 landed on
dev mid-session):

  $ PYTHONPATH=$(pwd)/src /home/logan/projects/frob/.venv/bin/python -m pytest \
      tests/unit/verify/test_verify_runner.py tests/unit/verify/test_quarantine.py \
      -p no:cacheprovider -q
  ................................................................         [100%]
  SUITE-RESULT: exitstatus=0 collected=64 failed=0

Also re-ran the pre-existing `TestDispose` cases that assert
`load_quarantine`'s post-clear cleared-record contract, to confirm no
regression:

  $ PYTHONPATH=$(pwd)/src /home/logan/projects/frob/.venv/bin/python -m pytest \
      tests/unit/verify/test_verify_runner.py tests/unit/rapid_sweep_suite/test_dispose.py \
      -p no:cacheprovider -q -k "quarantine or Quarantine"
  ....                                                                     [100%]
  SUITE-RESULT: exitstatus=0 collected=4 failed=0

Full-file run (88 tests across the two verify test files plus
test_dispose.py) also green.

Merge: dev merged clean into t-3082 TWICE during this session (no
conflicts requiring dev-side resolution either time) -- HEAD now
carries dev through T-4554's own land (0d4a05c42) plus everything
after.

Gates: `frob check --only gates --ticket T-3082 --files
src/frob/verify/_quarantine.py --files tests/unit/verify/test_quarantine.py
--files src/frob/app/verify_runner.py --files
tests/unit/verify/test_verify_runner.py --base dev`, run from inside the
worktree with the venv binary, re-run after every scope/code change
until clean. `frob ticket sweep T-3082` re-run before each gate check
(pre-work sweep goes stale on every file edit); gate:PRE and gate:SCOPE
both report 0 errors on the final run. Scope was widened twice beyond
the ticket's original single-file declaration, both via `frob ticket
scope --add` (no lease collisions):
  - tests/unit/verify/test_quarantine.py (regression tests for the new
    marker)
  - src/frob/app/verify_runner.py + tests/unit/verify/test_verify_runner.py
    (wiring the marker into `frob verify status`, per the coordinator's
    specific ask; this also fixed an initial gate:WIRE001 finding on
    `quarantine_status_marker` having no caller outside its own tests)
Per the tool's own scope-note, only gate:SCOPE/gate:PREWORK and the
diff-driven COV002/TODO001/FMT/AFFECT checks are ticket-scoped; every
other gate family's counts are repo-wide. Grepping the final gate run
for all 4 touched files specifically: zero COV001/COV002/WIRE001/WIRE002
findings remain on any of them (fixed two real findings along the way:
a WIRE001 on the initially-unwired marker, fixed by wiring it into
`build_status`/CLI output rather than waiving; and two COV002s on the
new/changed `TestBuildStatus` test methods missing a `frob:ticket
T-3082` edge, fixed by adding the directive). Every remaining FAIL gate
in the run (ARCH, COV, CROSSTICKET, DOC, DRIFT, DSL, LANG, PERF, TICK,
TODO, WIRE) locates its errors in files this ticket never touched (e.g.
`src/frob/tickets/_leases.py`'s COV001, `src/frob/dup/_legacy_cs.py`'s
WIRE002) -- pre-existing repo state.

Evidence: bound via `frob ticket evidence T-3082 <node> --accepts <N>
--base-ref dev` for all 7 node ids listed above, across 2 acceptance
criteria (both newly added via `frob ticket accept` -- the ticket had
none before this work started).

Commits (t-3082, chronological, ticket-ledger noise omitted):
  96b686055 fix(verify): tombstone quarantine.status on clear so stale never reads as live
  0f2a88b2f Merge branch 'dev' into t-3082
  5e244b896 feat(verify): surface the quarantine tombstone marker in frob verify status
  4d11e76a7 chore(verify): bind frob:tests directives for the quarantine status marker
  4a1a94045 chore(tests): bind frob:ticket edges for the new TestBuildStatus cases
  a72fc6835 Merge branch 'dev' into t-3082  <- HEAD

Filed: none (no out-of-scope work discovered; the two scope widenings
above went through `frob ticket scope --add`, not silent expansion).
...
```
 ...
 uv.lock                                            |    2 +-
 2134 files changed, 116272 insertions(+), 5495 deletions(-)
```

### Evidence
- `tests/unit/verify/test_quarantine.py::TestQuarantineStatusMarker::test_none_when_never_raised` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_quarantine.py::TestQuarantineStatusMarker::test_raised_after_raise` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_quarantine.py::TestQuarantineStatusMarker::test_cleared_after_clear` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_quarantine.py::TestQuarantineStatusMarker::test_cleared_after_retire_unidentifiable_findings` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_quarantine.py::TestQuarantineStatusMarker::test_stays_raised_when_retire_leaves_a_sibling_undisposed` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_verify_runner.py::TestBuildStatus::test_quarantine_status_marker_none_when_never_raised` (pytest node id, verified passing when recorded)
- `tests/unit/verify/test_verify_runner.py::TestBuildStatus::test_quarantine_status_marker_reflects_raise_and_clear` (pytest node id, verified passing when recorded)
