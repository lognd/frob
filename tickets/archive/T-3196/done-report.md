...
DRIFT001 src/frob/gates/__init__.py -- LIVE (not stale): fires on coverage_gate and run_gates (a different symbol pair than the ticket's own attribution chain, but the same (rule, file) identity per this sweep family's own file-level collapse rule). Verified both symbols still exist at their documented location/role in docs/modules/gates.md's Public API manifest, then bound frob:ack for both (body+sig facets). Re-run confirms gate:DRIFT dropped from 60 to 58 errors and no remaining finding on this file.

SYS003 src/frob/check/_python.py -- STALE: zero SYS003 findings anywhere in a full unscoped `frob check --only gates` run (only unrelated WAIVE004 notes about pre-existing SYS003 waivers elsewhere in the repo matching 0 findings, none touching this file). Already resolved by the time this sweep-filed ticket was read.

Evidence node id tests/test_gates.py::TestDriftGate::test_no_drift_when_clean characterizes the drift_gate/LockFile ack mechanism the fix relies on.

Filed: T-3285 (close-time disclosure check false-positives on split done-report.md -- unrelated frob tooling bug hit while closing this ticket, filed rather than worked around silently)

### Changed
```
 frob.lock                          | 40 ++++++++++++++++++++++++++++++++++++--
 tickets/T-3196/done-report.md      | 27 +++++++++++++++++++++++++
 tickets/T-3196/ticket.md           |  6 +++++-
 tickets/T-3285/ticket.md | 29 +++++++++++++++++++++++++++
 4 files changed, 99 insertions(+), 3 deletions(-)
```

### Evidence
- `tests/test_gates.py::TestDriftGate::test_no_drift_when_clean` (pytest node id, verified passing when recorded)

### Captured claims
- tests: 1 passed (from 1 evidence id(s))
- gates: 94 error(s), 4273 warning(s), 876 waived
