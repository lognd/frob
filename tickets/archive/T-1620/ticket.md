...
body_changes:
- mode: append
  reason: 'T-4709: preserve mass-invalidation-threshold rationale trimmed from _fix_engine_sync.py'
  actor: logan
  at: '2026-09-19'
  old_length: 2525
  new_length: 3246
...
T-4709 follow-up (condensed from _WAIVE004_MASS_INVALIDATION_THRESHOLD's
docstring in src/frob/gates/_fix_engine_sync.py, trimmed for
DOCARCH002's 12-line cap): a handful of a rule genuinely going stale
together (e.g. a refactor that deletes the pattern a few waivers
covered) is plausible; dozens going stale in the SAME run is not -- it
is the signature of the verification itself under-reporting, not of the
waivers. A rule with exactly 2 live `frob:waive` directives can never
reach the absolute threshold (5) no matter how degraded the run is, so
both waivers would silently pass through and get deleted, which is why
the PROPORTIONAL check exists: 2 of 2 is at least as suspicious as 40 of
40, arguably more so.