...
Root cause: src/frob/_cli_parsers/_shims.py::announce_shim (T-4690's one
deprecation-shim mechanism for every deleted/renamed CLI verb) logged
its notice via `_log.info(...)`. frob's default logging config
(src/frob/logging/config.toml, T-2979) routes INFO-level records to the
stdout handler, so every shimmed command (arch, dup, map, outline,
xref, gitlog, and more) prepended a literal
"cli shim: arch -> check --only arch (ticket=T-4690 ...)" line onto its
own stdout before any real output -- breaking json.loads() on every
--json invocation of a shimmed command. Measured directly: this was the
single root cause behind ~150 of the ~170 ubuntu CI failures in run
35717833933 (dev tip 197238c35e) -- every failing tests/system/test_cli_*.py
--json test, tests/system/test_system.py, tests/system/test_cli_scale.py,
tests/integration/test_gitlog.py all share this same JSONDecodeError
shape with the shim notice as the literal captured stdout prefix.

Fix: `_log.info` -> `_log.debug`. The human-readable deprecation notice
is already correctly delivered via the separate
`Renderer.for_stream(sys.stderr, ...)` call right below the log line;
the log call is a redundant diagnostic duplicate, not user-facing.
DEBUG-level records are excluded by the stdout handler's own
level=INFO threshold by default, only surfacing via FROB_LOG_LEVEL.

Measured: tests/unit/test_cli_shims.py (full file) + a representative
sample of the previously-failing --json tests (test_cli_arch.py,
test_cli_map.py, test_cli_outline.py, test_cli_dup.py, test_cli_xref.py,
test_cli_gitlog.py, test_cli_scale.py, test_system.py,
tests/integration/test_gitlog.py, test_cli_exports.py) -- 215 tests
total across these files, 0 failed.
...
```
 ...
 src/frob/_cli_parsers/_shims.py                    |   100 +
 ...
 .../ProjectSettings/ProjectVersion.txt             |     2 +
 tests/gates/test_comment_placement.py              |    11 +-
 ...
 tests/test_gates_milestone.py                      |    10 +
 ...
 tests/test_refactor.py                             |   374 +-
 tests/test_refs_gate.py                            |    61 +-
 tests/test_registry_corpus.py                      |     7 +-
 tests/test_registry_exhaustiveness.py              |    14 +
 ...
 tests/test_ticket_journal.py                       |     6 +
 ...
 tests/test_tickets_leases.py                       |    28 +-
 ...
 tests/unit/arch_suite/test_concurrency.py          |    55 +-
 ...
 tests/unit/arch_suite/test_type_design.py          |     9 +
 ...
 tests/unit/strata/test_access.py                   |    43 +-
 ...
 tests/unit/strata/test_circuit_breaker.py          |    28 +-
 ...
 tests/unit/strata/test_demand.py                   |    37 +-
 ...
 tests/unit/strata/test_multifile.py                |    19 +-
 tests/unit/strata/test_mutation_audit.py           |    31 +-
 tests/unit/strata/test_native_staleness.py         |    77 +-
 tests/unit/strata/test_native_test.py              |    19 +-
 tests/unit/strata/test_obligation_proof.py         |    28 +-
 ...
 tests/unit/strata/test_reliability.py              |    49 +-
 ...
 tests/unit/test_app_runners.py                     |    29 +
 ...
 tests/unit/test_cli_shims.py                       |   278 +
 ...
 tests/unit/test_land_duplicate_ticket_id.py        |     2 +
 ...
 uv.lock                                            |     2 +-
 3251 files changed, 426799 insertions(+), 16943 deletions(-)
```

### Evidence
- `tests/unit/test_cli_shims.py::TestAnnounceShim::test_never_writes_to_stdout` (pytest node id, verified passing when recorded)
- `tests/system/test_cli_arch.py::test_json_is_valid` (pytest node id, verified passing when recorded)
- `tests/system/test_cli_map.py::test_json_is_valid` (pytest node id, verified passing when recorded)
- `tests/system/test_cli_outline.py::test_json_is_valid` (pytest node id, verified passing when recorded)
- `tests/system/test_cli_dup.py::test_json_is_valid` (pytest node id, verified passing when recorded)
- `tests/system/test_cli_xref.py::test_json_is_valid` (pytest node id, verified passing when recorded)
- `tests/unit/test_cli_shims.py::TestAnnounceShim::test_notice_is_logged_at_debug_not_info` (pytest node id, verified passing when recorded)
