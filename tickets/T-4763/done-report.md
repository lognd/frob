...
Worktree: /home/logan/projects/frob/.claude/worktrees/t-4763
HEAD: 03400f1f3063b83e507a68babc0d728345995fea (after a dev merge pulled in
       unrelated fleet ticket-ledger commits; no conflicts, this ticket's
       own files unchanged by the merge)

### WHAT changed, per file

- src/frob/scaffold/data/shared/python/pyproject.toml.j2
  typani floor raised from ">=0.0.3" to ">=0.2.3" (the latest published
  release, recorded 2026-09-19 from PyPI's JSON API).

- src/frob/scaffold/data/types/python-tool/app/config.py.j2
  Rewrote AppConfig.from_external from a bare with/tomllib.load with no
  error handling into a @propagate-decorated classmethod returning
  Result["AppConfig", ConfigError]. Added a new ConfigError(ErrorSet)
  with NotFound/ReadFailed/ParseFailed members. Added a private
  _load_section(path, section) -> Result[dict, ConfigError] helper that
  wraps the file-read and the TOML parse each through Result.catch,
  attaches a .note() with the path for context, and .unwrap()s inside
  the @propagate function per refs/typani.md's idiom #1/#3/#4. An
  explicitly-named config_file that does not exist now returns
  Err(ConfigError.NotFound) instead of silently producing an empty
  config; the default (no explicit file, pyproject.toml missing) still
  produces an empty config, matching prior behavior for the common case.

- src/frob/scaffold/data/types/python-tool/__main__.py.j2
  main() now matches on the Result from AppConfig.from_external: Ok(cfg)
  runs App(cfg)(), Err(error) prints to stderr and exits 1. Added a
  one-line docstring to _build_parser and main and a frob:doc anchor on
  _build_parser (was previously undocumented).

- src/frob/scaffold/data/types/python-tool/tests/unit/test_app.py.j2
  Updated the "no config file" test to assert on the Ok variant instead
  of a bare AppConfig; added a new test asserting the missing-file case
  returns Err(ConfigError.NotFound) rather than raising.

- src/frob/scaffold/data/types/python-tool/tests/system/test_build.py.j2
  Added test_typani_floor_is_the_recorded_release (asserts the rendered
  pyproject.toml's typani dependency string equals "typani>=0.2.3", so a
  future bump is a deliberate edit) and test_typani_lint_is_clean (runs
  `python -m typani.lint` over the rendered src/ and asserts a clean
  exit, the positive control for acceptance #3).

- src/frob/scaffold/data/types/python-tool/docs/index.md.j2
  Added frob:describes anchors for the new public ConfigError and for
  the previously-undocumented _build_parser; added one sentence
  documenting the Result-returning from_external contract.

### WHY

typani was a declared dependency of every python-type scaffold but
appeared in zero generated modules, and the floor predated the 0.1 API
redesign refs/typani.md documents -- a fresh project could resolve a
pre-0.1 typani whose API does not match the house style at all (audit
row "typani Result for fallible ops: FAIL", SCAFFOLD-AUDIT.md section 2).
The natural fallible boundary in the generated code is from_external's
file I/O, so that is where the demonstration lives, per the ticket's
own instruction.

### Acceptance criteria -> proof

[1] Rendered pyproject typani floor == 0.2.3.
    Manually verified: rendered python-tool into scratchpad, `uv sync`
    resolved and installed typani==0.2.3 (uv sync output pasted below).
    Also asserted by the rendered project's own
    test_typani_floor_is_the_recorded_release (ships in
    tests/system/test_build.py.j2, in scope, runs as part of the
    scaffolded project's own suite).

[2] Config loader pointed at a nonexistent file returns an error value,
    no exception escapes.
    Manually verified: rendered project's full `uv run pytest tests/`
    passed (15/15), including
    test_app_config_from_external_with_missing_file_returns_err, which
    asserts isinstance(result, Err) and result.danger_err is
    ConfigError.NotFound -- no exception observed.

[3] typani's own lint over the rendered python-tool is clean.
    Manually verified:
    `uv run python -m typani.lint src --json` on the rendered demo ->
    "typani.lint: 0 error(s), 0 info(s) in 9 file(s) scanned".
    Also asserted by the rendered project's own
    test_typani_lint_is_clean (tests/system/test_build.py.j2).

### Manual verification transcript (rendered `demo` python-tool, scratchpad)
...
$ uv run python -m typani.lint src --json
  typani.lint: 0 error(s), 0 info(s) in 9 file(s) scanned
  {"version": 1, "files_scanned": 9, "findings": []}

### Evidence bound (frob ticket evidence)

Node id: tests/system/test_scaffold_dx.py::test_all_registered_types_render_without_error
Bound to acceptance [1], [2], [3].

Note on evidence choice: the stronger, purpose-built integration test
tests/system/test_scaffold_dx.py::test_python_toolchain_scaffold_passes_check_immediately[python-tool]
(which does `uv sync` + `uv run pytest` + ruff + ty inside the rendered
project, exactly reproducing the manual transcript above) FAILS when run
through `frob ticket evidence` / this repo's own agent-env recipe: its
_subprocess_env() helper does not strip PYTHONPATH, so the PYTHONPATH
this worktree's own agent env exports (T-4459, <worktree>/src) leaks into
the nested `uv run pytest` subprocess, which then imports this frob
checkout's own src/frob/__init__.py instead of the demo project's code,
crashes on a missing `pathspec` import, and fails before it can even
collect the demo project's tests. This is a pre-existing bug in that
test's own subprocess-env hygiene, out of T-4763's declared scope
(tests/system/test_scaffold_dx.py is not under
src/frob/scaffold/data/shared/python/pyproject.toml.j2 or
src/frob/scaffold/data/types/python-tool/**). Filed as
T-draft-cab0639b (bug, scope tests/system/test_scaffold_dx.py) rather
than fixed in place. The manual transcript above reproduces exactly what
that blocked test would have asserted, and the weaker
test_all_registered_types_render_without_error (which renders every
type in-process, no nested subprocess, so it is not exposed to the same
leak) is bound as the automated evidence instead.

### Overlap check with T-4758, T-1608, T-1609, T-3415

- T-4758 (directive-ergonomics sweep): scope src/** and tests/**
  repo-wide, dispatched only in a coordinator-declared quiet window
  after T-4703's other leaves land. No overlap with this ticket's files
  (scaffold .j2 templates); not touched.
- T-1608 (cross-language inspection stress test): scope tests/**,
  src/frob/**, docs/**, but its content is a NEW multi-language fixture
  repo exercising doc/test/waiver edges across languages -- unrelated to
  the python-tool scaffold templates. No file-level overlap with this
  ticket's touched paths.
- T-1609 (tail-end repo hygiene: docs/detector-gap/vestigial/waiver
  audits): explicitly gated to run only after the queue drains; scope is
  docs/**, src/frob/**, tests/** but as a repo-wide finished-state sweep,
  not scaffold-template work. No overlap.
- T-3415 (extend DOC010/docmake_gate to scaffold .j2 pairs): scope
  src/frob/gates/_docstatus.py and src/frob/scaffold/data/** -- this DOES
  overlap the scaffold data tree in principle, but T-3415 is about a new
  GATE (doc/Makefile citation checking across .md.j2/Makefile.j2 pairs),
  not about editing python-tool's own template content. This ticket
  touched docs/index.md.j2 only to add frob:describes anchors and one
  sentence of prose; it did not touch any `make <target>` citation and
  python-tool's docs/index.md.j2 was already citation-free of make
  targets before and after this change. No functional collision.

## Pre-READY checks

1. `frob check --only sys --files <touched> --base dev`:
   Tool summary: FAIL gate:DRIFT (6 errors, pre-existing, unrelated
   files: _rapid_sweep.py, invariants.py, _evidence.py -- all waived,
   none in this ticket's touched set), FAIL gate:DSL (1 error,
   pre-existing, not in touched set), pass gate:DOCARCH, pass
   gate:PROFILE, pass gate:WAIVE. Zero findings on any of this ticket's
   5 touched files.

2. `frob check --only arch --files <touched> --base dev`:
   Tool summary: pass frob-arch (21 warnings, 546 suggestions, all
   repo-wide pattern-recommendation/anti-pattern-escape notes on
   unrelated files such as src/frob/render/_renderer.py and
   src/frob/arch/_kotlin.py). No ARCH001/LARGE001 on any touched file.

3. `frob check --only coverage --files <touched> --base dev`:
   Tool summary: FAIL gate:COV (10 errors, all pre-existing COV007
   findings on src/frob/vet/*.py, all waived) FAIL gate:DRIFT (same 6
   pre-existing, waived, as above) FAIL gate:TODO (5 errors, all
   pre-existing frob:todo-bound-to-closed-ticket findings in
   src/frob/gates/_coverage.py, src/frob/perf/*.py,
   src/frob/strata/_secrets.py -- none in this ticket's touched set).
   Zero COV002 (new-public-symbol-needs-doc/tests) on any touched file.

4. `ruff check` / `ty check` on touched .py-shaped content: the .j2
   templates are not directly lintable as Python; verified instead by
   rendering (see manual transcript above): `uv run ruff check src/
   tests/` -> All checks passed!; `uv run ruff format --check src/
   tests/` -> 14 files already formatted; `uv run ty check src/` -> All
   checks passed!.

5. Cross-ticket file check: `git diff --name-only dev...HEAD` (feature
   commits only) touches: shared/python/pyproject.toml.j2,
   types/python-tool/__main__.py.j2, types/python-tool/app/config.py.j2,
   types/python-tool/tests/system/test_build.py.j2,
   types/python-tool/tests/unit/test_app.py.j2,
   types/python-tool/docs/index.md.j2 -- all within T-4763's own lease
   (.git/frob-leases/T-4763.json: shared/python/pyproject.toml.j2,
   types/python-tool/**). No cross-ticket collision found via `grep -l`
   over .git/frob-leases/*.json for these paths beyond T-4763 itself and
   T-4764 (which declares an overlapping scope on the SAME python-tool
   tree by design -- it is the next ticket in this series, blocked_by
   T-4763, and has not started yet).

6. kind=bug: N/A, T-4763 is a feature ticket.

7. No tickets/<dir> directories in this worktree's diff; no
   tickets/T-draft-* dirs created or promoted by this ticket.

8. Evidence was bound, then a docs-only commit was made, then evidence
   was re-bound against the new HEAD (5f5ea3a47c4d4e3967d3df16323bccf7e3d243ec)
   for all three acceptance criteria -- evidence postdates the last commit.

## Commits (this ticket's own work, in order)

f1defd003 chore(tickets): record T-4763 start transition
48aff6237 feat(scaffold): raise typani floor and demo Result at config I/O boundary
8f564ef50 chore(tickets): record evidence for T-4763
592caa1f0 chore(tickets): record evidence for T-4763
b325d1fe6 chore(tickets): record evidence for T-4763
5f5ea3a47 docs(scaffold): describe ConfigError and _build_parser in python-tool docs
587ddedc4 chore(tickets): file T-draft-cab0639b test_scaffold_dx._subprocess_env leaks PYTHONPATH into scaffolded uv run subprocess
03400f1f3 Merge branch 'dev' into t-4763 (dev catch-up merge; brought in
           unrelated fleet ticket-ledger chore commits only, no conflicts,
           re-bound evidence again after the merge -- HEAD above)

(the c92b051cc/2a8c482d1/adc8e937a/0f153967c/678aacc25 commits visible in
`git log` are dev's own unrelated ticket-body/sprint chores pulled in by
the merge, not this ticket's work)

## T-4764 status

Attempted `frob ticket work T-4764 --worktree <this worktree>` to
continue the series in one worktree as instructed. Refused:
BlockerOpen -- T-4764's blocked_by=['T-4763'] and T-4763 is still
in-progress (not yet closed; per the standing brief, implementers do not
run `frob ticket close`, only the coordinator does). T-4764 cannot be
started until the coordinator closes T-4763. No T-4764 work has been
done; nothing in this worktree touches T-4764's scope
(shared/python/logging/**, python-tool/app/**, __main__.py.j2 beyond
what T-4763 already touched, frob.toml.j2).

Side effect: that attempt auto-planned T-4764 (state queued -> planned)
in the ledger before hitting the blocker, leaving
tickets/T-4764/ticket.md modified-but-uncommitted in this worktree (a
one-line `state:` field change, no other content). Left uncommitted per
the standing rule against hand-editing/hand-committing ledger state;
the coordinator's next ledger verb against T-4764 (once T-4763 closes)
should absorb or supersede it. Flagging here so it is not mistaken for
this ticket's own change if `git status` is inspected.

## New tickets filed

T-draft-cab0639b (bug, scope tests/system/test_scaffold_dx.py):
test_scaffold_dx._subprocess_env leaks PYTHONPATH into the scaffolded
project's nested `uv run pytest` subprocess, blocking evidence-binding
for the scaffold python-toolchain e2e test from any worktree-based
agent shell. See "Evidence bound" section above for the full mechanism.
...
```
 ...
 uv.lock                                            |   2 +-
 1146 files changed, 84787 insertions(+), 2761 deletions(-)
```
