...
Root cause: _globs_intersect ran its wildcard DP on the raw fnmatch
character string. fnmatch's own '*' matches '/' too, so a trailing
wildcard in one glob could always absorb the other glob's literal
"/basename" suffix as ordinary matched text -- making '*' behave
exactly like '**'. Fixed by splitting both patterns on '/' first and
reserving cross-segment matching for a segment that is exactly '**';
every other segment matches within its own path component via the
original char-level DP (renamed _segment_intersect).

Gates: frob check --ticket T-3180 clean on gate:SCOPE and gate:PREWORK
(the two families the --ticket filter actually scopes; every other
family in that run is repo-wide and pre-existing, unaffected by this
change). frob test --base main ran the touched set (11 tests) exit=0.

Filed: none.

### Changed
```
 src/frob/tickets/_models.py | 96 ++++++++++++++++++++++++++++++++++++---------
 tests/test_tickets_lease.py | 46 ++++++++++++++++++++++
 tickets/T-3180/ticket.md    | 14 +++++++
