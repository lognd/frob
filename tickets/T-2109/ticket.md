...
- 2026-08-17 attempt 2: not repeating the already-disproven derived-formula approach: attempt 1 (2026-08-10) empirically proved the fix is tautological -- since the derived expected count is recomputed from the SAME raw design source (design/frob.strata) that an unintended addition would also modify, both sides of the equality move together and injecting an unintended node into the file still passed the test. The ticket's own acceptance criterion [0] mandates exactly this broken shape (assert elaborated count == SUM of raw decl counts from the same file), so it cannot be satisfied correctly as scoped. Per the ticket's own recorded coordinator direction ('push back rather than force a broken change'), filing this failure rather than re-attempting or hand-waiving the acceptance criterion. A genuinely different design (e.g. an explicit, ticket-updated golden SET of node ids checked for exact membership, not just a count) might work but changes the mechanism the acceptance criterion specifies and needs a coordinator decision, not a unilateral scope change under this ticket.
