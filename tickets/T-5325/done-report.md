...
WEBSEC needs a response-header lint engine before any header-family gate
rule can fire: it turns Django SECURE_* settings, Express helmet(), and
nginx/Caddy config directives into per-header PRESENT/MISSING findings.
A repo with no in-repo security surface at all (headers set purely at a
CDN/edge dashboard) reports ADVISORY rather than a false ERROR -- the
documented CDN-layer gap the owner directive called out. Built strictly
on T-5302's detect_frameworks (never re-detects) and gated the app-code
Django scan on it; helmet detection uses a text regex rather than
frob.lang.iter_identifiers because that walker has no JS/TS identifier
types yet (T-3232 follow-up), documented inline and in the module doc.
Positive and negative fixtures cover all four evidence sources plus the
no-evidence advisory case.
...
```
 ...
 src/frob/gates/_taint_gate.py                      |    63 +-
 ...
 src/frob/webapp/_websec_headers.py                 |   461 +
 ...
 tests/unit/test_webapp_websec_headers.py           |   114 +
 ...
 tickets/T-5323/ticket.md                           |   170 +
 tickets/T-5324/ticket.md                           |   130 +
 tickets/T-5325/done-report.md                      |  4014 ++++
 tickets/T-5325/ticket.md                           |   175 +
 ...
 tickets/T-draft-9fdc1fcd/ticket.md                 |    80 +
 ...
 uv.lock                                            |     2 +-
 4151 files changed, 690529 insertions(+), 23504 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_websec_headers.py::test_express_helmet_reports_default_headers_present` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_no_evidence_is_advisory_not_error` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_root_not_a_directory_is_err` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_full_evidence_all_present[nginx_full-nginx]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_full_evidence_all_present[caddy_full-caddy]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_full_evidence_all_present[django_full-app_django]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_one_missing_header_reports_missing[nginx_missing_csp-Content-Security-Policy-Strict-Transport-Security]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_one_missing_header_reports_missing[caddy_missing_hsts-Strict-Transport-Security-X-Frame-Options]` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_headers.py::test_one_missing_header_reports_missing[django_missing_xfo-X-Frame-Options-Content-Security-Policy]` (pytest node id, verified passing when recorded)
