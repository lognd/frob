...
T-3151: verified the premise first -- frob-exports genuinely missed
re-exports for frob.ci_report.*, frob.ci_validity.*,
frob.doctor.native_degrade_warning, frob.findings.* (not named in the
ticket text but in the same src/frob package, blocking the same cited
test, so included), frob.ghio.*, frob.repo_meta.is_frob_own_repo, and
testing._coverage_wait.CoverageLockUnavailable. Added the missing
re-exports (imports + __all__ entries) to src/frob/__init__.py and
src/frob/testing/__init__.py, following each file's existing convention.
Scope expanded via `frob ticket scope --add` to cover the two production
__init__.py files the fix actually touches (ticket originally named only
the test file).

Evidence: tests/unit/test_exports.py -- all 16 tests pass, including
TestFrobExportsPolicyResidue::test_all_nine_packages_report_zero_missing_symbols.
`import frob` succeeds (no circular-import regression from the new
re-exports). ruff check/format clean on both touched __init__.py files.

### Changed
```
 tickets/T-3151/done-report.md | 32 ++++++++++++++++++++++++++++++++
 tickets/T-3151/ticket.md      | 22 ++++++++++++++++++++--
...
- `tests/unit/test_exports.py::TestExportsPackage::test_basic_public_symbols` (pytest node id, verified passing when recorded)
...
- gates: 122 error(s), 690 warning(s), 872 waived
