...
Added the missing `frob status` command-table row to README.md's
Analysis section (T-2911 landed frob status as a real subcommand but
the land-time DOC005 pre-merge guard could not accept a same-diff
README edit against a still-stale registry). Measurement at pickup
also showed the live registry has grown to 49 subcommands total (not
47 as T-2940's own description assumed) and two more real subcommands,
frob narrative and frob refactor, had no README row either -- same
DOC005 obligation, same file, same scope glob. Added rows for all
three and corrected the "N total commands" claim to 49 so DOC005
passes clean for README.md.
