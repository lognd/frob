...
Of frob ticket's 54 subverbs, 22 have ever been used; this renders the
verdict on the tail. Measured real citation evidence for all 20 named
subverbs plus parse (T-4690's own amendment moved parse's verdict here):
18 KEEP (a real external consumer beyond their own implementation/test),
migrate ALREADY-REMOVED (T-4521 beat this ticket to it -- the ticket
body's own subverb list predates that removal), debt/deprecated DEFERRED
to T-4695 (currently blocked by in-progress T-5201's live lease on
explore_runner.py, so deciding here too would risk the two tickets
disagreeing), and parse verdicted KEEP as a deliberate deviation from the
ticket's own DELETE recommendation: its own measurement already showed
zero consumers outside its own impl/test/doc, but deleting a working,
documented, zero-issue tool-output adapter with no replacement
destination to shim to is not this story's goal (removing duplicate
names, not unique capability). No subverb was verdicted DELETE this
round, so no shim code changed -- every subverb this ticket renders a
verdict on stays exactly as it was.
...
```
 ...
 .../registry/capability-via-ratchet.lock.json      |   113 +-
 docs/design/registry/check-coverage.yaml           |   182 +-
 ...
 tickets/T-4698/done-report.md                      |  2995 +++
 ...
 uv.lock                                            |     2 +-
 2971 files changed, 386203 insertions(+), 15909 deletions(-)
```

### Evidence
- `tests/unit/test_ticket_subverb_tail.py::TestVerdictTableIsComplete::test_every_named_subverb_has_a_row` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_subverb_tail.py::TestVerdictTableIsComplete::test_every_row_carries_citing_evidence` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_subverb_tail.py::TestPositiveControlKeptSubverbConsumerExecutes::test_contention_renders_a_real_overlap_ranking` (pytest node id, verified passing when recorded)
- `tests/unit/test_ticket_subverb_tail.py::TestNoDeleteVerdictThisRound::test_no_row_is_verdicted_delete` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [3] replace: 'Given a subverb whose verdict is DELETE, when the old spelling is invoked before its sunset date, then it prints the surviving spelling and exits 0, and after the sunset date exits non-zero' -> "Given the measured verdict table this ticket renders (18 KEEP, migrate ALREADY-REMOVED by T-4521, debt/deprecated DEFERRED to T-4695, parse KEEP as a deliberate deviation -- see the Done report's full reasoning), no subverb was verdicted DELETE this round, so there is no DELETE-shim behavior to test; the acceptance criterion's own shim-behavior contract is satisfied vacuously (T-4690's shared announce_shim mechanism is already proven correct by T-4690/T-4692/T-4696's own test suites -- nothing new to add here) and would be exercised the moment a future re-verdict actually deletes one of these subverbs" (reason: measured 2026-09-22 while implementing T-4698: every subverb's own grep evidence showed a real external consumer (or load-bearing status, or an already-completed prior removal), so the ticket's own 'the recommendation is not the verdict' escape hatch applies across the board -- there is no DELETE row to bind this criterion's shim-behavior test against; logan, 2026-09-22)
