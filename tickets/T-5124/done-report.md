...
frob-suggest.py's attempt marker was keyed on the raw command string alone,
machine-global across every agent/session/repo -- two unrelated sessions
hitting the identical command shape shared one counter, so a second
session's first genuine attempt could read as an escalated repeat. Added
`_session_key` so the marker is scoped per agent/session, and
`dedupe_hook_registrations` in sync-claude-config.py so the hook's dual
project+user registration no longer double-fires. Merged dev (T-3851's
per-segment ack anchor, T-4625's ack-prefix test) with no behaviour loss on
either side; the worktree's stale `T-draft-36c347fe` directive/id tags were
rebound to the promoted T-5124 across source, tests and docs.
...
```
 ...
 tickets/T-5124/done-report.md                      |  1288 ++
 ...
 uv.lock                                            |     2 +-
 2784 files changed, 271385 insertions(+), 14968 deletions(-)
```

### Evidence
- `tests/test_hook_sync_claude_config.py::TestDedupeHookRegistrations::test_duplicate_basename_same_event_is_removed` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestDedupeHookRegistrations::test_distinct_basenames_are_kept` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestDedupeHookRegistrations::test_empty_group_after_removal_is_dropped` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestDedupeHookRegistrations::test_different_event_is_not_deduped` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestSyncDedupeHookRegistrations::test_writes_deduped_user_settings` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestSyncDedupeHookRegistrations::test_dry_run_does_not_write` (pytest node id, verified passing when recorded)
- `tests/test_hook_sync_claude_config.py::TestSyncDedupeHookRegistrations::test_missing_files_return_empty` (pytest node id, verified passing when recorded)
- `tests/test_hook_frob_suggest.py::test_different_sessions_do_not_share_the_attempt_counter` (pytest node id, verified passing when recorded)
- `tests/test_hook_frob_suggest.py::test_same_session_still_escalates_on_third_identical_attempt` (pytest node id, verified passing when recorded)
