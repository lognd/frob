...
24 entries in docs/design/registry/check-coverage.yaml claimed
`handled_by:<RULE>` enforcement, but none of their real enforcing
functions carried the `frob:enforces CHK-GATE-<RULE>` directive REG008
cross-checks against (T-0428's two-SSOT rule: the yaml's claim must be
backed by a real edge in code). All 24 rules were confirmed live and
already enforced in code -- this was a missing-directive gap, not a
missing-implementation gap, so the fix was additive only: one
`frob:enforces` comment above each rule's real violation-emitting
function (BASE001, CLAIM001, COV010, GUARD001, SYS900, PII013, RACE002,
WRAP001-003, SYS116, SYS117, PERF015-018, POL000, VET012, DSTACK001,
FMT002, TOOL001-003, SYS108), spanning gates/perf/vet/policy/strata.
Scope was widened from the original yaml-only filing to these 16 real
files once the investigation showed the fix could not land without
touching them.
...
```
 ...
 uv.lock                                            |     2 +-
 3899 files changed, 549411 insertions(+), 23243 deletions(-)
```
