...
Worktree: /home/logan/projects/frob/.claude/worktrees/t-draft-1cc03713
Branch: t-draft-1cc03713
HEAD: 5fc624174 ("chore(tickets): adopt promoted id T-4612 (was
T-draft-1cc03713)")
Ticket id: T-4612 (was T-draft-1cc03713)
Scope (narrowed): src/frob/strata/_pii.py, src/frob/gates/
_sys_provenance.py, docs/strata/provenance-trust-identity.md,
tests/test_pii_provenance_trust_identity.py
Rule ids: SYS116 (undeclared provenance), SYS117 (trust_identity
without carries) -- renamed from an initial SYS114/SYS115 pick that
clashed with T-4113.
Re-verified after merge+rename: ruff check clean, 40/40 tests passing.
`git diff --name-only dev...HEAD` = exactly the 4 scoped files + the
ticket file itself.

## UPDATE: scope narrowed off tests/**/test_sys*.py (blocked T-4509)

Coordinator flagged that this ticket's broad `tests/**/test_sys*.py`
scope glob was conservatively overlapping `tests/system/test_unity_
e2e.py` (the Unity capstone, T-4509), blocking it. That glob was
speculative from the start -- this ticket only ever wrote ONE test
file. Narrowed via `frob ticket scope T-4612 --remove 'tests/**/
test_sys*.py' --add tests/test_pii_provenance_trust_identity.py`.
`git diff --name-only dev...HEAD` now lists exactly 5 files: the 4
source/doc/test files plus `tickets/T-4612/ticket.md` -- no more glob
overreach.

## UPDATE: dev promoted the draft to T-4612

dev promoted `T-draft-1cc03713` to `T-4612` while this ticket was still
in progress in its own worktree. Merged dev into the worktree
(`git merge`, clean, git's own rename-detection already relocated
`tickets/T-draft-1cc03713/` -> `tickets/T-4612/`); fixed the ticket
file's own `id:` line (still read `T-draft-1cc03713` after the merge)
and every `frob:ticket`/prose reference to the old draft id in
`src/frob/gates/_sys_provenance.py` and `docs/strata/provenance-trust-
identity.md`. Renamed this file from `why-T-draft-1cc03713.txt` to
`why-T-4612.txt`. No evidence node ids changed by the promotion itself
(only the ticket id changed, not any test/symbol name), so evidence was
NOT re-bound for the promotion -- it stays bound as recorded under the
SYS114->SYS116/SYS115->SYS117 rename below, which happened before this
promotion was discovered.

## UPDATE: rule-id rename SYS114/SYS115 -> SYS116/SYS117

Coordinator flagged a clash after this ticket's first READY: T-4113
(worktree t-4112, held for landing) already claims SYS114/SYS115
(outbound-destination / foreign-flow-rate). Verified independently:
`git -C .claude/worktrees/t-4112 grep -n "SYS11[4-9]"` shows SYS114/
SYS115 fully claimed there (design registry, docs/strata/threat.md,
src/frob/strata/_outbound_destination.py) and SYS116/SYS117 unused in
both that worktree and dev's docs/modules/gates.md. Renamed everywhere
in this ticket's 4 files (rule id constants, doc anchors, module/test
docstrings, test class names `TestSys114...`->`TestSys116...` /
`TestSys115...`->`TestSys117...`, method names). Re-ran ruff (clean),
ty (clean), and the 40-test suite (still 40 passed, 0 failed) after the
rename. Committed as `880e5aab1`. Re-bound all 5 evidence node ids whose
names changed via `frob ticket evidence --replace` (the other 4,
whose test method names did not contain "sys114"/"sys115", were
untouched and remain valid). Evidence HEAD `1673b5523` is after the
rename commit `880e5aab1` -- evidence bound after the last source
commit, per rule 8.

Also appended the SYS116/SYS117 dispatch/`_KNOWN_GATE_RULES` wiring gap
to T-4605's body from the ROOT (`frob ticket body T-4605 --append`) --
the fold-in ticket the coordinator named as `T-draft-9a4eb7be` is no
longer reachable under that draft id (`frob ticket show T-draft-
9a4eb7be` returns "no ticket" even after this rename dispatch; no
promote-rename commit for it was found in `git log --all --grep`
either). `git grep -l "Wire CONFIGPATH001" -- tickets/T-4*/ticket.md`
found THREE near-identical tickets (T-4597, T-4602, T-4605) with the
same title and near-identical bodies -- diffing them shows T-4605 is a
strict superset of T-4597 and T-4602's content (it carries every append
they have, plus a later T-3962/INV011 append neither has), so T-4605 is
almost certainly the live, continuously-appended-to ticket the
coordinator meant, and T-4597/T-4602 are stale duplicate mirrors from
earlier land points. Appended there; flagging the T-4597/T-4602
duplication itself as something the coordinator or a cleanup ticket
should resolve, not something this ticket's scope covers.

## Done report -- T-4612 (implement T-3961 provenance/trust-as-identity)

Parent: T-3961 (design ticket, accepted per coordinator brief). This
ticket implements T-3961's design note, which itself found the real
implementation surface is `frob.strata._pii` (attr parsing/validation)
plus a new SYS10x rule module, NOT `src/frob/strata/_models.py` as
T-3961's original scope assumed -- confirmed independently: `derived_from`
and `trust_identity` reuse the existing `Node.attrs` opaque slot, the
same attr-desugar convention `pii=`/`code=`/T-4073's `no_pii` already
use. Zero grammar/kernel change.

### WHAT changed

- `src/frob/strata/_pii.py` (modified):
  - `_DERIVED_FROM_PREFIX = "derived_from:"`, `_TRUST_IDENTITY_PREFIX =
    "trust_identity:"` constants.
  - `node_derived_from(node) -> tuple[(tag, helper), ...]`: parses
    `derived_from:<tag>=<helper>` attrs.
  - `node_trust_identity_tags(node) -> tuple[str, ...]`: parses
    `trust_identity:<tag>` attrs.
  - `check_pii_derived_from_contradiction(model)` (PII005): two
    `derived_from` attrs naming the same tag with different helpers on
    one node is a contradiction, deny-by-default (mirrors PII001's
    shape). Wired into `evaluate_pii`'s aggregator and `__all__`.

- `src/frob/gates/_sys_provenance.py` (new): the "SYS10x consumer" T-3961
  acceptance[2] asks for.
  - `SYS_UNDECLARED_PROVENANCE = "SYS116"`: a node `carries` an
    `identifier.*` tag with NO matching `derived_from` attr -- the exact
    F-273 finding (raw client IP trusted behind a proxy) this epic
    traces to. Deliberately narrowed to `identifier.*` (not all seven
    `PII_CATEGORIES`) to avoid a mass false-positive wave on first
    landing; disclosed in the module docstring and doc file.
  - `SYS_TRUST_IDENTITY_WITHOUT_CARRIES = "SYS117"`: a
    `trust_identity:<tag>` attr naming a tag the node does not itself
    `carries` -- a provenance statement about data the node's own model
    says it does not hold.
  - `check_undeclared_provenance`, `check_trust_identity_without_carries`,
    `evaluate_provenance` -- gate-agnostic (`frob.gates._models.Violation`
    output only, no `_sys.py`/`_waive.py` import), same posture
    `_pii.py::evaluate_pii` already has.

  Rule id choice: T-3961's design note proposed SYS107/SYS108, but both
  are already taken (`SYS_VIA_LESS_LARGE_NODE`, `SYS_DUPLICATE_
  INTERFACE` in `_selfconform_ids.py`) -- verified against the live
  registry (`git grep '"SYS1[0-9]{2}"'` across src/+docs/) before
  picking SYS116/SYS117, the next genuinely free ids, per the design
  note's own explicit "re-check the live rule-id registry at
  implementation time" instruction.

- `tests/test_pii_provenance_trust_identity.py` (new): 12 unit tests.
  Positive controls per the brief's own framing: `test_undeclared_
  helper_fires_sys116` plants the F-273 shape (carries with no
  derived_from) and asserts it fires; `test_declared_helper_does_not_
  fire_sys116` asserts the declared-correctly case does not. Same
  pattern for PII005 (conflicting vs. single helper) and SYS117
  (trust_identity with/without matching carries).

- `docs/strata/provenance-trust-identity.md` (new): standalone doc row
  -- `docs/modules/gates.md` was leased by in-progress T-4111 at
  implementation time, so this stands alone rather than folding in;
  fold it into `docs/modules/gates.md` once that lease clears (noted in
  the doc file itself).

### WHY

T-3961's own body: F-273 (raw client IP trusted behind a proxy) is
invisible today because a wrong IP is still just a string; nothing
polices which code site is allowed to produce a value under a given PII
tag, or which node is authorized to treat a received value AS identity
(vs. merely holding it) -- two different axes strata's capability
ratchet (`may`) does not model.

### Acceptance criteria proof

This draft ticket has no formal `acceptance` list (created via `frob
ticket new` with only a title, per the coordinator's dispatch
instruction). Proof is against T-3961's own two acceptance criteria,
which this ticket implements:

- [1] design note attached before implementation: T-3961's own Done
  report (see `why-T-3961.txt`, already on disk) IS that note; this
  ticket only starts once that note exists.
- [2] "given the design is accepted, when implemented, then SYS100 (or
  a successor rule) can require that a single helper produce every
  client IP, and a trust-as-identity declaration exists distinct from
  the capability ratchet": SYS116 is that successor rule (a single
  `derived_from` helper per `identifier.*` tag, PII005 catching
  conflicting helpers); `trust_identity:<tag>` (SYS117) is the
  distinct-from-`may` trust-as-identity declaration.

### Test node ids (9, all passing)
...
Also re-ran the existing PII suite (unaffected): `tests/unit/strata/
test_pii.py` + `tests/unit/strata/test_litmus_pii.py` +
`tests/test_pii_provenance_trust_identity.py` = 40 passed, 0 failed.

### Evidence bound

All 9 node ids above bound via `frob ticket evidence T-4612
... --base-ref dev`, AFTER the source commit (see Commits below) --
evidence commit sha `0b255523f` follows source commit `7211e3afb`.

### Commits (branch t-draft-1cc03713, worktree
/home/logan/projects/frob/.claude/worktrees/t-draft-1cc03713)

- `7211e3afb` feat(strata): implement T-3961 provenance/trust-as-identity attrs
- `0b255523f` chore(tickets): record evidence for T-4612

### Gates run

- ruff check + format: clean on all 4 touched files (`src/frob/strata/
  _pii.py`, `src/frob/gates/_sys_provenance.py`, `tests/test_pii_
  provenance_trust_identity.py`, `docs/strata/provenance-trust-
  identity.md`) -- 2 E501 fixes applied during the pass.
- `ty check` on the 3 .py files: All checks passed.
- `frob check --only arch --files ... --base dev`: pass, 19 warnings
  (36 waived) + 542 suggestions, none attributable to my files (all
  pre-existing repo-wide `frob-arch` pattern-recommendation noise).
- `frob check --only sys --files ... --base dev`: ran DOCARCH/DRIFT/
  DSL/PROFILE/WAIVE/scope-note families; grepped full output for my 3
  files -- zero hits (no SELFAUDIT001/SYS finding attributable to this
  change). Pre-existing DRIFT/DSL errors elsewhere in the repo, none
  touching my files.
- `frob check --only coverage --files ... --base dev`: grepped full
  output for my 3 files -- zero hits (no COV002/COV007/TODO finding
  attributable to this change). First attempt hit a cache lock held by
  a concurrent `frob ticket land T-3943` process (transient, not a
  finding); retry succeeded.
- A bare `--only gates` combined run was attempted once but timed out
  under fleet load at 300s (per BRIEF: "gate check ... NEVER a bare or
  --ticket frob check; they take 10+ minutes under load") -- abandoned
  in favor of the per-family --only invocations above, which is what
  the PRE-READY CHECKLIST actually asks for.

### Pytest

`PYTHONPATH=.../src python -m pytest tests/unit/strata/test_pii.py
tests/unit/strata/test_litmus_pii.py tests/test_pii_provenance_trust_
identity.py -p no:cacheprovider -q` -> `SUITE-RESULT: exitstatus=0
collected=40 failed=0`.

### Scope / lease conflicts (disclosed, not worked around)

At `frob ticket scope --add` time, the following were REFUSED as leased
by other in-progress tickets and NOT touched:
- `src/frob/gates/_sys.py` -- held by T-4212. Consequence: SYS116/SYS117
  are NOT wired into `_sys.py::sys_gate`'s dispatch. `evaluate_provenance`
  is a complete, tested, standalone entrypoint ready for that wiring.
- `src/frob/gates/_waive.py` -- held by T-4212. Consequence: SYS116/
  SYS117 are NOT registered in `_KNOWN_GATE_RULES` (the merge-zone-
  managed rule-id registry). A `frob check` run that encounters these
  rule ids before that registration lands may flag them as unknown --
  expected until the follow-up wiring ticket lands.
- `tests/test_pii_structural_gate.py` -- held by T-4073; used
  `tests/test_pii_provenance_trust_identity.py` instead (new file).
- `docs/modules/gates.md` -- held by T-4111; used a standalone doc file
  instead (`docs/strata/provenance-trust-identity.md`), per the
  coordinator's own fallback instruction. Note: the coordinator's named
  fold-in target `T-draft-9a4eb7be` does not exist in this repo's
  ticket ledger (`frob ticket show` returns "no ticket") -- could not
  append there; used the standalone-file fallback instead.

Git diff vs dev is exactly my lease's 4 files -- no cross-ticket
overlap: `git diff --name-only dev...HEAD` = `docs/strata/provenance-
trust-identity.md`, `src/frob/gates/_sys_provenance.py`,
`src/frob/strata/_pii.py`, `tests/test_pii_provenance_trust_identity.py`.

### Simplifications disclosed (not silently dropped)

- SYS116 is scoped to `identifier.*` PII category only (not all seven
  `PII_CATEGORIES`) to avoid a mass false-positive wave against this
  repo's own existing `design/*.strata` declarations on first landing --
  matches the design note's own "cheap first step" posture; widening is
  real future work.
- T-3961's design note floated an "ENFORCING node" gate (mirroring a
  T-3985 subject-count concept) for SYS116's scope; no such "enforcing
  node" concept exists anywhere in this codebase today (verified via
  `git grep enforcing -- src/frob/strata/*.py`), so SYS116 applies to
  every node carrying an in-scope tag instead -- simpler and still a
  cheap first step, disclosed here rather than inventing an undefined
  concept.
- No structural code-tracing cross-check (the design note's own
  "follow-on step") -- pure declaration-shape checks only, exactly the
  design note's own disclosed-deferral posture (same as PII013/T-4073's
  taint-analysis deferral).

### Filed

None new (the two lease conflicts above are recorded here, not filed as
tickets -- both are pre-existing in-progress work, not new findings).

### T-3964 (second item in this dispatch)

BLOCKED, not started: T-3964 ("dataset construct under store with
append_only attribute") declares scope `src/frob/strata/_models.py`,
which is leased by in-progress T-3961 itself
(`.git/frob-leases/T-3961.json`, worktree `t-3961-strata`, branch
`t-3961`) at dispatch time. Per the brief's own explicit instruction
("if src/frob/strata/_models.py is leased by an in-progress ticket,
report BLOCKED with the holder"), T-3964 was not started. No worktree
was created for it, no ticket state was changed.
...
```
 ...
 src/frob/gates/_sys_provenance.py                  |  178 ++
 ...
 tests/test_pii_provenance_trust_identity.py        |  168 ++
 ...
 uv.lock                                            |    2 +-
 2151 files changed, 126513 insertions(+), 5514 deletions(-)
```

### Evidence
- `tests/test_pii_provenance_trust_identity.py::TestPii005DerivedFromContradiction::test_conflicting_helpers_on_same_tag_fires_pii005` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestPii005DerivedFromContradiction::test_single_helper_does_not_fire_pii005` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestSys116UndeclaredProvenance::test_undeclared_helper_fires_sys116` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestSys116UndeclaredProvenance::test_declared_helper_does_not_fire_sys116` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestSys116UndeclaredProvenance::test_non_identifier_category_is_out_of_sys116_scope` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestSys117TrustIdentityWithoutCarries::test_trust_identity_without_carries_fires_sys117` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestSys117TrustIdentityWithoutCarries::test_trust_identity_with_matching_carries_does_not_fire` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestEvaluateProvenance::test_evaluate_provenance_merges_both_rules` (pytest node id, verified passing when recorded)
- `tests/test_pii_provenance_trust_identity.py::TestEvaluateProvenance::test_evaluate_provenance_clean_model_has_no_violations` (pytest node id, verified passing when recorded)
