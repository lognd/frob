...
T-4116 land repair -- claim_lint_gate ARCH001 split

WHAT changed:
- src/frob/gates/_claim_lint.py: claim_lint_gate had grown to 65 lines
  (mixed per-file iteration, edge lookup, and per-symbol violation
  construction in one function), past ARCH001's long-AND-complex
  threshold. Split into:
  - _claim_violation(rel, parsed_path, sym, bound, waivers) -> Violation | None
    per-symbol verdict (claim word match, invariant-bound check, waiver
    check, Violation construction).
  - _claim_violations_for_file(f, root) -> tuple[list[Violation], bool]
    per-file scan: parse the file, build edges/bound/waivers, call
    _claim_violation over each symbol.
  claim_lint_gate itself is now a thin loop over iter_files calling
  _claim_violations_for_file and logging the summary. Behaviour and all
  CLAIM001 detection semantics (symref format, waiver honoring, claim
  word set) are unchanged -- pure extraction, no logic edits.
  Both new helpers are private (leading underscore), so no new COV002
  obligation (frob:doc/frob:tests apply to public symbols only).
  Added `from frob.lang import RawSymbol` for the new helper's type hint.

- Merged dev into t-4116 (fast, no conflicts on CHANGELOG.md/uv.lock/
  pyproject.toml/.frob-release.json/ratchet locks -- dev only added new
  ticket dirs and a T-4419 body edit, nothing touching this file).

WHY: land was refused by ARCH001 on src/frob/gates/_claim_lint.py:108
(claim_lint_gate at 65 lines, past the long-AND-complex threshold, not
already over it before this diff -- T-2214 family not relaxed by rapid
profile). Splitting the per-file and per-symbol logic into named helpers
is the natural decomposition already implied by the function's own two
loop levels; no behavior change needed.

Acceptance criteria / proof:
- All 7 existing CLAIM001 tests still pass unmodified against the split
  implementation (same public claim_lint_gate contract):
...
- Evidence re-bound to these 7 node ids via `frob ticket evidence T-4116
  ... --base-ref dev` AFTER the final commit (content was already
  identical to the pre-existing evidence list, so the write was a no-op
  on tickets/T-4116/ticket.md -- confirmed via `git status --short`
  clean and `git log -1 -- tickets/T-4116/ticket.md` unchanged at
  44e263997, i.e. no stale-then-invalidated ordering issue).

Commits:
- c7aa60a2d Merge branch 'dev' into t-4116
- 6f8828fa5 refactor(gates): split claim_lint_gate under ARCH001 threshold
  (HEAD)

## Pre-READY checks

frob check --only sys --files src/frob/gates/_claim_lint.py --base dev:
  FAIL gate:COV (unrelated, other files) / FAIL gate:DRIFT (unrelated,
  other files, all pre-existing and waived) / FAIL gate:DSL (unrelated:
  tests/test_app.py:387 malformed directive, not our file, matches the
  pre-existing land log WARNING from before this session) / pass
  gate:DOCARCH / pass gate:PROFILE / pass gate:WAIVE / pass gate:PLACE.
  Zero findings on src/frob/gates/_claim_lint.py in this run.

frob check --only arch --files src/frob/gates/_claim_lint.py --base dev:
  pass frob-arch 19 warnings (36 waived), 542 suggestions -- all findings
  listed are in other files (_kotlin.py, __init__.py, _daemon_proxy.py,
  etc., pattern-recommendation/anti-pattern-escape noise pre-existing in
  the repo); zero ARCH001/LARGE001 on _claim_lint.py. pass
  claude-config-drift.

frob check --only coverage --files src/frob/gates/_claim_lint.py --base dev:
  FAIL gate:COV / FAIL gate:DRIFT / FAIL gate:DSL / FAIL gate:TODO --
  all findings enumerated are in other files (src/frob/vet/_source.py,
  src/frob/vet/_typosquat.py, src/frob/app/ticket_runner/_rapid_sweep.py,
  src/frob/gates/invariants.py, src/frob/tickets/_evidence.py,
  tests/unit/test_ticket_store.py, all pre-existing and mostly waived).
  Zero COV002 (or any COV finding) on src/frob/gates/_claim_lint.py --
  confirmed no line in the raw output names _claim_lint.py. pass
  gate:PLACE, pass gate:PROFILE, pass gate:WAIVE.

ruff check src/frob/gates/_claim_lint.py: All checks passed!
ruff format --check src/frob/gates/_claim_lint.py: 1 file already formatted

ty check src/frob/gates/_claim_lint.py: All checks passed!

## Re-repair (second land refusal)

The land was refused a SECOND time by COV002: the ARCH001 split had
left the `# frob:doc`/`# frob:tests` directive block sitting above
`_claim_violation` (the first extracted helper) instead of moving with
`claim_lint_gate` itself, so the public gate function at its new
line (180) had no frob:tests edge.

WHAT changed:
- Moved the `# frob:doc docs/modules/gates.md#claim001-t-4116` and all
  four `# frob:tests tests/gates_suite/test_claim_lint.py::...` lines
  (with their `# noqa: E501` markers) so they sit directly above
  `def claim_lint_gate(root: Path) -> list[Violation]:` again, plus
  `# frob:ticket T-4116` restored on that same block.
- Added `# frob:ticket T-4116` above `_claim_violation` and
  `_claim_violations_for_file` (the two extracted helpers), replacing
  the misplaced frob:tests block that used to sit above
  `_claim_violation`.

Proof:
- `frob check --only coverage --files src/frob/gates/_claim_lint.py --base dev`:
  Tool summary: FAIL gate:COV (12 errors) / FAIL gate:DRIFT (5) / FAIL
  gate:DSL (1) / FAIL gate:TODO (4) -- all pre-existing findings in
  OTHER files (same shape as every prior run in this repair session).
  `grep -n "_claim_lint" <full raw output>` matches ONLY the
  REPEATED_FAILURE meta-warning about re-running the same command
  (a telemetry note about the harness's own retry, not a finding) --
  zero COV001/COV002/any finding naming _claim_lint.py anywhere in the
  raw output.
- PYTHONPATH=.../src python -m pytest tests/gates_suite/test_claim_lint.py -q
  SUITE-RESULT: exitstatus=0 collected=7 failed=0
- ruff check / ty check src/frob/gates/_claim_lint.py: All checks
  passed! (both tools, re-verified after the directive move).

Commit:
- 8609e68da fix(gates): restore frob:tests/frob:ticket directives after
  claim_lint split (HEAD)
...
```
 ...
 tickets/T-4116/done-report.md                      | 664 +++++++++++++++++
 ...
 uv.lock                                            |   2 +-
 567 files changed, 46042 insertions(+), 1948 deletions(-)
```

### Evidence
- `tests/gates_suite/test_claim_lint.py::test_claim001_fires_on_unbound_never_claim` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_quiet_when_invariant_directive_present` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_quiet_on_ordinary_prose_regardless_of_invariant_coverage` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_honors_frob_waive_escape_hatch` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_fires_once_per_offending_symbol_not_per_word` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_scopes_to_the_exact_symbol_not_the_whole_file` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_claim_lint.py::test_claim001_fires_on_class_docstring_too` (pytest node id, verified passing when recorded)
