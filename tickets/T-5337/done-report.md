...
Adds the ORM N+1/pooling/transaction/index rule family so the SQL
family has real lint coverage beyond raw injection, and so two sibling
epics (T-5147-6 WEBPERF pool config, T-5145-3 COMPLY GDPR migration-TTL
scan) stop being blocked on a helper that did not exist. Reuses
_extract.py's tree-sitter-via-raw_tree and tracked-file-scan shapes
rather than standing up a second walker. SQL103/SQL107 and the TS/Prisma
N+1 walk are left for a follow-up ticket rather than widened in scope.
Also grants core's fs.read via-list on src/frob/sql/*.py (T-5334 left
this for whichever leaf landed real reads first) in its own commit.
...
```
 ...
 tickets/T-draft-416b4c28/ticket.md                 |    36 +
 ...
 uv.lock                                            |     2 +-
 4153 files changed, 690899 insertions(+), 23508 deletions(-)
```

### Evidence
- `tests/unit/sql/test_orm_rules.py::test_sql101_lazy_relationship_in_loop_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql101_eager_loaded_loop_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql102_unbounded_all_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql102_limited_all_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql105_multi_write_no_transaction_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql105_wrapped_transaction_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_migration_scan_parses_create_index` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql104_missing_index_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql104_indexed_column_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql106_missing_pool_config_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_orm_rules.py::test_sql106_configured_pool_not_flagged` (pytest node id, verified passing when recorded)
