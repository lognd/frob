...
All three failures had one shared root cause, confirmed via git log -S:
T-4758's directive-ergonomics/change-narrative sweep stripped the literal
"(T-4522)"/"(T-4546)" ticket citations from the argparse description/help
strings in src/frob/_cli_parsers/_core.py (agent) and _misc.py (claude,
natives) -- the actual --help-rendering parser tree `_build_parser()`
uses, not src/frob/__main__.py where the ticket was originally (mis)filed.
Fix: restore the three citations verbatim. This is CLI-facing
documentation the test suite treats as a load-bearing contract (the
flattening/alias note), not internal change-narrative -- so the sweep's
removal was the regression, not the test.
...
```
 ...
 uv.lock                                            |     2 +-
 3905 files changed, 561230 insertions(+), 23384 deletions(-)
```

### Evidence
- `tests/unit/test_cli_single_child_groups.py::TestClaudeGroupFlattened::test_help_notes_alias` (pytest node id, verified passing when recorded)
...
- `tests/unit/test_cli_single_child_groups.py::TestNativesGroupFlattened::test_help_notes_alias` (pytest node id, verified passing when recorded)
