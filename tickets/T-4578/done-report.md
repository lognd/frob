...
T-4578: Wire 'frob scaffold unity-project <path>' into the scaffold CLI
Worktree: /home/logan/projects/frob/.claude/worktrees/t-4578
HEAD: d017430db (chore(tickets): record evidence for T-4578)

## WHAT changed, per file

- src/frob/_cli_parsers/_core.py
  Added a new `frob scaffold unity-project <dir> [--force]` argparse
  subparser inside `_populate_scaffold_actions`, alongside `list`/`apply`/
  `new`/`pool`. Positional `scaffold_unity_root` (dest matches the
  AppConfig field so the generic Namespace->kwargs copy in
  `_config_external.py` picks it up), plus a `--force` flag with dest
  `scaffold_unity_force`.

- src/frob/app/config.py
  Added `scaffold_unity_root: Path | None = None` and
  `scaffold_unity_force: bool = False` to `AppConfig`, matching the
  existing `scaffold_output`/`scaffold_force` field-naming convention
  T-2965's `ticket_parent_clear` and every other scaffold_* field already
  follow. Added `frob:ticket T-4578` alongside the class's existing
  ticket-edge comments (COV002 requires an open-ticket edge on a changed
  public symbol).

- src/frob/app/_config_external.py
  Added `scaffold_unity_root` to `_PATH_FIELDS` and `scaffold_unity_force`
  to `_BOOL_FLAGS` -- this is the module that copies the parsed
  `argparse.Namespace` onto `AppConfig` kwargs field-by-field
  (`_build_external_config_kwargs`); without this the CLI flag reached
  argparse fine but never reached `AppConfig` (verified: the CLI raised
  "frob scaffold unity-project requires <dir>" even when a dir was passed,
  until this file was also touched -- root-caused by running the new
  system test against the real CLI, not just the parser in isolation).
  Added to the ticket's scope via `frob ticket scope T-4578 --add` before
  editing (reason logged in the ticket's scope_changes).

- src/frob/app/scaffold_runner.py
  Added `_run_unity_project(cfg)`: a thin CLI wrapper (same shape as the
  existing `_run_pool`) that calls T-4503's `render_unity_project(Path(
  cfg.scaffold_unity_root), force=cfg.scaffold_unity_force)`, logs via the
  module's existing `_log` logger, and routes `Result.is_err` to
  `_log.error(...)` + `sys.exit(1)` -- no bare exception ever escapes to
  the CLI (matches every other branch in this file's `run()`). Wired into
  `run()`'s dispatch table under `cmd == "unity-project"`.

- src/frob/scaffold/_unity_project.py
  Removed the `frob:waive WIRE001 reason="... wiring a CLI form is out of
  scope" follow_up="T-4578"` waiver on `render_unity_project` now that
  `scaffold_runner._run_unity_project` is a real caller reachable from the
  CLI. Replaced it with a `frob:tests` binding to the new CLI success
  test. Added to ticket scope (reason logged in scope_changes) since the
  waiver removal is inside this file, even though it wasn't in the
  ticket's original declared scope list.

- docs/commands/scaffold.md
  Added a `frob scaffold unity-project <dir> [--force]` usage block
  (anchored `<!-- frob:describes src/frob/app/scaffold_runner.py::
  _run_unity_project -->`) next to the existing `pool` usage block --
  the doc already had extensive prose on `render_unity_project` from
  T-4503 but no example of the actual CLI invocation this ticket adds.
  Added to ticket scope (reason logged in scope_changes).

- tests/system/test_scaffold_unity_project_cli.py (new)
  Real `python -m frob scaffold unity-project ...` subprocess coverage,
  mirroring `tests/system/test_cli_scaffold_apply.py`'s pattern and
  reusing T-4512's `tests/fixtures/unity_sample_asmdef` fixture (same
  fixture `tests/unit/test_scaffold_unity_project.py` uses for the
  direct-call unit tests):
    - test_success: a real Unity project dir -> frob.toml + design/*.strata
      written, "created" on stdout, exit 0.
    - test_output_exists_refusal: second run without --force exits
      non-zero with "output already exists" and no Traceback; --force
      then succeeds.
    - test_not_a_unity_project: a plain directory (no Assets/Packages)
      exits non-zero with a clear Assets/Packages-naming message, no
      Traceback, and no frob.toml written.

## WHY

T-4503 landed `render_unity_project(root, *, force=False)` with a
WIRE001 waiver explicitly naming this ticket as the follow-up: its
root-only signature doesn't fit `render_project`'s uniform
`new <type> <name> [--output] [--force]` CLI dispatch (no separate
project name -- the type scaffolds ONTO an existing directory). This
ticket gives it its own CLI leaf instead of overloading `new`'s contract,
per the ticket body's "or equivalent frob init detection" acceptance
wording.

## Acceptance criteria proof

The ticket body has no separate numbered acceptance list (single prose
paragraph); `frob ticket evidence --accepts` reports 0 acceptance items
for T-4578, so evidence was bound without `--accepts` (matches how the
tool degrades when no such list exists).

1. `frob scaffold unity-project <dir>` end-to-end -> frob.toml + one
...
2. OutputExists refusal (no --force) is a clean non-zero exit with no
   traceback, and --force then overwrites:
   tests/system/test_scaffold_unity_project_cli.py::TestScaffoldUnityProjectCli::test_output_exists_refusal
3. A non-Unity directory gets a clear NotAUnityProject-derived message,
   non-zero exit, no traceback, no bogus frob.toml written:
   tests/system/test_scaffold_unity_project_cli.py::TestScaffoldUnityProjectCli::test_not_a_unity_project

All three bound via:
  frob ticket evidence T-4578 <node-id> --base-ref dev
(run individually per node id; the ticket carries no acceptance-item
list so no --accepts index was passed).

Also re-ran (unchanged, still green) as regression coverage for the
WIRE001-waiver removal and CLI wiring not disturbing existing behavior:
  tests/unit/test_scaffold_unity_project.py (9 tests, direct-call coverage)
  tests/system/test_cli_scaffold_apply.py (1 test, unrelated scaffold CLI leaf)
...
8628236f1 feat(scaffold): wire frob scaffold unity-project into the CLI
fc3be2474 fix(scaffold): satisfy COV002/COV007 on the unity-project CLI wiring
(plus frob-internal chore(tickets) mirror commits for scope/evidence:
 a48d4cfe7, 09fdc59ac, bbea0ce52 (scope adds), b2a0451d2, 7a1c4609d,
 d017430db (evidence binds) -- all --base-ref dev, evidence bound after
 the last code commit fc3be2474 per the PRE-READY CHECKLIST's ordering
 rule)

HEAD at READY: d017430db

## Scope

Ticket's original declared scope:
  src/frob/app/scaffold_runner.py
  src/frob/_cli_parsers/_core.py
  src/frob/app/config.py

Added during work (each via `frob ticket scope T-4578 --add ... --reason
...`, recorded in tickets/T-4578/ticket.md's scope_changes):
  src/frob/scaffold/_unity_project.py   (WIRE001 waiver removal lives here)
  src/frob/app/_config_external.py      (argparse Namespace -> AppConfig
                                          field-copy tuples; _core.py/
                                          config.py alone cannot populate
                                          the new fields)
  docs/commands/scaffold.md             (usage block + frob:doc anchor)

No cross-ticket lease conflicts encountered on any of these paths.

## Pre-READY checks

`FROB_SUGGEST_ACK=1 /home/logan/projects/frob/.venv/bin/frob check --only sys --files docs/commands/scaffold.md --files src/frob/_cli_parsers/_core.py --files src/frob/app/_config_external.py --files src/frob/app/config.py --files src/frob/app/scaffold_runner.py --files src/frob/scaffold/_unity_project.py --files tests/system/test_scaffold_unity_project_cli.py --base dev`
  -> Tool summary: gate:DOCARCH pass (0 errors), gate:DRIFT FAIL (5 errors,
     all in src/frob/app/ticket_runner/_rapid_sweep.py, src/frob/gates/
     invariants.py, src/frob/tickets/_evidence.py -- none of my files),
     gate:DSL FAIL (1 error, tests/test_app.py:387 -- not one of my files),
     gate:PROFILE pass, gate:WAIVE pass. No SELFAUDIT001 finding names any
     of the 7 touched files (grepped the full log for each filename --
     zero hits).

`FROB_SUGGEST_ACK=1 /home/logan/projects/frob/.venv/bin/frob check --only arch --files <same 7 files> --base dev`
  -> Tool summary: pass  frob-arch  19 warnings (36 waived), 542
     suggestions -- all repo-wide abstraction-opportunity/pattern-
     recommendation notes on unrelated files (checked: none name any of
     the 7 touched files). No ARCH001/LARGE001 finding on any touched
     file.

`FROB_SUGGEST_ACK=1 /home/logan/projects/frob/.venv/bin/frob check --only coverage --files <same 7 files> --base dev`
  -> First run found COV002 (AppConfig, run(), and the new test module's
     symbols lacked a frob:ticket edge) and COV007 (a frob:doc directive
     I'd put on the private `_run_unity_project` instead of leaving the
     doc<->code link in docs/commands/scaffold.md's own frob:describes
     anchor). Fixed both (commit fc3be2474), re-ran: gate:COV still shows
     10 errors overall but zero of them name any of the 7 touched files
     (checked by grepping the full log for each filename -- the surviving
     10 are all pre-existing COV002/COV006/COV007 findings on unrelated
     files: src/frob/vet/_capability_core.py, src/frob/tickets/_land_*.py,
     src/frob/dup/_core.py, etc.).

`/home/logan/projects/frob/.venv/bin/ruff check <7 files>` -> All checks passed!
`/home/logan/projects/frob/.venv/bin/ruff format --check <7 files>` -> 7 files already formatted (final state; re-verified per-subset at each edit)

`/home/logan/projects/frob/.venv/bin/ty check src/frob/_cli_parsers/_core.py src/frob/app/config.py src/frob/app/scaffold_runner.py src/frob/app/_config_external.py src/frob/scaffold/_unity_project.py` -> All checks passed!

## Notes for the coordinator

- No scope refusals encountered (all three additions granted cleanly).
- No new waivers added; one pre-existing waiver (WIRE001 on
  render_unity_project) was removed as instructed.
- No tests skipped.
- Version-skew warnings on every frob invocation are the known, already-
  documented condition (installed frob vs. this checkout's dev version)
  -- all measurement commands above used the worktree's own
  /home/logan/projects/frob/.venv/bin/frob per the standing brief, not
  the stale PATH-resolved binary.
...
```
 ...
 src/frob/_cli_parsers/_core.py                     |   54 +-
 ...
 uv.lock                                            |    2 +-
 2163 files changed, 133273 insertions(+), 5783 deletions(-)
```

### Evidence
- `tests/system/test_scaffold_unity_project_cli.py::TestScaffoldUnityProjectCli::test_success` (pytest node id, verified passing when recorded)
- `tests/system/test_scaffold_unity_project_cli.py::TestScaffoldUnityProjectCli::test_output_exists_refusal` (pytest node id, verified passing when recorded)
- `tests/system/test_scaffold_unity_project_cli.py::TestScaffoldUnityProjectCli::test_not_a_unity_project` (pytest node id, verified passing when recorded)
