...
WHY: T-4517's land left COV002 quarantine findings on src/frob/lang/
_support.py and src/frob/testing/_collect_csharp.py -- no open ticket's
scope covered either file once T-4517 closed to DONE. T-4561's own
declared scope (both files, plus docs/modules/testing.md and the new
test file) already satisfies COV002's "an open ticket whose scope glob
covers its file" clause structurally, so this ticket exists to close the
residue with real coverage rather than lean on that clause alone or a
frob:waive.

Fix: added tests/unit/test_collect_csharp.py -- 19 focused unit tests
exercising _collect_csharp.py's private helpers directly (_find_cs_files,
_match_dir_excluded, _cs_attribute_names, _cs_test_methods, _cs_node_id,
_collect_cs_file, _csharp_content_key, _load_cache/_store_cache) rather
than only through collect_csharp_tests's existing integration-style
coverage in tests/test_testing.py, plus one test pinning
_capability_test_discovery_status("csharp") as IMPLEMENTED, mirroring the
pre-existing kotlin coverage for the same _support.py function.

Measurement: `frob check --only coverage --files
src/frob/lang/_support.py,src/frob/testing/_collect_csharp.py,tests/unit/
test_collect_csharp.py --base dev` -- COV002 on the two T-4517 files: 0
both before (scope-covered) and after; docs/modules/testing.md already
carried T-4517's public-api anchors from the original land, so no doc
change was needed.
...
```
 ...
 src/frob/gates/_land_parity.py                     |    91 +-
 ...
 tickets/T-draft-667e124d/ticket.md                 |    37 +
 ...
 uv.lock                                            |     2 +-
 3365 files changed, 432347 insertions(+), 17074 deletions(-)
```

### Evidence
- `tests/unit/test_collect_csharp.py::TestCapabilityTestDiscoveryStatusCsharp::test_csharp_test_discovery_is_implemented` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_content_key_changes_when_file_content_changes` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_load_cache_key_mismatch_is_a_miss` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_load_cache_missing_file_is_a_miss` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_load_cache_unreadable_json_is_a_miss` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_store_cache_writes_sorted_node_ids` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestContentKeyAndCache::test_store_then_load_round_trips` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsAttributeNamesAndTestMethods::test_attribute_names_reads_bare_names_off_a_method_node` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsAttributeNamesAndTestMethods::test_parameterized_test_case_collapses_to_one_qualname` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsAttributeNamesAndTestMethods::test_setup_method_is_excluded` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsAttributeNamesAndTestMethods::test_test_attributed_method_is_found_with_its_qualname` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsNodeIdAndCollectCsFile::test_collect_cs_file_returns_node_ids` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsNodeIdAndCollectCsFile::test_node_id_shape` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestCsNodeIdAndCollectCsFile::test_unreadable_file_returns_empty_list_not_a_raise` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestFindCsFiles::test_finds_nested_cs_files_only` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestFindCsFiles::test_no_cs_files_is_empty_list` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestMatchDirExcluded::test_descendant_match` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestMatchDirExcluded::test_exact_dir_match` (pytest node id, verified passing when recorded)
- `tests/unit/test_collect_csharp.py::TestMatchDirExcluded::test_unrelated_dir_is_not_excluded` (pytest node id, verified passing when recorded)
