...
A gate stage is not a verb: frob check already ran dup/arch/cycle/bind
narrative/exports as analyses, so having each also be a top-level verb
doubled the CLI surface for zero new capability. Folds them into frob
check --only <stage> through deprecation shims (sunset 2026-12-01), adds
--list-stages for discoverability, and splits exports into its unchanged
CHECK half (--only exports) and its GENERATE half (frob scaffold
exports, owner decision). mutate/coverage/perf turned out, on measurement,
to have write-shaped standalone implementations genuinely distinct from
their same-named read-only gates -- folding them would have silently
broken the write path frob check's own PERF009 rule and CI coverage
refresh depend on, so they stay top-level, documented as a reasoned
deviation in the amended acceptance[2]. pool/profile stay top-level too,
per the ticket's own fallback clause (check has no subcommand tree to
graft them onto without a grammar collision risk).
...
```
 ...
 docs/design/registry/check-coverage.yaml           |   132 +-
 ...
 src/frob/_cli_parsers/_core.py                     |   303 +-
 src/frob/_cli_parsers/_design.py                   |    47 +-
 src/frob/_cli_parsers/_explore.py                  |    91 +-
 src/frob/_cli_parsers/_misc.py                     |    85 +-
 src/frob/_cli_parsers/_ops.py                      |    66 +-
 src/frob/_cli_parsers/_quality.py                  |    29 +-
 src/frob/_cli_parsers/_reporting.py                |    46 +-
 ...
 src/frob/_cli_parsers/_ticket/_metadata.py         |    57 +-
 src/frob/_cli_parsers/_ticket/_new.py              |    49 +-
 src/frob/_cli_parsers/_ticket/_progress.py         |   156 +-
 src/frob/_cli_parsers/_verify.py                   |    16 +-
 src/frob/app/_check_chunking_baseline.py           |     9 -
 src/frob/app/_config_external.py                   |    36 +-
 ...
 src/frob/app/ticket_runner/_land_cmd.py            |   707 +-
 src/frob/app/ticket_runner/_lifecycle.py           |   109 +-
 ...
 src/frob/gates/_fix_engine_text.py                 |    50 +-
 src/frob/gates/_fix_engine_tier_b.py               |     4 -
 src/frob/gates/_flag_coverage.py                   |    32 +-
 src/frob/gates/_fmt_directives.py                  |    65 +-
 ...
 src/frob/testing/_coverage_wait.py                 |    38 +-
 ...
 tests/system/test_cli_sys_audit.py                 |     7 +-
 tests/system/test_cli_ticket.py                    |    23 +-
 tests/system/test_fleet_status_ground_truth.py     |    25 +-
 tests/system/test_frob_self_model.py               |   240 +-
 tests/system/test_run_helper_env_leak.py           |    25 +-
 tests/system/test_scaffold_unity_project_cli.py    |    99 +
 ...
 tests/test_check_runner.py                         |   113 +-
 ...
 tests/test_hook_frob_suggest.py                    |   266 +-
 ...
 tests/test_ticket_leases.py                        |   456 +-
 tests/test_ticket_leases_cross_worktree.py         |   190 +-
 tests/test_ticket_reconcile.py                     |   154 +-
 tests/test_ticket_store_stale_snapshot.py          |    18 +-
 tests/test_ticket_work_and_land_finish.py          |   248 +-
 tests/test_tickets.py                              |   237 +-
 ...
 tests/test_tickets_parent.py                       |   288 +-
 tests/test_tickets_priority.py                     |   109 +-
 ...
 tests/unit/graph/test_dsl.py                       |   319 +-
 ...
 tests/unit/perf/test_cache_effects.py              |    87 +
 tests/unit/perf/test_hotpath_smells.py             |    24 +-
 tests/unit/perf/test_loop_variant.py               |    85 +
 ...
 tests/unit/rapid_sweep_suite/test_sweep_run.py     |    34 +-
 ...
 tests/unit/strata/test_outbound_destination.py     |   178 +
 ...
 tests/unit/test_check_scoped_files.py              |   627 +
 ...
 tests/unit/test_cli_shims.py                       |   248 +
 ...
 tests/unit/test_graph_hierarchy.py                 |    61 +
 ...
 tests/unit/test_layering_gate.py                   |   137 +
 ...
 tests/unit/test_run_commands.py                    |   177 +
 ...
 tests/unit/test_wrapper_drift.py                   |   158 +
 ...
 tests/unit/verify/test_drain.py                    |     7 +-
 ...
 uv.lock                                            |     2 +-
 2956 files changed, 369195 insertions(+), 15827 deletions(-)
```

### Evidence
- `tests/unit/test_check_only_stages.py::TestDupGoldenComparison::test_check_dup_gate_reports_the_planted_duplicate` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestDupGoldenComparison::test_flat_dup_runner_reports_the_same_duplicate` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestListStages::test_folded_stages_is_the_documented_set` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestListStages::test_every_folded_stage_is_a_known_check_only_name` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestExportsSplit::test_scaffold_exports_leaf_is_registered` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestExportsSplit::test_scaffold_exports_dispatches_to_the_unchanged_exports_runner` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestPoolProfileCarveOut::test_pool_and_profile_are_not_check_only_stages` (pytest node id, verified passing when recorded)
- `tests/unit/test_check_only_stages.py::TestPoolProfileCarveOut::test_pool_and_profile_remain_working_top_level_verbs` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [2] replace: 'Given frob check --list-stages, when it runs, then every folded stage name is printed and each printed name is accepted by frob check --only' -> 'Given frob check --list-stages, when it runs, then it prints exactly the folded stages (dup arch cycle bind perf mutate coverage narrative exports) and every printed name is accepted by frob check --only; pool, profile, debt, deprecated and parse are absent from that list because none of them is a check stage' (reason: 2026-09-19 coordinator review: pool/profile mutate state, debt/deprecated are read-only listings that move to T-4695 under explore, parse is a tool-output adapter that moves to T-4698 verdict form; logan, 2026-09-19)
- [3] replace: 'Given the exports generate mode, when the check half folds into frob check --only exports, then the generate half is reachable under its new home and a test exercises it' -> 'Given the exports verb is split, when this ticket closes, then its CHECK half runs as frob check --only exports and its GENERATE half (generate __init__.py from public symbols in a package directory) is reachable under frob scaffold -- owner decision 2026-09-19, scaffold not refactor -- and a test exercises each half' (reason: 2026-09-19 owner decision: exports check half into check --only, generate half under scaffold; logan, 2026-09-19)
- [2] replace: 'Given frob check --list-stages, when it runs, then it prints exactly the folded stages (dup arch cycle bind perf mutate coverage narrative exports) and every printed name is accepted by frob check --only; pool, profile, debt, deprecated and parse are absent from that list because none of them is a check stage' -> 'Given frob check --list-stages, when it runs, then it prints exactly the folded stages (dup arch cycle bind perf coverage narrative exports) and every printed name is accepted by frob check --only; pool, profile, debt, deprecated and parse are absent from that list because none of them is a check stage; mutate is ALSO absent -- it requires a mandatory single-file argument (mutate one file, report which mutants survived) that no other --only stage takes, so it stays a top-level frob mutate <file> verb, undeprecated, not folded' (reason: measured 2026-09-21 while implementing T-4692: frob mutate's own signature (cfg.mutate_file, required) is incompatible with --only's zero-arg whole-tree stage shape every other folded name shares; the ticket's own pool/profile carve-out ('your call with a reason') is the closest precedent and is applied here by the same logic; logan, 2026-09-21)
- [2] replace: 'Given frob check --list-stages, when it runs, then it prints exactly the folded stages (dup arch cycle bind perf coverage narrative exports) and every printed name is accepted by frob check --only; pool, profile, debt, deprecated and parse are absent from that list because none of them is a check stage; mutate is ALSO absent -- it requires a mandatory single-file argument (mutate one file, report which mutants survived) that no other --only stage takes, so it stays a top-level frob mutate <file> verb, undeprecated, not folded' -> "Given frob check --list-stages, when it runs, then it prints exactly the folded stages (dup arch cycle bind narrative exports) and every printed name is accepted by frob check --only; pool, profile, debt, deprecated and parse are absent because none of them is a check stage. mutate, coverage and perf are ALSO absent, each for a measured reason (2026-09-21, while implementing T-4692): frob mutate requires a mandatory single-file argument no --only stage takes; frob coverage's own docstring is 'refresh coverage.xml... via native_coverage_refresh' -- a WRITE operation, not a check, genuinely distinct from the read-only COV001-008 gate --only coverage already names; frob perf's profile/collect subcommands likewise WRITE telemetry/ratchet artifacts (frob perf collect is PERF009's own data producer) that the read-only PERF001-009 perf_gate --only perf already names only READS -- shimming any of the three flat verbs to --only would silently stop them doing the write their callers (including frob check's own PERF009 rule, T-0712's regression ratchet) depend on. All three stay top-level verbs, undeprecated, not folded -- the same reasoning the ticket's own pool/profile carve-out already applies, generalized to every verb this ticket's initial fold list named that turns out to mutate state" (reason: measured 2026-09-21: coverage_gate/perf_gate (frob.gates) are read-only findings gates genuinely distinct from frob coverage/frob perf's own write-shaped implementations (native_coverage_refresh writes coverage.xml; frob perf collect writes the PERF009 ratchet artifact) -- folding the verb into --only would silently break the write path every consumer of the verb (including check's own PERF009 rule) depends on; logan, 2026-09-21)
