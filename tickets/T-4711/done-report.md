...
Why: T-4703 measured 30,915 directive lines with only 88 carrying two or more
directives -- most stacks are same-kind repeats that a multi-target header would
collapse. This leaf builds the grammar (comma-separated frob:tests/frob:doc) and
documents the one continuation form the later merge/wrap leaves depend on, plus
the DSL001 check that names a mid-token break instead of a generic parse error.

What: frob:tests/frob:doc parse a comma-separated target list into one Edge per
target (comma inside a quoted value is never a separator); _parse_line keeps
returning a bare Edge for the single-target case (no behavior change for the
30,915 existing lines) and a list[Edge] only for a genuine multi-target line.
A join landing inside the bare target token now gets a DSL001-named reason
instead of the generic "bad attribute syntax" message; a quoted target/value or
a no-space identifier-spanning continuation (the repo's existing convention)
is untouched.

Left open: none in scope. Leaf 3 (T-4712) narrows frob fmt's own wrapper to
respect the same token-separation boundary this leaf's parser-side check
defines.
...
```
 ...
 src/frob/__main__.py                               |    98 +-
 src/frob/_cli_parsers/__init__.py                  |     5 +
 src/frob/_cli_parsers/_check.py                    |   304 +-
 src/frob/_cli_parsers/_core.py                     |   124 +-
 src/frob/_cli_parsers/_design.py                   |    28 +-
 src/frob/_cli_parsers/_explore.py                  |   102 +-
 src/frob/_cli_parsers/_misc.py                     |    66 +-
 src/frob/_cli_parsers/_ops.py                      |    38 +-
 src/frob/_cli_parsers/_quality.py                  |     7 +-
 ...
 src/frob/app/_config_external.py                   |    33 +-
 ...
 src/frob/app/check_runner.py                       |   201 +-
 src/frob/app/clean_runner.py                       |    57 +-
 src/frob/app/config.py                             |   116 +-
 ...
 src/frob/app/scaffold_runner.py                    |    61 +
 ...
 src/frob/gates/_tickets_gate.py                    |    48 +-
 src/frob/gates/_waive.py                           |   194 +-
 ...
 src/frob/graph/callgraph.py                        |    43 +-
 src/frob/graph/dsl.py                              |   762 +-
 ...
 tickets/T-draft-1c09ccb2/ticket.md                 |    30 +
 ...
 uv.lock                                            |     2 +-
 2908 files changed, 339198 insertions(+), 15568 deletions(-)
```

### Evidence
- `tests/unit/graph/test_dsl.py::TestMultiTargetDirectives::test_multi_target_tests_emits_one_edge_per_target` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestMultiTargetDirectives::test_multi_target_doc_emits_one_edge_per_target` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestMultiTargetDirectives::test_comma_inside_quoted_title_is_not_a_separator` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestMultiTargetDirectives::test_single_target_verbs_are_unaffected_by_a_comma` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestMultiTargetDirectives::test_multi_target_with_empty_entry_is_a_named_refusal` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestDsl001MidTokenBreak::test_join_mid_symbol_path_is_reported_by_dsl001` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestDsl001MidTokenBreak::test_same_target_broken_at_token_boundary_is_clean` (pytest node id, verified passing when recorded)
- `tests/unit/graph/test_dsl.py::TestMultiTargetFoldRoundTrip::test_multi_target_directive_survives_a_continuation_split` (pytest node id, verified passing when recorded)
