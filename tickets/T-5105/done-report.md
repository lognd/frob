...
Ships a structural gate (D-M8) refusing templated assumes so the 33
boilerplate CWE assumes in design/frob.strata stop passing as coverage.
Token-level substitution comparison (never lexical) satisfies the owner
directive that checks decide from parsed symbols, not keywords/regex.
Positive control (test_red_on_todays_design_frob_strata) proves the gate
fires on today's monolith; a green run there would mean the gate is
inert, same failure mode as T-4663's inert layering check.
...
```
 ...
 tickets/T-5105/done-report.md                      |  1369 ++
 ...
 uv.lock                                            |     2 +-
 2822 files changed, 286384 insertions(+), 15064 deletions(-)
```

### Evidence
- `tests/gates_suite/test_sys_assume_template.py::TestSelfaudit001TemplatedAssume::test_red_on_todays_design_frob_strata` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_sys_assume_template.py::TestFindTemplatedAssumes::test_distinct_weakness_class_not_merged` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_sys_assume_template.py::TestFindSharedExpiry::test_red_on_monolith_shared_date` (pytest node id, verified passing when recorded)
- `tests/gates_suite/test_sys_assume_template.py::TestSelfaudit001TemplatedAssume::test_module_owned_specific_assume_not_reported` (pytest node id, verified passing when recorded)
