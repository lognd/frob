...
sqlfluff needed a real frob-owned plugin home and a tool-gating category
that fails the SQL family (not just a gate) when it is missing, per the
owner's explicit REQUIRED_FOR_FAMILY directive -- OPTIONAL_FOR_GATE would
have silently folded a missing sqlfluff into "gate advisory", not a real
family-wide failure, when SQL surface genuinely exists in the repo.
Reusing frob.sql._extract.sql_relevance (T-5334) rather than
re-detecting SQL surface keeps the "repo has no SQL at all" case
correctly silent (no never-fail flag needed). Four of the fourteen named
rules (SELECT *, UPDATE/DELETE without WHERE, HAVING without an
aggregate, OFFSET pagination) fit a single-statement sqlfluff AST rule
cleanly; the other ten need schema/session state this leaf's declared
scope cannot reach, so they are filed as T-5446 per the
ticket's own contingency clause rather than forced to fit.
...
```
 ...
 src/frob/sql/_sqlfluff_plugin.py                   |   205 +
 ...
 src/frob/webapp/_a11y_statement.py                 |   244 +
 ...
 tests/unit/sql/test_sqlfluff_plugin.py             |   106 +
 ...
 tests/unit/test_webapp_a11y_statement.py           |    64 +
 ...
 tickets/T-5324/ticket.md                           |   135 +
 tickets/T-5325/ticket.md                           |   169 +
 ...
 tickets/T-5335/ticket.md                           |    76 +
 tickets/T-5336/ticket.md                           |    38 +
 tickets/T-5337/ticket.md                           |    82 +
 ...
 uv.lock                                            |     2 +-
 4247 files changed, 712304 insertions(+), 23512 deletions(-)
```

### Evidence
- `tests/unit/test_doctor.py::TestFamilyRequiredToolFindings::test_sql_relevant_missing_sqlfluff_is_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestFamilyRequiredToolFindings::test_no_sql_missing_sqlfluff_is_not_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestFamilyRequiredToolFindings::test_sql_relevant_present_sqlfluff_is_not_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestFamilyRequiredToolFindings::test_sqlfluff_is_required_for_family_category` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_select_star_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_named_columns_are_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_update_delete_without_where_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_update_delete_with_where_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_having_without_aggregate_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_having_with_aggregate_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_offset_pagination_is_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_limit_without_offset_is_not_flagged` (pytest node id, verified passing when recorded)
- `tests/unit/sql/test_sqlfluff_plugin.py::test_get_rules_registers_all_four` (pytest node id, verified passing when recorded)
