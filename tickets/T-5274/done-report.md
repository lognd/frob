...
DSTACK001's lint half (frob.gates._directive_stack.stack_lint_violations)
and its Tier-A merge fix (frob.gates._fix_engine_text.fix_dstack001_merge)
shipped tested but unreachable since T-4713 -- neither run_gates nor
TIER_A_HANDLERS ever called them, so the rule silently never fired.
This ticket wires stack_lint_violations into run_gates' self-check
violation set, registers fix_dstack001_merge in TIER_A_HANDLERS, and
adds the [gates] dstack_threshold frob.toml knob T-4713's own lease
collision with T-4663 blocked at the time. A new end-to-end positive
control (through the real run_gates/apply_tier_a_fixes entry points,
not synthetic Edge objects) proves the wiring fires and the fix is
idempotent; the pre-existing closed-set TIER_A_HANDLERS coverage test
is extended with DSTACK001.
...
```
 ...
 uv.lock                                            |     2 +-
 3738 files changed, 491965 insertions(+), 22382 deletions(-)
```

### Evidence
- `tests/gates_suite/test_fix_engine.py::TestDstack001Wiring::test_dstack001_fires_through_run_gates_and_fix_is_idempotent` (pytest node id, verified passing when recorded)
