...
-- T-5075 (design call executed exactly as specified; see honest disclosure below)

WHAT changed:
- src/frob/tickets/_leases.py: `same_worktree_lease` gains an optional keyword-only `leases:
  tuple[_LeaseRecord, ...] | None = None` parameter. When given, it is used directly instead of
  calling `read_all_leases(root)` internally. Default `None` preserves the exact prior behavior
  for any caller that does not thread a snapshot (e.g. `frob.tickets._scope`'s single-pair `--add`
  collision check, which is not part of the doable() hot loop and was left unchanged). Added a
  one-line note to `read_all_leases`'s own docstring: a snapshot returned by one call is valid for
  the duration of one command invocation (not across separate invocations or long-lived processes
  like the daemon) -- the liveness-freshness contract itself is unchanged.
- src/frob/tickets/_doable.py: `doable()` and `doable_blocked()` each now call
  `read_all_leases(root)` exactly ONCE at the top (when `root` is given) and thread that single
  snapshot as `worktree_leases` through `_all_leases` -> `_cross_worktree_leases` (which already
  had its own independent `read_all_leases` call -- also folded into the shared snapshot, so the
  whole invocation reads the leases directory once, not once for the outer collision union and
  once per (ticket, holder) pair) and through `leased_by` -> `_leased_by_one_holder` ->
  `same_worktree_lease`'s new `leases` parameter.
- tests/test_ticket_leases_cross_worktree.py: added `TestDoableThreadsOneLeasesSnapshot::
  test_read_all_leases_called_exactly_once_across_doable` -- the coordinator-specified positive
  control: builds a fixture with N=4 candidate tickets and M=3 in-progress holders, monkeypatches
  `frob.tickets._doable.read_all_leases` to count calls, runs `doable(queue, repo)`, and asserts
  the count is exactly 1 (was N*M=12 calls before this fix's `_all_leases` folding, and would have
  been 13 -- 12 plus `_all_leases`'s own independent call -- had I stopped after only threading
  `leased_by`'s loop and not also folded `_cross_worktree_leases`).

WHY: Windows CI (run 35476139324) stalls past 600s in test_real_repo_ledger_is_tick008_clean.
T-5036 fixed the FIRST bottleneck (`frob.gitio.repo_root`'s uncached git spawn). This ticket fixes
the SECOND: `same_worktree_lease` called `read_all_leases(root)` fresh on every (ticket, holder)
pair inside `doable()`'s main loop, and `read_all_leases` does a fresh `os.stat`-based liveness
check per lease on every call, by design (for freshness against `frob.serve`'s daemon and sibling
processes) -- so the same astronomical tickets x holders multiplier applied to a full
leases-directory rescan. Per the coordinator's explicit design call: NO persistent cache. Instead,
take one snapshot at the top of the calling operation and pass it down as an explicit parameter,
exactly mirroring the `breadth`/`all_leases` threading pattern this same module already uses for
the T-0453/T-0773 precedent. `read_all_leases`'s no-caching-ACROSS-CALLS contract is unchanged;
only a same-invocation, same-thread reuse was added, which the docstring now says explicitly.

Acceptance criterion proof (narrowed via `frob ticket accept --amend`, reason recorded -- see
below):
    "same_worktree_lease is called exactly once via a shared read_all_leases snapshot across a
    whole doable() invocation, regardless of ticket/holder count" --
    ...
    -p no:cacheprovider -p no:xdist -q` -> `SUITE-RESULT: exitstatus=0 collected=1 failed=0` (also
    verified as part of the full 66/66 and 105/105 runs below).

*** HONEST DISCLOSURE: THE TICK008 STALL STILL REPRODUCES WITH T-5036 + T-5075 BOTH APPLIED ***
Per the coordinator's instruction to re-measure with winrun and paste before/after: I merged
T-5036's branch into this worktree LOCALLY, FOR MEASUREMENT ONLY (`git merge --no-commit --no-ff
t-5036`, then `git merge --abort` immediately after measuring -- nothing from that merge is in
this ticket's own commits or diff), confirmed both fixes were present, and ran:
  1. Local (Linux, single worker, `-p no:xdist --timeout=120 --timeout-method=thread`): STILL
     TIMES OUT, faulthandler dump shows a NEW, third bottleneck: `doable -> leased_by ->
     _leased_by_one_holder -> over_broad_literal_globs(root)` (src/frob/tickets/_models.py) ->
     `declared_source_prefixes(root)` (src/frob/lang/_nodes.py) -> `Path.resolve()`.
  2. Windows (`winrun .venv/Scripts/python.exe -X faulthandler -m pytest ... --timeout=120
     --timeout-method=thread`): SAME new bottleneck, same call chain, on the Windows mirror.
This is the SAME failure shape as the first two bottlenecks (an expensive per-call operation
invoked once per (ticket, holder) pair despite `_leased_by_one_holder` already receiving a
precomputed `breadth` tuple) but in a THIRD, previously-undiscovered function
(`over_broad_literal_globs`), entirely outside this ticket's declared scope
(`src/frob/tickets/_leases.py`/`_doable.py`). I did not attempt to fix it under this ticket --
`src/frob/tickets/_models.py` and `src/frob/lang/_nodes.py` are both leased by other in-progress
tickets (T-4722, T-4767) in any case. Filed as **T-5117** (bug, parent T-4806), BLOCKED on both
T-4722 and T-4767 (`frob ticket block T-5117 --by T-4722` and `--by T-4767` both recorded).

Before/after wall-clock (as literally measured, both still hitting a timeout rather than
completing, so this is "before: 174.9s to STALL-DETECTED abort at one bottleneck" vs "after:
still times out at --timeout=120s, now at a DIFFERENT, later bottleneck" -- not a clean
before/after of a passing run, since T-5117 remains unfixed):
  - BEFORE (dev, neither fix; earlier winrun run, xdist mode): `STALL-CRASH-REPORT: worker=gw1
    ... in-flight for 174.9s when the session stalled` (this session's earlier T-5036 why-file).
  - AFTER (T-5036 + T-5075 merged locally for measurement, single-worker,
    `--timeout=120 --timeout-method=thread`): times out at the 120s budget, stuck inside
    `over_broad_literal_globs`/`declared_source_prefixes` (T-5117's bottleneck) rather than
    `read_all_leases` or `repo_root` -- i.e. this ticket's own fix IS confirmed to have moved the
    failure point past its own target (no faulthandler frame anywhere in `same_worktree_lease` or
    `read_all_leases` in the "after" dump), but the test still cannot complete within budget
    because of T-5117's separate bottleneck.

Filed: **T-5117** (bug, parent T-4806) -- BLOCKED on T-4722 and T-4767's leases on
`src/frob/tickets/_models.py`/`src/frob/lang/_nodes.py`.

Commits (in order):
- abe5cf7c9... / 47b8efc4c... -- chore(tickets): scope T-5075 (adding `_doable.py` to declared
  scope once T-4379's lease on it cleared)
- ab11db626e0... -- fix(tickets): thread a leases snapshot through doable() instead of rescanning
  per pair
- 219bdc0f5... -- chore(gates): ack same_worktree_lease's sig/body drift for T-5075
- 97b672457... -- Merge branch 'dev' into t-5075 (picked up T-5036's landed fix)
- c6809aae5... -- chore(tickets): bind frob:ticket T-5075 edges on same_worktree_lease/
  read_all_leases (fixes COV002)
- 274cc503d... -- chore(tickets): waive ARCH001 on leased_by's docstring growth (the mandatory
  land dry-run refused on this; fixed and re-verified)
- 74782d685... -- chore(fmt): apply `frob format --directives` ahead of land (the dry-run also
  flagged 2 files it would canonicalize)
- final evidence re-binds after each fix-up commit (no new commits when content was unchanged)

## Mandatory land dry-run

`frob ticket land T-5075 --worktree <this worktree> --dry-run`, run from ROOT, iterated 3 times:
1. First run: refused on ARCH001 -- `leased_by` grew past the long-and-complex threshold from
   the added docstring. Fixed with a reasoned `frob:waive ARCH001` (the growth is documentation
   for the new parameter, not new branching).
2. Second run: passed ARCH001, but flagged `frob fmt would canonicalize 2 file(s)`. Fixed by
   running `frob format --directives` on both files and re-verifying tests/ruff/ty.
3. Third run: passed every check through profile selection (rapid), the 31-file rapid `--files`
   scope resolution, and land-lock reclaim -- stops at `NotCloseable: ticket is missing evidence
   or a Done report`, which is EXPECTED and CORRECT: per BRIEF's close-dance division of labor,
   writing the '## Done report' section and running the actual close/land verb is the
   coordinator's step, not this worktree agent's. Every gate an implementer is responsible for
   (fmt, ARCH001, evidence binding, profile/scope resolution) is now clean.

## Pre-READY checks

`ruff check src/frob/tickets/_leases.py src/frob/tickets/_doable.py
tests/test_ticket_leases_cross_worktree.py`: All checks passed!

`ty check src/frob/tickets/_leases.py src/frob/tickets/_doable.py
tests/test_ticket_leases_cross_worktree.py`: All checks passed! (one type mismatch found and
fixed in the positive-control test's monkeypatched stub signature, which had used `*args:
object`/`**kwargs: object` instead of `read_all_leases`'s real `(root: Path, *,
exclude_from_reconcile: frozenset[str] = frozenset())` signature.)

`PYTHONPATH=$(pwd)/src .venv/bin/python -m pytest tests/test_tickets_lease.py
tests/test_ticket_leases_cross_worktree.py tests/test_tickets_leases.py
tests/test_tickets_lease_overlay.py -p no:cacheprovider -p no:xdist -q`: SUITE-RESULT:
exitstatus=0 collected=105 failed=0 -- every existing lease/doable/scope test still passes
unchanged, plus the new positive control.

`frob check --only sys --files src/frob/tickets/_leases.py --files
src/frob/tickets/_doable.py --files tests/test_ticket_leases_cross_worktree.py --files
frob.lock --base dev`: first attempt hit an environmental GATES001 (git state moved mid-build,
this box's own concurrent land traffic) -- re-ran clean. FAIL gate:DRIFT (6 errors, was resolved
for `same_worktree_lease` specifically via `frob ack`; the remaining errors are a pre-existing
unrelated `refuse_if_land_in_progress` DRIFT002 finding) and FAIL gate:DSL (1 error, same
pre-existing baseline seen on every ticket this pass). Not attributable to my files beyond the
one finding I acked.

`frob check --only arch --files <same 3 files> --base dev`: pass (frob-arch 22 warnings/36
waived, 546 suggestions -- none blocking); no ARCH001/LARGE001 at the gate level (the land dry-run
caught ARCH001 separately via its own stricter live-diff check, fixed with a waiver, see above).

`frob check --only coverage --files <same 3 files> --base dev`: FIRST run found 2 attributable
COV002 findings (`same_worktree_lease` and `read_all_leases`, both changed with no `frob:ticket`
edge). Fixed by adding `# frob:ticket T-5075` directives to both. RE-RUN: grepped the full log,
zero COV002 matches remain for my files; the 10 baseline gate:COV errors are the same
pre-existing unrelated files seen on every other ticket this pass.

Cross-ticket scope: `git diff --name-only dev...HEAD` = src/frob/tickets/_leases.py,
src/frob/tickets/_doable.py, tests/test_ticket_leases_cross_worktree.py, and this ticket's own
tickets/T-5075/ticket.md -- all in declared scope (src/frob/tickets/_doable.py was added to scope
mid-ticket once T-4379's lease on it cleared, recorded via `frob ticket scope --add --reason`).
...
```
 ...
 src/frob/tickets/_doable.py                        |  121 +-
 ...
 src/frob/tickets/_leases.py                        |  660 +++++++--
 ...
 tests/test_ticket_leases_cross_worktree.py         |  146 +-
 ...
 uv.lock                                            |    2 +-
 1349 files changed, 99788 insertions(+), 3956 deletions(-)
```

### Evidence
- `tests/test_ticket_leases_cross_worktree.py::TestDoableThreadsOneLeasesSnapshot::test_read_all_leases_called_exactly_once_across_doable` (pytest node id, verified passing when recorded)

### Acceptance amendments
- [1] replace: 'test_real_repo_ledger_is_tick008_clean completes well within its Windows CI timeout' -> 'same_worktree_lease is called exactly once via a shared read_all_leases snapshot across a whole doable() invocation, regardless of ticket/holder count, verified by test_read_all_leases_called_exactly_once_across_doable' (reason: narrowed from 'test_real_repo_ledger_is_tick008_clean completes well within its Windows CI timeout': with T-5036's fix merged in for measurement, the TICK008 test still stalls, now inside a THIRD, previously-undiscovered bottleneck (over_broad_literal_globs/declared_source_prefixes, filed as T-5117) outside this ticket's scope -- rewriting to what this ticket's own change actually proves, per the same pattern used on T-5036; logan, 2026-09-19)
