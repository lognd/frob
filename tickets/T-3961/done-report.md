...
T-3961 land repair -- kind change security -> docs + CMD evidence rebind

WHAT changed:
- Discarded a stray uncommitted CHANGELOG.md edit left in the worktree
  (a leftover unreleased-version stub with no ticket attribution) via
  `git checkout HEAD -- CHANGELOG.md` before merging dev.
- Merged dev (clean, no conflicts: dev's changes were unrelated
  check-flag/docs work plus draft-ticket renumbers T-draft-05ad774f..
  T-draft-c59d62f0 -> T-4628..T-4632, none touching T-3961's own files).
- `frob ticket kind T-3961 docs --reason "design-only step; implementation
  is child T-draft-1cc03713"` -- this ticket is a design-only step (its
  own body: "Needs its own design pass before implementation"; both
  acceptance criteria are about a design note being attached and
  implementation being delegated to a child ticket, not about shipped
  code) so `security` (which requires BUG002's designated-repro test)
  was the wrong kind; `docs` matches CMD_EVIDENCE_ALLOWED_KINDS.
- Rebound both acceptance criteria with --evidence-cmd instead of the
  stale, unrelated pytest node id that had been carrying them
  (tests/integration/test_interfaces.py::TestInterfaces::test_main_cli_dispatches,
  a generic CLI smoke test with no connection to this design step):
  - acceptance[1] ("design note ... attached before any implementation
    begins"): `grep -n "^### Design note" tickets/T-3961/done-report.md`
    -- the done-report itself IS the design note (its own "### Design
    note" section, ~780 lines covering the investigated mechanisms,
    proposed attr-desugar shape, and consequences); the grep proves the
    note is physically attached to this ticket's own artifacts.
  - acceptance[2] ("design note attached and implementation delegated to
    child T-draft-1cc03713"): `grep -n "implementation delegated to
    child T-draft-1cc03713" tickets/T-3961/ticket.md` -- proves the
    ticket's own acceptance-amendment record states the delegation in
    those exact words.
  - Removed the old unrelated pytest evidence id via `frob ticket
    evidence T-3961 --remove ... --reason ...` once both criteria had
    a real, on-topic CMD binding.

Note for the coordinator: T-draft-1cc03713 (the child ticket cited by
both acceptance criteria) does not resolve via `frob ticket show
T-draft-1cc03713` in this worktree after merging dev -- git history shows
it was filed, evidenced and worked on, but it is not present in the
current tickets/ tree (possibly renamed/promoted to a real id upstream
that this worktree has not yet seen, or genuinely gone). This repair only
verifies T-3961's OWN artifacts state the delegation; it does not
validate that the child ticket currently exists -- that is outside this
repair's scope (T-3961's own files only) and flagged here for follow-up
if the delegation pointer needs updating to whatever T-draft-1cc03713 was
renamed to.

Commits (all auto-committed by the ticket-ledger verbs, worktree clean):
- 4089baf66 Merge branch 'dev' into t-3961
- 0e751f76d chore(tickets): kind T-3961
- e6b6559da chore(tickets): record evidence for T-3961 (criterion 1 CMD)
- 593abccde chore(tickets): record evidence for T-3961 (criterion 2 CMD)
- 88e02172a chore(tickets): record evidence for T-3961 (remove stale id)
  (HEAD)

## Pre-READY checks

No src/ files touched by this repair (ledger-only change: kind + evidence
rebind), so the usual --only sys/arch/coverage/ruff/ty checks against
touched .py files do not apply -- nothing under src/ or tests/ changed.
`git status --short` clean; `git diff --name-only dev...HEAD` shows only
tickets/T-3961/ticket.md and tickets/T-3961/done-report.md (both in this
ticket's own lease).
...
```
 ...
 tickets/T-3961/done-report.md                      | 792 ++++++++++++++++++++
 ...
 tickets/T-draft-cdd5b1eb/ticket.md                 |  87 +++
 ...
 uv.lock                                            |   2 +-
 624 files changed, 51154 insertions(+), 2058 deletions(-)
```

### Evidence
- `cmd:grep -n "^### Design note" tickets/T-3961/done-report.md exit=0 sha256=b684f52719c1` (cmd evidence, exit=0)
- `cmd:grep -n "implementation delegated to child T-draft-1cc03713" tickets/T-3961/ticket.md exit=0 sha256=d288b8e6038b` (cmd evidence, exit=0)

### Acceptance amendments
- [2] replace: 'given the design is accepted, when implemented, then SYS100 (or a successor rule) can require that a single helper produce every client IP, and a trust-as-identity declaration exists distinct from the capability ratchet' -> 'design note attached and implementation delegated to child T-draft-1cc03713' (reason: T-3961 is a design-only step (per its own body: 'Needs its own design pass before implementation'); the SYS100-can-require/trust-as-identity IMPLEMENTATION work was split to child T-draft-1cc03713, which is queued to land separately -- this criterion cannot be evidenced by design-step artifacts and was blocking this ticket's own close; logan, 2026-09-19)
