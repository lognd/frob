...
Adds lighthouse to _RELEVANT_TOOLS so a repo relevant to dynamic Core
Web Vitals measurement (WEBPERF101-108) that lacks lighthouse gets a
real RelevantToolFinding instead of silence, mirroring T-5324's
axe-core/pa11y compound-predicate pattern exactly. Scope was narrowed
to drop docs/modules/gates.md after a lease collision with in-progress
T-3068; the lighthouse doc addition lives entirely in install.md's
tool paragraph per the coordinator's instruction, so nothing was lost.
Followed the ticket body's own text ("absence is a failing UNMEASURED
RelevantToolFinding") over a looser paraphrase, since the registry has
no never-fail flag and every existing row uses this same contract.
...
```
 ...
 uv.lock                                            |   265 +-
 4537 files changed, 790367 insertions(+), 23547 deletions(-)
```

### Evidence
- `tests/unit/test_doctor.py::TestLighthouseRelevant::test_lighthouse_relevant_and_missing_is_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLighthouseRelevant::test_lighthouse_irrelevant_missing_is_not_a_finding` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestLighthouseRelevant::test_lighthouse_relevant_present_is_not_a_finding` (pytest node id, verified passing when recorded)
