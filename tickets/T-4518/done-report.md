...
T-4518 is a story ticket; all deliverables already landed as leaves over the
past week, this closes the story rather than adding new code.

Item -> landed leaf:
- Unity project detection (Assets/, Packages/manifest.json,
  ProjectSettings/ProjectVersion.txt) -> T-4515 (src/frob/lang/_project_detect.py)
- Walker excludes Library/, Temp/, Logs/, obj/, *.meta -> T-4515/excludes work
  (src/frob/excludes.py::_unity_exclude_globs)
- One strata node per .asmdef, unity-project scaffold type, starter frob.toml ->
  T-4503 (src/frob/scaffold/_unity_project.py, scaffold/data/types/unity-project/**)
- frob doctor recognizing the Unity toolchain (Unity Hub / editor path) ->
  T-4501 (src/frob/doctor.py::UnityEditorStatus, _unity_hub_default_roots)

Verified on dev by rerunning the leaves' own test files directly (21 passed)
plus the T-4509 capstone e2e fixture test (6 passed), which exercises
detection + asmdef-node matching against a real fixture end to end.
...
```
 ...
 uv.lock                                            |     2 +-
 3261 files changed, 424804 insertions(+), 16943 deletions(-)
```
...
- `tests/test_excludes.py::TestUnityExcludeGlobs::test_non_unity_project_adds_nothing` (pytest node id, verified passing when recorded)
- `tests/unit/test_lang_project_detect.py::test_detects_unity_project` (pytest node id, verified passing when recorded)
- `tests/unit/test_doctor.py::TestUnityEditorStatus::test_present_via_env_reports_version` (pytest node id, verified passing when recorded)
