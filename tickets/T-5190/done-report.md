...
The known-gate-rule-id registry (src/frob/gates/_waive.py's _KNOWN_GATE_
RULES) had grown 19 ids behind the live code on dev tip: BASE001,
CLAIM001, CONFIGPATH001, COV010, GUARD001, INV010, INV011, PII005,
PII013, RACE001, RACE002, REL303, ROUTE001, SYS116, SYS117, TESTMOCK001,
WRAP001, WRAP002, WRAP003, plus SYS114/SYS115 (T-4113's own rule ids,
still in-progress -- registered here per coordinator decision so this
WARN-only gate stays green, but T-4113 still owns their full
check-coverage.yaml documentation entry). check-coverage.yaml already
had doc entries for all 19 (a prior half-done registration -- the doc
side landed but the code side never did), just with a stale declared
gate_rule_total (407 vs 412 actual); fixed the denominator only, no new
yaml entries needed (confirmed by removing my first duplicate-entry
attempt after test_gate_rule_entries_match_live_known_rules caught the
1:1 length mismatch). All three target tests green. Found-while-working:
DSTACK001 (T-5274's own new rule) missing from the checked-in
_KNOWN_RULE_FIXABILITY literal, filed separately (T-5350,
out of scope for a rule-id-registration ticket).
...
```
 ...
 src/frob/gates/_fix_engine.py                      |   433 +-
 ...
 tests/gates_suite/test_fix_engine.py               |   217 +
 ...
 tickets/T-5347/ticket.md                           |    52 +
 tickets/T-5348/ticket.md                           |    43 +
 tickets/T-5350/ticket.md                 |    36 +
 ...
 uv.lock                                            |     2 +-
 3755 files changed, 512010 insertions(+), 22394 deletions(-)
```
