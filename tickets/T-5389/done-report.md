...
T-5151's `frob ticket attach --remove PATH`/`--remove-all` parsed
correctly through argparse and had matching AppConfig fields, but their
dests (`ticket_attach_remove_path`, `ticket_attach_remove_all`) were
never added to `_config_external`'s `_STRING_FIELDS`/`_BOOL_FLAGS`
forwarding tuples, so `AppConfig.from_external` silently dropped both
flags and the fields stayed at their defaults regardless of the CLI
(FLAGCOV001). Added both dests to the correct tuples and added a
regression test class following the existing T-2320/T-0749 idiom (real
parser + real `AppConfig.from_external`, not a direct construction) so
this class of gap cannot regress silently again.
...
```
 ...
 uv.lock                                            |     2 +-
 3947 files changed, 608700 insertions(+), 23461 deletions(-)
```

### Evidence
- `tests/unit/test_app_config_flag_coverage.py::TestT5151TicketAttachRemoveFlagsReachAppConfig::test_from_external_carries_remove_path_from_parsed_argv` (pytest node id, verified passing when recorded)
- `tests/unit/test_app_config_flag_coverage.py::TestT5151TicketAttachRemoveFlagsReachAppConfig::test_from_external_carries_remove_all_from_parsed_argv` (pytest node id, verified passing when recorded)
- `tests/unit/test_app_config_flag_coverage.py::TestT5151TicketAttachRemoveFlagsReachAppConfig::test_absent_remove_flags_default_none_and_false` (pytest node id, verified passing when recorded)
