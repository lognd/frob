...
LANDPARITY001's new-public-symbol check only ever scanned the lexical
comment block directly above a symbol's def for a frob:tests directive,
never the graph's derived EdgeKind.TESTS edge T-4710 promises is
equivalent when the directive instead lives on the covering test symbol.
Once T-5289 fixed fix_test010_redundant_test_declaration to correctly
delete a now-redundant production-side frob:tests copy, this check began
refusing lands for symbols that always had real test coverage via the
test-side declaration. Consults the graph (cached load_graph first, a
real build_graph only on a genuine miss, the same pattern
_tdd_order_scoped_edges already uses) for a test-side TESTS edge before
reporting the frob:tests family missing.
...
```
 ...
 tickets/T-5299/done-report.md                      |  3385 +++
 tickets/T-5299/ticket.md                           |    83 +
 tickets/T-draft-b225e6c1/ticket.md                 |    70 +
 ...
 uv.lock                                            |     2 +-
 3361 files changed, 435236 insertions(+), 17074 deletions(-)
```
...
- `tests/unit/test_land_parity_gate.py::TestLandParityDocTestGate::test_new_public_symbol_with_only_test_side_tests_edge_is_quiet` (pytest node id, verified passing when recorded)
- `tests/unit/test_land_parity_gate.py::TestLandParityDocTestGate::test_new_public_symbol_with_neither_lexical_nor_test_side_edge_fires` (pytest node id, verified passing when recorded)
