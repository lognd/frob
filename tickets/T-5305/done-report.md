...
The scope's original filed file (src/frob/app/ticket_runner/_reconcile.py)
did not exist; the real reconcile module is src/frob/tickets/_reconcile.py
(dispatch in _lifecycle.py). Added strip_stale_fields() there: loads
every ticket, checks __pydantic_extra__ (T-0838's existing warning
mechanism) for undeclared fields, and (with --apply) rewrites the
ticket through write_ticket() using only the model's declared fields
so pydantic's extra="allow" no longer carries them forward. Dry-run
by default, matching reconcile's own report-first posture; idempotent
(verified with a dedicated test that runs apply twice and asserts the
second run finds nothing). Wired the CLI flag (--strip-stale-fields),
AppConfig field, and its external-config allowlist entry. All 28 tests
in tests/test_ticket_reconcile.py pass, including 4 new ones for this
feature. This unblocks T-5292 (TICK008 flags the real repo ledger) --
once this lands, running `frob ticket reconcile --strip-stale-fields
--apply` on the real ledger is T-5292's own remaining step, not
duplicated here.
...
```
 ...
 src/frob/app/ticket_runner/_lifecycle.py           |   454 +-
 ...
 tests/test_ticket_reconcile.py                     |   270 +-
 ...
 uv.lock                                            |     2 +-
 3979 files changed, 626253 insertions(+), 23493 deletions(-)
```
