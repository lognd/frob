...
Every WEBSEC session/CSRF rule (cookie flags, timeouts, CSRF wiring) needs
the same few facts out of Django/Flask/Express/Rails' own session config,
each spelled a different way. Parsing each dialect once into one
SessionConfig model here means every downstream rule reads one normalized
shape instead of re-parsing the config file itself -- the exact contract
this ticket's description states, built directly on T-5302's
FrameworkKind/detect_frameworks (never re-detects) and frob.lang's
raw_tree escape hatch. Rails' .rb has no frob.lang grammar entry, so it
parses via tree_sitter_language_pack.get_parser("ruby") directly, scoped
to this one module. Also widened the core node's fs.read via-grant to a
src/frob/webapp/*.py glob (coordinator request) so the six sibling WEBSEC
substrate lands stop contending this same design/frob.strata hunk.
...
```
 ...
 src/frob/webapp/_websec_session_config.py          |   434 +
 ...
 tests/unit/test_webapp_websec_session_config.py    |   160 +
 ...
 uv.lock                                            |     2 +-
 3989 files changed, 614124 insertions(+), 23464 deletions(-)
```

### Evidence
- `tests/unit/test_webapp_websec_session_config.py::test_django_compliant_reads_secure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_django_violating_reads_insecure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_flask_compliant_reads_secure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_flask_violating_reads_insecure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_express_compliant_reads_secure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_express_violating_reads_insecure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_rails_compliant_reads_secure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_rails_violating_reads_insecure_posture` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_missing_config_file_is_err` (pytest node id, verified passing when recorded)
- `tests/unit/test_webapp_websec_session_config.py::test_unsupported_framework_is_err` (pytest node id, verified passing when recorded)
