...
threat: null
```
Filed while working T-0209 (capability scanner comment-span filtering). T-0209's own scope (src/frob/vet/_capability.py, frob.lang/**, tests/**, tickets.md) has no per-line observation shape to dedupe -- scan_file_capabilities returns a bare frozenset[str] of capability kinds (no file/line), and scan_file_operations returns each DANGEROUS_OPERATIONS registry entry at most once per file by construction (one loop iteration per entry, appended at most once), so there is no in-scope duplicate to fix there. The 'same site reported twice' symptom described in T-0209's filed body most plausibly comes from src/frob/strata/_selfconform.py's SYS_UNDECLARED_INTERFACE (SYS100) join: _core_undeclared_violations (T-0158/THREAT004 delegate) and _extended_kind_violations (T-0169's eval/env/ffi/install-hook slice) both independently walk observed capabilities per node and can each emit a SYS100 finding for the same underlying capability on the same node -- core's finding carries line=0 (no line tracking), extended's carries a real line via _effects.py, so a single false (or now-fixed) observation can still surface as two separate findings for a caller that does not itself dedupe by (file, line, kind). Investigate whether check_self_conformance's finding aggregation should dedupe SYS100/SYS101 findings by (node, capability_kind) [or (file, line, kind) once core tracks a real line] before returning, and add a regression fixture with one observed-but-undeclared capability that both the core and extended join paths would independently flag.
