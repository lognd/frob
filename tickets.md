...
from the fresh run (verified by direct grep, not inference). The 15
...
pre-existing and outside this ticket's scope/diff:
  - COV003 x13 on tickets/T-0065, T-0148, T-0168 (stale test-collection
    ...
    instructions)
  - SYS004 x1 on design/frob.strata (strata_core native extension not
    installed in this worktree -- known env artifact per prior session
    notes, not a regression)
  ...
  - `ty`: 2 diagnostics, both `frob_core` unresolved-import in
    tests/unit/test_dup_core.py -- same native-extension worktree
    artifact as SYS004 above, not touched by this ticket's files
