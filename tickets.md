...
parent: null
scope:
- src/frob/strata/_selfconform.py,src/frob/vet/_capability.py,design/frob.strata,tests/**,tickets.md
evidence: []
attachments: []
acceptance: []
threat: null
```
Discovered while merging main into the T-0169 worktree (post T-0181 close): tests/unit/strata/test_selfconform.py::TestRealGateGreen::test_repo_design_and_declarations_are_self_conformant now fails against the real repo tree with SYS100 'html_render' observed but not declared on the vet node. Root cause: T-0181 added new html_render registry needles (e.g. innerHTML, dangerouslySetInnerHTML) as literal string DATA inside src/frob/vet/_capability_registry.py itself -- scanning that file's own text for html_render needles self-matches trivially (the exact documented false-positive class vet.scan_directory_capabilities already excludes via a private _is_self_path check on _capability.py/_capability_registry.py, docs in _capability.py module docstring Self-match note). _selfconform.py's own file-level SYS100/SYS101 joins scan node-owned files directly (never through scan_directory_capabilities), so they never got this exclusion and were quietly relying on the self-match noise to satisfy design/frob.strata's vet node may declarations (eval/exec/deserialize/sql were declared and self-conformance's own scan of _capability_registry.py's literal needle text was the ONLY thing observing them -- confirmed by prototyping an exclusion fix: doing so correctly removes the html_render false SYS100, but then produces four NEW SYS101 stale-design findings for eval/exec/deserialize/sql on vet, meaning design/frob.strata's may list for the vet node needs to be recalibrated against what the real (non-self-match) code actually exercises, not just patched in _selfconform.py/_capability.py alone). Fix needs both: (1) expose the existing self-match exclusion (_capability.py::_is_self_path, or a public equivalant) to _selfconform.py's file-level scans, and (2) audit + correct design/frob.strata's vet node may declarations for eval/exec/deserialize/sql/html_render against genuine (non-registry-data) usage in src/frob/vet/**, restamping T-0150's self-conformance baseline honestly. NOT fixed as part of T-0169 (which is scoped to per-language TS/JS/Rust/C-C++ wiring, not this self-match/design-recalibration issue) -- this was reverted out of that ticket's diff to avoid unbounded scope creep into design/frob.strata, which T-0169's scope excludes.
