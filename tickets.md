...
labels: []
```
found while working T-0726 (TICK006 phantom-filing gate): T-0367 existed in tickets-archive.md as a fully-formed yaml frontmatter block + body (title/state/scope/created all present) but with NO <!-- ticket:T-0367 -->
```yaml
id: T-0367
...
```yaml
id: T-0800
title: 'perf: PERF004 AST check may over-fire on sort inside a def nested in a loop
  body'
state: queued
kind: bug
origin: agent
...
attachments: []
acceptance:
- text: GIVEN a function defined inside a loop body whose own body sorts WHEN PERF004
    evaluates THEN the finding decision is deliberate (fire or not) with a regression
    test locking it, instead of incidental byte-containment
  evidence: []
threat: null
component: null
labels: []
```
Promotion of T-0367's worktree draft 63dd867e (reviewer note): _enclosing_loop_body_hit's ancestor walk does not stop at an intervening function_definition boundary, so a sort inside a def nested in a loop fires on byte-containment. Over-fire direction (safe), but make it a deliberate documented choice.
