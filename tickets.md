...
<!-- ticket:T-draft-2a3adb6d -->
```yaml
id: T-draft-2a3adb6d
title: bump version + frob release stamp for T-0253's is_self_pattern_path signature
  change
state: queued
kind: bug
origin: human
...
threat: null
```
T-0253 (fixing the self-pattern-path scan-target evasion hole) changed the public frob.vet._capability.is_self_pattern_path signature (added an optional root: Path | None = None param, default-safe/fail-closed). frob check flags REL001: public API changed (major) since 0.2.0, requiring a version bump to >= 1.0.0 and a CHANGELOG entry, then frob release stamp. Both pyproject.toml and .frob-release.json are outside T-0253's declared scope, so that ticket could not carry the bump itself -- do it here. Decide honestly whether a full 1.0.0 bump is warranted pre-1.0 or whether the project's own semver policy needs a documented pre-1.0 exception; either way, run frob release stamp to re-baseline the API manifest so REL001 goes quiet.
