...
# Import/binding-aware resolution for Rust: `_rust_use_table` maps
# `use std::process::Command as C;` -> alias C to
# `std::process::Command`, so `C::new(...)` resolves like
# `Command::new(...)` -- grouped/nested/glob `use` forms supported.
#
# Scope-awareness is POSITION-sensitive: a `let` binding does not hoist
# in Rust, so a call textually BEFORE a same-named `let` must still
# resolve to the `use`-bound alias (`_rust_scope_bound_names` maps
# name -> byte position from which it shadows; a call site is only
# shadowed when `site.start_byte >= that position`). Parameters always
# shadow. A name rebound multiple times keeps its EARLIEST position.
