...
T-2833: the worktree-sweep family (`sweep_worktrees`/`remove_worktree` and
their shared verdict machinery) that used to live here moved to `frob.
tickets._worktree_sweep` -- a distinct CLI-surface concern (reachable from
`frob worktree sweep`/`remove` and the rapid-profile auto-sweep, never
from the lease-CRUD/ledger-commit machinery the rest of this module
serves) that needed its own `may` capability grant in `design/frob.strata`
before it could land as a real file (see that module's own docstring, and
T-2833's Done report, for the exact grant added).
"""
