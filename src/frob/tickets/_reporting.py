...
# frob:ticket T-4287
def _worktrees_carrying_terminal_copy(root: Path, ticket_id: str) -> tuple[str, ...]:
    """Live cross-worktree leases (T-0473's `read_all_leases`) whose OWN
    on-disk copy of `ticket_id` is currently `done`/`dropped` (T-4287) --
    named to the operator by `reopen_ticket` BEFORE it performs the
    `done -> queued` transition, since a `frob ticket land` run from
    exactly one of these worktrees is the audience the T-1914 sibling-
    state-regression guard (`frob.tickets._land._assert_no_sibling_state_
    regression`) was written to catch.

    Measured incident (T-4287): a ticket was reopened with no warning
    that two unrelated, already-finished worktrees still carried its old
    terminal state -- both then failed to land, three attempts each, with
    no forward move available to the worktree itself (the state it would
    need to change belongs to someone else's ticket). Naming the affected
    worktrees before the transition turns that surprise into a decision
    the person reopening can act on (rebase those worktrees, or land them
    first) instead of discovering the cost from a blocked land.

    Best-effort and non-blocking, matching `read_all_leases`'s own fail-
    open posture: a lease whose worktree cannot be resolved or whose
    ticket store cannot be loaded contributes nothing rather than raising
    -- this is disclosure, not a second gate. Distinct worktree paths
    only (a lease is per-ticket, one worktree can hold several); sorted
    ...
    for record in read_all_leases(root):
        if record.worktree in seen_worktrees:
            continue
        seen_worktrees.add(record.worktree)
        loaded = load_all(Path(record.worktree))
        if loaded.is_err:
            continue
        sibling = loaded.danger_ok.get(ticket_id)
        if sibling is None:
            continue
        if sibling.state in (TicketState.DONE, TicketState.DROPPED):
            named.add(record.worktree)
...
def reopen_ticket(
    ...
    # frob:ticket T-4287
    # T-4287: name every live worktree whose own copy of `ticket_id` is
    # still terminal BEFORE performing the transition below -- the
    # measured incident this ticket fixes had no such warning, so the
    # cost (two finished tickets stranded) was discovered only after
    # their lands started failing, not at reopen time.
    stranded = _worktrees_carrying_terminal_copy(root, ticket_id)
    if stranded:
        _log.warning(
            "tickets: %s reopen will leave %d live worktree(s) carrying a "
            "stale terminal copy of this ticket: %s -- their next `frob "
            "ticket land` recognizes this reopen (T-4287) and will not be "
            "refused over it, but rebasing them onto the reopened state "
            "first is still the cleaner path",
            ticket_id,
            len(stranded),
            ", ".join(stranded),
        )
