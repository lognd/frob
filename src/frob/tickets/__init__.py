...
# frob:doc docs/modules/tickets.md#public-api
def add_evidence(
    root: Path, ticket_id: str, node_ids: Sequence[str], collected: frozenset[str]
) -> Result[Ticket, TicketError]:
    ...
    dependency frob.testing pulls in."""
    loaded = _load_one(root, ticket_id)
    if loaded.is_err:
        return Err(loaded.danger_err)
    ticket = loaded.danger_ok

    unresolved = [nid for nid in node_ids if not _matches_collected(nid, collected)]
    if unresolved:
        _log.warning(
