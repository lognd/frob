...
# frob:waive ARCH102 reason="this module implements ticket persistence across the \
# three coexisting on-disk representations its own docstring names (v2 per-ticket tree \
# / single monofile ledger / legacy dir-mode) plus their shared locking primitives -- \
# one cohesive persistence concern by design, same shape as this file's own \
# tickets/_models.py and tickets/_evidence.py sibling ARCH102 waivers. This finding \
# was NOT present before this ticket's own migration- function extraction (LARGE001 \
# remainder batch 2): the extracted functions are now plain import re-export bindings \
# here, with their real call-graph edges into this module's helper functions \
# (ledger_path/_dir_glob/atomic_write/etc) severed by the move -- those edges used to \
...
# `frob.tickets._store.migrate_*`/`_split_done_report`/`_migrate_one_v2`/
# `_migrate_missing_ids` call site keeps working with no caller-side change.
from frob.tickets._store_migrate import (
    _migrate_missing_ids,  # noqa: F401 -- re-exported: migrate_missing_v2's own helper
    _migrate_one_v2,  # noqa: F401 -- re-exported: tests/test_tickets_migration.py
    _split_done_report,  # noqa: F401 -- re-exported: _setters.py's own amend path
    migrate_missing_v2,  # noqa: F401 -- re-exported: T-2355 gap-fill migrator
...
class TicketLockUnavailable(RuntimeError):
    ...
    advisory-lock primitive on this platform. A loud refusal, never a
    silent no-op: proceeding unlocked would reopen the exact ledger
...
def _coverage_tracer_active() -> bool:
    ...
    direct-import test coverage and any external caller still importing
    it from this path."""
    return _coverage_tracer_active_impl()
...
def _yaml_loader() -> type[yaml.SafeLoader]:
    ...
    (`_tickets_yaml_loader`) that already depends on this import path."""
    return fast_yaml_loader()
...
def ledger_lock(root: Path) -> Iterator[None]:
    ...
    Uses `fcntl.flock` on `.frob/tickets.lock` (POSIX) or, on Windows,
    `msvcrt.locking` (T-2934). If NEITHER primitive exists, raises
    `TicketLockUnavailable` rather than the pre-T-2934 silent
    unconditional no-op. Re-entrant per thread (see `_lock_local`) so a
    locked primitive called
    ...
    if fcntl is None and msvcrt is None:
        raise TicketLockUnavailable(
            ...
            f"proceed unlocked against {root} (T-2934)"
        )
