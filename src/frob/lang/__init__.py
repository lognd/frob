...
_EXTENSION_TABLE: dict[str, tuple[str, str]] = {
    ...
    # names -- see `frob.lang._walk_css`'s `walk_css`/`walk_scss` for the
    # thin rule-set/variable walker and its publicness (always-public,
    # CSS has no visibility keyword) decision. A11Y/SEO contrast,
    # target-size, outline, and hidden-text lint rules (T-5146-3,
    # T-5146-4, T-5147-3) query the raw tree these entries make
    # reachable, not this walker's symbol shape.
    ".css": ("css", "css"),
    ".scss": ("scss", "scss"),
    # frob:ticket T-5300
...
# (the demoted case). T-1062: leaked Unknown traces to Path.as_posix, a plain \
# string-formatting call the resolver cannot statically bound; the one real raise path \
# (relative_to outside cwd) is caught above"
...
# frob:waive EXHAUST003 reason="T-1402: EXHAUST001 narrowed to fire for an own \
# ambiguous bare re-raise; this leaked Unknown traces to an unresolved callee instead \
# (the demoted case). T-1062: fn is a caller-supplied zero-arg callable (a \
