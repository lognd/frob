...
def run(cfg: AppConfig) -> None:
    ...
    unguarded call here would leak that line into `--json` output. Wrapped
    in `quiet_stdout_logs()` (the same pattern every other `--json` runner
    uses, e.g. `frob.app.xref_runner`) to keep stdout exactly the rendered
    result, nothing else."""
