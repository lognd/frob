...
# frob:invariant terminates reason="mutually recurses with _mutation_for_eval, which \
# re-tokenizes only the tokens strictly after the resolved 'eval' token (tokens[start \
# + 1:]); each hop through eval therefore drops at least the 'eval' token itself \
# before recursing, so the token-list length strictly decreases and is bounded below \
# by 0" measure="len(tokens) at each recursive hop through eval, strictly decreasing"
...
# frob:invariant terminates reason="mutually recurses with _mutation_for_command by \
# handing it sub_tokens re-tokenized from tokens[start + 1:] -- strictly fewer source \
# tokens than this call's own `tokens`, since `start` always points at or past the \
