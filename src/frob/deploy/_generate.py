...
from frob.strata import HostManifest, KernelModel
from frob.strata._host import HostOwns
...
# T-2358: `ManifestEntry`/`sorted_manifest_entries`/`manifest_digest`/
# `DIGEST_HEADER_PREFIX` are re-exported here (not just imported for local
# use) because every existing external caller
# (`frob.deploy.__init__`/`_conform.py`/tests) imports them FROM
# `frob.deploy._generate` -- moving their DEFINITION to `_generate_common`
# (the actual cycle fix; see that module's own docstring) without also
# re-exporting them here would be a breaking rename for every one of those
# callers. `ruff`'s unused-import check does not flag this because each
# name IS used below (`sorted_manifest_entries`/`manifest_digest` in
# `generate_all`, `ManifestEntry` in type-shaped call sites,
# `DIGEST_HEADER_PREFIX` in `_header`).
