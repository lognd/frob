...
# frob:ticket T-3003
# frob:ticket T-3018
# T-3003 fixed this module's own `os.kill(pid, 0)` liveness probe for
# Windows (unsafe there -- see `frob.process._pid_liveness`'s module
# docstring for the full TerminateProcess/PID-reuse hazard); T-3018 found
# TWO more independent copies of the same unsafe pattern elsewhere in this
# repo and extracted the fixed implementation here so a future fix never
# again has to be applied to one copy while the others stay dangerous.
# Imported by its BARE name (never `frob.process._pid_liveness.pid_alive`
# dot-qualified, and never re-aliased) so `frob.gates._wire`'s WIRE001
# reach-scan -- which, for a plain FUNCTION record, only recognizes an
...
def _is_stale(entry: _MutationJournalEntry) -> bool:
    ...
    if current is None:
        ...
        # between pid_alive's probe and this read) -- conservatively NOT
