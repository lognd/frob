...
# all sharing one severity channel (advisory-only suggestions, never build-blocking) \
# and one structural-walk substrate (_iter_py_functions/_child/_node_text) this \
# package already provides. Each detector rule is short; the bulk is the volume of \
# distinct hallmark/anti-pattern rules under one shared design-constraint header \
# (advisory-only, strong-hallmark-only), not a bundle of unrelated concerns -- \
# splitting by rule-count would be an arbitrary cut with no consumer-set boundary, \
# since every rule is reached only through this module's own single entrypoint."
