...
def _ctest_content_key(root: Path, build_dirs: list[Path]) -> str:
    """Sha256 over every discovered build dir's `CTestTestfile.cmake`
    content -- the file ctest itself reads to answer `--show-only`, so
    hashing it is sufficient to invalidate the cache on any test-set change
    (a reconfigure, an added/removed `add_test()`) without needing to hash
    the whole source tree."""
    hasher = hashlib.sha256()
    testfiles = sorted(d / "CTestTestfile.cmake" for d in build_dirs)
...
def collect_cpp_tests(root: Path) -> Result[CollectedTests, TestingError]:
    ...
    per-crate shape as closely as ctest's own model allows. KNOWN
    APPROXIMATION: a ctest test name has no required source-file locality,
    so its node id anchors to the build directory itself
    (`<build_dir>::<test name>`) rather than a real source file -- the
    closest available symref, the same kind of approximation
    `_integration_module_path_to_symref` makes for a rust integration
    binary's submodule tree. Cached on every discovered
    `CTestTestfile.cmake`'s content hash. Degrades to an empty, `Ok` result
    (with a warning, never a hard failure) when `ctest` is not on PATH."""
