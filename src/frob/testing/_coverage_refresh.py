...
#: coverage recipe measures. `native_coverage_refresh`'s default now
#: RESOLVES the real target from the scanned repo's own
#: `pyproject.toml` `[project].name` via `_resolve_cov_target` below,
#: so a consumer repo whose package is not literally `frob` measures
#: its own code instead of silently measuring frob's. This literal is
#: reached only when that resolution cannot determine an answer
#: (missing/malformed pyproject.toml, or a declared name whose
#: src-layout directory does not exist) -- an explicit `cov_target=`
#: caller always wins over both. Full incident history: T-4718 sweep,
#: T-3275 ticket body.
# frob:ticket T-1516
# frob:ticket T-3275
...
#: retry serially. `INTERNALERROR` is pytest's own marker for an
#: uncaught exception in its own machinery; `worker '?gwNN'?  crashed`
#: (both quote styles, and none) is xdist's own summary line;
#: `replacing crashed worker gwNN` is execnet's node-down report.
#: Matching any is enough to classify the abort as an ENVIRONMENT
#: failure (a worker got killed, most often OOM) rather than a genuine
#: test regression. All three alternatives are kept even though only
#: one may fire on a given xdist version, since each guards a different
#: failure-reporting path. Full investigation history: T-4718 sweep,
#: T-1672/T-1677/T-2087 ticket bodies.
