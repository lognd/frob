...
_LEAK = re.compile(r"(definitely lost|indirectly lost|possibly lost|still reachable):\s*([\d,]+) bytes")
_INVALID = re.compile(r"(Invalid read|Invalid write|Use of uninitialised|Conditional jump)")
...
def _parse_text(text: str, exit_code: int) -> ToolResult:
    ...
    clean = [_ERROR_PAD.sub("", l).strip() for l in lines]
    ...
    for block in error_blocks:
        ...
        if _INVALID.search(first) or "Invalid" in first:
            ...
            loc_line = next((l for l in block if "(" in l and ":" in l), None)
            ...
            diagnostics.append(Diagnostic(
                file=file_ref, line=lineno,
                severity="error",
                message=first[:120],
            ))

    # Leak summary
    leak_lines = [l for l in clean if _LEAK.search(l)]
    ...
    # Summary line
    err_summary = next((l for l in clean if "ERROR SUMMARY:" in l), None)
    errors = sum(1 for d in diagnostics if d.severity == "error")
    warnings = sum(1 for d in diagnostics if d.severity == "warning")
    summary = err_summary or (f"{errors} errors, {warnings} warnings" if errors or warnings else "clean")
    return ToolResult(tool="valgrind", exit_code=exit_code, diagnostics=diagnostics, summary=summary.strip())
...
def _parse_xml(text: str, exit_code: int) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(tool="valgrind", exit_code=exit_code, summary="XML parse error")
    ...
    return ToolResult(tool="valgrind", exit_code=exit_code, diagnostics=diagnostics, summary=summary)
