...
def acquire_singleton_lock(root: Path) -> Result[IO[Any], DaemonError]:
    ...
    if fcntl is None and msvcrt is None:
        ...
        return Err(DaemonError.LockUnavailable)
    path = lock_path(root)
...
class _RequestHandler(socketserver.StreamRequestHandler):
    ...
    # ambiguous bare re-raise; this leaked Unknown traces to an unresolved callee \
    # instead (the demoted case). T-1062: leaked Unknown traces to self._write_line's \
    # own json.dumps call, a stdlib call the resolver cannot statically bound as \
    # raise-free; the two documented raise paths (OSError, ValueError) are caught below"
...
if sys.platform != "win32":
    ...
    # (_DaemonServer.__init__), never to the enclosing class itself or the \
    # assignment statement -- this position is the only one that resolves \
    # via_following to _DaemonServer, matching the violation's own \
    # enclosing_qualname symref exactly (T-2438 exact-match rule)"
...
# (the demoted case). T-1062: leaked Unknown traces to the _log.info call's own \
# %-formatting, a stdlib logging call the resolver cannot statically bound; the one \
# real raise path (Path.unlink) is caught below"
