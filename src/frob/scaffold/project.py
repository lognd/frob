...
#
# T-1742: the guard used to refuse ANY staged change to a land-owned file,
# with no check for whether the staged content actually diverges from
# main's own current tip -- so an ordinary `git merge main` forward-merge
# that legitimately carries main's own land-generated changes to these
# files (zero local divergence: `git diff main -- <file>` empty) was
# refused outright, turning routine hygiene into a blocked commit. Fixed
# by comparing staged content against `main`'s current tip per file
# (`_staged_diverges_from_main` below) and refusing only when they
# actually differ -- a real hand-edit still refuses exactly as before; a
# merge commit that reproduces main's own content byte-for-byte no longer
# does. If `main` cannot be resolved at all (e.g. no such ref in this
# checkout), the check fails safe and treats the file as diverging, so the
# guard never goes silently inert in an unexpected repo shape.
...
if [ -z "$FROB_LAND_INTERNAL" ]; then
    # T-1742: true only when the staged blob for "$1" differs from what
    # `main`'s current tip has for that same path -- a merge commit that
    # just fast-forwards main's own land-generated content is not a
    # divergence and must not be refused; a genuine hand-edit is.
    _t1742_staged_diverges_from_main() {
        if ! git rev-parse -q --verify main >/dev/null 2>&1; then
            return 0
        fi
        ! git diff --cached --quiet main -- "$1"
    }
    staged=$(git diff --cached --name-only)
    case "$staged" in
        *CHANGELOG.md*)
            if _t1742_staged_diverges_from_main CHANGELOG.md; then
                echo "frob: refusing commit -- CHANGELOG.md is land-owned (T-0731)" >&2
                echo "frob: changelog entry is generated by \\`frob ticket land\\`" >&2
                exit 1
            fi
            ;;
    esac
    case "$staged" in
        *uv.lock*)
            if _t1742_staged_diverges_from_main uv.lock; then
                ...
                echo "frob: lockfile update happens at land time" >&2
                exit 1
            fi
            ;;
    esac
    if echo "$staged" | grep -qx "pyproject.toml"; then
        ...
        if git diff --cached -- pyproject.toml | grep -qE "$version_line" \\
            && _t1742_staged_diverges_from_main pyproject.toml; then
