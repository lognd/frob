# frob:waive SCOPE001 reason="T-0385's declared scope is src/frob/vet/+docs/design/registry/patterns.yaml; tests/** is leased in-progress by T-0160 so the scope cannot be formally extended here, same ad-hoc precedent as tests/test_check_coverage_registry.py's existing T-0424 SCOPE001 waiver"  # noqa: E501
"""Real-data EXHAUSTIVENESS meta-test for T-0385 (registry reconciliation:
patterns, 346 entries) -- docs/design/registry/patterns.yaml,
...
loads the REAL `docs/design/registry/patterns.yaml` against the REAL live
ticket queue -- the whole point of T-0385's acceptance criterion is that
the catalogued count (346) equals enforced+excused+deferred RIGHT NOW in
this build, not on a fixture standing in for it. Same posture as
`tests/test_check_coverage_registry.py` (T-0424)."""
...
# frob:ticket T-0385
_PATTERNS_CATALOGUED_TOTAL = 346
...
class TestPatternsExhaustiveness:
    ...
    # frob:ticket T-0385
    def test_declared_total_is_346(self) -> None:
...
class TestExhaustivenessGateOverRealPatterns:
    ...
    # frob:ticket T-0385
    def test_no_patterns_violations(self) -> None:
        ...
        patterns_violations = [
            v for v in violations if v.file == "docs/design/registry/patterns.yaml"
        ]

        assert patterns_violations == []
