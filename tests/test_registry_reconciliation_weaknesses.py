...
class TestWeaknessesExhaustiveness:
    ...
    def test_every_deferred_entry_targets_an_open_ticket(self) -> None:
        ...
        if not deferred:
            ...
            # (precedent: test_registry_reconciliation_system_design.py's
            # identical fix in commit 6baef20d) -- every previously-deferred
            # weaknesses.yaml entry has since been resolved to
            # checkable/duplicate-of/out-of-scope, so the positive case
            # below simply has nothing left to check.
            return
