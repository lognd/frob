...
class TestLangConformanceGate:
    ...
    # frob:ticket T-0405
    # frob:tests src/frob/gates/_lang_conformance.py::lang_conformance_gate
...
# frob:ticket T-1601
class TestJavaCapabilityConformance:
    """Java-specific positive/negative controls (T-1601) -- mirrors the
    csharp controls above."""

    # frob:ticket T-1601
    # frob:waive DUP001 reason="deliberately mirrors TestCSharpCapabilityConformance:: \
    # test_csharp_registered_capabilities_pass's shape (T-1600) -- both are the SAME \
    # per-language clean-control convention this file already established (one test \
    # class, one such test, per new adapter language); extracting a shared helper \
    # would touch T-1600's own already-landed, already-closed test class for a \
    # five-line loop, not a real reuse win"
    def test_java_registered_capabilities_pass(self, tmp_path: Path) -> None:
        """CLEAN CONTROL: every capability the live registry claims
        IMPLEMENTED for java actually works against the real fixture."""
        registry = derive_capability_registry()
        support = registry["java"]
        implemented = [
            capability
            for capability, status in support.capabilities.items()
            if status.state is FacetState.IMPLEMENTED
        ...
        for capability in implemented:
            ok, detail = _behavioral_capability_check("java", capability, tmp_path)
            assert ok, f"java/{capability}: {detail}"

    # frob:ticket T-1601
    def test_java_broken_continuation_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """MUST-FAIL POSITIVE CONTROL: a Java fixture whose `frob:tests \\`
        continuation's second physical line is DROPPED must make
        `_behavioral_capability_check` report failure."""
        import frob.gates._lang_conformance as module
        ...
        monkeypatch.setitem(module._CAPABILITY_FIXTURE_SOURCES, "java", broken_source)
        ok, detail = _behavioral_capability_check("java", "directive_parse", tmp_path)
        assert not ok, f"broken java fixture was wrongly reported as passing: {detail}"

    # frob:ticket T-1601
    def test_java_no_symbols_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """A second, independent MUST-FAIL positive control: an empty Java
        fixture (no classes/members at all) must fail `symbol_walk`'s
        behavioral check."""
        import frob.gates._lang_conformance as module

        monkeypatch.setitem(
            module._CAPABILITY_FIXTURE_SOURCES, "java", "// just a comment\n"
        )
        ok, detail = _behavioral_capability_check("java", "symbol_walk", tmp_path)
        assert not ok, f"empty java fixture was wrongly reported as passing: {detail}"


# frob:ticket T-1602
class TestCudaCapabilityConformance:
    """CUDA-specific positive/negative controls (T-1602) -- mirrors the
    java controls above."""

    # frob:ticket T-1602
    # frob:waive DUP001 reason="deliberately mirrors TestJavaCapabilityConformance:: \
    # test_java_registered_capabilities_pass's shape (T-1601) -- both are the SAME \
    # per-language clean-control convention this file already established (one test \
    # class, one such test, per new adapter language); extracting a shared helper \
    # would touch T-1601's own already-landed, already-closed test class for a \
    # five-line loop, not a real reuse win"
    def test_cuda_registered_capabilities_pass(self, tmp_path: Path) -> None:
        """CLEAN CONTROL: every capability the live registry claims
        IMPLEMENTED for cuda actually works against the real fixture."""
        registry = derive_capability_registry()
        support = registry["cuda"]
        implemented = [
            capability
            for capability, status in support.capabilities.items()
            if status.state is FacetState.IMPLEMENTED
        ...
        for capability in implemented:
            ok, detail = _behavioral_capability_check("cuda", capability, tmp_path)
            assert ok, f"cuda/{capability}: {detail}"

    # frob:ticket T-1602
    def test_cuda_missing_directive_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """MUST-FAIL POSITIVE CONTROL: a CUDA fixture whose `frob:tests`
        comment is DROPPED entirely must make `_behavioral_capability_
        check` report failure. (Not a dropped-continuation-line control
        like csharp/java's -- CUDA inherits C/C++'s own documented
        line-splice quirk, docs/modules/lang.md, so its OWN clean fixture
        above is deliberately single-physical-line, mirroring `.c`/`.cpp`;
        this control instead proves the oracle catches a directive that
        never bound at all.)"""
        import frob.gates._lang_conformance as module
        ...
        monkeypatch.setitem(module._CAPABILITY_FIXTURE_SOURCES, "cuda", broken_source)
        ok, detail = _behavioral_capability_check("cuda", "directive_parse", tmp_path)
        assert not ok, f"broken cuda fixture was wrongly reported as passing: {detail}"

    # frob:ticket T-1602
    def test_cuda_no_symbols_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """A second, independent MUST-FAIL positive control: an empty CUDA
        fixture (no functions/classes at all) must fail `symbol_walk`'s
        behavioral check."""
        import frob.gates._lang_conformance as module

        monkeypatch.setitem(
            module._CAPABILITY_FIXTURE_SOURCES, "cuda", "// just a comment\n"
        )
        ok, detail = _behavioral_capability_check("cuda", "symbol_walk", tmp_path)
        assert not ok, f"empty cuda fixture was wrongly reported as passing: {detail}"


# frob:ticket T-1603
class TestZigCapabilityConformance:
    """Zig-specific positive/negative controls (T-1603) -- mirrors the
    cuda controls above."""

    # frob:ticket T-1603
    # frob:waive DUP001 reason="deliberately mirrors TestCudaCapabilityConformance:: \
    # test_cuda_registered_capabilities_pass's shape (T-1602) -- both are the SAME \
    # per-language clean-control convention this file already established (one test \
    # class, one such test, per new adapter language); extracting a shared helper \
    # would touch T-1602's own already-landed, already-closed test class for a \
    # five-line loop, not a real reuse win"
    def test_zig_registered_capabilities_pass(self, tmp_path: Path) -> None:
        """CLEAN CONTROL: every capability the live registry claims
        IMPLEMENTED for zig actually works against the real fixture."""
        registry = derive_capability_registry()
        support = registry["zig"]
        implemented = [
            capability
            for capability, status in support.capabilities.items()
            if status.state is FacetState.IMPLEMENTED
        ...
        for capability in implemented:
            ok, detail = _behavioral_capability_check("zig", capability, tmp_path)
            assert ok, f"zig/{capability}: {detail}"

    # frob:ticket T-1603
    def test_zig_broken_continuation_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """MUST-FAIL POSITIVE CONTROL: a Zig fixture whose `frob:tests \\`
        continuation's second physical line is DROPPED must make
        `_behavioral_capability_check` report failure."""
        import frob.gates._lang_conformance as module
        ...
        monkeypatch.setitem(module._CAPABILITY_FIXTURE_SOURCES, "zig", broken_source)
        ok, detail = _behavioral_capability_check("zig", "directive_parse", tmp_path)
        assert not ok, f"broken zig fixture was wrongly reported as passing: {detail}"

    # frob:ticket T-1603
    def test_zig_no_symbols_fixture_is_caught_not_rubber_stamped(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """A second, independent MUST-FAIL positive control: an empty Zig
        fixture (no functions/types at all) must fail `symbol_walk`'s
        behavioral check."""
        import frob.gates._lang_conformance as module

        monkeypatch.setitem(
            module._CAPABILITY_FIXTURE_SOURCES, "zig", "// just a comment\n"
        )
        ok, detail = _behavioral_capability_check("zig", "symbol_walk", tmp_path)
        assert not ok, f"empty zig fixture was wrongly reported as passing: {detail}"
...
class TestCapabilityConformanceGate:
    ...
    # frob:ticket T-3801
    # frob:ticket T-2365
    # frob:tests src/frob/repo_meta.py::is_frob_own_repo
    # frob:tests src/frob/gates/_lang_conformance.py::capability_conformance_gate
