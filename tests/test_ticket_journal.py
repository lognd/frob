...
class TestWriteIntent:
    def test_write_then_read_round_trips(self, tmp_path: Path) -> None:
        ...
        result = write_intent(tmp_path, "T-0001", tmp_path / "worktree")
        assert result.is_ok
        records = read_all_intents(tmp_path)
    ...
    def test_write_failure_returns_err(
        ...
        result = write_intent(tmp_path, "T-0002", tmp_path)
        assert result.is_err
        assert result.danger_err == JournalError.WriteFailed
...
class TestClearIntent:
    def test_clear_removes_the_file(self, tmp_path: Path) -> None:
        ...
        write_intent(tmp_path, "T-0003", tmp_path)
        assert len(read_all_intents(tmp_path)) == 1
        clear_intent(tmp_path, "T-0003")
        assert read_all_intents(tmp_path) == ()
    ...
    def test_clear_missing_file_is_a_no_op(self, tmp_path: Path) -> None:
        ...
        clear_intent(tmp_path, "T-does-not-exist")  # must not raise
...
class TestReadAllIntents:
    def test_reads_every_recorded_intent(self, tmp_path: Path) -> None:
        ...
        write_intent(tmp_path, "T-0010", tmp_path)
        write_intent(tmp_path, "T-0011", tmp_path)
        records = read_all_intents(tmp_path)
    ...
    def test_malformed_record_is_skipped_not_fatal(self, tmp_path: Path) -> None:
        ...
        write_intent(tmp_path, "T-0020", tmp_path)
        records = read_all_intents(tmp_path)
...
class TestLandIntent:
    def test_model_round_trips_via_json(self) -> None:
        ...
        assert LandIntent.model_validate_json(intent.model_dump_json()) == intent
