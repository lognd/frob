...
loads the REAL `docs/design/registry/pii.yaml` against the REAL live
ticket queue -- the whole point of T-0387's acceptance criterion is that
the catalogued count (7) equals enforced+excused+deferred RIGHT NOW in
this build, not on a fixture standing in for it. Same posture as
`tests/test_check_coverage_registry.py` (T-0424) and
`tests/test_registry_reconciliation_patterns.py` / `_secrets.py` (T-0385,
T-0386)."""
...
class TestPiiRegistryFile:
    ...
    def test_is_in_registry_files(self) -> None:
        """`pii.yaml` is one of the files `registry_gate` actually scans --
        an entry missing from `REGISTRY_FILES` is invisible to every gate
        no matter how well-dispositioned its own YAML is."""
        assert "pii.yaml" in REGISTRY_FILES

    # frob:ticket T-0387
    def test_loads_without_error(self) -> None:
        """The real file parses under the unified model, not just under
        hand-rolled regex."""
        loaded = load_registry_dir(_REGISTRY_DIR, ("pii.yaml",))
        assert "pii.yaml" in loaded
    ...
    def test_no_malformed_entries(self) -> None:
        ...
        count."""
        registry_file = load_registry_dir(_REGISTRY_DIR, ("pii.yaml",))[
            "pii.yaml"
        ].danger_ok
...
class TestPiiExhaustiveness:
    ...
    # frob:ticket T-0387
    def test_declared_total_is_7(self) -> None:
    ...
    def test_every_deferred_entry_targets_an_open_ticket(self) -> None:
        ...
        pii.yaml currently has no deferred entries (all 7 are
        handled_by:PII010), so this is vacuously satisfied but stays
        wired for the day one appears."""
        registry_file = load_registry_dir(_REGISTRY_DIR, ("pii.yaml",))[
            "pii.yaml"
        ].danger_ok
        queue = _real_queue()
...
class TestExhaustivenessGateOverRealPii:
    ...
    def test_no_pii_violations(self) -> None:
        ...
        pii_violations = [
            v for v in violations if v.file == "docs/design/registry/pii.yaml"
        ]
