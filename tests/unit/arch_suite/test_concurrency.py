...
class TestForkPoolHazards:
    ...
    def test_self_join_deadlock_discharges_on_real_repo_vet_scan(self):
        """Asserts `src/frob/vet` carries zero fork/pool-hazard findings
        across all four hazard categories, regression-locking the
        discharge so a future refactor reintroducing a
        self-join-deadlock (or any sibling hazard shape) fails loudly.
        The synthetic fixture above
        (`test_self_join_deadlock_fires_when_dispatched_task_joins_its_
        pool`) proves the detector itself still fires. See T-0794/T-0767
...
class TestAsyncEventLoopHazards:
    ...
    def test_sequential_independent_awaits_does_not_fire_on_single_await(
        ...
        result = analyze_project(src_dir)
        hits = [
            # frob:tests src/frob/arch/_lock_ordering.py::_check_lock_ordering_hazards
            s
            for s in result.suggestions
            if s.category == "sequential-independent-awaits"
        ]
...
class TestConcurrencyModelMismatch:
    ...
    # frob:tests src/frob/arch/_concurrency_model.py::_check_concurrency_model_mismatch
