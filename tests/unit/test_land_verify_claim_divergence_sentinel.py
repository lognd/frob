...
class TestQueue001CodedSentinelIsAlsoExcluded:
    """Asserts the exclusion above recognizes `_gates_error_result`'s
    `code="QUEUE001"`/`file=None` QueueUnavailable sentinel shape. See
    T-2684/T-1549 for the design rationale."""
    ...
    def test_queue001_coded_sentinel_does_not_refuse(self, tmp_path: Path) -> None:
        """Asserts a `("QUEUE001", "")` finding as the only new finding
        does not refuse the land. See T-2684 for the design rationale."""
        ticket = _make_ticket(tmp_path, scope=("src/frob/tickets/**",))
        claims = DoneReportClaims(
            test_count=1,
            evidence_count=1,
