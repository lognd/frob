...
`stale_detail` message formatter, and (T-0174 REJECT round)
`split_waiver_rule`/`validate_waiver_fields`, the sub-target grammar and
...
class TestSplitWaiverRule:
    # frob:tests src/frob/strata/_waive.py::split_waiver_rule kind="unit"
    def test_bare_rule_has_no_sub_target(self):
        assert split_waiver_rule("LINT004") == ("LINT004", None)

    # frob:tests src/frob/strata/_waive.py::split_waiver_rule kind="unit"
    def test_qualified_rule_splits_on_first_colon(self):
        assert split_waiver_rule("SYS100:fs-write") == ("SYS100", "fs-write")

    # frob:tests src/frob/strata/_waive.py::split_waiver_rule kind="unit"
    def test_cwe_sub_target_with_its_own_dash(self):
        assert split_waiver_rule("THREAT003:CWE-78") == ("THREAT003", "CWE-78")

    # frob:tests src/frob/strata/_waive.py::split_waiver_rule kind="unit"
    def test_trailing_colon_with_nothing_after_it_is_no_sub_target(self):
        assert split_waiver_rule("SYS100:") == ("SYS100", None)
...
class TestValidateWaiverFields:
    # frob:tests src/frob/strata/_waive.py::validate_waiver_fields kind="unit"
    def test_bare_single_instance_rule_with_reason_is_ok(self):
        assert validate_waiver_fields("LINT004", "a real reason").is_ok

    # frob:tests src/frob/strata/_waive.py::validate_waiver_fields kind="unit"
    def test_qualified_multi_instance_rule_with_reason_is_ok(self):
        assert validate_waiver_fields("SYS100:fs-write", "a real reason").is_ok

    # frob:tests src/frob/strata/_waive.py::validate_waiver_fields kind="unit"
    def test_empty_reason_rejected(self):
        result = validate_waiver_fields("LINT004", "")
    ...
    def test_bare_multi_instance_rule_rejected(self):
        result = validate_waiver_fields("THREAT003", "a real reason")
        assert result.is_err
        assert result.danger_err is StrataError.MalformedWaiver
    ...
    def test_every_multi_instance_family_requires_sub_target(self):
        for family in sorted(MULTI_INSTANCE_WAIVER_FAMILIES):
            result = validate_waiver_fields(family, "a real reason")
            assert result.is_err, f"{family} should require a sub-target"
            result_qualified = validate_waiver_fields(f"{family}:x", "a real reason")
