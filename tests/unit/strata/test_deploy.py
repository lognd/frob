...
class TestScenarioEvaluationErrorPropagation:
    def test_evaluate_scenarios_error_propagates(self, monkeypatch):
        # frob:tests src/frob/strata/_deploy.py::evaluate_deploy_contracts kind="unit"
        """A failing `evaluate_scenarios` call inside
        `_evaluate_generated_scenarios` must abort with its own error, not
        a partially-built report."""
        import frob.strata._deploy as deploy_mod
