...
def _git_init_committed(root: Path) -> None:
    """T-4434: `git init` + commit everything currently under `root` --
    `_tracked_source_digest` (the seeder's fix) compares GIT-TRACKED
    content via `git ls-files`, so a `TestSeedWorktreeNativeSourceMtimes`
    fixture must be a real (committed) git repo for that comparison to see
    anything at all; an uncommitted `tmp_path` reads as "git ls-files
    failed" and the seeder correctly treats that as "cannot verify
    identical, skip" rather than crashing."""
    run = lambda *argv: subprocess.run(  # noqa: E731
        ["git", *argv], cwd=str(root), check=True, capture_output=True, text=True
    )
    run("init", "-q")
...
class TestSeedWorktreeNativeSourceMtimes:
    ...
    def test_identical_source_is_backdated_and_reads_fresh(
        ...
        # T-4434: the seeder compares GIT-TRACKED content, so both sides
        # must be real (committed) git repos for `git ls-files` to see
        # anything -- committing does not touch the mtimes set above.
        _git_init_committed(repo)
        _git_init_committed(worktree)
    ...
    def test_repo_side_untracked_file_does_not_block_seeding(
        ...
        _write_frob_toml(repo, name)
        _write_frob_toml(worktree, name)
        _fake_native_package(repo, name, b"\x00compiled-untracked")
        monkeypatch.syspath_prepend(str(repo))
        importlib.invalidate_caches()
        artifact = repo / name / f"{name}.abi3.so"
        artifact_mtime = artifact.stat().st_mtime
        ...
        _git_init_committed(repo)
        _git_init_committed(worktree)
        # repo-only, root-gitignored, generated AFTER the commit -- exactly
        # the real `uv.lock` shape: present on disk, never tracked, never
        # checked out into a fresh worktree. Backdated like the rest of
        # repo's own source so `stale_natives(repo)` itself stays clean --
        # this file's presence-vs-absence, not its mtime, is what the fix
        # under test must tolerate.
        uv_lock = repo / source_dir / "uv.lock"
        uv_lock.write_text("# generated by uv sync\n")
        os.utime(uv_lock, (artifact_mtime - 1000.0, artifact_mtime - 1000.0))
        assert stale_natives(repo) == ()
        assert len(stale_natives(worktree)) == 1
