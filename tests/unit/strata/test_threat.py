...
class TestCaughtByAuditExhaustive:
    ...
    def test_every_shipped_entry_has_a_substantive_caught_by(self):
        ...
        # + 13 BenignCapability (T-0771 added net.connect/net.listen once
        # `net` joined WIRED_MODE_FAMILIES -- 9 + 2; T-1075 added
        # env.read/env.write once `env` joined WIRED_MODE_FAMILIES too --
        # 11 + 2). A future add to any of these tuples that forgets
        # `caught_by` still satisfies pydantic's `min_length=1` (whitespace
        # aside) but would change this count -- bumping it is the forcing
        # function that re-triggers this audit's placeholder scan below.
        assert len(all_out_of_scope) == 21
        assert len(DEFAULT_BENIGN_CAPABILITIES) == 13
