...
class TestCaughtByAuditExhaustive:
    ...
    def test_every_shipped_entry_has_a_substantive_caught_by(self):
        ...
        # + 15 BenignCapability (T-0771 added net.connect/net.listen once
        ...
        # A future add to any of these tuples that forgets `caught_by`
        # still satisfies pydantic's `min_length=1` (whitespace aside) but
        # would change this count -- bumping it is the forcing function
        # that re-triggers this audit's placeholder scan below.
        assert len(all_out_of_scope) == 21
        assert len(DEFAULT_BENIGN_CAPABILITIES) == 15
