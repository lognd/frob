...
def test_exclude_glob_and_test_dir_are_honored_not_scanned_as_production(
    ...
    """Asserts EXHAUST002 fires for the production copy only when the
    identical genuine boundary is placed in a real production file, a
    `[graph].exclude`-matched directory, and a nested `tests/` directory
    (a nested directory, not a bare `test_*.py` filename, is required so
    `is_excluded`/`is_test_file` are actually exercised on a POSIX-style
    `rel`). See T-3948 for the design rationale."""
