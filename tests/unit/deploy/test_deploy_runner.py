...
class TestGenerate:
    ...
    def test_generate_check_clean_no_exit(self, tmp_path):
        ...
        cfg = AppConfig(
            deploy_command="generate", deploy_path=tmp_path, deploy_check=True
        )
...
def _fake_attestation(*, passed: bool) -> AuditAttestation:
    checkpoint = CheckpointResult(
        label="C0",
        status_assertion="unit=fake inactive",
        status_assertion_passed=passed,
    )
...
class TestAudit:
    ...
    def test_audit_skipped_exits_2(self, tmp_path):
        ...
        with patch(
            ...
            return_value=AuditRunResult(
                status="skipped", reason="VBoxManage not found"
            ),
        ):
            with pytest.raises(SystemExit) as excinfo:
                run(cfg)
