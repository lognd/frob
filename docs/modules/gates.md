...
| REG008 | registry | (warn, advisory) an entry dispositioned `handled_by:<rule>` has no corresponding `frob:enforces` edge in the code graph binding that rule id to real enforcement -- a claimed-but-unwired handler; T-2369 burned this from 36 down to 1 (the last entry blocked by a cross-ticket file lease), promotion to ERROR deferred until that last entry clears (`frob.gates._registry_exhaustiveness`) |
