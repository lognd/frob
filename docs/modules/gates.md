...
| REG008 | registry | (warn, advisory) an entry dispositioned `handled_by:<rule>` has no corresponding `frob:enforces` edge in the code graph binding that rule id to real enforcement -- a claimed-but-unwired handler; T-2369 burned this from 36 down to 3, the remaining 3 blocked on T-2729's (queued) scope claim over src/frob/strata/_selfconform.py, promotion to ERROR deferred until those clear (`frob.gates._registry_exhaustiveness`) |
