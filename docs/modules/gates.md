...
| DOC011 | docanchor | (warn, T-1486 first-turn-on) a doc's prose mentions a ticket id (`T-####`/`T-draft-<hex>`) that does not resolve to any active or archived ticket -- a typo'd or long-since-renumbered id reads as a real, followable reference but silently resolves to nothing; see "DOC011" below |
