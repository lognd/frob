...
| REG008 | registry | (warn, advisory) an entry dispositioned `handled_by:<rule>` has no corresponding `frob:enforces` edge in the code graph binding that rule id to real enforcement -- a claimed-but-unwired handler (`frob.gates._registry_exhaustiveness`) |
