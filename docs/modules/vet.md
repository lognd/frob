...
gaps this does NOT cover; T-2479 extends the same `net-mutate` signal to
`boto3.client("service")`/`boto3.resource("service")` INSTANCE-method
calls -- S3/DynamoDB/IAM's mutating verbs specifically -- via a binding-
aware resolver (`frob.vet._capability_python._resolve_py_boto3_client_
call`) that resolves the service-name STRING LITERAL a variable was bound
from, since boto3's mutating method names are per-service and carry no
...
- **bash and csharp** (T-2906): both are first-class `_capability_registry.
  LANGUAGES` entries with real `DANGEROUS_OPERATIONS` patterns (`.sh`/
  `.bash` and `.cs` in `_capability_core._EXT_LANGUAGE`) -- bash covers
  `eval`, pipe-to-shell (`curl|bash`), `curl`/`wget` (`fetch_url`),
  `rm -rf` (`fs-write`), `source`/`.` (`fs-read`), `export`/`printenv`
  (`env-write`/`env-read`), `/dev/tcp/HOST/PORT` (`net-connect`), `nc -l`
  (`net-listen`), and `kill` (`process-control`); csharp covers
  `Process.Start` (`exec`), `CSharpScript`/`CodeDomProvider` (`eval`),
  `HttpClient`/`WebClient` (`fetch_url`), `TcpClient`/`Socket` and
  `TcpListener`/`HttpListener` (`net-connect`/`net-listen`), `File.Delete`/
  `Directory.Delete` (`fs-write`), `File.ReadAllText`/`ReadAllBytes`
  (`fs-read`), `Environment.SetEnvironmentVariable`/
  `GetEnvironmentVariable` (`env-write`/`env-read`), `DllImport`/`Marshal`
  (`ffi`), and `BinaryFormatter.Deserialize` (`deserialize`). Every
  remaining (kind, language) cell for both is a written `_MatrixExcuse` --
  the structural/normalized-vocabulary kinds (`net`, `net.connect`,
  `net.listen`, `net-mutate`, `env`, `env.read`, `env.write`, `fs`,
  `fs.read`, `fs.write`) are generated by `_new_adapter_matrix_excuses`
  (mirrors `frob.dup._exhaustiveness._non_python_excuses`'s generated-not-
  hand-copied shape), and the substantive un-surveyed idioms
