# Agent playbook appendix: full narrative and incident record

**Start with `docs/guides/agent-playbook.md` (the hot-path checklist), not
this file.** T-2909 split the original single 1,446-line playbook in two
because roughly 40% of it was narrative/incident write-up rather than an
in-the-moment checklist item, and every dispatched agent paid the token
cost of reading all of it, every ticket, whether or not the narrative was
relevant to that ticket. The hot-path file keeps every RULE an agent must
obey in the moment (the warm-up, the mid-ticket discipline, the land
ritual) plus a short pointer into this file wherever the full WHY, the
exact incident, or a rarely-needed recipe lives here instead.

This file is the unabridged original: every section, every incident,
every measurement this repo has paid for once, kept in full so nothing
from the pre-split playbook is lost -- read a specific section here when
the hot-path file's pointer sends you, or when you want the full story
behind a rule rather than just the rule.

Each incident referenced below actually happened in this repo's history
(tickets.md / tickets-archive.md Done reports, or `tickets/T-####/` under
ledger v2). This is not theoretical caution.
