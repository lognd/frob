...
**What "green" means as of T-3512.** The T-3425 windows-latest ADVISORY leg (`continue-on-error: true`) was removed under T-3512 on 2026-09-13, once CI run 34758499278 (head `020d2db1f`) measured green on all three legs, with windows-latest passing 13962/13962 tests and a 0-error self-gate -- see `docs/design/windows-portability.md`'s closure note. `windows-latest` is now a normal, blocking, REQUIRED leg like `ubuntu-latest` and `macos-latest`: `scripts/verify_release_ci_status.py` reads GREEN off all three legs again, not just ubuntu/macos. A red windows-latest leg now flips `ci.yml`'s overall conclusion and blocks the release gate, the same as the other two legs.
