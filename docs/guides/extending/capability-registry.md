...
1. New dangerous operation: append an `_op(language=..., library=...,
   ...
   kotlin/c-cpp), `_BASH_CSHARP_OPERATIONS` in
   `_dangerous_ops_bash_csharp.py` (bash/csharp, T-2906), or
   `_JAVA_OPERATIONS` in `_dangerous_ops_java.py` (java, T-3492) -- all
   four feed the combined `DANGEROUS_OPERATIONS` table in `_matrix.py`.
   `needles` are the literal substrings the scanner matches on.
